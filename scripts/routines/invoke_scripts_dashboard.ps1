...
$commands = @(
    ...
    @{ Name = 'start-worker [-Force] [-Background]'; Purpose = 'Inicia worker+API (Force=reinicia, Background=daemon)'; Backend = 'scripts/ops/start_worker.ps1' },
...
$activeScripts = @(
    ...
    @{ Category = 'Ops'; Path = 'scripts/ops/start_worker.ps1'; Use = 'Inicia worker (start-worker [-Force] [-Background])' }
)
...
$lines.Add("| Recovery controlado | `stop-worker`, `start-worker -Force`, `nexus-fallback-prune-legacy` | [VERMELHO] |")
$lines.Add('')
