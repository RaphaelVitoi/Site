...
try {
    ...
    # SOTA: ErrorAction SilentlyContinue previne crash em buffers corrompidos (OneDrive/Reparse Points).
    # Exclusao nativa de dependencias e caches para compressao ultra rapida sem ruidos de I/O.
    $AllFiles = Get-ChildItem -Path $ProjectRoot -Recurse -Force -File -ErrorAction SilentlyContinue | Where-Object {
