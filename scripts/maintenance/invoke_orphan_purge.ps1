...
Write-Host '=== [SISTEMA] INICIANDO EXPURGO DE DEPENDENCIAS ORFAS SOTA ===' -ForegroundColor Magenta

try {
    & $PythonCmd $ExecutorScript 'db-purge-orphans'
    if ($LASTEXITCODE -eq 0) {
        Write-Host '[VITORIA] Higiene da fila concluida com sucesso (Friccao Zero).' -ForegroundColor Green
    }
}
catch {
