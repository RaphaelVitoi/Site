        ...
        if env_path.exists():
            try:
                with open(env_path, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        match = re.search(r'(?:\$env:|\$)?([\w]+)\s*[:=]\s*[\'"]?([^\'"\s#]+)[\'"]?', line)
                        if match:
...
_RATE_LIMITERS: Dict[str, AsyncTokenBucket] = {
    "gemini-pro": AsyncTokenBucket(
        capacity=TOTAL_PRO_RPM,
        fill_rate_per_minute=TOTAL_PRO_RPM
    ),
    "gemini-flash": AsyncTokenBucket(
        capacity=TOTAL_FLASH_RPM,
        fill_rate_per_minute=TOTAL_FLASH_RPM
    ),
}
