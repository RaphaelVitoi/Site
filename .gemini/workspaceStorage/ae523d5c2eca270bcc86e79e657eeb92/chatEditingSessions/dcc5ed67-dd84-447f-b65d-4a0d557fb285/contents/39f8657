...
def check_candidate_patterns(p: Path, rel: Path, name: str) -> str | None:
    """Check if a path matches any archive candidate patterns."""
    ...
    if (
        str(rel).lower().startswith("queue")
        and p.is_file()
        and p.suffix.lower() in [".bak", ".backup", ".json"]
    ):
        return "queue_backup"
...
def find_candidates(root: Path):
    """Find all candidate files and directories for archiving."""
    candidates = []
    for p in root.rglob("*"):
        try:
            name = p.name.lower()
        ...
        reason = check_candidate_patterns(p, rel, name)
        if reason:
...
def find_task_dbs(root: Path):
    """Find all existing task database files."""
    candidates = [
...
def analyze_db(db_path: Path, cutoff_days: int):
    """Analyze a task database to gather statistics."""
    ...
    try:
        ...
        cur.execute(
            "SELECT COUNT(*) as c FROM tasks WHERE timestamp < ?", (cutoff_iso,)
        )
        out["stale_count"] = cur.fetchone()["c"]
        cur.execute(
            "SELECT agent, COUNT(*) as c FROM tasks WHERE timestamp < ? GROUP BY agent",
            (cutoff_iso,),
        )
...
def main():
    ...
    parser.add_argument(
        "--cutoff", type=int, default=30, help="Days cutoff for stale tasks"
    )
    parser.add_argument(
        "--out", type=str, default=str(REPORTS_DIR / "dry_run_report.json")
    )
