...
## 9. NOTA: Workflow v5 Completamente Assíncrono (Proposta Futura)

> **Status:** PROPOSTA NÃO IMPLEMENTADA
> Autor da proposta: @chatbot | Data: 2026-03-11

Uma proposta futura descreve um modelo v5 completamente assíncrono com:
- Um orquestrador central de fila de tarefas
- Processamento em background sem bloqueio do usuário
- Registro centralizado em `logs/task_log.md` (ainda não implementado)

Esta seção é preservada como referência arquitetônica para uma próxima evolução, mas **NÃO funciona no estado atual**. O sistema atual (v5 com scripts `do.ps1`, `status.ps1`, `cleanup.ps1`) oferece enfileiramento manual e consulta de status, não orquestração automática em background.

Para detalhes sobre a arquitetura proposta v5, consulte `.backups/2026-03-12_auditoria_sistema/MANUAL_WORKFLOW_AGENTES.md` linhas 660-750.
