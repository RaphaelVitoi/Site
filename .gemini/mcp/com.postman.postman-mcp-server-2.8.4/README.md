...
If you're using the EU server (`mcp.eu.postman`), a [local server](https://github.com/postmanlabs/postman-mcp-server/blob/main/docs/developer/postman-api/postman-mcp-server/postman-mcp-local-server#codex), or prefer API key authentication, use the API key method. Set the `POSTMAN_API_KEY` environment variable and invoke the MCP server using `npx`.
...
For Docker set up and installation, see [DOCKER.md](https://github.com/postmanlabs/postman-mcp-server/blob/main/DOCKER.md).
