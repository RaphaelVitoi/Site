...
import {
  startCommand,
  startServer
} from "./chunk-BH2Z44L2.js";
import {
  exitCli,
  require_source
} from "./chunk-34I3QK5T.js";
import {
  DEFAULT_PORT,
  GEMMA_MODEL_NAME,
  ...
  resolveGemmaConfig,
  saveModelChange
} from "./chunk-ACF4JPUT.js";
import {
  RELAUNCH_EXIT_CODE
} from "./chunk-WCQLND73.js";
import "./chunk-FZWS2GMN.js";
import {
  cleanupCheckpoints,
  registerCleanup,
  registerSyncCleanup,
...
async function loadSandboxConfig(settings, argv) {
  ...
  const image = process.env["GEMINI_SANDBOX_IMAGE"] ?? "us-docker.pkg.dev/gemini-code-dev/gemini-cli/sandbox:0.42.0-nightly.20260512.gc987b9939" ?? customImage ?? packageJson?.config?.sandboxImageUri;
...
async function runNonInteractive({
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    if (process.env["GEMINI_CLI_ACTIVITY_LOG_TARGET"]) {
      const { setupInitialActivityLogger } = await import("./devtoolsService-DOAGOR3Q.js");
...
async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd(), resumedSessionData, initializationResult) {
  const { startInteractiveUI: doStartUI } = await import("./interactiveCli-OO2ACBTQ.js");
  await doStartUI(
    config,
    settings,
    startupWarnings,
...
${finalArgs[promptIndex + 1]}`;
  ...
  }
  {
    ...
    initAppHandle?.end();
    import("./liteRtServerManager-5DXMTYK6.js").then(({ LiteRtServerManager }) => {
