    ...
    <div className="glass-panel p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(250px,350px)_1fr] xl:grid-cols-[minmax(300px,400px)_1fr] gap-6 xl:gap-10 items-start mb-8">
        ...
        <div className="flex flex-col gap-3.5">
          ...
          {/* Display do RP */ }
          <div className={`p-6 rounded-2xl border text-center min-h-40 flex flex-col items-center justify-center transition-all duration-400 ${resultCardBgClass} ${resultCardBorderClass}`}>
            { rp === null ? (
              <>
      ...
      <div className="mb-8">
        ...
        <div className="overflow-x-auto">
          <table className="border-separate border-spacing-0.75 font-mono tabular-nums text-[0.6rem] w-full">
            <thead>
              <tr>
                <th className="text-left text-slate-500 font-semibold py-1 px-1.5 text-[0.58rem] w-13">
                  A \ D
                </th>
                { env.stacks.map( p => (
                  <th key={ p.id } className={`text-center py-1 px-0.5 text-[0.58rem] whitespace-nowrap min-w-8 ${defensor === p.id ? 'text-rose-500 font-black' : 'text-slate-500 font-bold'}`}>
                    { p.pos.split( ' ' )[ 0 ] }
                  </th>
                ) ) }
              </tr>
            ...
            <tbody>
              { env.stacks.map( rowPlayer => (
                <tr key={ rowPlayer.id }>
                  <td className={`py-0.5 pr-1.5 pl-1 text-[0.58rem] whitespace-nowrap ${agressor === rowPlayer.id ? 'font-black text-indigo-400' : 'font-bold text-slate-400'}`}>
                  ...
                  { env.stacks.map( colPlayer =>
                  {
                    ...
                    return (
                      <td
                        ...
                        className={`text-center py-1 px-0.5 rounded transition-all ${bg} ${color} ${isHighlighted ? 'font-black outline-2 outline-indigo-500 -outline-offset-1' : 'font-bold'} ${isDiag ? 'cursor-default text-transparent' : 'cursor-pointer'}`}
                      >
