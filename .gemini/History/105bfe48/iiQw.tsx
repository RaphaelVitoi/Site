...
/**
 * IDENTITY: Matchup Selector — Pressão de Risco por Confronto v4.6 GOLD
...
export default function MatchupSelector() {
  ...
  const env =
    FT_ENVIRONMENTS.find((e) => e.id === activeEnvId) ?? FT_ENVIRONMENTS[0];
  if (!env) return null;
  ...
  const rp =
    agressor && defensor ? (env.rpMatrix[agressor]?.[defensor] ?? null) : null;
  const classification = rp === null ? null : classifyRp(rp);

  const agressorData = agressor
    ? env.stacks.find((p) => p.id === agressor)
    : null;
  const defensorData = defensor
    ? env.stacks.find((p) => p.id === defensor)
    : null;

  let bgGlowClass = "text-accent-emerald";
  if (classification?.color.includes("rose"))
    bgGlowClass = "text-accent-danger";
  else if (classification?.color.includes("amber"))
    bgGlowClass = "text-accent-amber";
    ...
    <div className="glass-panel p-6 sm:p-8 lg:p-12 xl:p-16 flex flex-col gap-12 animate-sota-in overflow-hidden relative rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl transition-all duration-500">
      ...
      <div className="pt-8">
        <p className="text-[0.6rem] font-black text-text-darker uppercase tracking-[0.4em] m-0 mb-4 px-2">
          Estrutura de Premiação &middot;{" "}
          <span className="text-text-dim">$10k Ref</span>
        </p>
