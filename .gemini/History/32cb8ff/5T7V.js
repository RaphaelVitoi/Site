...
const ALL_SCENARIOS = [...SCENARIOS, ...TOY_GAMES];
...
class GameEngine {
    constructor() {
        ...
        this.isMuted = false; // Estado inicial do som
        this.init();
    }
    ...
    loadScenario(id) {
        ...
        // Reset agression factor ao mudar de cenário
        this.agressionFactor = 1.0;

        // Atualiza Gauges (Reatividade via Attributes)
        // Cruzamento de dados: Passamos o RP do oponente para calcular Predator Mode
        ...
        this.ui.content.innerHTML = `
            <div id="stats-panel-container">
                ${this._generateStatsPanel(this.solver.solve(data.data.ip.rp, data.data.oop.rp, this.agressionFactor))}
            </div>
        ...
        // Renderizar Gráfico Principal com ApexCharts
    ...
    _generateStatsPanel(sol) {
        ...
        const muteText = this.isMuted ? 'Unmute' : 'Sons';
        ...
        return `
            ...
            <div class="flex items-center justify-between mb-6 bg-slate-900/30 p-3 rounded-lg border border-white/5">
                ...
                <button id="btn-export-pdf" class="text-[9px] font-bold uppercase tracking-widest text-slate-400 hover:text-white flex items-center gap-2 transition-colors">
                    <i class="fa-solid fa-file-pdf"></i> Exportar Relatório
                </button>
                <div class="flex gap-4">
                    <button id="btn-toggle-mute" class="text-[9px] font-bold uppercase tracking-widest ${this.isMuted ? 'text-slate-600' : 'text-slate-400 hover:text-white'} flex items-center gap-2 transition-colors">
    ...
    updateComparisonChart() {
        ...
        const comparison = this.solver.compareScenarios(s1.data, s2.data);
    ...
    exportPDF() {
        // Importa html2pdf dinamicamente se não estiver presente
        if (!window.html2pdf) {
