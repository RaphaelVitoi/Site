...
class GameEngine {
    ...
    renderComparisonInterface() {
        const optionsHtml = SCENARIOS.map(s => `<option value="${s.id}">${s.label}</option>`).join('');
        ...
        if(SCENARIOS.length > 1) sel2.value = SCENARIOS[1].id;
    ...
    updateComparisonChart() {
        ...
        const s1 = SCENARIOS.find(s => s.id === id1);
        const s2 = SCENARIOS.find(s => s.id === id2);
