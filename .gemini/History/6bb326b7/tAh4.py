...
async def process_agent_task(task: Task, manager: QueueManager, timing_metrics: Dict) -> str:
    ...
    # Roteamento SOTA: Garantia de Quota RPM (Prevencao de 429)
    await global_rate_limiter.consume(1)

    agent_clean = task.agent.replace("@", "")

    # OTIMIZACAO DE TOKENS: Numero de resultados RAG dinamico
    ...
    # SOTA: Gatilhos Inteligentes e Adaptativos de Web Search por Perfil Cognitivo
    normalized_description = enforce_pure_ascii((task.description or "").lower())
    web_context = ""

    # SOTA 8.0: Nova diretriz para o @verifier auditar com WebSearch
    if task.agent == '@verifier' and task.metadata.get('priority') in ['high', 'critical']:
        task.description += "\n\n[DIRETRIZ DE AUDITORIA SOTA 8.0] Alem da validacao funcional, voce DEVE realizar uma busca externa (WebSearch) para confrontar a solucao apresentada com o estado da arte mais recente da internet (ultimas 24h). Valide contra mudancas de API, novas versoes de bibliotecas ou metodos depreciados. O sistema deve se auto-corrigir contra a obsolescencia."
        logger.info(f"[[{te._c(task.agent)}]{task.agent}[/]] [bold yellow]AUDITORIA SOTA 8.0[/] ativada para tarefa critica.")

    # SOTA: Garante que lemos do sistema de heuristicas
    ...
    if (TAVILY_KEYS or PERPLEXITY_KEYS) and needs_web_search:
        try:
            ...
            if cached_result and (time.monotonic() - cache_time < WEB_SEARCH_CACHE_TTL):
                ...
                # SOTA: Tenta Tavily primeiro, com fallback para Perplexity
                if TAVILY_KEYS:
    ...
    try:
        t0 = time.monotonic()
        rag = te.get_rag()
        # SOTA: Economia Generalizada. Usamos local_only=True para nao gastar 1 LLM request
        # por tarefa apenas para expandir a query. Busca vetorial hibrida 100% local.
    ...
    # Escalada de Raciocinio (Deep Thinking) para o @securitychief em tarefas vitais.
    ...
    # SOTA (Correcao L2.5): Blindagem contra Prompt Injection via encapsulamento XML-like
    ...
    # SOTA: Purificacao Absoluta dos Inputs (Friccao Zero de Encoding)
...
"""
            if await _create_system_task(manager, fix_id, fix_desc, task.agent, "critical"):
                logger.info(f"[bold orange3][+] AUTO-CURA[/] Anticorpos acionados para a tarefa [cyan]{task.id}[/]")
