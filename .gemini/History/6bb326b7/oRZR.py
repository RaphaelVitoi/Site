...
def _assemble_prompt(task: Task, project_context: str, web_context: str, collective_memory: str, agent_memory: str, task_docs: str, agent_clean: str, autonomy_mode: str = "default") -> Tuple[str, str]:
    ...
    if task.agent not in ["@dispatcher", "@bibliotecario"]:
        ...
        # SOTA 3.2: Matriz de Autonomia Hierarquica e Auditoria
        user_prompt += f"\n\n[MATRIZ DE AUTONOMIA VITOI 3.2] Nivel Atual: {autonomy_mode.upper()}\n"
        if autonomy_mode == "stop":
            user_prompt += "Foco: Homeostase Sistemica. Aplique auto-cura, security patches e fixes de sintaxe silenciosos.\n"
        elif autonomy_mode == "partial":
            user_prompt += "Foco: Estrategista Hibrido. Aplique auto-debugging e refatoracoes proativas de BAIXO impacto. Para alteracoes Core/GTO, bloqueie e proponha a alteracao antes.\n"
        elif autonomy_mode == "full":
            user_prompt += "Foco: Agencia Total (Motor de Inovacao). Refatoracao proativa trans-modular, mudancas arquiteturais e execucao de terminal ativadas em grau maximo.\n"

        derived = "PARTIAL" if autonomy_mode == "full" else "DEFAULT"
