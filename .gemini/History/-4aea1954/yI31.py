    ...
    async with _global_http_session_lock:
        if _global_http_session is None or _global_http_session.closed:
            ...
            connector = aiohttp.TCPConnector(
                ...
                keepalive_timeout=15, # previne "Connection Closed" de APIs LLM agressivas
            ...
            _global_http_session = aiohttp.ClientSession(trust_env=True, connector=connector, headers=default_headers, timeout=global_timeout)
        return _global_http_session


def _sync_fallback_request(url: str, payload: Dict[str, Any], headers: Dict[str, str], timeout_seconds: float) -> Tuple[int, str]:
    """
    ...
    try:
        ...
        return 0, f"Ruptura DNS/TCP (URLError): {str(e.reason)}"
    except Exception as e:
        return 0, f"Entropia Nativa: {str(e)}"
