...
export default function RangeMatrix( { ipRp, oopRp, scenarioId }: Readonly<RangeMatrixProps> ) {
    ...
    const getBgClass = ( status: string ) => {
        switch ( status ) {
            case 'core': return 'bg-[var(--sim-range-core)]';     // Emerald
            case 'marginal': return 'bg-[var(--sim-range-marginal)]';// Amber
            case 'bluff': return 'bg-[var(--sim-range-bluff)]';    // Indigo
            case 'death': return 'bg-[var(--sim-range-death)]';    // Rose (Death Zone)
            default: return 'bg-[var(--sim-range-fold)]';          // Slate (Fold)
        }
    };
        ...
        <div>
            ...
            <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                    ...
                    <div className="flex rounded-md overflow-hidden border border-white/5 bg-bg-deep">
                        { ( ['ip', 'oop'] as Perspective[] ).map( ( p ) => {
                            ...
                            const activeClasses = p === 'ip'
                                ...
                                : 'bg-rose-500/20 text-accent-danger';
                            return (
                                <button
                                    key={ p }
                                    type="button"
                                    onClick={ () => setPerspective( p ) }
                                    className={ `px-2.5 py-1 text-[0.58rem] font-extrabold uppercase tracking-[0.08em] cursor-pointer border-none transition-all duration-200 ${isActive ? activeClasses : 'bg-transparent text-text-darker'}` }
                                >
                ...
                <div className="flex gap-2 text-[0.58rem] font-bold text-text-muted uppercase flex-wrap">
                    <span className="flex items-center gap-1"><span className={ `w-2 h-2 rounded-sm ${getBgClass( 'core' )}` }></span> Core</span>
                    <span className="flex items-center gap-1"><span className={ `w-2 h-2 rounded-sm ${getBgClass( 'marginal' )}` }></span> Misto</span>
                    <span className="flex items-center gap-1"><span className={ `w-2 h-2 rounded-sm ${getBgClass( 'bluff' )}` }></span> Bluff/Float</span>
                    { activeRp >= 40 && (
                        <span className="flex items-center gap-1 text-accent-danger"><span className={ `w-2 h-2 rounded-sm ${getBgClass( 'death' )}` }></span> Death Fold</span>
                    ) }
                </div>
            </div>
                        ...
                        { RANKS.map( ( r2, j ) => {
                            ...
                            return (
                                <button
                                    type="button"
                                    key={ hand }
                                    className={ `${styles.rangeCell} ${bgClass} border-none cursor-pointer transition-colors ${status === 'fold' ? 'text-text-dim' : 'text-text-main'} ${overrides[hand] ? 'opacity-100' : 'opacity-[0.85]'}` }
