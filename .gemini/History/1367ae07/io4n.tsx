    ...
    return (
        <div className="glass-panel flex flex-col gap-10 p-6 sm:p-8 lg:p-12 rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl relative overflow-hidden transition-all duration-300">
            ...
            <div className="w-full flex justify-center py-4 overflow-x-auto scrollbar-hide">
                <div className="min-w-150 max-w-2xl w-full grid grid-cols-13 gap-px bg-white/5 p-px rounded-3xl overflow-hidden border border-white/10 shadow-3xl transform transition-transform duration-700 hover:scale-[1.01]">
                    { RANKS.map( ( r1, i ) => (
                            ...
                            { RANKS.map( ( r2, j ) => {
                                ...
                                return (
                                    <button
                                        type="button"
                                        key={ hand }
                                        className={ `aspect-square flex items-center justify-center text-[0.6rem] font-black font-mono transition-all duration-300 cursor-pointer ${ bgClass } ${ overrides[ hand ] ? 'z-10 scale-110 shadow-[0_0_20px_rgba(255,255,255,0.3)] border border-white/40 ring-1 ring-white/20' : 'opacity-85 hover:opacity-100 hover:scale-[1.08] hover:z-20 border border-white/5' }` }
                                        onClick={ () => handleCellClick( hand, status ) }
                                        title={ `${ hand } - ${ status.toUpperCase() }` }
                                    >
                                        <span className={status === 'fold' ? 'opacity-20' : 'opacity-100'}>{ hand }</span>
                                    </button>
                                );
                            } ) }
                        </React.Fragment>
