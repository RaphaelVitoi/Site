            ...
            <div className="max-w-300 mx-auto px-6 pb-12">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    { posts.map( ( post ) => {
                        ...
                        const excerpt = post.markdown_body.substring( 0, 140 ).replace( /[#*`>]/g, '' ) + '...';
                                    ...
                                    { tags.length > 0 && (
                                        <div className="flex flex-wrap gap-2 mt-auto">
                                            { tags.slice( 0, 3 ).map( ( tag, idx ) => (
                                                <span key={ idx } className="text-[0.6rem] font-mono text-text-darker uppercase tracking-widest bg-white/5 px-2 py-1 rounded-sm">
                                                    { tag }
                                                </span>
                                            ) ) }
                                        </div>
