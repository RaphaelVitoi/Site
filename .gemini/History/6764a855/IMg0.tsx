        ...
        <header className="sota-container pt-32 pb-12 animate-sota-in">
            <div className="flex flex-col gap-6">
                ...
                <nav className="flex items-center gap-3 text-label opacity-40 hover:opacity-100 transition-opacity">
                    ...
                    <span className="text-text-darker truncate max-w-[200px]">{ title }</span>
                </nav>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="space-y-4 max-w-3xl">
                        <div className="flex items-center gap-4">
                            ...
                            { category && (
                                <span className="px-3 py-1 rounded-full bg-white/[0.03] border border-white/5 text-[0.6rem] font-black text-text-muted uppercase tracking-[0.2em]">
                                    { category }
                                </span>
                            ) }
                        </div>

                        <h1 className="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-text-bright via-text-muted to-text-darker tracking-tighter leading-[1.1]">
                            { title }
                        </h1>

                        { subtitle && (
                            <p className="text-lg md:text-xl text-text-muted font-medium leading-relaxed max-w-2xl border-l-2 border-accent-indigo/30 pl-6 py-1">
                                { subtitle }
                            </p>
                        ) }
                    </div>
