"""Módulo de gerenciamento da fila de tarefas SOTA (Queue Manager)."""
import gc
import json
import logging
import os
...
class QueueManager:
        ...
        try:
            ...
            if len(backups) > max_backups:
                for old_backup in backups[max_backups:]:
                    # SOTA: missing_ok=True erradica race conditions em execuções concorrentes.
    ...
    async def _archive_and_tasks(self, cutoff: str) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            insert_query = (
                f"INSERT OR IGNORE INTO archive_tasks SELECT * FROM tasks "
            ...
            delete_query = (
                f"DELETE FROM tasks WHERE status IN ('completed', 'failed') "
    ...
    def _process_directory_purge(
        ...
        for item in target_dir.rglob("*"):
            if item.is_dir():
                ...
                mtime_dt = datetime.fromtimestamp(item.stat().st_mtime, tz=timezone.utc)
