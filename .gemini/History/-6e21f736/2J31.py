...
class QueueManager:
    ...
    def _resolve_db_path(self, queue_path: Optional[str]) -> None:
        if queue_path is None:
            ...
            self._memory_conn = sqlite3.connect(MEMORY_DB_TARGET, check_same_thread=False)
    ...
    def _validate_path_traversal(self) -> None:
        """Blindagem de Seguranca SOTA: Previne Path Traversal."""
    ...
    async def cleanup(self, days: int = 30) -> None:
        ...
        await self._archive_and_purge_tasks(cutoff)
        self._purge_obsolete_files(cutoff_date)
        gc.collect()

    async def _archive_and_purge_tasks(self, cutoff: str) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("""
    ...
    def _process_directory_purge(self, target_dir: Path, cutoff_date: datetime) -> Tuple[int, List[Path]]:
        ...
        for item in target_dir.rglob("*"):
            if item.is_dir():
                ...
                try:
                    if item.stat().st_size == 0 or datetime.fromtimestamp(item.stat().st_mtime) < cutoff_date:
                        item.unlink()
                        deleted_count += 1
                except OSError:
                    pass
