    ...
    def _enforce_backup_retention_policy(
        ...
        try:
            ...
            if not backup_dir.resolve().is_relative_to(self.base_path.resolve()):
                logging.error(
                    f"[SEC] Bloqueio de Path Traversal no expurgo de backups: {backup_dir}"
                )
                return
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if row["metadata"]:
                try:
                    ...
                    logging.exception(
                        f"[DAL] Entropia semantica detectada: Corrupcao JSON na tarefa {task_id}"
                    )
                    current = {}
    ...
    def _row_to_task(self, row: sqlite3.Row) -> Task:
        ...
        if row["metadata"]:
            try:
                ...
                logging.exception(
                    f"[DAL] Entropia semantica extrema: Impossivel decodificar a carga JSON para a tarefa {row['id']}"
                )
