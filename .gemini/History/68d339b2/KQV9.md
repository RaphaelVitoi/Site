...
*   **Roteamento Cognitivo Premium (A Regra de Ouro):**
    *   **Claude Pro (Opus / 3.5 Sonnet) -> "O Cirurgião":** Usado para lógicas complexas, refatoração e design patterns perfeitos (ex: arquitetura NextJS/Prisma). Encarna: `@planner`, `@auditor`, `@implementor` (complexo) e `@curator`.
    *   **Gemini Advanced (3.1 Pro) -> "O Devorador de Mundos":** Usado para devorar janelas de contexto massivas, cruzar a COSMOVISAO com logs antigos, pesquisar e gerar relatórios de alta velocidade. Encarna: `@pesquisador`, `@maverick`, `@organizador` e `@validador`.
