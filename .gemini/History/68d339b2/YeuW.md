...
> **Status:** Ativo e Integrado (Workflow v6.3 - Trios Dinamicos & Economia Generalizada)
...
*   **Roteamento Cognitivo e Anti-Exclusividade (Trios Dinamicos - Web Premium):**
    ...
    *   **Trilha "O Cirurgiao" (Logica/Arquitetura Estrita - @planner, @auditor, @implementor):** 
        ...
        *   *Terceira Via (Tier 3):* **Gemini 2.5 Pro / Flash (Free Tier)** via terminal `nexus` (para automacao material sem custo financeiro, com recomendacao de handoff).
    *   **Trilha "O Devorador de Mundos" (Contexto Massivo/Padroes - @pesquisador, @maverick, @organizador):** 
        ...
        *   *Terceira Via (Tier 3):* **Gemini 2.5 Pro / Flash (Free Tier)** via terminal `nexus` (para orquestracao invisivel, com recomendacao de handoff).
...
*   **Roteamento Cirurgico (Otimizacao de Contexto e API - Trios Dinamicos):** O ecossistema local nao consome recursos financeiros. Ele opera sobre o Free Tier da Google API.
    *   **Trios de Contingencia:** Cada agente possui um trio de modelos configurado em `task_executor.py` (Primario Free Tier, Secundario Free Tier, Terciario Pago/Fallback).
    *   **Prioridade:** Modelos Flash (gemini-2.5-flash) para volume e velocidade; Modelos Pro (gemini-2.5-pro) para complexidade. Claude (Anthropic) como ultima linha de defesa (paga).
