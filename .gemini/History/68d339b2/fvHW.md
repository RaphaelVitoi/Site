...
*   **Roteamento Cognitivo e Anti-Exclusividade (As Três Vias):**
    Não existe exclusividade absoluta. Embora existam modelos "ideais" para certas tarefas, **qualquer modelo Premium pode executar qualquer tarefa** dependendo da escolha, disponibilidade ou estratégia momentânea do usuário. Claude e Gemini operam com Consciência Mútua Plena (um sabe o que o outro fez, lê seus artefatos e continua o trabalho sem refazer do zero).
    *   **Trilha "O Cirurgião" (Lógica/Arquitetura Estrita - @planner, @auditor, @implementor):** 
        *   *Ideal (Tier 1):* **Claude Pro (Opus/Sonnet)** na IDE (pela precisão sintática).
        *   *Alternativa Premium (Tier 2):* **Gemini Advanced (3.1 Pro)** na IDE (plenamente capaz de assumir e continuar com excelência).
        *   *Execução Local (Tier 3):* **Gemini 2.5 Pro / Flash (Free Tier)** via terminal `nexus` (automação material sem custo financeiro).
    *   **Trilha "O Devorador de Mundos" (Contexto Massivo/Padrões - @pesquisador, @maverick, @organizador):** 
        *   *Ideal (Tier 1):* **Gemini Advanced (3.1 Pro)** na IDE (pela janela de contexto colossal e inteligência analítica).
        *   *Alternativa Premium (Tier 2):* **Claude Pro (Opus/Sonnet)** na IDE (plenamente capaz de sintetizar com precisão).
        *   *Execução Local (Tier 3):* **Gemini 2.5 Pro / Flash (Free Tier)** via terminal `nexus` (para orquestração invisível).
