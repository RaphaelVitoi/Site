...
if ($Description) {
    ...
    if ($ExplicitAgent -eq '@gemma' -or $ExplicitAgent -eq '@gemma4') {
        ...
        $Uri = 'http://127.0.0.1:11434/generate'
        $AuthToken = 'sota-token-2026'
        ...
        try {
            $Response = Invoke-RestMethod -Uri $Uri -Method Post -Body $Payload -ContentType 'application/json' -Headers @{'X-Vitoi-Auth' = $AuthToken }
