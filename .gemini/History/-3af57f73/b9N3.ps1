...
if ($DailyReport) {
    ...
    $ReportDesc = "SISTEMA: VITOI 3.2`nOBJETIVO: Gerar o Relatorio Diario de Autonomia SOTA para o Arquiteto (Tier 0).`nDATA: $ReportDate`n`nDADOS EXTRAIDOS DO KERNEL:`n$DailyStats`n`nINSTRUCAO: 1. Escreva um relatorio executivo e analitico em Markdown detalhando a performance do ecossistema hoje. 2. Identifique padroes de falha, gargalos de entropia e recomende acoes estrategicas baseadas nos dados. 3. OBRIGATORIO: Forje o resultado no arquivo 'docs/reports/daily_report_$ReportDate.md'."

    $TaskJson = [ordered]@{ id = "REPORT-$(Get-Date -Format 'yyyyMMdd-HHmmss-ffff')"; description = $ReportDesc; status = 'pending'; timestamp = (Get-Date -Format 'o'); agent = '@historian'; metadata = @{ priority = 'high'; type = 'daily_report'; observers = @('@maverick', '@chico') } } | ConvertTo-Json -Depth 10 -Compress
    ...
    Write-Host "[OK] Tarefa de Relatorio Diario ($ReportDate) enfileirada. O @historian triturara a telemetria do SQLite." -ForegroundColor Green
    exit 0
}
