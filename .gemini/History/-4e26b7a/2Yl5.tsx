...
export default function SimulatorNavigation({
  ...
  return (
    <nav className="flex items-center gap-3 p-2 bg-slate-950/40 backdrop-blur-2xl rounded-3xl border border-white/5 shadow-inner relative overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] snap-x snap-mandatory group/nav w-full max-w-full">
          ...
          >
            {isActive && (
              <motion.div
                ...
                {...({
                  className:
                    "absolute inset-0 bg-slate-800/80 border border-white/10 rounded-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]",
                } as any)}
            ...
            {isActive && (
              <motion.div
                ...
                {...({
                  className:
                    "relative z-10 w-1.5 h-1.5 rounded-full bg-accent-indigo shadow-[0_0_8px_var(--accent-indigo)]",
                } as any)}
              />
            )}
          </button>
