...
export interface CCDHeatmapProps {
...
const CCDHeatmapComponent: React.FC<CCDHeatmapProps> = ( { collisionMatrix } ) => {
    ...
    const gridCells = useMemo( () => {
        return Array.from( { length: 169 } ).map( ( _, index ) => {
            ...
            // SOTA: Escudo Defensivo absoluto contra objetos indefinidos
    ...
    return (
        ...
        >
            <div className="grid grid-cols-13 gap-px bg-gray-900 p-px rounded-sm">
                { gridCells }
            </div>
            <div className="mt-5 flex items-center justify-between text-xs font-mono text-gray-500 uppercase tracking-widest">
                ...
                <div className="h-1 flex-1 mx-6 rounded-full bg-linear-to-r from-gray-900/50 via-gray-700 to-[#cd5c5c]" />
...
// SOTA: Fricção Zero. Evita a recriação das 169 células verificando o array com profundidade,
// protegendo o Event Loop contra o bypass de referência (Object.is) quando o pai gera novos arrays.
const CCDHeatmap = memo( CCDHeatmapComponent, ( prevProps, nextProps ) => {
    if ( !prevProps.collisionMatrix || !nextProps.collisionMatrix ) return false;
    if ( prevProps.collisionMatrix.length !== nextProps.collisionMatrix.length ) return false;

    for ( let i = 0; i < 169; i++ )
    {
        if ( prevProps.collisionMatrix[ i ] !== nextProps.collisionMatrix[ i ] )
        {
            return false;
        }
    }
