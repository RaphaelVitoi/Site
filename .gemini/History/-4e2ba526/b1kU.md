...
### 6. Erradicacao de Entropia de I/O no PowerShell (Backup SOTA)

**Causa:** O script `safeguard_system.ps1` sofria de `ParameterBindingException` e `IOException` (Reparse Points do OneDrive), alem de congelamento (`Deadlock`) no `Compress-Archive`.
**Decisao:** Refatoracao completa do script para utilizar `robocopy` com a flag `/XJ` (isolamento de Reparse Points) e a API nativa `[System.IO.Compression.ZipFile]` do .NET para compressao de alta performance, contornando o pipeline ineficiente do PowerShell.
