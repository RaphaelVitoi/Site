...
/**
 * IDENTITY: Calculadora Malmuth-Harville de Equidade ICM v4.6 GOLD
...
export default function EquityCalculator() {
  ...
  const insolvency: InsolvencyMetrics | null =
    wasmContext?.insolvencyMatrixData ?? null;
  ...
  const { results, isWorkerCalculating, totalChips, totalPrizes } =
    useIcmCalculations({
  ...
  return (
    <div className="glass-panel flex flex-col gap-10 p-6 sm:p-8 lg:p-12 rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl relative overflow-hidden transition-all duration-300">
      ...
      <div className="flex justify-between items-start border-b border-white/5 pb-6">
        <div>
          <h3 className="text-[0.75rem] font-black text-text-main uppercase tracking-[0.2em] m-0">
            Calculadora Malmuth-Harville
          </h3>
          <p className="m-0 mt-1.5 text-[0.6rem] text-text-dim font-medium uppercase tracking-wider">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {showParser ? "Config Manual" : "Parser HH"}
          </button>
          <button
      ...
      {showParser ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
              ...
              {prizes.map((val, i) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={val}
                    onChange={(e) =>
                      updatePrize(i, Number.parseFloat(e.target.value) || 0)
                    }
            ...
            <div
              className={`mt-4 p-4 rounded-2xl border flex justify-between items-center font-mono ${totalPrizes === 100 ? "bg-accent-emerald/5 border-accent-emerald/20 text-accent-emerald" : "bg-accent-amber/5 border-accent-amber/20 text-accent-amber"}`}
            >
              <span className="text-[0.6rem] font-black uppercase tracking-widest">
                Soma Total:
              </span>
              <span className="text-[0.8rem] font-black">
                {totalPrizes.toFixed(1)}%
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="pt-10 border-t border-white/5 flex flex-col gap-8">
                  ...
                  >
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-[0.75rem] font-bold text-text-light">
                          {r.name}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-right font-mono text-[0.75rem] text-text-dim">
                      {player?.stack.toFixed(1)}
                    </td>
                    <td className="px-4 py-4 text-right font-mono text-[0.75rem] text-text-darker">
                      {chipPct.toFixed(1)}%
                    </td>
                    ...
                    <td
                      className={`px-4 py-4 text-right font-mono text-[0.75rem] font-black ${delta >= 0 ? "text-accent-emerald" : "text-accent-danger"}`}
                    >
                      {delta >= 0 ? "+" : ""}
                      {delta.toFixed(1)}%
                    </td>
        ...
        >
          <InsolvencyRioPanel
            ...
            isCalculating={wasmContext?.isCalculatingInsolvency ?? false}
          />
        </div>
      </div>
    </div>
