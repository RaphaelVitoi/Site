...
export default function EquityCalculator ()
{
  ...
  const isCalculatingICM = players !== deferredPlayers || prizes !== deferredPrizes;

  // Cálculo ICM memoizado
  const results = useMemo(
    () =>
    {
      const res = calculateMalmuthHarville( deferredPlayers, deferredPrizes );

      // SOTA Audit: Loga métrica de conservação de valor
      const totalEq = res.reduce( ( sum, r ) => sum + r.equityPercent, 0 );
      logger.metric( 'EquityCalculator', 'icm_total_equity', totalEq, { playerCount: deferredPlayers.length } );
  ...
  return (
    <div className={ styles.calcPanel }>
      ...
      {/* Resultados */ }
      <div className={ `${ styles.calcResultPanel } transition-all duration-300 ${ isCalculatingICM ? 'opacity-50 blur-[1px] scale-[0.99]' : 'opacity-100 blur-none scale-100' }` }>
