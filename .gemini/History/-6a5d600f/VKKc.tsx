...
function getBfTextColorClass(val: number): string {
  ...
  if (val === 0) return 'text-transparent';
...
function getBfBgClass(val: number): string {
  ...
  if (val === 0) return 'bg-slate-900/50';
...
function BfMatrixRow({ row, r, mPlayers, mStacks, mRp }: Readonly<{ row: number[], r: number, mPlayers: string[], mStacks: number[], mRp: number[][] }>) {
  return (
    <tr>
      ...
      { row.map( ( val: number, c: number ) => {
        ...
        return (
          <td key={ mPlayers[ c ] } className={`py-2.5 px-2 text-center min-w-17.5 border border-white/5 leading-snug align-middle ${bgClass}`}>
            { val === 0 ? '' : (
              <>
              ...
              <table className="border-collapse text-[0.85rem]">
                ...
                <tbody>
                  { mBf.map( ( row, r ) => (
                    ...
                    <BfMatrixRow key={ mPlayers[ r ] } row={row} r={r} mPlayers={mPlayers} mStacks={mStacks} mRp={mRp} />
                  ) ) }
                </tbody>
              </table>
            </div>
