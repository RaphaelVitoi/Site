...
export default function ReferencialAula12 ()
{
  ...
  const handleAnaliseMotor = useCallback(() => {
    ...
    const prompt = `Cenário Estrutural: ${matrixView}\nBoard: K-J-T-2-3\nBTN (Agressor): ${heroStack}bb (RP: ${rpBtn}% / BF: ${bfBtn})\nBB (Defensor): ${villainStack}bb (RP: ${rpBb}% / BF: ${bfBb})\nAnalise este embate considerando a Assimetria de Risco e a Teoria Vitoi. Formate a resposta em Pure ASCII.`;
    executeLlama(prompt, { systemPrompt: 'Você é a Mente Coletiva SOTA. Um estrategista de poker focado em Teoria dos Jogos, RIO e ICM.', mode: 'chat' });
  }, [matrixView, matrixData, activeBtnIdx, activeBbIdx, rpBtn, bfBtn, rpBb, bfBb, executeLlama]);
          ...
          <ClientOnly fallback={<div className="mt-8 pt-8 border-t border-white/5 animate-pulse"><div className="h-10 w-48 bg-white/5 rounded-lg mb-6"></div><div className="h-24 w-full bg-black/20 rounded-xl border border-white/5"></div></div>}>
            <div className="mt-8 pt-8 border-t border-white/5">
              <div className="flex gap-4 mb-6">
                ...
                >
                  {isLlamaProcessing ? 'Calculando Trajetória...' : 'Disparar Motor SOTA'}
                </button>
                {isLlamaProcessing && (
                  <button onClick={stopLlama} className="px-6 py-3 rounded-lg text-sm font-bold uppercase tracking-widest bg-rose-500/10 text-rose-400 border border-rose-500/30 hover:bg-rose-500/20 transition-colors">
                    Abortar
                  </button>
                )}
              </div>
              {llamaError && <div className="mb-4 text-rose-500 font-mono text-xs p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg">{llamaError}</div>}
              <TerminalLog lines={llamaLines} isProcessing={isLlamaProcessing} title="ANCORA EMPÍRICA - BARE METAL GPU" height="h-64" />
            </div>
          </ClientOnly>
        </div>
      </details>
