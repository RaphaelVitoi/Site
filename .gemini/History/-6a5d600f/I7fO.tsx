...
function IcmRulers({ matrixView, setMatrixView, matrixData }: { matrixView: 'FT' | 'BUBBLE' | 'EG', setMatrixView: (v: 'FT' | 'BUBBLE' | 'EG') => void, matrixData: { mPlayers: string[], mStacks: number[], mBf: number[][], mRp: number[][] } }) {
...
function EngineSotaPanel({ isStreaming, isPreparing, isCompleted, streamedText, error, handleAnaliseMotor, stopStream }: {
  isStreaming: boolean;
  isPreparing: boolean;
  isCompleted: boolean;
  streamedText: string;
...
export default function ReferencialAula12 ()
{
  ...
  const handleAnaliseMotor = () => {
    // 1. Extração SOTA de Contexto Dinâmico
    const btnIdx = mPlayers.includes('BTN') ? mPlayers.indexOf('BTN') : 0;
    const bbIdx = mPlayers.includes('BB') ? mPlayers.indexOf('BB') : mPlayers.length - 1;
    const btnIdx = matrixData.mPlayers.includes('BTN') ? matrixData.mPlayers.indexOf('BTN') : 0;
    const bbIdx = matrixData.mPlayers.includes('BB') ? matrixData.mPlayers.indexOf('BB') : matrixData.mPlayers.length - 1;

    const heroStack = mStacks[btnIdx];
    const villainStack = mStacks[bbIdx];
    const currentRp = mRp[bbIdx][btnIdx]; // RP do BB contra o BTN
    const currentBf = mBf[bbIdx][btnIdx]; // Bubble Factor
    const heroStack = matrixData.mStacks[btnIdx];
    const villainStack = matrixData.mStacks[bbIdx];
    const currentRp = matrixData.mRp[bbIdx][btnIdx];
    const currentBf = matrixData.mBf[bbIdx][btnIdx];

    // 2. Transmutação para Linguagem Natural
    const prompt = `Cenário Estrutural: ${matrixView}
Board: K-J-T-2-3
BTN (Agressor): ${heroStack}bb | BB (Defensor): ${villainStack}bb
Risk Premium (BB vs BTN): ${currentRp}% | Multiplicador ICM (BF): ${currentBf}
Analise este embate.`;
    const prompt = `Cenário Estrutural: ${matrixView}\nBoard: K-J-T-2-3\nBTN (Agressor): ${heroStack}bb | BB (Defensor): ${villainStack}bb\nRisk Premium (BB vs BTN): ${currentRp}% | Multiplicador ICM (BF): ${currentBf}\nAnalise este embate.`;
    ...
    <div id="anchor-aula12" className="max-w-7xl mx-auto px-6">
      <details className="border-b border-white/5 group">
        ...
        <div className="flex flex-col gap-12 pb-12 pt-4">
          ...
          <RangesDuel />
          <IcmRulers matrixView={matrixView} setMatrixView={setMatrixView} matrixData={matrixData} />
          <ToyGamesFramework />
          <EngineSotaPanel
