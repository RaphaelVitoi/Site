...
function _buildDiagnostico( perspectivaPct: number, rioLiability: number, payjumpBonus: number, edgePenalty: number, investidoAcumulado: number | undefined, stackHero: number, kappa: number, valuation: number ): string {
  let diagnostico = perspectivaPct > 0 ? "Ação Soberana." : "Insolvência de Perspectiva.";
  if ( valuation < 0.5 ) diagnostico += " Paradoxo do Valuation: Risco desproporcional ao dano causado ao vilão.";
...
export function calculatePerspectivaVitoi( input: PerspectivaInput ): PerspectivaResult {
  ...
  // Diagnóstico
  const diagnostico = _buildDiagnostico( perspectivaPct, rioLiability, payjumpBonus, edgePenalty, input.investidoAcumulado, stackHero, kappa, valuation );
