...
function _calculateValuationAndRio( current: MapaICMResult, deltaWinPct: number, perspWin: MapaICMResult, villainIdx: number, totalPrizes: number, potSize: number, numPlayersInPot: number ) {
  ...
  const icmPerChip = currentVillainEq > 0 ? ( ( currentVillainEq / totalPrizes ) * 100 ) / ( current.totalChips / numPlayersInPot ) : 0; // Proxy de conversao
...
function _calculateFoldPressure( input: PerspectivaInput, stacksWin: number[], deltaFoldPct: number ) {
  const { prizes, heroIdx, stacks, isNearPayjump: _isNearPayjump = false, blindsRisingSoon: _blindsRisingSoon = false, currentEquityPct: _currentEquityPct = 0 } = input; // Contexto estendido
