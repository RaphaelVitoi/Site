...
export function calculatePerspectivaVitoi ( input: PerspectivaInput ): PerspectivaResult {
  ...
  const valuation = villainDeltaLoss > 0 ? deltaWinPct / ( villainDeltaLoss / totalPrizes * 100 ) : 1.0;
  ...
  const survivalPressure = isVacuum ? 0 : Math.min( 1.0, 1.0 / handsToBust );
  ...
  const tierBonus = ( !isVacuum && winTier !== currentTier ) ? 0.15 : 0.0;
  const fgsHealth = isVacuum ? 1.0 : ( 1.0 + tierBonus + ( survivalPressure * 0.2 ) );
  ...
  const edgePenalty = ( !isVacuum && isVillainShort && ratio > 3 ) ? 0.3 : 1.0;
  ...
  if ( input.investidoAcumulado && input.investidoAcumulado > ( stackHero * 0.3 ) ) diagnostico += " Alerta: Pot Entrapment Severo.";

  return {
    currentEquityPct, deltaWinPct, deltaLosePct,
