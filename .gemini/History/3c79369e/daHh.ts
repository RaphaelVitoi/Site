...
export interface PerspectivaInput {
  ...
  blindCost?: number;
  heroPosition?: string; // SOTA: Injeção de Antevisão Posicional
}
...
function _calculateValuationAndRio( current: MapaICMResult, deltaWinPct: number, perspWin: MapaICMResult, villainIdx: number, totalPrizes: number, potSize: number, numPlayersInPot: number ) {
  ...
  const villainDeltaLossPct = ( villainDeltaLoss / totalPrizes ) * 100;

  // SOTA Valuation: O "Câmbio Desleal" do Paradoxo do Valuation.
  // Extraído puramente para diagnóstico. Não multiplica o EV final para evitar dupla contagem da distorção ICM (já contida no deltaWinPct).
  const rawValuation = villainDeltaLossPct > 0 ? deltaWinPct / villainDeltaLossPct : 1;
  const valuation = Math.max( 0.1, Math.min( 5, rawValuation ) ); // Limite expandido para precisão de análise

  // SOTA RIO (Reverse Implied Odds): Escalonamento Quadrático (x^2) para Multiway
  // A dimensionalidade usa a % do pote sobre o total de fichas, refletindo o dano estrutural ao ecossistema.
  let rioLiability = 0;
  if ( numPlayersInPot > 2 && current.totalChips > 0 ) {
    const opponents = numPlayersInPot - 1;
    const p_d = 0.15; // Frequência de domínio estrutural
    const potTotalPct = ( potSize / current.totalChips ) * 100;
    rioLiability = Math.pow( opponents, 2 ) * p_d * potTotalPct;
  }
...
function _calculateFoldPressure( input: PerspectivaInput, stacksWin: number[], deltaFoldPct: number ) {
  const { prizes, heroIdx, stacks, isNearPayjump = false, blindsRisingSoon = false, currentEquityPct: _currentEquityPct = 0 } = input; // Contexto estendido
  ...
  // SOTA: Erosão Proporcional (Decaimento não ultrapassa o limite zero de equity)
  const erosionPenalty = ( blindsRisingSoon && !isVacuum ) ? Math.abs( deltaFoldPct * 0.5 ) + 0.1 : 0;
...
function _buildDiagnostico( params: Readonly<DiagnosticoParams> ): string {
  let diagnostico = params.perspectivaPct > 0 ? "Ação Soberana." : "Insolvência de Perspectiva.";
  if ( params.valuation < 0.5 ) diagnostico += " Paradoxo do Valuation: Risco desproporcional ao dano causado ao vilão.";
  if ( params.rioLiability > 1 ) diagnostico += " Alerta: Colapso Multiway.";
  if ( params.payjumpBonus > 0 ) diagnostico += " Laddering favorece o Fold.";
  if ( params.edgePenalty < 1 ) diagnostico += " Punição: Restaurando árvore do oponente.";
  if ( params.investidoAcumulado && params.investidoAcumulado > ( params.stackHero * 0.3 ) ) diagnostico += " Alerta: Pot Entrapment Severo.";
  if ( Math.abs( params.perspectivaPct ) <= 5 && params.kappa < 0.4 ) diagnostico += " Credibilidade Baixa: Intuição filtrada pelo Baseline Matemático.";
  return diagnostico;
}
...
export function calculatePerspectivaVitoi( input: PerspectivaInput ): PerspectivaResult {
  ...
  const { valuation, rioLiability } = _calculateValuationAndRio( current, deltaWinPct, perspWin, villainIdx, totalPrizes, potSize, numPlayersInPot );
  ...
  // A EQUAÇÃO UNIFICADA SOTA v4.1: PM = [(Equity * R) * Gain] + [(1 - Equity) * Loss] - [EV_fold + RIO_mw]
  // A variável 'valuation' atua apenas como métrica de observação do Paradoxo (não multiplica para não inflar a distorção já aplicada no ICMev).
  const expectativaReal = ( bayesianWinProb * deltaWinPct * R * fgsHealth ) + ( ( 1 - bayesianWinProb ) * deltaLosePct );
