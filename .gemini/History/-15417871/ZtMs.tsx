...
type TabId = 'theory' | 'ranges' | 'multiway' | 'bubble' | 'spr' | 'exploit' | 'quiz';
...
function calculateHandPower ( i: number, j: number, isSuited: boolean, isPair: boolean ): number {
  ...
  {
    power = ( high * 2.3 ) + ( low * 1.4 ) + 12;
    if ( isSuited ) power += 7;
    const gap = high - low;
    if ( gap === 1 ) power += 5;
    else if ( gap === 2 ) power += 3;
    else if ( gap === 3 ) power += 1;
    if ( high >= 10 && low >= 10 ) power += 3;
  }
...
function calculatePureEv ( power: number ): number {
  if ( power >= 75 ) return ( power - 70 ) * 0.15;
  if ( power >= 60 ) return ( power - 55 ) * 0.08;
  if ( power >= 50 ) return 0.05;
  return -0.5;
}
...
function calculateInsolvencyDelta ( power: number, isSuited: boolean, isPair: boolean, rpFactor: number, heroInvested: number, currentPot: number, activePlayers: number ): number {
  ...
  let rioVulnerability = 1.2; // Offsuit
  if ( isSuited ) rioVulnerability = 0.5;
  else if ( isPair ) rioVulnerability = 0.3;

  // SOTA FIX: Alinhamento Exato da Entropia Multiway (N² * p_d) com o rpDeriver.ts
  const N = activePlayers > 1 ? activePlayers - 1 : 1;
  const mwEntropy = activePlayers > 2 ? ( N * N * 0.15 ) : 0.1; // 0.1 baseline (10%) para HU
  const rioLiability = currentPot * rioVulnerability * mwEntropy;

  // 2. Teto de RP + Piso Dinâmico (Sunk Cost)
  const rpPenalty = ( rpFactor / 100 ) * currentPot;
  const evFold = -heroInvested;

  // A vulnerabilidade marginal decai proporcionalmente à Força (Power) da mão
  const marginality = Math.max( 0.05, ( 90 - power ) * 0.015 );
  return -( ( rioLiability + rpPenalty ) * marginality ) - evFold * 0.5;
}
...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  // SOTA P1 FIX: Arredondamento para 3 casas decimais para estabilizar dependências de useEffect
  ...
  const ipRp = Math.round( Number( ipRpRaw ) * 1000 ) / 1000;
  const oopRp = Math.round( Number( oopRpRaw ) * 1000 ) / 1000;
