  ...
  const matrixDataToRender = useMemo( () => {
    ...
    for ( let i = 0; i < ranks.length; i++ )
    {
      for ( let j = 0; j < ranks.length; j++ )
      {
        ...
        // Heurística de ChipEV (Vácuo / GTO)
        let pureEv = -0.5;
        if ( rankValue >= 16 ) pureEv = ( rankValue - 15 ) * 0.8;
        else if ( rankValue >= 13 && ( isSuited || isPair ) ) pureEv = ( rankValue - 12 ) * 0.3;
        else if ( rankValue >= 10 && isSuited ) pureEv = 0.1;

        // Passivo Sistêmico (Punição RIO e ICM)
        let insolvencyDelta = 0;
        if ( rpFactor > 0 && pureEv > -1 )
        {
          // Mãos conectadas/suited sofrem menos RIO, mãos offsuit off-board sofrem mais
          const rioVulnerability = isSuited ? 0.5 : ( isPair ? 0.3 : 1.2 );
          insolvencyDelta = -( rpFactor * 0.05 ) * rioVulnerability * Math.max( 0.5, ( 22 - rankValue ) * 0.15 );
        }

        fallback.push( { combo, pureEv, insolvencyDelta, isSuited, isPair } );
      }
    }
    return fallback;
