...
// SOTA: Extrator isolado para mitigar Complexidade Cognitiva
const generateMockMatrixData = (): NashCellData[] => {
  ...
  for ( let i = 0; i < ranks.length; i++ )
  {
    for ( let j = 0; j < ranks.length; j++ )
    {
      ...
      // SOTA (SonarLint S3358): Erradicação de ternários aninhados
      let combo = `${ranks[ j ]}${ranks[ i ]}o`;
      if ( isPair ) combo = `${ranks[ i ]}${ranks[ j ]}`;
      else if ( isSuited ) combo = `${ranks[ i ]}${ranks[ j ]}s`;

      // SOTA (SonarLint S7773): Uso explícito de Number.parseFloat
      const baseEv = ( ( 13 - i ) + ( 13 - j ) ) / 5 - ( isOffsuit ? 2.5 : 1.5 );
      const pureEv = Number.parseFloat( baseEv.toFixed( 2 ) );
      const insolvencyDelta = Number.parseFloat( ( -( Math.random() * ( pureEv > 0 ? 1.2 : 0.2 ) ) ).toFixed( 2 ) );
...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  const mockMatrixData = useMemo( () => generateMockMatrixData(), [ scenario.id ] );
