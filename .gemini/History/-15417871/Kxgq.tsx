      ...
      <div className={styles.animateFadeUp} key={`${scenario.id}-${activeTab}`}>
        ...
        {activeTab === 'quiz' && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            {/* Consome os dados do cenário local. Fornece array vazio se os dados estiverem legados. */}
            <QuizEngine questions={Array.isArray(scenario.quiz) ? scenario.quiz : ((scenario.quiz as any)?.questions || [])} />
          </div>
        )}
      </div>
    </div>
