...
/**
 * IDENTITY: Painel de Teoria com Tabs (SOTA v4.0)
 ...
 *       Visão Arquitetural SOTA com alta densidade de informação.
...
import type { Quiz, Scenario, SprStage } from '../engine/types';
...
function BubbleFactorDiagnostic ( { ipRp, oopRp }: { ipRp: number; oopRp: number } ) {
    ...
    <div className="space-y-6">
      <div className="p-4 rounded-xl bg-slate-900/50 border border-white/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-lg bg-indigo-950/20 border border-indigo-500/20 relative overflow-hidden">
          ...
          <div className="p-4 rounded-lg bg-rose-950/20 border border-rose-500/20 relative overflow-hidden">
...
export default function TheoryPanel ( { scenario, effectiveSprData }: Readonly<TheoryPanelProps> ) {
      ...
      .map( ( q: Record<string, unknown>, index: number ): NormalizedQuestion | null => {
        ...
        return {
          ...
          options: rawOptions.map( ( opt, i ) => ( {
            id: `opt-${i}`,
            label: typeof opt === 'string' ? opt : ( opt.label || opt.text || JSON.stringify( opt ) ),
          } ) ),
  ...
  return (
    <div className="glass-panel border border-white/5 rounded-2xl shadow-2xl shadow-slate-900/50 bg-[#0b0f19]/80 backdrop-blur-md flex flex-col transition-all duration-500">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-linear-to-r from-slate-900/80 to-transparent">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[0.65rem] text-slate-500 m-0 font-mono tracking-widest mt-0.5">ESTADO DA ARTE V4.0</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-6 flex flex-col min-h-87.5 transition-all duration-500">
        <div className="animate-fade-in w-full" key={ `${scenario.id}-${activeTab}` }>
          ...
          { activeTab === 'bubble' && (
            <BubbleFactorDiagnostic ipRp={ scenario.ipRp } oopRp={ scenario.oopRp } />
          ) }
