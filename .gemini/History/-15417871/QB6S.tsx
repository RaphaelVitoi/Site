...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  }, [ activeTab, scenario.id, ipRp, oopRp, spotContext?.heroInvested, spotContext?.spotData?.pot, spotContext?.activePlayers, spotContext?.spotData?.heroRole, wasmActions, isDynamicRpActive ] );
  ...
  {
    const pot = spotContext.spotData.pot;
    const costToCall = Math.abs( spotContext.actionMetrics.fold.chipEv );
    derivedPotOddsPct = ( pot + costToCall ) > 0
      ? ( costToCall / ( pot + costToCall ) ) * 100
      : 33;
  }
