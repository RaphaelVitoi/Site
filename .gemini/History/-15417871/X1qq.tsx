...
function calculateInsolvencyDelta ( rankValue: number, isSuited: boolean, isPair: boolean, rpFactor: number, heroInvested: number, currentPot: number, activePlayers: number ): number {
  ...
  // O Delta de Insolvência é a gravidade que pune mãos marginais (RIO + RP contra a âncora do Sunk Cost)
  const marginality = Math.max( 0.1, ( 22 - rankValue ) * 0.15 );
...
function calculateFallbackCell ( i: number, j: number, rpFactor: number, ranks: string[], heroInvested: number, currentPot: number, activePlayers: number ) {
  ...
  const insolvencyDelta = calculateInsolvencyDelta( rankValue, isSuited, isPair, rpFactor, heroInvested, currentPot, activePlayers );
