...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  useEffect( () => {
    if ( activeTab === 'ranges' && wasmActions?.dispatchInsolvencyMatrix )
    {
      ...
      // SOTA: O filtro dinâmico engatilha a distorção. Se inativo, submetemos 0 para revelar o vácuo GTO (ChipEV puro).
  ...
  const matrixDataToRender = useMemo( () => {
    ...
    // Se o filtro estiver desativado, o fallback também deve refletir o ChipEV
    return generateFallbackMatrix( isDynamicRpActive ? ipRp : 0, isDynamicRpActive ? oopRp : 0 );
  }, [ wasmState?.insolvencyMatrixData, ipRp, oopRp, isDynamicRpActive ] );
