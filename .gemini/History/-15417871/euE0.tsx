...
export default function TheoryPanel({ scenario }: Readonly<TheoryPanelProps>) {
  ...
  const normalizeQuizData = (quizData: any) => {
    if (!quizData) return [];
    // Formato novo já convertido ou array existente
    ...
    // Formato legado provável: objeto único com question, options[], correctAnswer e explanation
    if (quizData.question && Array.isArray(quizData.options)) {
      return [{
        id: `legacy-${scenario.id}`,
        text: quizData.question,
        options: quizData.options.map((opt: any, i: number) => ({
          id: `opt-${i}`,
          label: typeof opt === 'string' ? opt : (opt.label || String(opt))
        })),
        correctOptionId: `opt-${quizData.correctAnswer ?? 0}`,
