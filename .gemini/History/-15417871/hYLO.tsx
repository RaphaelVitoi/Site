...
import { NashConvergenceMatrix, type NashCellData } from '../../TheoryPanel/NashConvergenceMatrix';
...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  // SOTA: Gerador de dados mock para a Matriz de Convergência.
  // Substituir pela chamada ao Web Worker que consome o WASM quando a Fase 3.1 estiver completa.
  const mockMatrixData = useMemo( (): NashCellData[] => {
    const ranks = [ 'A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2' ];
    const data: NashCellData[] = [];
    for ( let i = 0; i < ranks.length; i++ )
    {
      for ( let j = 0; j < ranks.length; j++ )
      {
        const isPair = i === j;
        const isSuited = !isPair && i < j;
        const isOffsuit = !isPair && i > j;
        const combo = isPair ? `${ranks[ i ]}${ranks[ j ]}` : ( isSuited ? `${ranks[ i ]}${ranks[ j ]}s` : `${ranks[ j ]}${ranks[ i ]}o` );

        // Lógica de simulação para EV e Insolvência
        const baseEv = ( ( 13 - i ) + ( 13 - j ) ) / 5 - ( isOffsuit ? 2.5 : 1.5 );
        const pureEv = parseFloat( baseEv.toFixed( 2 ) );
        const insolvencyDelta = parseFloat( ( -( Math.random() * ( pureEv > 0 ? 1.2 : 0.2 ) ) ).toFixed( 2 ) );

        data.push( { combo, pureEv, insolvencyDelta, isPair, isSuited } );
      }
    }
    return data;
  }, [ scenario.id ] ); // Recalcula se o cenário mudar
            ...
            <div className="h-full space-y-4">
              <div className="flex items-center justify-between gap-3 mb-4 p-3 rounded-lg bg-slate-900/50 border border-white/5">
                ...
                <div className="relative inline-block w-10 align-middle select-none transition duration-200 ease-in">
                  ...
                  />
                  <label htmlFor="rp-toggle" className="toggle-label block overflow-hidden h-6 rounded-full bg-zinc-700 cursor-pointer"></label>
                </div>
              </div>
              <NashConvergenceMatrix matrixData={ mockMatrixData } isDynamicRpActive={ isDynamicRpActive } />
            </div>
          ) }
