...
import { SotaMetricsContext, SotaSpotContext, SotaWasmActionsContext, SotaWasmStateContext } from '../SotaContext';
import QuantumSynthesis from '../ui/QuantumSynthesis';
import SprPipeline from '../ui/SprPipeline';
import BayesianRegressionPanel from './BayesianRegressionPanel';
import FgsDegradationTimeline from './FgsDegradationTimeline';
import MultiwayTensionVisualizer from './MultiwayTensionVisualizer';
...
type TabId = 'theory' | 'ranges' | 'multiway' | 'bayes' | 'fgs' | 'bubble' | 'spr' | 'exploit' | 'quiz';

const TABS: { id: TabId; label: string; icon: string }[] = [
  { id: 'theory', label: 'Fundamento', icon: 'fa-book-journal-whills' },
  { id: 'ranges', label: 'Ranges SOTA', icon: 'fa-border-all' },
  { id: 'multiway', label: 'Topologia MW', icon: 'fa-users-rays' },
  { id: 'bayes', label: 'Incerteza', icon: 'fa-microscope' },
  { id: 'fgs', label: 'Erosão (FGS)', icon: 'fa-clock-rotate-left' },
  ...
  { id: 'quiz', label: 'Validação', icon: 'fa-vial' },
];
...
// SOTA: Heurística de Força Unificada. Mantém simetria matemática com a RangeMatrix.
function calculateHandPower ( i: number, j: number, isSuited: boolean, isPair: boolean ): number {
  const v1 = 14 - i;
  const v2 = 14 - j;
  ...
  if ( isPair )
  {
    power = 45 + ( high * 3.5 ); // Pares escalam rápido
  } else
  {
    power = ( high * 2.5 ) + ( low * 1.5 ) + 10;
    if ( isSuited ) power += 8;
    const gap = high - low;
    if ( gap === 1 ) power += 6; // Connectors
    else if ( gap === 2 ) power += 3; // One-gappers
    if ( high >= 10 && low >= 10 ) power += 5; // Broadway bonus
  }
...
function calculatePureEv ( power: number ): number {
  // Escala de BB: Mãos premium > 1bb, Mãos jogáveis ~0.2bb, Lixo < -0.5bb
  if ( power >= 80 ) return ( power - 75 ) * 0.25;
  if ( power >= 60 ) return ( power - 55 ) * 0.12;
  if ( power >= 45 ) return 0.05;
  return -0.6;
}

// SOTA: Passivo Sistêmico isolado para previsibilidade matemática
function calculateInsolvencyDelta ( power: number, isSuited: boolean, isPair: boolean, rpFactor: number, heroInvested: number, currentPot: number, activePlayers: number ): number {
  if ( rpFactor <= 0 && heroInvested <= 0 ) return 0; // Vácuo ChipEV Absoluto

  // 1. Teoria Sistêmica: RIO Exponencial em Potes Multiway (N²)
  let rioVulnerability = 1.4; // Offsuit (mais frágil a RIO)
  if ( isSuited ) rioVulnerability = 0.6;
  else if ( isPair ) rioVulnerability = 0.4;

  const N = activePlayers > 1 ? activePlayers - 1 : 1;
  const mwEntropy = activePlayers > 2 ? ( N * N * 0.2 ) : 0.15;
  const rioLiability = currentPot * rioVulnerability * mwEntropy;

  // 2. Teto de RP + Piso Dinâmico (Sunk Cost)
  const rpPenalty = ( rpFactor / 100 ) * currentPot * 1.5; // Inflação de colisão
  const evFold = -heroInvested;

  // A vulnerabilidade marginal aumenta conforme a mão enfraquece
  const marginality = Math.max( 0.05, ( 95 - power ) * 0.02 );

  // SOTA v4.2: O Delta é a gravidade que puxa o EV para baixo
  return -( ( rioLiability + rpPenalty ) * marginality ) + ( evFold * 0.2 );
}

// SOTA: Avaliação vetorial de uma célula
function calculateFallbackCell ( i: number, j: number, rpFactor: number, ranks: string[], heroInvested: number, currentPot: number, activePlayers: number ) {
  const isPair = i === j;
  const isSuited = j > i;
  ...
  const power = calculateHandPower( i, j, isSuited, isPair );
  const pureEv = calculatePureEv( power );
  const insolvencyDelta = calculateInsolvencyDelta( power, isSuited, isPair, rpFactor, heroInvested, currentPot, activePlayers );
...
function generateFallbackMatrix ( ipRp: number, oopRp: number, heroInvested: number, currentPot: number, activePlayers: number ) {
  ...
  for ( let i = 0; i < ranks.length; i++ )
  {
    ...
    {
      fallback.push( calculateFallbackCell( i, j, rpFactor, ranks, heroInvested, currentPot, activePlayers ) );
    }
  }
  return fallback;
}
...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  // SOTA P1 FIX: Arredondamento para 4 casas decimais para sincronia total com o motor quântico
  const ipRpRaw = spotContext?.effectiveIpRp ?? effectiveIpRp ?? scenario.ipRp;
  const oopRpRaw = spotContext?.effectiveOopRp ?? effectiveOopRp ?? scenario.oopRp;

  const ipRp = Math.round( Number( ipRpRaw ) * 10000 ) / 10000;
  const oopRp = Math.round( Number( oopRpRaw ) * 10000 ) / 10000;
  ...
  // SOTA Audit: O useEffect agora depende estritamente das ações e dados de input, nunca do status de loading.
  useEffect( () => {
    if ( activeTab === 'ranges' && wasmActions?.dispatchInsolvencyMatrix )
    {
      // SOTA: Vácuo ChipEV Absoluto. Se o filtro está OFF, erradicamos TODAS as tensões estruturais,
      // restaurando EV_fold para 0 e diluindo a entropia de múltiplos jogadores para o GTO natural.
      const heroInvested = isDynamicRpActive ? ( spotContext?.heroInvested ?? 1 ) : 0;
      const currentPot = spotContext?.spotData?.pot ?? 2.5;
      const activePlayers = isDynamicRpActive ? ( spotContext?.activePlayers ?? 2 ) : 2;

      // SOTA P1 FIX: Usa o RP correto baseado na posição do Hero para os dados fazerem sentido
      const isHeroIp = spotContext?.spotData?.heroRole?.includes( 'IP' );
      const heroRp = isHeroIp ? ipRp : oopRp;
      const rpFactor = isDynamicRpActive ? heroRp : 0;

      const villainRange = "100%";
      const board = "";

      wasmActions.dispatchInsolvencyMatrix( villainRange, board, rpFactor, heroInvested, currentPot, activePlayers );
    }
  }, [ activeTab, scenario.id, ipRp, oopRp, spotContext?.heroInvested, spotContext?.spotData?.pot, spotContext?.activePlayers, spotContext?.spotData?.heroRole, wasmActions?.dispatchInsolvencyMatrix, isDynamicRpActive ] );
  ...
  const matrixDataToRender = useMemo( () => {
    if ( wasmState?.insolvencyMatrixData && wasmState.insolvencyMatrixData.length > 0 ) return wasmState.insolvencyMatrixData;

    // Sincronia térmica da Teoria Sistêmica para o visualizador
    const heroInvested = isDynamicRpActive ? ( spotContext?.heroInvested ?? 1 ) : 0;
    const currentPot = spotContext?.spotData?.pot ?? 2.5;
    const activePlayers = isDynamicRpActive ? ( spotContext?.activePlayers ?? 2 ) : 2;
    ...
    let fallbackRp = 0;
    if ( isDynamicRpActive )
    {
      fallbackRp = isHeroIp ? ipRp : oopRp;
    }

    return generateFallbackMatrix( fallbackRp, fallbackRp, heroInvested, currentPot, activePlayers );
  }, [ wasmState?.insolvencyMatrixData, ipRp, oopRp, isDynamicRpActive, spotContext?.heroInvested, spotContext?.spotData?.pot, spotContext?.activePlayers, spotContext?.spotData?.heroRole ] );


  // SOTA: Fallback de Pot Odds (Erradicação de Ternário Aninhado - SonarLint S3358)
  let derivedPotOddsPct = 33;
  if ( spotContext?.potOddsPct !== undefined )
  {
    derivedPotOddsPct = spotContext.potOddsPct;
  } else if ( spotContext?.spotData && spotContext?.actionMetrics?.fold )
  {
    const fallbackPot = spotContext.spotData.pot;
    const fallbackSunkCost = Math.abs( spotContext.actionMetrics.fold.chipEv );
    derivedPotOddsPct = ( fallbackPot + fallbackSunkCost ) > 0
      ? ( fallbackPot / ( fallbackPot + fallbackSunkCost ) ) * 100
      : 33;
  }
  ...
  return (
    <div className="glass-panel border border-white/5 rounded-2xl shadow-2xl shadow-slate-900/50 bg-(--bg-deep)/80 backdrop-blur-md flex flex-col transition-all duration-500">
      ...
      {/* Summary Visual de Colisão SOTA */ }
      <div className="px-6 py-2">
        <QuantumSynthesis
          scenarioName={ scenario.name || 'Simulação Analítica' }
          verdict={ isDynamicRpActive ? 'Motor Quântico Ativo' : 'Baseline GTO' }
          ipRp={ ipRp }
          oopRp={ oopRp }
          isNearPayjump={ scenario.isNearPayjump ?? false }
          blindsRisingSoon={ scenario.blindsRisingSoon ?? false }
          isVacuum={ ipRp === 0 && oopRp === 0 }
        />
      </div>
        ...
        <div className="animate-fade-in w-full" key={ `${scenario.id}-${activeTab}` }>
          ...
          { activeTab === 'ranges' && (
            <div className="h-full space-y-4">
              ...
              { wasmState?.isCalculatingInsolvency ? (
          ...
          {/* TAB: MULTIWAY TENSION (O ABISMO VISUAL) */ }
          { activeTab === 'multiway' && (
            <MultiwayTensionVisualizer />
          ) }

          {/* TAB: INCERTEZA BAYESIANA */ }
          { activeTab === 'bayes' && (
            <BayesianRegressionPanel />
          ) }

          {/* TAB: EROSÃO TEMPORAL (FGS) */ }
          { activeTab === 'fgs' && (
            <FgsDegradationTimeline />
          ) }
            ...
            <div className="space-y-6">
              ...
              { metricsContext?.quantumPerspectiva && spotContext?.spotData && spotContext?.actionMetrics && (
                <DegradationChart
                  ...
                  // SOTA: Consumo da Store Global purificado via variável independente
                  potOddsPct={ derivedPotOddsPct }
                  // SOTA: Erradicação de ternários. Mapeado para a Tensão Dinâmica (RIO) do Call.
                  potSizePct={ ( spotContext.actionMetrics.call?.tension ?? 0.1 ) * 100 }
                />
              ) }
            </div>
          ) }
