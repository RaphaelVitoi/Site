...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> ) {
  ...
  // SOTA P1 FIX: Arredondamento para 2 casas decimais para estabilizar dependências de useEffect
  ...
  const ipRp = Math.round( Number( ipRpRaw ) * 100 ) / 100;
  const oopRp = Math.round( Number( oopRpRaw ) * 100 ) / 100;
  ...
  useEffect( () => {
    if ( activeTab === 'ranges' && wasmActions?.dispatchInsolvencyMatrix )
    {
      ...
      const rpFactor = isDynamicRpActive ? Math.max( ipRp, oopRp ) : 0;
  ...
  }, [ activeTab, scenario.id, ipRp, oopRp, spotContext?.heroInvested, spotContext?.spotData?.pot, spotContext?.activePlayers, wasmActions?.dispatchInsolvencyMatrix, isDynamicRpActive ] );
  ...
  const matrixDataToRender = useMemo( () => {
    ...
    const currentPot = spotContext?.spotData?.pot ?? 2.5;
    const activePlayers = isDynamicRpActive ? ( spotContext?.activePlayers ?? 2 ) : 2;
    const fallbackRp = isDynamicRpActive ? Math.max( ipRp, oopRp ) : 0;

    return generateFallbackMatrix( fallbackRp, fallbackRp, heroInvested, currentPot, activePlayers );
  }, [ wasmState?.insolvencyMatrixData, ipRp, oopRp, isDynamicRpActive, spotContext?.heroInvested, spotContext?.spotData?.pot, spotContext?.activePlayers ] );
