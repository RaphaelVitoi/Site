...
# Carregamento robusto do Kernel para evitar erro de path nulo
$KernelPath = if ($Global:AgentPaths -and $Global:AgentPaths.Kernel) { $Global:AgentPaths.Kernel } else { Join-Path $script:ProjectRoot "Agent-TaskManager.psm1" }
...
3.  **Contagem de Agentes:** Valide o numero total de entidades (atualmente 18: Incluindo o @architect como guardiao estrutural).
