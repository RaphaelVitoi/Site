...
const Header = () => {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-bg-base/80 border-b border-white/5 transition-all duration-300 shadow-[0_4px_30px_rgba(0,0,0,0.4)]">
        <div className="sota-container h-20 flex items-center justify-between gap-8">
            ...
            <ul className="flex items-center gap-1">
              { [
                ...
                <li key={ item.href }>
                  <Link
                    href={ item.href }
                    className="px-3 xl:px-4 py-2 text-[0.7rem] font-black uppercase tracking-widest text-text-muted hover:text-accent-indigo-light hover:bg-white/5 rounded-lg transition-all flex items-center gap-2 whitespace-nowrap"
                  >
              ...
              <li>
                <Link href="/templo/analytics" className="px-3 xl:px-4 py-2 text-[0.7rem] font-black uppercase tracking-widest text-accent-rose hover:text-accent-rose-light hover:bg-accent-rose/10 rounded-lg transition-all flex items-center gap-2 whitespace-nowrap">
          ...
          <div className="flex items-center gap-4">
            <Link href="/simulador" className="hidden sm:flex btn-primary px-6 py-2.5 rounded-xl text-[0.65rem] font-black tracking-widest shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:-translate-y-0.5 transition-all">
              LAUNCH ENGINE
            </Link>
            <button
      ...
      <div className={ `mobile-nav backdrop-blur-xl bg-slate-950/95 ${mobileOpen ? 'open' : ''}` }>
        ...
        <Link href="/biblioteca/estado-da-arte-2025" onClick={ closeMobile }>
          <i className="fa-solid fa-lightbulb" /> Estado da Arte 2025
        </Link>
        <Link href="/biblioteca/protocolo-smart-sniper" onClick={ closeMobile }>
          <i className="fa-solid fa-bullseye" /> Protocolo Smart Sniper
        </Link>
        <Link href="/biblioteca/psicologia-high-stakes" onClick={ closeMobile }>
