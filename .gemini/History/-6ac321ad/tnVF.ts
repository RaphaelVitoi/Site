    ...
    try
    {
        ...
        try
        {
            ...
            if ( response.ok )
            {
                ...
                if ( data.status === 'SUCCESS' ) return createSSEStream( data.answer, 'llm' );
            }
        ...
        // Blindagem Anti-Deadlock: timeout de 15s previne o congelamento da interface se o DB estiver em lock.
        const { stdout } = await execFileAsync( PYTHON_CMD, [ SCRIPT_PATH, 'query', question ], { timeout: 15000 } );

        const answer = stdout.trim() || 'A Mente Coletiva não encontrou fragmentos relevantes.';
        return createSSEStream( answer, 'context_only' );
    } catch ( error: unknown )
    {
