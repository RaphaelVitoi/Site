...
function assertSafeByokBaseUrl ( input: string ): URL {
    ...
    {
        throw new Error( 'Credenciais embutidas na URL nao sao permitidas.' );
    }

    if ( url.protocol !== 'https:' )
    {
        const allowDevHttp = isLocalDev && ( hostname.toLowerCase() === 'localhost' || hostname.startsWith( '127.' ) );
        if ( !allowDevHttp )
        {
            throw new Error( 'A URL customizada deve usar HTTPS.' );
        }
    }

    if ( isPrivateHost && !( isLocalDev && ( hostname.toLowerCase() === 'localhost' || hostname.startsWith( '127.' ) ) ) )
    {
        throw new Error( 'Destinos locais, privados ou de loopback nao sao permitidos no modo BYOK.' );
    }
...
async function fetchByokAnswer ( contextStr: string, question: string, customApiKey: string, customBaseUrl: string, customModelName: string ): Promise<string> {
    const safeBaseUrl = assertSafeByokBaseUrl( customBaseUrl );
    const targetUrl = `${safeBaseUrl.toString().replace( /\/$/, '' )}/chat/completions`;
    ...
    if ( byokRes.ok )
    {
        ...
        return byokData.choices[ 0 ]?.message?.content || contextStr;
    }
    console.warn( '[ORACULO] BYOK retornou status nao exitoso:', byokRes.status );
    return contextStr;
}
...
export async function POST ( request: Request ) {
    try
    {
        ...
        {
            finalAnswer = await fetchByokAnswer( contextStr, question, customApiKey, customBaseUrl, customModelName );
        }
