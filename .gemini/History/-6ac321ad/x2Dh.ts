...
export async function POST ( req: NextRequest ) {
    try
    {
        ...
        try
        {
            ...
            // SOTA: Bypass de Isolamento Cloud. Impede a invocação do Python no Serverless.
            if ( process.env.NEXT_PUBLIC_IS_VERCEL === 'true' || process.env.NEXT_PUBLIC_IS_VERCEL === '1' )
            {
                ragContext = '[MODO CLOUD SOTA] Motor Python e RAG Local suprimidos para preservar a Homeostase Serverless. Inferência operando via conhecimento inerente.';
                console.log( '[Oráculo Híbrido] Bypass Vercel ativo. Child Process bloqueado.' );
            } else
            {
