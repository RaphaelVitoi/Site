...
export default function ComparisonRadar ( { scenarios, currentId, nashFlop }: Readonly<ComparisonRadarProps> ) {
  ...
  const radarElement = useMemo( () => (
    <ResponsiveContainer width="100%" height={ 300 } minWidth={ 1 } minHeight={ 1 } initialDimension={ INITIAL_CHART_DIMENSION }>
  ...
  ), [ radarData, currentScenario?.name, compareScenario?.name ] );
