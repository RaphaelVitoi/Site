...
foreach ($Agent in $AgentNames) {
    ...
    $Model = if ($AgentProps.primary_model) { $AgentProps.primary_model } else { 'meta-llama/llama-3.1-8b-instruct' }
    ...
    [System.IO.File]::WriteAllText($AgentDocPath, $Template, $Utf8NoBom)
    ...
    if (-not (Test-Path $MemoryPath)) {
        ...
        [System.IO.File]::WriteAllText($MemoryPath, $MemTemplate, $Utf8NoBom)
