...
const StreetDashboards = ({ ipRp, oopRp, current }: { ipRp: number, oopRp: number, current: any }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
    <div className="flex flex-col gap-3 p-6 bg-slate-900/40 rounded-4xl border border-white/5 shadow-inner hover:border-accent-indigo/30 hover:bg-slate-900/60 transition-all duration-500 group/ip">
      <div className="flex justify-between items-center px-1">
          <span className="text-[0.55rem] text-text-darker uppercase font-black tracking-[0.4em] group-hover/ip:text-accent-indigo-light transition-colors">Pressão Agressor (IP)</span>
      ...
      <div className="flex items-baseline gap-2">
        <span className={`text-3xl font-black font-mono tabular-nums tracking-tighter text-white ${current.textShadowClass}`}>{ ipRp.toFixed( 1 ) }</span>
        <span className="text-[0.65rem] font-black text-text-darker uppercase tracking-widest">RP %</span>
      </div>
    ...
    <div className="flex flex-col gap-3 p-6 bg-slate-900/40 rounded-4xl border border-white/5 shadow-inner hover:border-accent-amber/30 hover:bg-slate-900/60 transition-all duration-500 group/oop md:items-end md:text-right">
      <div className="flex flex-row-reverse md:flex-row justify-between items-center px-1 w-full">
          ...
          <span className="text-[0.55rem] text-text-darker uppercase font-black tracking-[0.4em] group-hover/oop:text-accent-amber transition-colors">Pressão Defensor (OOP)</span>
      </div>
      <div className="flex items-baseline gap-2">
        <span className="text-3xl font-black font-mono tabular-nums tracking-tighter text-accent-amber [text-shadow:0_0_20px_rgba(245,158,11,0.2)]">{ oopRp.toFixed( 1 ) }</span>
        <span className="text-[0.65rem] font-black text-text-darker uppercase tracking-widest">RP %</span>
      </div>
...
const ActionStrategies = ({ current, activeStreet, onStreetFreqChange }: { current: any, activeStreet: keyof StreetChipEvFreqs, onStreetFreqChange: (s: keyof StreetChipEvFreqs, f: ChipEvFreqs) => void }) => (
...
const EntropyModulators = ({ aggressionFactor, pkoValue, onAggressionChange, onPkoChange }: { aggressionFactor: number, pkoValue: number, onAggressionChange: (v: number) => void, onPkoChange: (v: number) => void }) => (
...
export default function NashPanel ( {
  ...
  const streetData = {
    flop: { nash: nashFlop, freqs: streetFreqs.flop, rps: streetRps.flop, label: 'FLOP', color: 'var(--color-accent-indigo-light)', bgClass: 'bg-accent-indigo-light', shadowClass: 'shadow-[0_15px_30px_-10px_rgba(99,102,241,0.2)]', textShadowClass: '[text-shadow:0_0_20px_rgba(99,102,241,0.4)]' },
    turn: { nash: nashTurn, freqs: streetFreqs.turn, rps: streetRps.turn, label: 'TURN', color: 'var(--color-accent-emerald)', bgClass: 'bg-accent-emerald', shadowClass: 'shadow-[0_15px_30px_-10px_rgba(16,185,129,0.2)]', textShadowClass: '[text-shadow:0_0_20px_rgba(16,185,129,0.4)]' },
    river: { nash: nashRiver, freqs: streetFreqs.river, rps: streetRps.river, label: 'RIVER', color: 'var(--color-accent-danger)', bgClass: 'bg-accent-danger', shadowClass: 'shadow-[0_15px_30px_-10px_rgba(244,63,94,0.2)]', textShadowClass: '[text-shadow:0_0_20px_rgba(244,63,94,0.4)]' },
  };
  ...
  return (
    <div className="glass-panel flex flex-col gap-10 p-8 sm:p-10 lg:p-14 transition-all duration-700 rounded-4xl bg-bg-panel/80 backdrop-blur-3xl border border-white/10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.7)] relative overflow-hidden group/nash">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start border-b border-white/5 pb-8 gap-6 relative z-10">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
             <div className="w-2 h-2 rounded-full bg-accent-indigo shadow-[0_0_15px_var(--accent-indigo)] animate-pulse" />
             <h3 className="m-0 text-[0.75rem] font-black text-white uppercase tracking-[0.4em]">
        ...
        <div className="flex items-center gap-3">
            <span className="text-[0.5rem] font-black text-text-darker uppercase tracking-[0.3em]">Instabilidade δ</span>
            <div className={`px-4 py-2 rounded-xl bg-slate-950/80 border border-white/10 text-[0.8rem] font-black font-mono tabular-nums whitespace-nowrap shadow-2xl flex items-center gap-2 transition-colors ${deltaRp > 0 ? 'text-accent-amber' : 'text-accent-emerald'}`}>
              <div className={`w-1 h-1 rounded-full ${deltaRp > 0 ? 'bg-accent-amber animate-pulse' : 'bg-accent-emerald'}`} />
      ...
      {/* Toggles Táticos */}
      <div id="quantum-controls" className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
        <button
            onClick={ () => onPayjumpToggle( !isNearPayjump ) }
            aria-pressed={isNearPayjump ? 'true' : 'false'}
            className={`py-4 px-6 rounded-2xl text-[0.65rem] font-black uppercase tracking-[0.25em] transition-all duration-500 cursor-pointer border flex items-center justify-center gap-3 group/btn active:scale-95 shadow-2xl ${isNearPayjump ? 'bg-accent-emerald/10 border-accent-emerald/40 text-accent-emerald shadow-emerald-500/10' : 'bg-slate-900/40 border-white/5 text-text-muted hover:bg-slate-900/60 hover:border-white/20'}`}
        >
          <div className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${isNearPayjump ? 'bg-accent-emerald shadow-[0_0_12px_var(--accent-emerald)] scale-110' : 'bg-text-darker group-hover/btn:bg-text-muted'}`} />
          { isNearPayjump ? 'Payjump Iminente' : 'Salto de Prêmios' }
        </button>

        <button
            onClick={ () => onBlindsToggle( !blindsRisingSoon ) }
            aria-pressed={blindsRisingSoon ? 'true' : 'false'}
            className={`py-4 px-6 rounded-2xl text-[0.65rem] font-black uppercase tracking-[0.25em] transition-all duration-500 cursor-pointer border flex items-center justify-center gap-3 group/btn active:scale-95 shadow-2xl ${blindsRisingSoon ? 'bg-accent-danger/10 border-accent-danger/40 text-accent-danger shadow-rose-500/10' : 'bg-slate-900/40 border-white/5 text-text-muted hover:bg-slate-900/60 hover:border-white/20'}`}
        >
          <div className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${blindsRisingSoon ? 'bg-accent-danger shadow-[0_0_12px_var(--accent-danger)] animate-pulse scale-110' : 'bg-text-darker group-hover/btn:bg-text-muted'}`} />
      ...
      <div className="flex gap-3 p-1.5 bg-slate-950/60 rounded-3xl border border-white/5 shadow-inner relative z-10 overflow-x-auto scrollbar-hide">
        { ( [ 'flop', 'turn', 'river' ] as const ).map( s => {
          ...
          const inactiveClasses = 'bg-transparent border-transparent text-text-darker opacity-40 hover:opacity-100 hover:text-text-muted';
          ...
          return (
            ...
            >
              <span className={`text-[0.7rem] font-black uppercase tracking-[0.25em] ${isActive ? 'text-white' : 'text-text-darker'}`}>{ d.label }</span>
              <div className="flex items-center gap-1.5">
                  <div className={`w-1 h-1 rounded-full ${d.bgClass}`} />
                  <span className={`text-[0.55rem] font-mono font-black tabular-nums tracking-tighter ${isActive ? 'text-text-muted' : 'text-text-darker'}`}>RP { avgRp.toFixed( 1 ) }%</span>
              </div>
            </button>
          );
        } ) }
      ...
      <EntropyModulators aggressionFactor={aggressionFactor} pkoValue={pkoValue} onAggressionChange={onAggressionChange} onPkoChange={onPkoChange} />
