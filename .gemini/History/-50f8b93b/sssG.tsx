    ...
    <div className="flex flex-col gap-3 p-6 bg-slate-900/40 rounded-4xl border border-white/5 shadow-inner hover:border-accent-indigo/30 hover:bg-slate-900/60 transition-all duration-500 group/ip">
      ...
      <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden mt-1">
        <motion.div
          ...
          {...({
            className:
              "h-full bg-accent-indigo shadow-[0_0_10px_var(--accent-indigo)]",
          } as any)}
        />
      </div>
    </div>
    ...
    <div className="flex flex-col gap-3 p-6 bg-slate-900/40 rounded-4xl border border-white/5 shadow-inner hover:border-accent-amber/30 hover:bg-slate-900/60 transition-all duration-500 group/oop md:items-end md:text-right">
      ...
      <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden mt-1 flex justify-end">
        <motion.div
          ...
          {...({
            className:
              "h-full bg-accent-amber shadow-[0_0_10px_var(--accent-amber)]",
          } as any)}
        />
      </div>
    </div>
...
export default function NashPanel({
  ...
  const handleConsultGemma = () => {
    const prompt = `> SYSTEM: Atue como Arquiteto de Teoria dos Jogos SOTA. Foco na Distorção de Nash.
> DATA: Street: ${current.label} | IP RP: ${ipRp.toFixed(1)}% | OOP RP: ${oopRp.toFixed(1)}% | Agressão (Fator Ψ): ${aggressionFactor} | PKO Bounty: ${pkoValue}
> PROFILE: ${JSON.stringify(predictiveProfile || {})}
> TASK: Forneça uma análise visceral (máx 200 palavras) explicando o desvio da estratégia GTO pura. Como as pressões assimétricas do ICM e a telemetria do jogador justificam essa topologia de frequências (Check/Bet/Fold)? Use formatação avançada.`;
    generateAnalysis(prompt, 512, "auto", undefined, predictiveProfile);
  };
      ...
      <div className="mt-6 pt-10 border-t border-white/5 relative z-10">
        <div className="flex items-center justify-between mb-8">
          <h4 className="text-[0.8rem] font-black text-white uppercase tracking-[0.3em] flex items-center gap-3">
            <i className="fa-solid fa-microchip text-accent-indigo"></i>
            Análise Preditiva (Gemma Edge)
          </h4>
          <button
          ...
          >
            {isStreaming ? (
              <>
                <i className="fa-solid fa-atom animate-spin"></i> Processando...
              </>
            ) : (
              <>
                <i className="fa-solid fa-radar"></i> Injetar Telemetria
              </>
            )}
          </button>
        </div>
