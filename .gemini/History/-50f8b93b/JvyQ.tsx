        ...
        { ( [ 'flop', 'turn', 'river' ] as const ).map( s => {
          ...
          return (
            <button
                ...
                className={`flex-1 flex flex-col items-center gap-1.5 py-4 px-3 rounded-2xl border transition-all duration-700 ease-out cursor-pointer min-w-28 ${isActive ? `bg-slate-900/90 border-white/10 -translate-y-1 scale-[1.02] ${d.shadowClass}` : 'bg-transparent border-transparent text-text-darker opacity-40 hover:opacity-100 hover:text-text-muted'}`}
            >
