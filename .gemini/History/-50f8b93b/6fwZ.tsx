      ...
      <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-8 transition-all hover:bg-slate-900/60 hover:border-accent-indigo/30 group/Ψ shadow-inner relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-8 px-1 relative z-10">
          <div className="space-y-1">
            <span className="text-[0.7rem] font-black text-text-muted uppercase tracking-[0.3em] group-hover/Ψ:text-white transition-colors">
              Modulador Ψ
            </span>
        ...
        <input
          ...
          onChange={(e) =>
            onAggressionChange(Number.parseFloat(e.target.value))
          }
...
export default function NashPanel({
  ...
  const [activeStreet, setActiveStreet] = useState<"flop" | "turn" | "river">(
    "flop",
  );
  ...
  const payjumpAria = { "aria-pressed": isNearPayjump ? "true" : "false" };
  const blindsAria = { "aria-pressed": blindsRisingSoon ? "true" : "false" };
  ...
  return (
    <div className="glass-panel flex flex-col gap-10 p-8 sm:p-10 lg:p-14 transition-all duration-700 rounded-4xl bg-bg-panel/80 backdrop-blur-3xl border border-white/10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.7)] relative overflow-hidden group/nash">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start border-b border-white/5 pb-8 gap-6 relative z-10">
        <div className="space-y-3">
          ...
          <p className="m-0 text-[0.6rem] text-text-dim font-medium uppercase tracking-[0.2em] flex items-center gap-2 leading-none">
            <span className="text-accent-indigo-light font-black">
              Motor SOTA v4.2
            </span>
      ...
      >
        ...
        >
          <div
            className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${isNearPayjump ? "bg-accent-emerald shadow-[0_0_12px_var(--accent-emerald)] scale-110" : "bg-text-darker group-hover/btn:bg-text-muted"}`}
          />
        ...
        >
          <div
            className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${blindsRisingSoon ? "bg-accent-danger shadow-[0_0_12px_var(--accent-danger)] animate-pulse scale-110" : "bg-text-darker group-hover/btn:bg-text-muted"}`}
          />
