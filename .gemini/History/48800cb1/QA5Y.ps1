...
try {
    # Friccao Zero: Tenta puxar direto da API SOTA em memoria
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:17042/status?status=all" -Method Get -UseBasicParsing -TimeoutSec 1 -ErrorAction Stop
    $JsonOutput = $response.Content
}
catch {
    # Fallback para I/O tradicional de disco se a API estiver offline
