...
async def run_sota_tests():
    ...
    try:
        ...
        test_task = Task(id="TEST-SOTA-001", description="Validacao SOTA", agent="@implementor", timestamp="2026-03-24T00:00:00")
        await manager.add_task(test_task)
        retrieved = await manager.get_task("TEST-SOTA-001")
        assert retrieved.id == "TEST-SOTA-001", "Falha de I/O no SQLite"
