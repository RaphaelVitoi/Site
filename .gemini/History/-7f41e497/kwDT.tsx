...
import { calculateCallEV, calculateRequiredEquity, calculateRiskPremium } from "@/lib/icm";
...
const CustomTooltip = ( { active, payload, label, chartData }: any ) => {
    ...
    {
        const currentDataPoint = chartData.find( ( d: any ) => Number( ( d.rp ).toFixed( 1 ) ) === Number( label ) ); // Encontra o ponto de dados por RP
        return (
...
export function DownwardDriftSimulator () {
    ...
    const [ handEquity, setHandEquity ] = useState( 0.4 );
