...
function Search-DeepPropertyRegexSOTA {
    ...
    try {
        if ($Node -is [array]) {
            foreach ($item in $Node) { Search-DeepPropertyRegexSOTA -Node $item -RegexPattern $RegexPattern -Accumulator $Accumulator }
        } elseif ($null -ne $Node.PSObject -and $null -ne $Node.PSObject.Properties) {
            foreach ($prop in $Node.PSObject.Properties) {
                if ($prop.Name -match $RegexPattern) { [void]$Accumulator.Add([pscustomobject]@{ Key = $prop.Name; Value = $prop.Value }) }
                if ($null -ne $prop.Value) { Search-DeepPropertyRegexSOTA -Node $prop.Value -RegexPattern $RegexPattern -Accumulator $Accumulator }
            }
        }
    } catch { }
...
function nexus-db {
    ...
    if ($Arguments.Length -eq 0) {
        ...
        try {
            ...
            $commandsJson = $commandsJsonRaw | Where-Object { $_ -match '^\s*\{' } | Select-Object -First 1
