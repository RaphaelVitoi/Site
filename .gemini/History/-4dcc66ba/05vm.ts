...
export interface GameState {
    stack_efetivo: number;        // Profundidade do stack em BBs (ex: 10 a 100)
    jogadores_ativos: number;     // De 2 (HU) a 9 (MW massivo)
    pot_odds_imediatas: number;   // Razão imediata (ex: 0.20 para 4:1)
    distancia_payjump: DistanciaPayjump;
    tempo_blinds_min: number;     // Erosão por inanição
    edge_oponentes: EdgeOponentes;

    // Parâmetros internos providos pelo simulador (baseline)
    equity: number;               // Equidade bruta da mão (0.0 a 1.0)
    realization_factor: number;   // R: Fator de Realização (ex: 0.8)
    valuation_stack: number;      // Peso em $ do stack
    rio_base: number;             // Passivo basal por colisão (ex: 0.1 BB)
}
...
export class MathEngine {
    ...
    public static calculateEVFold ( t: number, dpj: DistanciaPayjump ): number {
        let evFold = -this.BASE_ANTE;

        // Efeito Payjump (d_pj): Adiciona utilidade passiva
        switch ( dpj )
        {
            case 'Bolha': evFold += 1.5; break;   // Passar a vez gera lucro por eliminação alheia
            case 'FT': evFold += 0.8; break;
            case 'Moderado': evFold += 0.2; break;
            case 'Longe': break;
        }
    ...
    public static calculateEdgeRelativa ( stack: number, edge: EdgeOponentes ): number {
        const sigma_delta = edge === 'Superior' ? 1.5 : ( edge === 'Inferior' ? 0.7 : 1.0 );
    ...
    public static simulate ( state: GameState ): PerspectiveResult {
        ...
        const decisao = is_mecanico ? 'Mecânico (Nash)' : ( c_i >= 1 ? 'Solvente (Agressão)' : 'Insolvente (Fold)' );
