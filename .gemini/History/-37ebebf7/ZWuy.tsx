...
export default function KGExplorer () {
    ...
    useEffect( () => {
        fetch( '/api/kg' )
            ...
            .catch( ( err: unknown ) => {
                ...
                console.error( "[CORTEX SHIELD] Erro ao carregar Topologia KG:", errorMessage );
                setLoading( false );
            } );
    }, [] );
    ...
    const handleNodeClick = ( node: Partial<GraphNode> ) => {
        setSelectedNodeQuery( `Explique a entidade ontológica: ${node.name || 'Desconhecido'} (${node.group || 'N/A'})` );
    };
