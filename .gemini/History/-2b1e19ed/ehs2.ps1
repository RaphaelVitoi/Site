...
.DESCRIPTION
    ...
    Ele sobrescreve os arquivos MEMORY.md dos 18 agentes com conteudo profundo,
    filosofico e atrelado as recentes evolucoes (SQLite, RAG, God Mode 2.0),
    e em seguida roda a indexacao no ChromaDB.
#>

Write-Host '=== INICIANDO INJECAO MASSIVA DE INTELIGENCIA (18 AGENTES) ===' -ForegroundColor Cyan
...
try {
    ...
    Invoke-WebRequest -Uri $apiUrl -Method Post -Headers $headers -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop | Out-Null
    Write-Host '  [+] Ingestao delegada ao Orquestrador SOTA (API Hibrida) com sucesso!' -ForegroundColor Green
}
catch {
    Write-Host '  [-] Orquestrador offline. Acionando Ingestao Nativa (Cold-Start)...' -ForegroundColor DarkGray
    $pythonCmd = 'python'
