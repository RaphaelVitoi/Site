          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {tasks?.slice(0, 10).map((task: any) => (
              <div key={task.id} className="p-3 bg-gray-800/50 rounded-lg border border-gray-700/50 flex justify-between items-center">
                ...
                <span className={`text-[10px] px-2 py-1 rounded-full uppercase tracking-wider font-bold shrink-0 ${
                  task.status === 'completed' ? 'bg-green-900/30 text-green-400 border border-green-800/50' :
