...
function _calculateAmortizedEdge ( input: PerspectivaInput, stackHero: number, stackVillain: number, isVacuum: boolean )
{
...
export function calculatePerspectivaVitoi ( input: PerspectivaInput ): PerspectivaResult
{
  ...
  const { edgePenalty, amortizedEdge } = _calculateAmortizedEdge( input, stackHero, stackVillain, isVacuum );
...
export function computeQuantumMetrics ( quantumPerspectiva: PerspectivaResult | null, activePlayers: number, heroInvested: number, stacks: number[], humanNoiseFactor: number = 0 )
{
  ...
  const sEff = Math.min( safeStacks[ 0 ] ?? 40, safeStacks[ 1 ] ?? 40 );
