﻿﻿﻿﻿/**
 * IDENTITY: Motor de Perspectiva Matemática SOTA v4.6 (VITOI - GOLD)
...
export function calculateMapaICM(
  stacks: number[],
  prizes: number[],
): MapaICMResult {
  ...
  const positionProbs: number[][] = Array.from({ length: n }, () =>
    new Array(k).fill(0),
  );
  ...
  function compute(
    ...
    const posC: number[][] = Array.from({ length: n }, () =>
      new Array(k).fill(0),
    );
...
function _calculateValuationAndRio(
  ...
  const rawValuation =
    villainDeltaLoss > 0
      ? deltaWinPct / ((villainDeltaLoss / totalPrizes) * 100)
      : 1;
  ...
  const volatilityMultiplier =
    stackHero > 0
      ? Math.pow(numPlayersInPot / Math.max(1, stackHero / 5), 2)
      : 1;

  const rioPenaltyChips =
    potSize * rioPenaltyFactor * (0.15 + volatilityMultiplier * 0.05);
  const icmPerChip =
    currentVillainEq > 0
      ? ((currentVillainEq / totalPrizes) * 100) /
        (current.totalChips / numPlayersInPot)
      : 0;
...
function _calculateFoldPressure(
  ...
  const payjumpBonus =
    isNearPayjump && !isVacuum
      ? Math.max(1.2, Math.abs(deltaFoldPct) + 0.25)
      : 0;
...
export function calculateAmortizedEdge(
  ...
  const effectiveStackForEdge =
    spr === undefined ? stackHero : Math.max(2, spr * 5);
...
function _buildDiagnostico(params: {
  ...
  let diagnostico =
    perspectivaPct > 0 ? "Ação Soberana." : "Insolvência de Perspectiva.";
  ...
  if (Math.abs(perspectivaPct) <= 5 && kappa < 0.4)
    diagnostico +=
      " Credibilidade Baixa: Intuição filtrada pelo Baseline Matemático.";
...
export function calculatePerspectivaVitoi(
  input: PerspectivaInput,
): PerspectivaResult {
  ...
  const { valuation, rioLiability } = _calculateValuationAndRio(
    current,
    deltaWinPct,
    perspWin,
    input,
  ...
  const prospectDeltaLose = calculateUtilityEV(
    baseDeltaLose,
    "baseline",
    2.25,
    effectiveStack,
  ...
  const chipLoseExpectativa = (1 - bayesianWinProb) * prospectDeltaLose; // Perda é física, não amortizável pela skill.

  const bountyExpectativa = bayesianWinProb * bountyValue * R; // Exige vitoria e Realizacao(R), mas imune a Valuation/FGS.

  const expectativaReal =
    chipWinExpectativa + chipLoseExpectativa + bountyExpectativa;
  ...
  if (Math.abs(denom) > 1e-6) {
    const rawThresh =
      (dynamicEvFold + rioLiability - prospectDeltaLose) / denom;
  ...
  const marginInstability =
    Math.max(0.01, 1 / Math.max(2, effectiveStack)) * 100;
...
export function calculateRioTension( // NOSONAR
  ...
  const potEntrapment =
    (heroInvested + betToCall) / Math.max(0.1, heroRawStack);
...
export type ReferencePointStatus =
  | "baseline"
  | "tilt"
  | "protecting"
  | "bubble";
...
export function computeQuantumMetrics(
  ...
  _activePlayers: number,
  _heroInvested: number,
  _stacks: number[],
  _humanNoiseFactor: number = 0,
) {
