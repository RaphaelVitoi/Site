...
export function calculatePerspectivaVitoi ( input: PerspectivaInput ): PerspectivaResult
{
  ...
  const effectiveStack = Math.min( stackHero, stackVillain );
  const baseDeltaLose = deltaLosePct * (1.0 / Math.max(0.1, fgsHealth));
  const prospectDeltaLose = calculateUtilityEV(baseDeltaLose, effectiveStack);
  ...
  const ci = threshEq > 0 ? ( bayesianWinProb / threshEq ) : ( perspectivaPct > 0 ? 1.5 : 0.5 );
...
export function computeQuantumMetrics ( quantumPerspectiva: PerspectivaResult | null, activePlayers: number, heroInvested: number, currentPot: number, stacks: number[], humanNoiseFactor: number = 0.0 )
{
  ...
  const mwFactor = Math.pow( opponents, 2.0 + humanNoiseFactor );
  ...
  const baseDeltaLose = deltaLosePct * (1.0 / Math.max(0.1, fgsHealth));
  const prospectDeltaLose = calculateUtilityEV(baseDeltaLose, sEff);
