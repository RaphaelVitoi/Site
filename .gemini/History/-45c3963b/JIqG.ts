...
export function computeQuantumMetrics ( quantumPerspectiva: PerspectivaResult | null, activePlayers: number, heroInvested: number, currentPot: number, stacks: number[] )
{
  ...
  // SOTA FIX: Dimensionalidade Restaurada. A Expectativa exige 'Hand Equity' (winProb), não a representação ICM da Stack (currentEquityPct).
  ...
  // SOTA: Desacoplamento Dimensional (Chips vs Bounty) na Camada de Renderizacao (WASM-UI Bridge)
  const bountyPower = quantumPerspectiva.bountyPower ?? 0;
  const chipEsperanca = ( eq * adjustedDeltaWin ) + ( ( 1 - eq ) * deltaLosePct );
  const esperanca = chipEsperanca + (eq * bountyPower);

  const chipExpectativa = ( eq * adjustedDeltaWin * rFactor * fgsHealth ) + ( ( 1 - eq ) * deltaLosePct );
  const bountyExpectativa = eq * bountyPower * rFactor;
  const expectativa = chipExpectativa + bountyExpectativa;
  ...
  const denom = ( adjustedDeltaWin * rFactor * fgsHealth ) - deltaLosePct + (bountyPower * rFactor);
  let threshEq = null;
  // SOTA: A Equidade Limite (Teto) DEVE subir para compensar o passivo do RIO.
  // OBSERVAÇÃO EMPÍRICA: O Teto de Equidade no River estabiliza empiricamente em ~41-45%.
  // Deixamos a matemática calcular o limite orgânico (sem hard-cap artificial).
  if ( Math.abs( denom ) > 1e-6 )
  {
