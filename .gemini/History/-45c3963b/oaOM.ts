...
function _calculateValuationAndRio ( current: MapaICMResult, deltaWinPct: number, perspWin: MapaICMResult, villainIdx: number, totalPrizes: number, potSize: number, numPlayersInPot: number, stackHero: number )
{
  ...
  const rioPenaltyFactor = numPlayersInPot > 2 ? ( 1 + Math.log2( numPlayersInPot - 1 ) ) : 1;
...
export function calculatePerspectivaVitoi ( input: PerspectivaInput ): PerspectivaResult
{
  ...
  const { valuation, rioLiability } = _calculateValuationAndRio( current, deltaWinPct, perspWin, villainIdx, totalPrizes, potSize, numPlayersInPot, stackHero );
...
export function computeQuantumMetrics ( quantumPerspectiva: Record<string, any>, activePlayers: number, heroInvested: number, currentPot: number, stacks: number[] )
{
  ...
  let ci = null;
  const potOdds = ( currentPot + heroInvested ) > 0 ? heroInvested / ( currentPot + currentPot ) : 0;
