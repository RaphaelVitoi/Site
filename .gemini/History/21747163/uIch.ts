...
/**
 * Calcula o Valor Esperado (EV) utilitário de um call sob pressão de ICM.
 * Diferente do ChipEV, o ICM penaliza assimetricamente a derrota multiplicando o risco pelo Bubble Factor.
 *
 * Equação: EV = [Equidade * (Pote + Aposta)] - [(1 - Equidade) * (Aposta * Bubble Factor)]
 *
 * @param pot Tamanho do pote antes da aposta (Dead Money).
 * @param bet Tamanho da aposta enfrentada (Risco nominal).
 * @param bubbleFactor Fator de Bolha (Dor financeira da eliminação/perda).
 * @param handEquity Equidade real da mão contra o range do agressor (0.0 a 1.0).
 * @returns O Expected Value (EV) da decisão. Valores > 0 indicam Call lucrativo.
 */
...
{
  if ( pot < 0 || bet < 0 || bubbleFactor < 1 || handEquity < 0 || handEquity > 1 )
  {
    return 0;
  }

  const reward = pot + bet;
  const penalizedRisk = bet * bubbleFactor; // O custo letal do ICM

  const ev = ( handEquity * reward ) - ( ( 1 - handEquity ) * penalizedRisk );
  return ev;
}

/**
 * Calcula o Bubble Factor (BF) real baseado no risco e recompensa monetaria (EV).
 * SOTA: Esta e a fundacao para a V2 do Laboratorio Universal.
 *
 * @param initialEquityEV A expectativa matematica ($EV) do jogador antes de pagar o all-in.
 * @param winEquityEV A expectativa matematica ($EV) se o jogador pagar e VENCER o all-in.
 * @param loseEquityEV A expectativa matematica ($EV) se o jogador pagar e PERDER o all-in.
 * @returns O Bubble Factor exato para o cenario. Retorna 1.0 (ChipEV) se nao houver pressao.
 */
export function calculateBubbleFactor (
  initialEquityEV: number,
  winEquityEV: number,
  loseEquityEV: number
): number
{
  // Reward: O quanto ganhamos de utilidade real ao vencer
  const reward = winEquityEV - initialEquityEV;

  // Risk: O quanto perdemos de utilidade real ao ser derrotado
  const risk = initialEquityEV - loseEquityEV;

  // Prevencao contra divisoes por zero ou dados anomalos (vazios matematicos)
  if ( reward <= 0 || risk <= 0 )
  {
    return 1.0;
  }

  const bf = risk / reward;

  // O BF nunca pode ser menor que 1 (A dor da perda e sempre igual ou maior que a alegria do ganho)
  if ( bf < 1.0 )
  {
    return 1.0;
  }
