...
while ($true) {
    ...
    Write-Host " 📊 NEXUS DASHBOARD :: Visão Global do Fluxo de Trabalho" -ForegroundColor Yellow
    ...
    if (-not (Test-Path $QueuePath)) {
        ...
        try {
            ...
            Write-Host " [ CONCENTRAÇÃO DE ENTROPIA ]" -ForegroundColor White
            Write-Host " ⏳ Pendentes : $($pending.Count)" -ForegroundColor Gray
            Write-Host " ⚡ Rodando   : $($running.Count)" -ForegroundColor Magenta
            Write-Host " ✅ Concluídas: $($completed.Count)" -ForegroundColor Green
            Write-Host " ❌ Falhas    : $($failed.Count)`n" -ForegroundColor Red
            ...
            if ($activeTasks.Count -eq 0) {
                Write-Host " Nenhuma tarefa na fila de execução." -ForegroundColor DarkGray
            }
            else {
                foreach ($t in $activeTasks) {
                    ...
                    Write-Host " $icon [$($t.id.Substring(0,18))_] $agent | $desc" -ForegroundColor $color
                }
            }
        }
        catch {
    ...
    Write-Host "`nAtualizando a cada 3 segundos... Pressione [Ctrl+C] para sair." -ForegroundColor DarkGray
