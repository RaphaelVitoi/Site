...
function OldRangeGrid ( { freqs, color, title, subtitle, role }: { freqs: number[][], color: 'indigo' | 'emerald', title: string, subtitle: string, role: 'aggressor' | 'defender' } ) {
  return (
    <div style={ { position: 'relative' } } className="group">
      ...
      <table style={ { borderCollapse: 'collapse', fontSize: '0.55rem', fontFamily: 'monospace', background: 'rgba(15,23,42,0.4)', borderRadius: '8px', overflow: 'hidden' } }>
        <tbody>
          { RANKS.map( ( r1, i ) => (
            <tr key={ r1 }>
              { RANKS.map( ( r2, j ) => {
      ...
      <div className="absolute top-0 left-full ml-4 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 duration-300" style={ { width: '320px' } }>
          ...
          <div style={ { marginTop: '1rem' } }>
            ...
            <div style={ {
              ...
              <div style={ { flex: '0 0 auto' } }>
                <OldRangeGrid freqs={ BTN_FREQS } color="indigo" title="BTN (RFI)" subtitle="33.6% open" role="aggressor" />
              </div>
              <div style={ { flex: '0 0 auto' } }>
                <OldRangeGrid freqs={ BB_FREQS } color="emerald" title="BB (Defesa)" subtitle="82.9% continue" role="defender" />
              </div>
            </div>
          </div>
