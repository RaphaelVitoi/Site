...
import { BB_FREQS, BF_MATRIX, BF_PLAYERS, BF_STACKS, BTN_FREQS, BUBBLE_BF_MATRIX, BUBBLE_PLAYERS, BUBBLE_RP_MATRIX, BUBBLE_STACKS, PRIZES, RANKS, RP_MATRIX, TABLE_PLAYERS, TOTAL_POOL } from './ReferencialData';
...
export default function ReferencialAula12 () {
  ...
  const [ matrixView, setMatrixView ] = useState<'FT' | 'BUBBLE'>( 'FT' );
  const isFT = matrixView === 'FT';
  const isBubble = matrixView === 'BUBBLE';
  const mPlayers = isFT ? BF_PLAYERS : BUBBLE_PLAYERS;
  const mStacks = isFT ? BF_STACKS : BUBBLE_STACKS;
  const mBf = isFT ? BF_MATRIX : BUBBLE_BF_MATRIX;
  const mRp = isFT ? RP_MATRIX : BUBBLE_RP_MATRIX;
