...
function OldRangeGrid ( { freqs, color, title, subtitle }: { freqs: number[][], color: 'indigo' | 'emerald', title: string, subtitle: string } ) {
  return (
    <div style={ { position: 'relative', width: '100%', maxWidth: '340px', margin: '0 auto' } }>
      ...
      <table style={ { width: '100%', tableLayout: 'fixed', borderCollapse: 'collapse', fontSize: '0.5rem', fontFamily: 'monospace', background: 'rgba(15,23,42,0.4)', borderRadius: '8px', overflow: 'hidden' } }>
        <tbody>
          { RANKS.map( ( r1, i ) => (
            <tr key={ r1 }>
              { RANKS.map( ( r2, j ) => {
                ...
                return (
                  <td key={ r2 } style={ { background: cellBg( freq, color ), color: cellText( freq ), border: '1px solid rgba(255,255,255,0.03)', height: '22px', textAlign: 'center', verticalAlign: 'middle', cursor: 'default' } }>
