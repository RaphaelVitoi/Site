...
import { RangeHeatmap, type CellData } from '@/app/ICMlaboratory/RangeHeatmap';
...
// SOTA: Adaptador Fractal para injetar o Heatmap Estocastico na Matriz de Frequencias Antiga
function convertToHeatmapMatrix ( freqs: number[][], role: 'aggressor' | 'defender' ): CellData[][] {
  return RANKS.map( ( rowRank, r ) =>
    RANKS.map( ( colRank, c ) => {
      const freq = freqs[ r ][ c ] / 100;
      const raise = role === 'aggressor' ? freq : 0;
      const call = role === 'defender' ? freq : 0;
      const fold = 1 - freq;
      return {
        combo: getHandLabel( r, c ),
        isSuited: r < c,
        frequencies: { fold, call, raise },
        ev: 0 // Matriz de frequencia basica nao contem EV individual
      };
    } )
  );
}

function OldRangeGrid ( { freqs, color, title, subtitle, role, tooltipDirection = 'right' }: { freqs: number[][], color: 'indigo' | 'emerald', title: string, subtitle: string, role: 'aggressor' | 'defender', tooltipDirection?: 'left' | 'right' } ) {
  const tooltipPos = tooltipDirection === 'right' ? 'left-full ml-4' : 'right-full mr-4';
  const glowColor = color === 'indigo' ? '99,102,241' : '16,185,129';

  return (
    <div style={ { position: 'relative' } } className="group cursor-help">
      ...
      <div style={ { position: 'relative' } }>
        <table style={ { borderCollapse: 'collapse', fontSize: '0.55rem', fontFamily: 'monospace', background: 'rgba(15,23,42,0.4)', borderRadius: '8px', overflow: 'hidden', transition: 'box-shadow 0.3s ease' } } className={ `group-hover:shadow-[0_0_20px_rgba(${glowColor},0.25)]` }>
          <tbody>
            { RANKS.map( ( r1, i ) => (
              <tr key={ r1 }>
                { RANKS.map( ( r2, j ) => {
                  ...
                  return (
                    <td key={ r2 } style={ { background: cellBg( freq, color ), color: cellText( freq ), border: '1px solid rgba(255,255,255,0.03)', width: '24px', height: '24px', textAlign: 'center', verticalAlign: 'middle' } }>
        ...
        <div className={ `absolute top-0 ${tooltipPos} opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-50 duration-300 transform group-hover:scale-100 scale-95 origin-top` } style={ { width: '320px' } }>
          <RangeHeatmap matrix={ convertToHeatmapMatrix( freqs, role ) } title={ `SOTA: ${title}` } subtitle="Estocástico (CCD)" />
        </div>
      </div>
    </div>
  );
          ...
          <div style={ { marginTop: '1rem' } }>
            ...
            <div style={ {
              ...
              <div style={ { flex: '0 0 auto' } }>
                <OldRangeGrid freqs={ BTN_FREQS } color="indigo" title="BTN (RFI)" subtitle="33.6% open" role="aggressor" tooltipDirection="right" />
              </div>
              <div style={ { flex: '0 0 auto' } }>
                <OldRangeGrid freqs={ BB_FREQS } color="emerald" title="BB (Defesa)" subtitle="82.9% continue" role="defender" tooltipDirection="left" />
              </div>
            </div>
          </div>
