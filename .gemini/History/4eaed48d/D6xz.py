...
def _format_predictive_profile(profile: Optional[dict]) -> str:
    ...
    return f"\n[PERFIL COGNITIVO (TELEMETRIA BAYESIANA)]\n{prof_str}\nDiretriz SOTA: Adapte sua argumentação e justifique a jogada mitigando ativamente as maiores fraquezas numéricas deste perfil.\n[END_PROFILE]\n"
...
def _get_rag_context(prompt: str) -> str:
    if not RAG_AVAILABLE:
        return ""
    try:
        # SOTA (Plano Hacker): Fetch expandido e corte de Distância (Strictness)
        ...
        if results:
            ...
            if docs is not None and docs[0] and distances is not None and distances[0]:
                        strictness_threshold = 1.1  # ChromaDB L2 Default: Menor é mais próximo. > 1.1 é ruído.
                ...
                # SOTA: Log explícito dos fragmentos RAG no terminal do backend para auditoria epistemológica
                logger.info(
                    f"=== [RAG INJECTED FRAGMENTS: {len(top_docs)} CHUNKS (Threshold < {strictness_threshold})] ==="
                )
                for i, d in enumerate(top_docs):
                    logger.info(f"Fragmento {i + 1}:\n{d[:200]}...[TRUNCADO]\n")
                logger.info("=====================================================")
...
async def generate_response(
    ...
    sys_prompt = (
        req.system_prompt if req.system_prompt is not None else VITOI_SYSTEM_PROMPT
    )
    final_prompt = (
        sys_prompt
        + rag_context
