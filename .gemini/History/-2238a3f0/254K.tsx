...
function QuizSynthesis( { onRestart }: { readonly onRestart: () => void } ) {
    return (
        ...
        >
            ...
            <p className="text-text-muted text-[0.95rem] leading-relaxed max-w-[600px] mx-auto mb-8">
            ...
            <div className="flex flex-col gap-4 text-left mb-10">
                <div className="bg-slate-800/40 border-l-[3px] border-accent-indigo p-4 rounded-md">
                    <strong className="text-accent-indigo-light block mb-1 font-bold">1. Equidade não é Destino (Fator R)</strong>
                    <span className="text-[0.85rem] text-text-dim">Estar Fora de Posição dilui estruturalmente o seu poder de fogo. O Fator R revela que mãos que parecem lucrativas no vácuo tornam-se insustentáveis no mundo real.</span>
                </div>
                ...
                <div className="bg-slate-800/40 border-l-[3px] border-accent-pink p-4 rounded-md">
                    <strong className="text-accent-pink-light block mb-1 font-bold">2. Solvers preveem perfeição; humanos entregam entropia (Axioma)</strong>
                    <span className="text-[0.85rem] text-text-dim">O Axioma Lipe Piv utiliza a Regressão Bayesiana para forçar a equidade teórica de volta ao baseline passivo sempre que a Credibilidade (κ) for baixa. Não confie cegamente no vácuo.</span>
                </div>
                ...
                <div className="bg-slate-800/40 border-l-[3px] border-accent-amber p-4 rounded-md">
                    <strong className="text-accent-amber-light block mb-1 font-bold">3. O Zero Não Existe (Sunk Cost)</strong>
                    <span className="text-[0.85rem] text-text-dim">Qualquer decisão agressiva negativa (-3bb) será lucrativa pela Lente da Perspectiva se o Custo Afundado da desistência for ainda pior (-5bb). O EV do Fold é o seu piso inegociável.</span>
                </div>
                <div style={ { background: 'rgba(30,41,59,0.4)', borderLeft: '3px solid var(--accent-pink)', padding: '1rem', borderRadius: '6px' } }>
                ...
                <div className="bg-slate-800/40 border-l-[3px] border-accent-pink p-4 rounded-md">
                    <strong className="text-accent-pink-light block mb-1 font-bold">4. A Ilusão das Pot Odds (Coeficiente Cᵢ)</strong>
                    <span className="text-[0.85rem] text-text-dim">O preço barato mascara o Passivo Estrutural das Reverse Implied Odds (RIO). Se o Cᵢ for menor que 1, as odds estão mentindo e a sua equity real será devorada pela entropia (Multiway).</span>
                </div>
            </div>
            ...
            <button onClick={ onRestart } className="py-3 px-8 bg-accent-indigo text-white border-none rounded-lg font-extrabold cursor-pointer uppercase tracking-[0.05em]">
        ...
        >
            ...
            <div className="mb-8">
                ...
                <div className="bg-red-500/10 border-l-[3px] border-accent-danger px-4 py-3 rounded-md">
                    <span className="text-[0.8rem] font-extrabold text-accent-danger-light uppercase">⚠ Armadilha Cognitiva: </span>
                    <span className="text-[0.85rem] text-text-light">{ scenario.trap }</span>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
                { scenario.choices.map( choice => {
                    ...
                    const opacityClass = isAnswered && !isSelected ? 'opacity-40' : 'opacity-100';
                    const cursorClass = ( isAnswered || isRevealing ) ? 'cursor-default' : 'cursor-pointer';
                    return (
                        <button
                            key={ choice.id }
            ...
            { isRevealing && (
                ...
                <motion.div initial={ { opacity: 0 } } animate={ { opacity: 1 } } className="mt-8 p-6 text-center bg-slate-900/40 rounded-lg border border-dashed border-accent-indigo/20">
                    <span className="text-[0.75rem] text-accent-indigo uppercase tracking-[0.1em] font-bold">
                        <i className="fa-solid fa-microchip fa-fade mr-2" />Processando Vetores de Perspectiva...
                    </span>
                </motion.div>
            ) }
                ...
                >
                    ...
                    <div className="flex gap-8 flex-wrap font-mono">
                        <div>
                            ...
                            <span className="block text-[0.7rem] text-text-dim uppercase">Expectativa (P)</span>
                        ...
                        <div>
                            ...
                            <span className="block text-[0.7rem] text-text-dim uppercase">EV_Fold (1ª Ordem)</span>
                        ...
                        <div>
                            ...
                            <span className="block text-[0.7rem] text-text-dim uppercase">Perspectiva (PM)</span>
                            <span className={ `text-[1rem] font-extrabold ${choiceData.isCorrect ? 'text-accent-emerald' : 'text-accent-danger'}` }>
                        ...
                        { choiceData.metricImpact.ci !== undefined && (
                            <div>
                                ...
                                <span className="block text-[0.7rem] text-text-dim uppercase">Insolvência (Cᵢ)</span>
                                <span className={ `text-[1rem] font-extrabold ${Number( choiceData.metricImpact.ci ) < 1 ? 'text-accent-danger' : 'text-accent-emerald'}` }>
...
export default function IcmQuizVisceral() {
        ...
        return (
            ...
            <div className="p-8 text-center text-text-dim italic tracking-[0.05em]">
                ...
                <div className="flex gap-1.5 justify-center mt-4">
                    ...
                    <div className="w-1.5 h-1.5 rounded-full bg-accent-indigo animate-pulse" style={{ animationDelay: '0.2s' }} />
                    <div className="w-1.5 h-1.5 rounded-full bg-accent-indigo animate-pulse" style={{ animationDelay: '0.4s' }} />
                </div>
            </div>
        );
    }
    ...
    return (
        ...
        <div className="max-w-[800px] my-12 mx-auto p-8 bg-slate-900/60 rounded-xl border border-white/5">
            ...
            { !isFinished && (
                ...
                <div className="flex justify-between mt-8 pt-4 border-t border-white/5">
                    <button onClick={ () => setCurrentStep( prev => Math.max( 0, prev - 1 ) ) } disabled={ currentStep === 0 } className={ `py-2 px-4 rounded-md font-semibold transition-all duration-200 border ${currentStep === 0 ? 'bg-transparent text-white/10 border-white/5 cursor-not-allowed' : 'bg-transparent text-text-dim border-white/20 cursor-pointer'}` }>
                        &larr; Voltar
                    </button>
                    ...
                    <button onClick={ () => setCurrentStep( prev => prev + 1 ) } disabled={ !isAnswered || isRevealing } className={ `py-2 px-6 rounded-md font-bold transition-all duration-200 border-none ${isAnswered && !isRevealing ? 'bg-accent-indigo text-white cursor-pointer' : 'bg-indigo-500/10 text-white/30 cursor-not-allowed'}` }>
