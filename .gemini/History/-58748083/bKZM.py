...
def _compute_dynamic_route(task: Task) -> tuple[list, list]:
    """Mapeia dinamicamente a rota de agentes baseada em Antevisao e heuristica."""
    ...
    stage_prompts = {
        "@architect": (
            f"Fallback automatico do dispatcher para a tarefa base {task.id}. "
        ...
        "@maverick": (
            f"Refine estrategicamente a direcao da tarefa base {task.id}, validando tradeoffs, riscos e antevisao "
            f"antes da fase de planejamento detalhado."
        ),
        "@pesquisador": (
            f"Execute pesquisa aplicada para a tarefa base {task.id}: referencias externas, benchmark e dados de apoio "
        ...
        "@securitychief": (
            f"Audite vetores de seguranca da tarefa base {task.id} (auth, segredos, permissao, superficie de ataque) "
            f"e entregue diretrizes obrigatorias para implementacao segura."
        ),
        "@validador": (
            f"Valide premissas de dominio da tarefa base {task.id} (regras, calculos e consistencia tecnica) "
            f"antes da implementacao."
        ),
        "@implementor": (
            f"Execute a implementacao da tarefa base {task.id} conforme plano aprovado, preservando routing, integridade "
            f"e estabilidade sistemica."
        ),
        "@verifier": (
            f"Valide funcionalmente a entrega da tarefa base {task.id} (task -> queue -> worker -> resposta) e reporte "
            f"riscos residuais com criterio tecnico."
        ),
        "@curator": (
            f"Curadoria final da tarefa base {task.id} apos verificacao: refinar clareza, consistencia e alinhamento "
    ...
    for idx, agent in enumerate(route_agents, start=1):
        fallback_specs.append({
            "suffix": f"SUB-{idx}",
            "agent": agent,
            "description": stage_prompts.get(agent, f"Execute a sua etapa para a tarefa base {task.id}."),
