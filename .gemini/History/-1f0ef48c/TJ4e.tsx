...
export async function GET ( req: NextRequest ) {
    try
    {
        ...
        // SOTA: Fricção Zero Input Validation (Mitigação C1)
        if ( bf.length > 10 || rp.length > 10 || pureEv.length > 10 || icmEv.length > 10 || pot.length > 10 )
        {
            return new Response( 'Invalid bounds for mathematical parameters.', { status: 400 } );
        }
    ...
    } catch ( e: unknown )
    {
        ...
        console.error( '[API SOTA OG] Erro crítico de renderização de ICM Chart:', msg );
