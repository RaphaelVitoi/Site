...
// Instância SOTA: Evita exaustão de conexões no modo dev
const globalForPrisma = global as unknown as { prisma: PrismaClient }
const prisma = globalForPrisma.prisma || new PrismaClient()
if (process.env.NODE_ENV !== "production") globalForPrisma.prisma = prisma

export const revalidate = 0 // Fricção Zero: Desativa cache estático para dados analíticos (Tempo Real)
...
export default async function AnalyticsDashboard() {
    ...
    const sortedCategories = Object.entries(lossByCategory).sort((a, b) => b[1] - a[1])
                                ...
                                return (
                                    <div key={category} className="w-full">
                                        <div className="flex justify-between text-sm mb-1 text-white/70">
                                            <span>{category}</span>
                                            <span className="text-red-400 font-mono">-{loss.toFixed(2)} EV</span>
                                        </div>
