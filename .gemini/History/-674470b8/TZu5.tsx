...
export function SotaTooltip({
  ...
  const gapBridge =
    position === "bottom"
      ? "-top-4 left-0 w-full h-4"
      : "-bottom-4 left-0 w-full h-4";
  const arrowClasses =
    position === "bottom"
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-3 border-b border-white/10 pb-3">
          <div
            className={`w-2 h-2 rounded-full ${theme === "indigo" ? "bg-accent-indigo shadow-[0_0_10px_rgba(99,102,241,0.8)]" : theme === "emerald" ? "bg-accent-emerald shadow-[0_0_10px_rgba(16,185,129,0.8)]" : "bg-accent-rose shadow-[0_0_10px_rgba(244,63,94,0.8)]"} animate-pulse`}
          />
          <p
            className={`text-[0.7rem] font-black uppercase tracking-[0.25em] m-0 ${theme === "indigo" ? "text-accent-indigo-light" : theme === "emerald" ? "text-accent-emerald-light" : "text-accent-rose-light"}`}
          >
            {title}
          </p>
        </div>
