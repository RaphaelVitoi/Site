...
def load_markdown_body(file_path: Path) -> str:
    """
    Lê o conteúdo do arquivo bruto apontado fisicamente, eliminando fragilidade de saltos relativos.
    """
    if not file_path.exists():
        print(
            f"[ERRO] O arquivo {file_path.resolve()} não foi encontrado na infraestrutura."
        )
        sys.exit(1)
    ...
    if row:
        ...
        """,
            (
                ...
                markdown_body,
                "Lesson",
                "ICM,Pós-Flop,Risk Premium",
                now_iso,
                now_iso,
            ),
        )
        print(
            f"[SUCESSO] Aula SOTA injetada na Máquina de Conteúdo! Rota-alvo: /aulas/{slug}"
        )
...
def upsert_content(
    ...
    """
    description = (
        "Compreenda o ICM e suas heurísticas através da análise de RPs e Toy Games."
    )
...
def main() -> None:
    ...
    if len(sys.argv) < 4:
        ...
        print(
            'Exemplo: python seed_lesson.py amortizacao-da-edge "A Amortização da Edge" "../../../research/icm-materials/icmteoriaadicionalpt1.txt"'
        )
        sys.exit(1)
