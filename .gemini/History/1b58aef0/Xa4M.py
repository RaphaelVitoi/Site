...
def load_markdown_body(slug: str) -> str:
    ...
    """
    md_path = Path(__file__).parent.parent.parent.parent / f"{slug}.md"
    if not md_path.exists():
        print(f"[ERRO] O arquivo {slug}.md não foi encontrado. Execute o script de conversão primeiro.")
        sys.exit(1)
...
def main() -> None:
    ...
    """
    if len(sys.argv) < 3:
        print("Uso: python seed_lesson.py <slug> <titulo_entre_aspas>")
        print("Exemplo: python seed_lesson.py entendendo-o-icm-e-suas-heuristicas \"Entendendo o ICM\"")
        sys.exit(1)
