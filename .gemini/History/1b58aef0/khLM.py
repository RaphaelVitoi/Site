...
def load_markdown_body(file_path: Path) -> str:
    ...
    if not file_path.exists():
        print(f"[ERRO] O arquivo {file_path.resolve()} não foi encontrado na infraestrutura.")
        sys.exit(1)
...
def upsert_content(cursor: sqlite3.Cursor, slug: str, title: str, markdown_body: str, now_iso: str) -> None:
    ...
    if content_row:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (str(uuid.uuid4()), slug, 'biblioteca', title, description, markdown_body, 1, now_iso, now_iso))
...
def main() -> None:
    ...
    if len(sys.argv) < 4:
        ...
        print("Exemplo: python seed_lesson.py amortizacao-da-edge \"A Amortização da Edge\" \"../../../research/icm-materials/icmteoriaadicionalpt1.txt\"")
        sys.exit(1)
