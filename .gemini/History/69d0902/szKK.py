...
def _test_quantum_physics_parity() -> str:
    ...
    try:
        ...
        assert res["rio_mw"] > 0, "Dívida RIO não calculada para Multiway."  # noqa: S101
        ...
        expected_persp = res["expectativa"] - (res["rio_mw"] + (-1.0))
        assert abs(res["perspectiva"] - expected_persp) < 1e-6, (
            f"Divergência vetorial na Perspectiva: {res['perspectiva']} != {expected_persp}"
        )  # noqa: S101
