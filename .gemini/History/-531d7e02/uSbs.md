...
4.  ~~**Executar Comando:** `cd frontend ; npm install prisma @prisma/client`~~ (CONCLUÍDO)
5.  ~~**Executar Comando:** `cd frontend ; npx prisma init`~~ (CONCLUÍDO)
6.  **Verificar Arquivo:** Conferir se o `frontend/prisma/schema.prisma` possui o schema exato listado acima. Se faltar algo, sobrescreva via God Mode.
7.  **Materializar Arquivos:** Criar a estrutura de rotas vazias no NextJS.
