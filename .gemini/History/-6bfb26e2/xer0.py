...
async def _handle_cli_command(cmd: str, argv: list, manager: QueueManager):
    if cmd == "db-init":
    ...
    if cmd == "db-get-notify":
        await _cmd_get_notify(manager, argv)
        return

    if cmd == "db-reset-budget":
    ...
    if cmd in ("db-retry-failed", "retry-failed"):
        await _cmd_retry_failed_tasks(manager)
        return

    if cmd in ("db-complete", "complete"):
        if len(argv) < 3:
    ...
    if cmd in ("gemini-health", "nexus-gemini-health"):
    ...
    if cmd in ("check-keys", "nexus-keys"):
