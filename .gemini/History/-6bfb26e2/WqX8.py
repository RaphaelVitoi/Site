...
async def _fetch_gemini_health(session, key, model, system_prompt, health_prompt, client_timeout):
    ...
    try:
        ...
        await call_gemini(session, model, system_prompt, health_prompt, key, client_timeout=client_timeout, require_json=False)
        return True, True, "ListModels+GenerateContent OK"
    except Exception as ge:  # noqa: BLE001
        return True, False, f"generateContent falhou ({model}): {str(ge)[:180]}"

async def _test_single_gemini_key(session, entry, manager, req_timeout, pro_model, flash_model, system_prompt, health_prompt, window_minutes):
    key = entry["key"]
    pool = entry["pool"]
    ...
    try:
        lm_ok, gen_ok, detail = await _fetch_gemini_health(session, key, model, system_prompt, health_prompt, req_timeout)
        ...
        return {
            ...
            "listmodels_ok": lm_ok, "generate_ok": gen_ok,
    ...
    async with aiohttp.ClientSession(connector=connector, trust_env=True, timeout=timeout) as session:
        for entry in entries:
            res = await _test_single_gemini_key(
                session, entry, manager, timeout, pro_model, flash_model,
                system_prompt, health_prompt, window_minutes
            )
            results.append(res)
...
async def _cli_ingest(argv: list, manager: QueueManager) -> None:
        if len(argv) < 3:
            ...
            sys.exit(1)
        try:
            filepath = Path(argv[2]).resolve()
            if not filepath.is_relative_to(Path.cwd()):
                print("ERROR: [SEC] Path traversal detectado. Caminho fora do escopo do projeto.")
            ...
            if not filepath.is_relative_to(dropzone):
                print(f"ERROR: [SEC] Escopo Invalido. O arquivo '{filepath.name}' deve estar restrito a '{dropzone.relative_to(project_root)}' para evitar delecao do nucleo.")
                sys.exit(1)
