...
async def _test_single_gemini_key(session, entry, manager, client_timeout, pro_model, flash_model, system_prompt, health_prompt, window_minutes):
    key = entry["key"]
    pool = entry["pool"]
    ...
    try:
        lm_ok, gen_ok, detail = await _fetch_gemini_health(session, key, model, system_prompt, health_prompt, client_timeout)
        latency_ms = int((time.monotonic() - started) * 1000)
        status = "success" if (lm_ok and gen_ok) else "error"
