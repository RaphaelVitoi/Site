...
async def _cli_ingest(argv: list, manager: QueueManager) -> None:
    ...
    try:
        ...
        if not filepath.is_relative_to(dropzone):
            print(
                f"ERROR: [SEC] Escopo Invalido. O arquivo '{filepath.name}' deve estar restrito a '{dropzone.relative_to(project_root)}' para evitar delecao do nucleo."
            )
            sys.exit(1)
