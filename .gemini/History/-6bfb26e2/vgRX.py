...
async def _cli_ingest(argv: list, manager: QueueManager) -> None:
    ...
    try:
        ...
        # SOTA: Purificacao total do filename prevenindo Path Traversal (LFI)
        filename = Path(argv[2]).name
        filepath = (dropzone / filename).resolve()
            # SOTA: Validacao estrita de Path Traversal (Security Report Fix)
            filepath = (dropzone / argv[2]).resolve()

        if filepath.parent != dropzone:
            if not filepath.is_relative_to(dropzone):
            print(
                f"ERROR: [SEC] Escopo Invalido. O arquivo '{filename}' deve estar estritamente dentro de '{dropzone.relative_to(project_root)}'."
