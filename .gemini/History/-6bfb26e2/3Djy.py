...
async def _cmd_get_budget(manager: QueueManager) -> dict:
    ...
    async with aiosqlite.connect(manager.db_path) as db, db.execute("SELECT call_count FROM daily_usage WHERE date = ?", (today,)) as cursor:
        row = await cursor.fetchone()
        current_count = row[0] if row else 0
        return {"used": current_count, "total": DAILY_API_BUDGET}
...
async def _format_notify_row(t: Task, manager: QueueManager) -> tuple:
    ...
    return (display_id, t.agent, f"[{st_color}]{t.status.upper()}[/]", f"[{pri_color}]{pri.upper()}[/]", desc, ts)
...
async def _cmd_get_notify(manager: QueueManager, argv: list) -> None:
    ...
    table = Table(title="[bold orange3]🔔 NEXUS ORCHESTRATOR - Auditorias Sentinela Pendentes[/]", border_style="orange3", show_lines=True)
    ...
    for t in notify_tasks:
        ...
        table.add_row(*row)
    if not notify_tasks:
        table.add_row("", "", "", "", "[dim]Nenhuma auditoria sentinela pendente no momento.[/dim]", "")
    console.print(table)
...
async def _cmd_get_watchdog_stats(manager: QueueManager) -> dict:
    ...
    if last_timestamp_str and last_failed is not None:
        ...
        time_delta = datetime.now(timezone.utc) - datetime.fromisoformat(last_timestamp_str)
...
async def _cmd_retry_task(manager: QueueManager, task_id: str) -> None:
    ...
    await manager.update_task_metadata(task_id, {"workflow_status": "pending_retry_forced", "retry_timestamp": datetime.now().isoformat()}, merge=True)
    await manager.update_task_metadata(task_id, {"workflow_status": "pending_retry_forced", "retry_timestamp": datetime.now(timezone.utc).isoformat()}, merge=True)
...
async def _cmd_retry_failed_tasks(manager: QueueManager) -> None:
    ...
    for task in tasks:
        ...
        await manager.update_task_metadata(task.id, {"workflow_status": "pending_retry_forced", "retry_timestamp": datetime.now().isoformat()}, merge=True)
        await manager.update_task_metadata(task.id, {"workflow_status": "pending_retry_forced", "retry_timestamp": datetime.now(timezone.utc).isoformat()}, merge=True)
...
async def _cmd_complete_task(manager: QueueManager, task_id: str) -> None:
    ...
    await manager.update_task_metadata(task_id, {"workflow_status": "completed", "completedAt": datetime.now().isoformat()}, merge=True)
    await manager.update_task_metadata(task_id, {"workflow_status": "completed", "completedAt": datetime.now(timezone.utc).isoformat()}, merge=True)
...
async def _cmd_check_integrity(manager: QueueManager) -> dict:
    report: dict = {"integrity_check": "ok", "zombie_tasks": [], "orphan_dependencies": []}
    try:
        async with aiosqlite.connect(manager.db_path) as db:
            db.row_factory = sqlite3.Row
            ...
            try:
                ...
                if list_models_ok:
                    try:
                        ...
                        _text, _usage = await call_gemini(session, model, system_prompt, health_prompt, key, client_timeout=timeout, require_json=False)
                        generate_ok = True
...
async def _cmd_run_health_parallel(manager: QueueManager) -> dict:
    ...
    async with aiosqlite.connect(manager.db_path) as db, db.execute("SELECT call_count FROM daily_usage WHERE date = ?", (budget_day,)) as cursor:
        row = await cursor.fetchone()
        used_budget = row[0] if row else 0
    return {
        "status": "ok",
    ...
    async with semaphore:
        ...
        try:
            if provider_id == "gemini":
                ...
                await call_anthropic(session, "claude-3-haiku-20240307", "ping", "ping", key, client_timeout=aiohttp.ClientTimeout(total=15), require_json=False)
            ...
            root_cause = e.__cause__ if e.__cause__ else e
            return (provider, mask, STATUS_FAIL, f"Conexao: {type(root_cause).__name__}: {root_cause}", LATENCY_NA)
...
async def _cli_db_init(argv: list, manager: QueueManager) -> None:
    ...
    try:
        if os.name == 'nt':
            ...
            lock_file = open(lock_file_path, 'w')  # noqa: ASYNC230, SIM115
...
async def _cli_db_fallback_stats(argv: list, manager: QueueManager) -> None:
        window_minutes = int(argv[2]) if len(argv) > 2 else 180
        print(json.dumps({"window_minutes": window_minutes, "rows": await _cmd_fallback_stats(manager, window_minutes)}, indent=2))
...
async def _cli_db_rate_limits(argv: list, manager: QueueManager) -> None:
        days = int(argv[2]) if len(argv) > 2 else 7
        print(json.dumps({"days": days, "rate_limits_429": await _cmd_get_rate_limits(manager, days)}, indent=2))
...
async def _cli_db_commands(argv: list, manager: QueueManager) -> None:
        db_commands = {
...
async def _cli_autonomy(argv: list, manager: QueueManager) -> None:
        ...
        te = _get_runtime()
        labels = {"stop": "W0", "default": "Homeostase", "partial": "Equilibrio", "full": "Agencia Total"}
...
async def _cli_ingest(argv: list, manager: QueueManager) -> None:
        ...
        try:
            ...
            te = _get_runtime()
            await te.apply_god_mode(content, manager)
...
async def _cli_query(argv: list, manager: QueueManager) -> None:
        ...
        try:
            te = _get_runtime()
            rag = te.get_rag()
            from task_executor import get_rag
            rag = get_rag()
...
async def _cli_gemini_health(argv: list, manager: QueueManager) -> None:
        ...
        print(json.dumps(await _cmd_run_gemini_health(manager, window_minutes), indent=2, ensure_ascii=True))
...
async def _handle_cli_command(cmd: str, argv: list, manager: QueueManager) -> None:
    handlers = {
        ...
        "db-fallback-stats": _cli_db_fallback_stats, "fallback-stats": _cli_db_fallback_stats, "nexus-fallback": _cli_db_fallback_stats,
        "db-fallback-prune": _cli_db_fallback_prune, "fallback-prune": _cli_db_fallback_prune,
        "db-fallback-prune-legacy": _cli_db_fallback_prune_legacy, "fallback-prune-legacy": _cli_db_fallback_prune_legacy,
        "db-rate-limits": _cli_db_rate_limits, "rate-limits": _cli_db_rate_limits, "nexus-rate-limits": _cli_db_rate_limits,
        "watchdog": _cli_watchdog, "watchdog-stats": _cli_watchdog,
        "db-route-health": _cli_db_route_health, "route-health": _cli_db_route_health, "nexus-route-health": _cli_db_route_health,
        "get": _cli_get,
        "db-retry": _cli_retry, "retry": _cli_retry,
        "db-retry-failed": _cli_retry_failed, "retry-failed": _cli_retry_failed,
        "db-complete": _cli_complete, "complete": _cli_complete,
        ...
        "ingest": _cli_ingest,
        "query": _cli_query,
        "gemini-health": _cli_gemini_health, "nexus-gemini-health": _cli_gemini_health,
        "check-keys": _cli_check_keys, "nexus-keys": _cli_check_keys,
        "health": _cli_health,
        "worker": _cli_worker, "worker-api": _cli_worker,
    }

    if cmd in handlers:
        await handlers[cmd](argv, manager)
    else:
