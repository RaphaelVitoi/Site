...
export default function MasterSimulator() {
  ...
  useEffect(() => {
    if (isSyncHydrated && physics) {
      ...
      if (aggressionFactor !== physics.edgeFactor)
        setAggressionFactor(physics.edgeFactor);
    }
  }, [
    isSyncHydrated,
    physics,
  ...
  const insolvencyRadarData = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        Ameaça: Math.min(
          100,
          Math.max(0, Math.abs(apiQuantumMetrics.adjustedEvFold) * 30),
        ),
      },
      {
        subject: "Risco FGS",
        Ameaça: Math.min(
          100,
          Math.max(0, 100 - apiQuantumMetrics.expectativa * 50),
        ),
      },
      {
        ...
        Ameaça: Math.min(
          100,
          Math.max(0, apiQuantumMetrics.marginInstability * 100),
        ),
      },
      { subject: "Insolvência", Ameaça: apiQuantumMetrics.isSolvent ? 10 : 90 },
      {
        ...
        Ameaça: Math.min(
          100,
          Math.max(0, 100 - apiQuantumMetrics.esperanca * 50),
        ),
      },
    ];
  }, [apiQuantumMetrics]);
  ...
  const dispatchInsolvencyMatrixAdapter = useCallback(
    (payload: import("./SotaContext").InsolvencyPayload) => {
      if (typeof dispatchInsolvencyMatrix === "function") {
        (dispatchInsolvencyMatrix as any)(
          payload.villainRange,
          payload.board,
          payload.rpFactor,
          payload.heroInvested,
          payload.currentPot,
          payload.activePlayers,
          payload.kappaOverride,
          payload.heroRange,
          payload.betSizing,
        );
      }
  ...
  const wasmContextValue = useMemo(
    () => ({
      ...
      insolvencyMatrixData,
      isCalculatingInsolvency,
      dispatchInsolvencyMatrix: dispatchInsolvencyMatrixAdapter,
      dispatchIcmDistortion,
      nashResults,
      setManualEquity,
    }),
    [
      ...
      insolvencyMatrixData,
      isCalculatingInsolvency,
      dispatchInsolvencyMatrixAdapter,
      dispatchIcmDistortion,
      nashResults,
      setManualEquity,
    ],
              ...
              >
                <div className="max-w-4xl mx-auto mb-16 text-center space-y-8">
                  ...
                  <p className="text-[0.9rem] text-text-dim leading-relaxed mx-auto max-w-3xl font-medium">
                    ...
                    . O motor ingere a estrutura de premiação e os stacks reais
                    da Mesa Final para erguer as fundações matemáticas do
                    cálculo de{" "}
              ...
              <div className="sota-container px-6 flex flex-col items-center justify-center gap-10 relative z-10">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-[0.65rem] font-bold text-text-muted uppercase tracking-[0.3em] m-0">
                    Estado da Arte em Teoria de Jogo & Perspectiva Matemárica
                  </p>
                </div>
