...
export default function MasterSimulator ()
{
    ...
    useEffect(() => {
        if (isSyncHydrated && physics) {
            ...
            if (heroPosition !== physics.position) setHeroPosition(physics.position as any);
            if (aggressionFactor !== physics.edgeFactor) setAggressionFactor(physics.edgeFactor);
        }
    }, [isSyncHydrated, physics, currentPot, heroInvested, heroPosition, aggressionFactor, setCurrentPot, setHeroInvested, setHeroPosition, setAggressionFactor]);
    ...
    useEffect( () =>
    {
        // SOTA FIX: Instanciação estrita via module para evitar parsing EOF do Turbopack e proteger o escopo do GC.
    ...
    const apiQuantumMetrics = useMemo(() => {
        if (!quantumPerspectiva || !scenario.stacks || scenario.stacks.length === 0) return null;
        if (!quantumPerspectiva || !scenario.stacks || scenario.stacks.length < 2) return null;
        try {
            return computeQuantumMetrics(quantumPerspectiva, safeActivePlayers, safeHeroInvested, scenario.stacks);
        } catch (e) {
            console.warn("[SOTA] Fricção evitada: Motor quântico aguardando simetria topológica (Hidratação Pendente).", e);
            console.warn("[SOTA] Fricção evitada: Motor quântico aguardando simetria topológica (Hidratação Pendente).");
            return null;
        }
    }, [quantumPerspectiva, safeActivePlayers, safeHeroInvested, scenario.stacks]);
    ...
    const wasmContextValue = useMemo( () => ( {
        nativeRangeMetric, insolvencyMatrixData, isCalculatingInsolvency, dispatchInsolvencyMatrix, dispatchIcmDistortion, nashResults, setManualEquity
    } ), [ nativeRangeMetric, insolvencyMatrixData, isCalculatingInsolvency, dispatchInsolvencyMatrix, dispatchIcmDistortion, nashResults, setManualEquity ] );
    ...
    const activeToolContent = useMemo( () =>
    {
        const toolContents: Record<ActiveTool, React.ReactNode> = {
                ...
                <Suspense fallback={ <LoadingFallback /> }>
                    <div className="flex flex-col gap-10">
                        <ScenarioStage
                            ...
                            dynamicDeathZone={ apiQuantumMetrics?.threshEq ? apiQuantumMetrics.threshEq * 100 : undefined }
                        />
            ...
            comparar: <Suspense fallback={ <LoadingFallback /> }><div className="h-187.5 w-full relative">{scenarios && scenarios.length > 0 ? <ComparisonRadar key={`comp-${scenario.id}`} scenarios={ scenarios } currentId={ scenario.id } nashFlop={ nashFlop } /> : <LoadingFallback />}</div></Suspense>,
                ...
                <SotaWasmContext.Provider value={ wasmContextValue }>
                    ...
                    <div className="flex flex-col gap-24 pt-20 pb-32">
                        ...
                        <section className="sota-container" aria-label="Laboratório ICM">
                                ...
                                <div className={ `grid gap-12 items-start ${ sidebarOpen ? 'grid-cols-1 xl:grid-cols-[360px_1fr]' : 'grid-cols-1' }` }>
                                    { sidebarOpen && (
