...
export default function MasterSimulator ()
{
    ...
    const activeToolContent = useMemo( () =>
    {
        const toolContents: Record<ActiveTool, React.ReactNode> = {
            ...
            comparar: <Suspense fallback={ <LoadingFallback /> }><div className="h-[600px] w-full relative"><ComparisonRadar key={`comp-${scenario.id}`} scenarios={ scenarios } currentId={ scenario.id } nashFlop={ nashFlop } /></div></Suspense>,
            ...
            cfr: <Suspense fallback={ <LoadingFallback /> }><div className="min-h-[500px] w-full relative"><CfrRegretPanel key={`cfr-${scenario.id}`} initialPot={safeCurrentPot} initialStack={Math.min(heroUpdatedStack, villainUpdatedStack)} initialEquity={nativeRangeMetric.equity} /></div></Suspense>
        };
