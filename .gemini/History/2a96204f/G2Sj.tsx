...
export default function MasterSimulator ()
{
    ...
    const activeToolContent = useMemo( () =>
    {
        const toolContents: Record<ActiveTool, React.ReactNode> = {
            ...
            comparar: <Suspense fallback={ <LoadingFallback /> }><div className="h-187.5 w-full relative"><ComparisonRadar key={`comp-${scenario.id}`} scenarios={ scenarios } currentId={ scenario.id } nashFlop={ nashFlop } /></div></Suspense>,
                ...
                <SotaWasmContext.Provider value={ wasmContextValue }>
                    ...
                    <div className="flex flex-col gap-24 pt-20 pb-32">
                        ...
                        <section className="sota-container" aria-label="Laboratório ICM">
                                ...
                                <main className="flex flex-col gap-12 min-w-0 transition-all duration-500" role="main" aria-label="Painel de Ferramentas do Simulador">
                                        <SimulatorNavigation activeTool={ activeTool } onSelectTool={ setActiveTool } />
                                        <div className={ `transition-all duration-500 ${ isPending ? 'opacity-40 blur-sm scale-[0.99]' : 'opacity-100 scale-100' }` }>
                                            { activeToolContent }
                                        </div>

                                        {/* SOTA: Terminal Cognitivo Llama.cpp Global */}
                                        <div className="mt-8 p-10 glass-panel border-accent-indigo/20 animate-sota-in relative overflow-hidden group/terminal">
                                            <div className="absolute inset-0 bg-radial-[at_bottom_left] from-accent-indigo/5 to-transparent pointer-events-none" />
                                            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-6 relative z-10">
                                                <div className="flex items-center gap-5">
                                                    <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-accent-indigo group-hover/terminal:border-accent-indigo/30 transition-all">
                                                        <i className="fa-solid fa-terminal text-lg" />
                                                    </div>
                                                    <div>
                                                        <h3 className="text-sm font-black text-text-bright uppercase tracking-[0.2em] m-0">Terminal Analítico (llama.cpp)</h3>
                                                        <p className="text-[0.65rem] text-text-dim mt-1 font-mono uppercase tracking-widest">Processamento Neural Bare Metal</p>
                                                    </div>
                                                </div>
                                                <div className="flex gap-4" aria-live="polite">
                                                    <button
                                                        ...
                                                        className="btn-primary px-8 py-3 text-[0.7rem] font-black uppercase tracking-widest rounded-xl disabled:opacity-50 shadow-2xl transition-all hover:scale-[1.02] active:scale-95"
                                                    >
                                                        {isLlamaProcessing ? 'Processando...' : 'Injetar Llama'}
                                                    </button>
                                                    {isLlamaProcessing && (
                                                        <button onClick={stopLlama} className="px-6 py-3 text-[0.7rem] font-black uppercase tracking-widest bg-rose-500/10 text-rose-400 border border-rose-500/30 rounded-xl hover:bg-rose-500/20 transition-colors shadow-lg active:scale-95">
                                                            Abortar
                                                        </button>
                                                    )}
                                                </div>
                                            ...
                                            {llamaError && (
                                                <div role="alert" className="mb-6 text-rose-500 font-mono text-[0.7rem] p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl shadow-inner uppercase tracking-widest font-black">{llamaError}</div>
                                            )}
                                            <div aria-live="polite" aria-atomic="true" className="relative z-10">
                                                <TerminalLog lines={llamaLines} isProcessing={isLlamaProcessing} title="NEXUS ORCHESTRATOR - BARE METAL GPU" height="h-80" />
                                            </div>
                                        </div>
                                    </main>
                                </div>
