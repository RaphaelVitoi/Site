...
export default function MasterSimulator ()
{
    ...
    // SOTA: Dados Dinâmicos processados para alimentar o Radar de Insolvência
        ...
        const toolContents: Record<ActiveTool, React.ReactNode> = {
            scenario: (
                <Suspense fallback={ <LoadingFallback /> }>
                    ...
                    <div className="mb-12 glass-panel animate-sota-in p-8">
                        <div className="p-8 bg-black/40 border border-white/5 rounded-3xl shadow-inner mb-12">
                            <h3 className="text-center text-lg font-black text-accent-rose-light uppercase tracking-widest mb-2">
                                <i className="fa-solid fa-radar mr-2"></i> Diagnóstico de Insolvência
                            </h3>
                            <p className="text-center text-xs text-text-muted mb-6">Mapeamento vetorial de ameaças sistêmicas e colapso de equidade.</p>
                            <div className="h-100 w-full relative">
                                {insolvencyRadarData.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%" minWidth={10} minHeight={10}>
                                        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={insolvencyRadarData}>
                                            <PolarGrid stroke="rgba(255,255,255,0.15)" strokeWidth={1} />
                                            <PolarAngleAxis dataKey="subject" tick={{ fill: '#fb7185', fontSize: 11, fontWeight: 'bold' }} />
                                            <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
                                            <Radar name="Ameaça (%)" dataKey="Ameaça" stroke="#e11d48" strokeWidth={2} fill="#e11d48" fillOpacity={0.3} />
                                            <RechartsTooltip contentStyle={{ backgroundColor: '#0a0f1c', border: '1px solid rgba(225,29,72,0.3)', borderRadius: '8px' }} itemStyle={{ color: '#fb7185', fontWeight: 'bold' }} />
                                        </RadarChart>
                                    </ResponsiveContainer>
                                ) : (
                                    <div className="flex items-center justify-center h-full text-text-dim text-sm uppercase tracking-widest font-mono">
                                        Aguardando dados quânticos...
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            comparar: <Suspense fallback={ <LoadingFallback /> }><div className="h-150 w-full relative"><ComparisonRadar key={`comp-${scenario.id}`} scenarios={ scenarios } currentId={ scenario.id } nashFlop={ nashFlop } /></div></Suspense>,
            ...
            cfr: <Suspense fallback={ <LoadingFallback /> }><div className="min-h-125 w-full relative"><CfrRegretPanel key={`cfr-${scenario.id}`} initialPot={safeCurrentPot} initialStack={Math.min(heroUpdatedStack, villainUpdatedStack)} initialEquity={nativeRangeMetric.equity} /></div></Suspense>
        };
        ...
        <SotaSpotContext.Provider value={ spotContextValue }>
            <SotaMetricsContext.Provider value={ metricsContextValue }>
                <SotaWasmContext.Provider value={ wasmContextValue }>
                    ...
                    <section className="sota-container mt-12 mb-16 animate-sota-in" aria-label="Dashboard Quântico">
