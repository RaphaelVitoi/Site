...
try {
    ...
    # SOTA: Expurgo de bloqueios (Friccao Zero).
    # Remove o banco vetorial e locks temporarios da pasta de staging antes de zipar.
    $ChromaDbPath = Join-Path $SnapshotDir '.claude\agent-memory\.chroma_db'
    if (Test-Path $ChromaDbPath) {
        Remove-Item -Path $ChromaDbPath -Recurse -Force -ErrorAction SilentlyContinue
    }
    Get-ChildItem -Path $SnapshotDir -Include *.db-wal, *.db-shm -Recurse -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
