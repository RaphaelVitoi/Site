...
// SOTA: Helpers Puros para Erradicação de Complexidade Ciclomática (SonarLint S3776/S3358/S7735)
const formatEvValue = ( val?: number | null, decimals = 2, suffix = '' ) => {
    if ( val == null ) return '—';
    const sign = val > 0 ? '+' : '';
    return `${sign}${val.toFixed( decimals )}${suffix}`;
};

const getEvColor = ( val?: number | null, posClass = 'text-teal-400', negClass = 'text-rose-500' ) => {
    if ( val == null ) return 'text-neutral-500';
    return val >= 0 ? posClass : negClass;
};

const RiskPremiumDashboardComponent: React.FC<RiskPremiumDashboardProps> = ( { spot } ) => {
    const icmEvVal = spot.isBaseline ? spot.chipEv : ( spot.icmEv ?? ( spot.chipEv - spot.riskPremium ) );
    const expectativaVal = spot.apiQuantumMetrics?.expectativa;
    const perspectivaVal = spot.apiQuantumMetrics?.perspectiva;

    const esperancaDisplay = useMemo( () => {
        if ( spot.apiQuantumMetrics?.esperanca != null )
        {
            const { esperanca } = spot.apiQuantumMetrics;
            const sign = esperanca > 0 ? '+' : '';
            return `Esp: ${sign}${esperanca.toFixed( 2 )}bb`;
        }
        const sign = spot.chipEv > 0 ? '+' : '';
        return `cEV: ${sign}${spot.chipEv.toFixed( 1 )}`;
    }, [ spot.apiQuantumMetrics, spot.chipEv ] );

    const pisoDisplay = useMemo( () => {
        if ( spot.apiQuantumMetrics?.adjustedEvFold != null )
        {
            return `${spot.apiQuantumMetrics.adjustedEvFold.toFixed( 2 )}bb (RIO)`;
        }
        const pisoValue = ( ( 1 - ( spot.chipEv / ( spot.pot || 1 ) ) ) * spot.riskPremium );
        return `${pisoValue.toFixed( 1 )}%`;
    }, [ spot.apiQuantumMetrics, spot.chipEv, spot.pot, spot.riskPremium ] );
    ...
    // Fallback Determinístico de Matriz de Risco SOTA (se o Motor ainda não tiver injetado)
    const safeCollisionMatrix = useMemo( () => {
        if ( spot.collisionMatrix ) return spot.collisionMatrix;
        ...
        return Array.from( { length: 169 } ).map( ( _, index ) => {
            ...
            let strength = rankValue + ( isPair ? 4 : 0 ) + ( isSuited ? 2 : 0 );

            // SOTA: Força Bruta Estrutural da mão (0.0 para 32o, 1.0 para AA)
            const rawPower = Math.max( 0, ( strength - 5 ) / 27 );

            // Lei de Shannon (Decaimento Exponencial): RP esmaga o range, preservando o topo e aniquilando a base
            const tensionExponent = 1 + ( spot.riskPremium / 15 );

            return Math.pow( rawPower, tensionExponent );
        } );
    ...
    return (
        <div className="flex flex-col w-full bg-black text-neutral-300 font-mono border border-neutral-900 rounded-xl shadow-[0_0_40px_rgba(0,0,0,0.8)] relative">
            ...
            {/* CADEIA DE TRANSMUTAÇÃO QUÂNTICA SOTA */ }
            <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-neutral-900/60 bg-[#050505] border-b border-neutral-900/50">
                <MetricTooltip align="center" title="ICMev (Snapshot)" desc="A fotografia estática do torneio. Avalia a utilidade das fichas no vácuo, descontando o Risk Premium da Equidade Bruta, mas sem considerar dinâmica de mesa, tempo ou entropia humana.">
                    <div className="p-4 flex flex-col items-center justify-center text-center hover:bg-white/2 transition-colors">
                        <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">ICMev (Snapshot)</span>
                        <span className={ `text-xl font-light ${getEvColor( icmEvVal, 'text-emerald-500/80', 'text-[#cd5c5c]' )}` }>
                            { formatEvValue( icmEvVal, 1, '%' ) }
                        </span>
                    </div>
                </MetricTooltip>
                <MetricTooltip align="center" title="Expectativa (Preditiva)" desc="Adiciona a Antevisão de Saúde (FGS) e a Realização Posicional (R). Mede a urgência de sobrevivência frente ao tempo e à posição.">
                    <div className="p-4 flex flex-col items-center justify-center text-center hover:bg-white/2 transition-colors">
                        <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">Expectativa</span>
                        <span className={ `text-xl font-light ${getEvColor( expectativaVal, 'text-indigo-400', 'text-rose-400' )}` }>
                            { formatEvValue( expectativaVal, 2, ' bb' ) }
                        </span>
                    </div>
                </MetricTooltip>
                <MetricTooltip align="right" title="Perspectiva (Síntese)" desc="O Veredito Final. Subtrai o Piso Dinâmico (EV_fold penalizado por RIO MW) da Expectativa. Se > 0, o investimento supera matematicamente o custo de desistência.">
                    <div className="p-4 flex flex-col items-center justify-center text-center hover:bg-white/2 transition-colors">
                        <span className="text-[10px] text-teal-600 font-bold uppercase tracking-widest mb-1">Perspectiva</span>
                        <span className={ `text-2xl font-bold ${getEvColor( perspectivaVal, 'text-teal-400', 'text-rose-500' )}` }>
                            { formatEvValue( perspectivaVal, 2 ) }
                        </span>
                    </div>
                </MetricTooltip>
            </div>
            ...
            { showDeathZone && (
                <div className="w-full bg-[#050505] border-b border-neutral-900/60 p-6 flex justify-center items-center shadow-inner relative z-10">
                        ...
                        <div className="z-10 relative flex-1">
                            ...
                            <div className="mt-6 pt-4 border-t border-neutral-900/50 flex justify-between items-end">
                                <div>
                                    ...
                                    <p className="text-lg font-medium text-neutral-500">{ spot.isBaseline ? '1.00' : spot.bubbleFactor.toFixed( 2 ) }</p>
                                </div>
                ...
                <div className="flex flex-col p-8 flex-1">
                    <MetricTooltip align="right" title="Distorção Estratégica" desc="Representação visual do desvio comportamental exigido pelo ICM. Um Risk Premium alto dilata a frequência de Fold, obliterando os limites permissivos de ChipEV puro.">
                        ...
                        <div className="mt-5 flex justify-between text-[9px] font-bold uppercase tracking-widest text-neutral-600 whitespace-nowrap gap-2">
                            <span>{ esperancaDisplay }</span>
                            <span>Piso: { pisoDisplay }</span>
                        </div>
                    </MetricTooltip>
                </div>
            </div>
