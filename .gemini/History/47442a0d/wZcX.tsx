        ...
        <div className="flex flex-col w-full bg-black text-neutral-300 font-mono border border-neutral-900 rounded-xl shadow-[0_0_40px_rgba(0,0,0,0.8)] relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-neutral-900/60 bg-[#050505] border-b border-neutral-900/50">
                <MetricTooltip align="center" title="ICMev (Snapshot)" desc="A fotografia estática do torneio. Avalia a utilidade das fichas no vácuo, descontando o Risk Premium da Equidade Bruta, mas sem considerar dinâmica de mesa, tempo ou entropia humana.">
                    <div className="p-4 flex flex-col items-center justify-center text-center hover:bg-white/[0.02] transition-colors">
                        <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mb-1">ICMev (Snapshot)</span>
                        <span className={ `text-xl font-light ${icmEvVal >= 0 ? 'text-emerald-500/80' : 'text-[#cd5c5c]'}` }>
                            { icmEvVal != null ? `${icmEvVal > 0 ? '+' : ''}${icmEvVal.toFixed( 1 )}%` : '—' }
                        </span>
                    </div>
                </MetricTooltip>
                <MetricTooltip align="center" title="Expectativa (Preditiva)" desc="Adiciona a Antevisão de Saúde (FGS) e a Realização Posicional (R). Mede a urgência de sobrevivência frente ao tempo e à posição.">
                    <div className="p-4 flex flex-col items-center justify-center text-center hover:bg-white/[0.02] transition-colors">
                        ...
                        <span className={ `text-xl font-light ${expectativaVal != null && expectativaVal >= 0 ? 'text-indigo-400' : 'text-rose-400'}` }>
                            { expectativaVal != null ? `${expectativaVal > 0 ? '+' : ''}${expectativaVal.toFixed( 2 )} bb` : '—' }
                        </span>
                    </div>
                </MetricTooltip>
                <MetricTooltip align="center" title="Perspectiva (Síntese)" desc="O Veredito Final. Subtrai o Piso Dinâmico (EV_fold penalizado por RIO MW) da Expectativa. Se > 0, o investimento supera matematicamente o custo de desistência.">
                    <div className="p-4 flex flex-col items-center justify-center text-center hover:bg-white/[0.02] transition-colors">
                        ...
                        <span className={ `text-2xl font-bold ${perspectivaVal != null && perspectivaVal >= 0 ? 'text-teal-400' : 'text-rose-500'}` }>
                            { perspectivaVal != null ? `${perspectivaVal > 0 ? '+' : ''}${perspectivaVal.toFixed( 2 )}` : '—' }
                        </span>
                    </div>
                </MetricTooltip>
            </div>
