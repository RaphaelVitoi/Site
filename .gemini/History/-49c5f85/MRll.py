...
class MemoryRAG:
    ...
    async def query_memory(
        ...
        try:
            ...
            # A busca expandida retorna uma lista de resultados por variacao de query.
            # Mesclamos os resultados mantendo o melhor (menor) distance score.
            unique_docs = {}
            ...
            for i in range(len(res_docs)):
                if not res_docs[i]:
                    continue
                for j, doc in enumerate(res_docs[i]):
                    current_meta = res_metas[i][j] if res_metas and i < len(res_metas) else {"agent": "Unknown", "source": "N/A"}
                    current_dist = res_dists[i][j] if res_dists and i < len(res_dists) else 0.0

                    if doc not in unique_docs:
                        unique_docs[doc] = {"meta": current_meta, "dist": current_dist}
                    else:
                        if current_dist < unique_docs[doc]["dist"]:
                            unique_docs[doc]["dist"] = current_dist
