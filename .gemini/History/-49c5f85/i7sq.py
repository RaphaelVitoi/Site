...
class MemoryRAG:
    ...
    async def _extract_pdf(self, file_path: Path) -> str:
        ...
        try:
            ...
            logger.error(f"[RAG] Falha ao extrair texto do PDF ({file_path.name}): {e}")
            return ""
    ...
    async def _extract_csv(self, file_path: Path) -> str:
        ...
        try:
            ...
            logger.error(f"[RAG] Falha ao extrair texto do CSV ({file_path.name}): {e}")
            return ""
    ...
    async def _process_single_file(
        ...
        if chunks:
            logger.info(
                f"[RAG] Vetorizando {len(chunks):02d} fragmentos de '{source_name}'... {file_info}"
            )
