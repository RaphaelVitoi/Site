...
class MemoryRAG:
    ...
    async def _extract_text_from_file(self, file_path: Path) -> str:
        if file_path.suffix.lower() == ".docx":
            ...
            try:
                # SOTA: Isolamento de I/O sincrono pesado para nao afogar o Async Event Loop
                return await asyncio.to_thread(_read_pdf_sync)
            except Exception as e:  # noqa: BLE001
                logger.error(
                    f"[RAG] Falha ao extrair texto do PDF ({file_path.name}): {e}"
                )
                return ""
            ...
            try:
                # SOTA: Isolamento de I/O sincrono pesado
                return await asyncio.to_thread(_read_csv_sync)
            except Exception as e:  # noqa: BLE001
                logger.error(
                    f"[RAG] Falha ao extrair texto do CSV ({file_path.name}): {e}"
                )
                return ""
            ...
            try:
                # SOTA: Isolamento de I/O sincrono pesado
                return await asyncio.to_thread(_read_xlsx_sync)
            except Exception as e:  # noqa: BLE001
                logger.error(
                    f"[RAG] Falha ao extrair texto do XLSX ({file_path.name}): {e}"
                )
                return ""
        else:
    ...
    async def _process_single_file(self, file_path: Path) -> list[str]:
        ...
        if chunks:
            logger.info(
                f"[RAG] Vetorizando {len(chunks):02d} fragmentos de '{source_name}'... (Aguarde, processamento intensivo de CPU)"
            )
            await asyncio.to_thread(
                self.collection.upsert, documents=chunks, metadatas=metadatas, ids=ids
            )
            logger.info(
                f"[OK] Ingeridos {len(chunks):02d} fragmentos de: {source_name}"
            )
        return ids
