    ...
    def _flatten_and_deduplicate_results(
        ...
        for i in range(len(res_docs)):
            ...
            for j, doc in enumerate(res_docs[i]):
                current_meta = (
                    res_metas[i][j]
                    if res_metas and i < len(res_metas)
                    else {"agent": "Unknown", "source": "N/A"}
                )
                current_dist = (
                    res_dists[i][j] if res_dists and i < len(res_dists) else 0.0
                )

                if doc not in unique_docs:
                    unique_docs[doc] = {"meta": current_meta, "dist": current_dist}
                else:
                    # Ruff PLR1730: Substituicao de condicional por comparacao matematica SOTA
                    unique_docs[doc]["dist"] = min(
