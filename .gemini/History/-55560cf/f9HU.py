...
try:
    ...
    print("Execute no terminal: pip install google-generativeai rich psutil")
...
def load_api_key():
    ...
    env_path = base_dir / "_env.ps1"
    if env_path.exists():
        with open(env_path, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                match = re.search(r'(?:\$env:|\$)?GEMINI_API_KEY\s*[:=]\s*[\'"]?([^\'"\s#]+)[\'"]?', line, re.IGNORECASE)
                if match:
...
if not api_key:
    print("[ERRO] Chave GEMINI_API_KEY nao encontrada no _env.ps1 ou variaveis de ambiente.")
...
def get_extension_host_memory():
    ...
    for p in psutil.process_iter(['name', 'cmdline', 'memory_info']):
        try:
            ...
            if name and 'code' in name.lower() and cmd and any('extensionHost' in c for c in cmd):
