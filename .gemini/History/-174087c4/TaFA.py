        ...
        try:
            ...
            if is_protected:
                if effective_mode == "full" and agent_name == AGENT_CHICO:
                    ...
                    "Re-escrevendo arquivo de Kernel: %s", filepath
                    )
                else:
                    logger.error(
                    "[SEC] Bloqueio de escrita em arquivo protegido. "
                    "%s nao possui privilegios de Tier 1: %s", agent_name, filepath
                    )
                    continue
...
def _validate_command(cmd: str, effective_mode: str, agent_name: str) -> bool:
    ...
    if effective_mode not in ["full", "full_restricted"] and any(
        ...
        error_msg = (f"Encadeamento, sub-expressoes ou redirecionamento bloqueado no modo "
    ...
    if effective_mode == "partial" and any(
        ...
        logger.warning(
            ...
            "Comando interceptado: '%s'", agent_name, cmd
        )
...
async def _execute_commands(text: str, effective_mode: str, agent_name: str) -> None:
    ...
    if effective_mode in ["stop", "default"]:
        logger.warning(
            "[SEC] Execucao de terminal bloqueada por hardware-lock no modo '%s'.", effective_mode
        )
        return
