...
def _read_legacy_autonomy_config() -> str:
    ...
    if config_path.exists():
        try:
            ...
            logger.debug(f"[AUTONOMY] Falha ao ler configuracao legada: {e}")
    return "stop"
...
async def get_autonomy_mode(manager: QueueManager) -> str:
    ...
    if mode not in VALID_AUTONOMY_MODES:
        logger.warning(
            f"[AUTONOMY] Modo invalido '{mode}' detectado. Revertendo para 'stop' (W0)."
        )
        mode = "stop"
        ...
        try:
            ...
            if is_protected:
                if effective_mode == "full" and agent_name == AGENT_CHICO:
                    logger.warning(
                        f"[GOD MODE W3] Override de Seguranca Absoluto (TIER 1). Re-escrevendo arquivo de Kernel: {filepath}"
                    )
                else:
                    logger.error(
                        f"[SEC] Bloqueio de escrita em arquivo protegido. {agent_name} nao possui privilegios de Tier 1: {filepath}"
                    )
                    continue
            ...
            logger.exception(f"[FAIL] Falha ao forjar {filepath}")
...
def _validate_command(cmd: str, effective_mode: str, agent_name: str) -> bool:
    ...
    # SOTA: Expansão implacável dos vetores de bypass (sub-expressões, backticks, redirecionamentos).
    if effective_mode not in ["full", "full_restricted"] and any(
        char in cmd for char in [";", "|", "&&", "&", "$", "`", ">", "<"]
    ):
        error_msg = f"Encadeamento, sub-expressoes ou redirecionamento bloqueado no modo '{effective_mode}'. Privilegio insuficiente para {agent_name}."
    ...
    if effective_mode == "partial" and any(
        ...
        logger.warning(
            f"[SEC TIER 3] O agente {agent_name} tentou mutar o estado do ecossistema. Comando interceptado: '{cmd}'"
        )
...
async def _execute_commands(text: str, effective_mode: str, agent_name: str) -> None:
    ...
    if effective_mode in ["stop", "default"]:
        logger.warning(
            f"[SEC] Execucao de terminal bloqueada por hardware-lock no modo '{effective_mode}'."
        )
        return
    ...
    for match in re.finditer(cmd_pattern, text, re.DOTALL | re.IGNORECASE):
        ...
        try:
            ...
            logger.exception(f"[FAIL] Arritmia no comando {cmd}")
            raise
...
async def apply_god_mode(
    ...
    if not agent_name:
        ...
        # SOTA: Obliteração da escalada de privilégios (Zero-Trust). Fallback para Tier inferior.
    ...
    if global_mode in ["stop", "default"]:
        ...
        logger.info(
            "[TIER 1] @chico invocando autoridade executiva máxima. Operando sob subordinação exclusiva ao Tier 0."
        )
    elif agent_name == "@maverick":
        effective_mode = (
            "full_restricted" if global_mode == "full" else global_mode
        )  # W2.5
    else:
        effective_mode = (
            "partial" if global_mode in ["partial", "full"] else global_mode
        )  # W2 max
    ...
    if effective_mode == "default":
        logger.info(
            "[GOD MODE] W1 (Default) ativo. Homeostase. Arquivos forjados, execução de comandos bloqueada."
        )
        return modified_files
