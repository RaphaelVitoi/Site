/**
 * IDENTITY: Motor de Propagação de Frequências (Entanglement Quântico)
 * PATH: src/components/simulator/engine/propagation.ts
 * ROLE: Orquestrar a reação em cadeia das frequências entre streets (Flop/Turn/River).
 * BINDING: [src/components/simulator/MasterSimulator.tsx]
 */

import type { ChipEvFreqs, StreetChipEvFreqs } from './types';

/**
 * Aplica a lógica de reação em cadeia baseada em desvios de equilíbrio.
 * Se o oponente desvia no Flop, o range que chega no Turn é filtrado e tensionado.
 */
export function applyFrequencyPropagation ( sourceDeltas: Partial<ChipEvFreqs>, targetFreqs: ChipEvFreqs, attenuation: number ): ChipEvFreqs {
...
/**
 * Calcula a diferença entre dois estados de frequência.
 */
export function calculateFreqDeltas ( oldFreqs: ChipEvFreqs, newFreqs: ChipEvFreqs ): { deltas: Partial<ChipEvFreqs>, hasChange: boolean } {
...
/**
 * Orquestra a propagação entre as streets.
 */
export function propagateFrequencies ( prev: StreetChipEvFreqs, street: keyof StreetChipEvFreqs, deltas: Partial<ChipEvFreqs> ): StreetChipEvFreqs {
    const next = {
        flop: { ...prev.flop },
        turn: { ...prev.turn },
        river: { ...prev.river }
