...
const ExportAntevisaoButton: React.FC<ExportAntevisaoProps> = ( props ) => {
    ...
    const handleExport = async () => {
        try
        {
            ...
            const params = new URLSearchParams( {
                bf: props.bf.toString(),
                rp: props.rp.toString(),
                pureEv: props.pureEv.toString(),
                icmEv: props.icmEv.toString(),
                pot: props.pot.toString(),
                bet: props.bet.toString(),
            } );

            const url = `/api/og/icm-chart?${params.toString()}`;
            const response = await fetch( url );

            if ( !response.ok ) throw new Error( 'Falha ao acionar a matriz Vercel OG.' );
    ...
    return (
        <motion.button
            ...
            whileTap={ { scale: 0.98 } }
            className={ `px-5 py-2.5 flex items-center justify-center gap-2 rounded-lg font-mono text-sm tracking-wide transition-all shadow-lg backdrop-blur-sm border ${status === 'success' ? 'border-green-500/50 text-green-400 bg-green-900/20 shadow-[0_0_15px_rgba(34,197,94,0.2)]' :
                    status === 'error' ? 'border-[#cd5c5c]/50 text-[#cd5c5c] bg-[#cd5c5c]/20 shadow-[0_0_15px_rgba(205,92,92,0.2)]' :
                        'border-gray-700 text-gray-300 bg-gray-900/60 hover:bg-gray-800 hover:text-white hover:border-gray-500'
                }` }
        >
            { status === 'idle' && (
            ...
            { status === 'idle' ? 'Exportar O.G.' : status === 'loading' ? 'Forjando...' : status === 'success' ? 'Materializado' : 'Falha na Matriz' }
        </motion.button>
    );
};
