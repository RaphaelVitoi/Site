...
export function generateDynamicICMQuiz( state: SimulatorState ): QuizQuestion[] {
    ...
    const isMassiveChipleader = sortedStacks.length > 0 && totalChips > 0 && ( sortedStacks[0] ?? 0 ) > totalChips * 0.4;
    const isTopHeavy = totalPrizes > 0 && ( ( prizes[0] ?? 0 ) / totalPrizes ) > 0.35;
    const lastStack = sortedStacks.at( -1 );
    const hasCriticalShortStack = sortedStacks.length > 0 && totalChips > 0 && lastStack !== undefined && ( lastStack / totalChips ) < 0.1;
    ...
    if ( isMassiveChipleader && stacks.length > 2 ) {
        questions.push( {
            ...
            category: 'Vantagem de Risco (God Mode)'
        } );
    }
