...
export default function PsychologyHub({ posts }: Readonly<PsychologyHubProps>) {
  const safePosts = useMemo(
    () => (Array.isArray(posts) ? posts : EMPTY_POSTS),
    [posts],
  );
    ...
    return (
      <div className="text-center p-10 glass-panel animate-fade-up">
        <p className="text-slate-400 font-mono text-sm uppercase tracking-widest">
          Nenhuma anomalia psicológica detectada no banco de dados.
        </p>
      </div>
    );
  }
        ...
        <AnimatePresence mode="popLayout">
          {filteredPosts.map((post) => (
            <motion.article
              ...
              {...({
                className:
                  "bg-slate-900/60 backdrop-blur-md border border-white/5 rounded-xl p-6 transition-all duration-300 hover:-translate-y-1 hover:border-fuchsia-500/30 hover:shadow-[0_10px_30px_-15px_rgba(217,70,239,0.3)] flex flex-col h-full group",
              } as any)}
            >
              ...
              <h3 className="text-xl font-heading font-bold text-white mb-3 group-hover:text-fuchsia-300 transition-colors">
                {post.title}
              </h3>
              <p className="text-slate-400 text-sm grow leading-relaxed">
                {post.excerpt}
              </p>
