...
import { IQuantumPerspectiva, IcmDistortionResult, Scenario, SprStage, StreetChipEvFreqs } from '../engine/types';
...
export interface QuantumEngineResult {
    ...
    quantumPerspectiva: IQuantumPerspectiva | null;
}

export function useQuantumEngine ( { scenario, pkoValue, isNearPayjump, blindsRisingSoon, streetFreqs, aggressionFactor, heroIsIp = false, anteSize = 12.5, heroInvestedBb, currentPotBb, activePlayers = 2 }: QuantumEngineParams ): QuantumEngineResult {
    const pendingTelemetryRef = useRef<{ latency: number; scenarioId: string; stacksCount: number; pko: number } | null>( null );
    ...
    }, [ scenario.stacks, scenario.prizes, pkoValue, isNearPayjump, blindsRisingSoon, preflopDeadMoney, foldEvBb, heroIsIp ] );
    ...
    const derivedRp = useMemo( () => {
        ...
        try
        {
            ...
            if ( latency > 50 )
            {
                pendingTelemetryRef.current = {
                    ...
                    stacksCount: scenario.stacks.length,
                    pko: pkoValue
                };
            }
            return res;
        ...
        {
            console.error( '[useQuantumEngine] Failed to derive RPs via Malmuth-Harville:', error );
            return null;
        }
    ...
    useEffect( () => {
        ...
        void logTelemetryEvent( {
            ...
            componentName: `MasterSimulator:${payload.scenarioId}:deriveRps`,
            latency: payload.latency,
            metadata: { stacksCount: payload.stacksCount, pko: payload.pko }
        } );
    }, [ derivedRp ] );
    ...
    const effectiveIpRp = Math.round( Math.max( 0, ( ( derivedRp?.ipRp ?? Number( scenario.ipRp ) ) || 0 ) + rpAdjustment ) * 10000 ) / 10000;
    const effectiveOopRp = Math.round( Math.max( 0, ( ( derivedRp?.oopRp ?? Number( scenario.oopRp ) ) || 0 ) + rpAdjustment ) * 10000 ) / 10000;
