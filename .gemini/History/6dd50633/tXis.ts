...
export function useQuantumEngine ( { scenario, pkoValue, isNearPayjump, blindsRisingSoon, streetFreqs, aggressionFactor, heroIsIp = false, anteSize = 12.5, heroInvestedBb, currentPotBb }: QuantumEngineParams ) {
    ...
    const foldEvBb = useMemo( () => {
        ...
        const baseInvested = heroInvestedBb !== undefined ? Math.abs( heroInvestedBb ) : ( heroIsIp ? 0 : 1 );
    ...
    }, [ quantumPerspectiva, scenario.category, scenario.prizes.length, foldEvBb, scenario.sprData, preflopDeadMoney ] );
