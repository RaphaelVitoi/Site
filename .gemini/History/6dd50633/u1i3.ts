...
export function useQuantumEngine ( { scenario, pkoValue, isNearPayjump, blindsRisingSoon, streetFreqs, aggressionFactor, heroIsIp = false, anteSize = 12.5, heroInvestedBb, currentPotBb, isMultiway = false }: QuantumEngineParams ) {
    ...
    // SOTA VITOI: Consciência Topológica de Platôs vs Escada Direta
    const topologicAggression = useMemo( () => {
        if ( isBaseline ) return aggressionFactor;

        // Vale de Pressão (Platô): Sem payjump imediato, a sobrevivência passiva rende $0.
        // A Perspectiva Matemática obriga o aumento da agressividade para alavancar stack visando FT/Cravada.
        if ( !isNearPayjump ) return aggressionFactor * 1.25;

        // Borda do Platô (Sub-Bolha): A um passo do payjump (ex: 14 left), o valor da sobrevivência volta a ser imperativo.
        return aggressionFactor * 0.75;
    }, [ aggressionFactor, isNearPayjump, isBaseline ] );

    const { nashFlop, nashTurn, nashRiver } = useMemo( () => ( {
        nashFlop: solveIcmDistortion( ipRpFlop, oopRpFlop, streetFreqs.flop, topologicAggression, isMultiway ),
        nashTurn: solveIcmDistortion( ipRpTurn, oopRpTurn, streetFreqs.turn, topologicAggression, isMultiway ),
        nashRiver: solveIcmDistortion( ipRpRiver, oopRpRiver, streetFreqs.river, topologicAggression, isMultiway ),
    } ), [ ipRpFlop, oopRpFlop, ipRpTurn, oopRpTurn, ipRpRiver, oopRpRiver, streetFreqs, topologicAggression, isMultiway ] );
