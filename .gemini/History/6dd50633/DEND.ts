...
export function useQuantumEngine ( { scenario, pkoValue, isNearPayjump, blindsRisingSoon, streetFreqs, aggressionFactor, heroIsIp = false, anteSize = 12.5, heroInvestedBb, currentPotBb, isMultiway = false }: QuantumEngineParams ) {
    ...
    const { nashFlop, nashTurn, nashRiver } = useMemo( () => ( {
        nashFlop: solveIcmDistortion( ipRpFlop, oopRpFlop, streetFreqs.flop, aggressionFactor, isMultiway ),
        nashTurn: solveIcmDistortion( ipRpTurn, oopRpTurn, streetFreqs.turn, aggressionFactor, isMultiway ),
        nashRiver: solveIcmDistortion( ipRpRiver, oopRpRiver, streetFreqs.river, aggressionFactor, isMultiway ),
    } ), [ ipRpFlop, oopRpFlop, ipRpTurn, oopRpTurn, ipRpRiver, oopRpRiver, streetFreqs, aggressionFactor, isMultiway ] );
