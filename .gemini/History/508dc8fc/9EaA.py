...
def _isolated_fit_and_extract(df: Any) -> tuple[Any, dict[str, float]]:
    ...
    model = RandomForestClassifier(
        n_estimators=100, max_depth=None, min_samples_leaf=2, random_state=42, class_weight="balanced"
    )
...
class PredictiveForestEngine:
    ...
    def _extract_profile(self, df: Any, X: Any, y: Any) -> dict[str, float]:
        if pd is None or self.model is None:
            return {}
        ...
        if "category" in df.columns:
            for col in X.columns:
                ...
                proba = model.predict_proba(sample)[0]
                vuln = (
                    proba[list(model.classes_).index(0)] if 0 in model.classes_ else 0.0
                )
                profile[cat_name] = round(float(vuln), 2)
        else:
    ...
    def train_model(self) -> bool:
        ...
        try:
            ...
            logger.exception("[PREDICTIVE] Falha catastrófica no isolamento do processo de treinamento.")
            return False
