...
def _isolated_fit_and_extract(df: Any) -> tuple[Any, dict[str, float]]:
    ...
    try:
        ...
        def objective(trial):
            ...
            return cross_val_score(rf, X, y, cv=2, scoring="accuracy", fit_params={"sample_weight": sample_weights}).mean()

        study = optuna.create_study(direction="maximize")
        study.optimize(objective, n_trials=10) # Fricção Zero: Apenas 10 trials para não estourar a CPU

        model = RandomForestClassifier(**study.best_params, random_state=42, class_weight="balanced", max_features="sqrt")
        ...
        # SOTA: Fallback resiliente caso falte a lib Optuna ou ocorra colapso no Cross Validation (classes minúsculas)
        model = RandomForestClassifier(
            n_estimators=100,
            max_depth=None,
            min_samples_leaf=2,
...
class PredictiveForestEngine:
    ...
    def get_predictive_profile(self) -> dict[str, float]:
        ...
        try:
            ...
            # Recupera contagem de amostras para ponderação Bayesiana
            with self._get_db_connection() as conn:
                count = conn.execute("SELECT COUNT(*) FROM VitoiPerspectiveMetric").fetchone()[0]
            ...
            for k in fallback_profile:
                blended[k] = round((trained_profile.get(k, fallback_profile[k]) * weight) + (fallback_profile[k] * (1.0 - weight)), 2)
            return blended

        except Exception:
            logger.exception("[PREDICTIVE] Erro na síntese Bayesiana. Revertendo para Prior.")
            return fallback_profile
