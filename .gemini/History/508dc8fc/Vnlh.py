...
class PredictiveForestEngine:
    ...
    def _fetch_training_data(self) -> pd.DataFrame | None:
        try:
            ...
            logger.error(f"[PREDICTIVE] Falha no I/O de dados: {e}")
            return None
                ...
                if (
                    ...
                    return False
    def _extract_profile(self, df: pd.DataFrame, X: pd.DataFrame, y: pd.Series) -> dict[str, float]:
        profile = {}
        if "category" in df.columns:
            ...
            importances = dict(zip(X.columns, self.model.feature_importances_, strict=False))
            profile = {
    ...
    def _persist_profile(self, profile: dict[str, float]) -> bool:
        try:
            ...
            with open(self.profile_path, "w", encoding="ascii", errors="backslashreplace") as f:
                json.dump(profile, f, ensure_ascii=True)
            self._is_trained = True
            logger.info(f"[PREDICTIVE] Mente Preditiva calibrada. Assinaturas bayesianas extraidas: {len(profile)}")
            return True
        except Exception as e:
            logger.error(f"[PREDICTIVE] Falha ao persistir perfil: {e}")
            return False
    ...
    def train_model(self) -> bool:
        ...
        if len(df) < 10:  # Threshold dinâmico reduzido para calibração ultra-rápida (Cold-Start)
            logger.info("[PREDICTIVE] Amostragem insuficiente (<10 registros). Treinamento abortado para prevenir overfitting.")
            return False
        ...
        df = df.dropna(subset=["is_correct"])
        if df.empty or df["is_correct"].nunique() < 2:
            logger.info("[PREDICTIVE] Variancia nula na amostra (apenas acertos/erros). O modelo exige falibilidade para treinar.")
            return False
                ...
                if "category" in df.columns:
                    ...
                    profile = {
                        ...
                        "Excesso de Agressão": round(
                            global_vuln + importances.get("feat_over_aggression", 0.0),
                            2,
                        ),
        ...
        profile = self._extract_profile(df, X, y)
