...
interface QuizEngineProps {
  ...
  onQuizRestart?: () => void;
  onAnswer?: (isCorrect: boolean, evLoss: number, category?: string) => void;
}
...
export const QuizEngine: React.FC<QuizEngineProps> = ({
  ...
  const handleSelectOption = (optionId: string) => {
    ...
    logTelemetryEvent({
      category: (currentQ.category || "Fundamentos SOTA") as
        | "quiz"
        | "simulator"
        | "performance"
        | "error"
    ...
    if (onAnswer) {
      onAnswer(isCorrect, evLoss, currentQ.category);
    }
  };
      ...
      <AnimatePresence mode="wait">
        {isFinished ? (
          <motion.div
            ...
            {...({
              className:
                "glass-panel rounded-[3rem] bg-bg-panel/60 border border-white/10 shadow-2xl overflow-hidden relative",
            } as any)}
          >
          ...
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            {...({ className: "flex flex-col gap-10" } as any)}
          >
            <QuizProgress
                  ...
                  answers[safeQuestions[currentIndex].id] && (
                    <motion.button
                      ...
                      {...({
                        onClick: handleNext,
                        className:
                          "px-8 py-4 bg-accent-indigo text-white border border-accent-indigo-light/20 rounded-2xl cursor-pointer font-black transition-all uppercase tracking-widest text-[0.7rem] flex items-center gap-4 hover:shadow-[0_0_30px_rgba(99,102,241,0.4)] active:scale-95",
                      } as any)}
                    >
