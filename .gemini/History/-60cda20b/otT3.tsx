...
function updateSizingDom(
  ...
  const flopPct = Math.min(
    1.5,
    Math.max(0.2, path[0].y * 1.5 * equityMultiplier),
  );
  ...
  if (elFlop)
    elFlop.textContent = `${flopBet.toFixed(1)} bb (${Math.round(flopPct * 100)}%)`;
  if (elTurn)
    elTurn.textContent = `${turnBet.toFixed(1)} bb (${Math.round(turnPct * 100)}%)`;
  if (elRiver)
    elRiver.textContent = `${Math.max(0, riverJam).toFixed(1)} bb (JAM)`;
}

function extractPathfinding(
  matrix: Float32Array,
  nodes: number,
): { x: number; y: number }[] {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-white/5 gap-6">
        <div>
          <h3 className="text-[0.75rem] font-black text-accent-indigo-light uppercase tracking-[0.2em] m-0">
            IA Laboratory &middot;{" "}
            <span className="text-text-muted">CFR Engine</span>
          </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2 bg-black/60 p-4 rounded-2xl border border-white/5">
                ...
                <input
                  ...
                  value={pot}
                  onChange={(e) =>
                    setPot(Number.parseFloat(e.target.value) || 0)
                  }
        ...
        <div className="flex flex-col gap-6">
          ...
          <p className="text-[0.65rem] text-text-muted leading-relaxed text-center px-4 italic font-medium">
            O heatmap visualiza a densidade de arrependimento (regret) em cada
            nó da árvore. O pathfinding busca o equilíbrio dinâmico entre
            pot-odds e pressão ICM estrutural.
          </p>
        </div>
      </div>
    </div>
