...
function extractPathfinding(matrix: Float32Array, nodes: number): {x: number, y: number}[] {
  ...
  if (path.length > 0 && path.at(-1)?.x !== 1) {
      let maxVal = -1;
      let bestY = 0;
      const x = nodes - 1;
      for (let y = 0; y < nodes; y++) {
  ...
  useEffect(() => {
    ...
    workerRef.current.onmessage = (e: MessageEvent) => {
      ...
      if (pathfindingRef.current && path.length > 0) {
         const w = 450; const h = 450; // Dimensões viewBox SVG
         let d = `M ${path[0].x * w} ${path[0].y * h}`;
         for(let i=1; i<path.length; i++) {
             d += ` L ${path[i].x * w} ${path[i].y * h}`;
         }
        ...
        <div className="space-y-8">
           <div className="bg-black/40 p-6 sm:p-8 rounded-3xl border border-white/5 shadow-inner space-y-6">
              ...
              <div className="space-y-4">
                ...
                <input id="cfr-kappa" title="Alpha Kappa (Regret)" aria-label="Alpha Kappa (Regret)" type="range" min="0.1" max="1" step="0.05" value={ kappa } onChange={ ( e ) => setKappa( Number.parseFloat( e.target.value ) ) } className="w-full h-1 accent-accent-indigo bg-white/10 rounded-full appearance-none cursor-pointer" />
              </div>
              ...
              <div className="space-y-4">
                ...
                <input id="cfr-equity" title="Equity Hero" aria-label="Equity Hero" type="range" min="0" max="100" step="1" value={ equity } onChange={ ( e ) => setEquity( Number.parseFloat( e.target.value ) ) } className="w-full h-1 accent-accent-emerald bg-white/10 rounded-full appearance-none cursor-pointer" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-2 bg-black/60 p-4 rounded-2xl border border-white/5">
                    <label htmlFor="cfr-pot-size" className="text-[0.5rem] text-text-darker uppercase font-black tracking-widest block">Pot Size (BB)</label>
                    <input id="cfr-pot-size" aria-label="Pot Size (BB)" type="number" value={ pot } onChange={(e) => setPot(Number.parseFloat(e.target.value) || 0)} className="w-full bg-transparent border-none text-[0.85rem] font-mono font-black text-white focus:outline-none focus:ring-0" />
                 </div>
                 <div className="space-y-2 bg-black/60 p-4 rounded-2xl border border-white/5">
                    <label htmlFor="cfr-eff-stack" className="text-[0.5rem] text-text-darker uppercase font-black tracking-widest block">Eff. Stack (BB)</label>
                    <input id="cfr-eff-stack" aria-label="Eff. Stack (BB)" type="number" value={ stack } onChange={(e) => setStack(Number.parseFloat(e.target.value) || 0)} className="w-full bg-transparent border-none text-[0.85rem] font-mono font-black text-white focus:outline-none focus:ring-0" />
                 </div>
              </div>
           </div>
        ...
        <div className="flex flex-col gap-6">
            <div className="relative aspect-square w-full max-w-112.5 mx-auto rounded-3xl overflow-hidden border border-white/10 bg-black/60 shadow-2xl group">
                ...
                <div className="absolute inset-0 w-full h-full group-hover:scale-[1.02] transition-transform duration-700">
                    ...
                    <svg viewBox="0 0 450 450" className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-[0_0_12px_rgba(16,185,129,0.8)]">
                        <path
                            ref={pathfindingRef}
                            fill="none"
