...
function updateSizingDom(
  ...
  if (elFlop)
    elFlop.innerText = `${flopBet.toFixed(1)} bb (${Math.round(flopPct * 100)}%)`;
  if (elTurn)
    elTurn.innerText = `${turnBet.toFixed(1)} bb (${Math.round(turnPct * 100)}%)`;
  if (elRiver)
    elRiver.innerText = `${Math.max(0, riverJam).toFixed(1)} bb (JAM)`;
}
