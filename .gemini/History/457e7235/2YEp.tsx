...
interface CfrCanvasProps {
    matrix: Float32Array | null;
    nodes: number;
}

export function CfrCanvas({ matrix, nodes }: Readonly<CfrCanvasProps>) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const deviceRef = useRef<GPUDevice | null>(null);
    const pipelineRef = useRef<GPURenderPipeline | null>(null);
    const matrixBufferRef = useRef<GPUBuffer | null>(null);
    ...
    useEffect(() => {
        ...
        async function initWebGPU() {
            ...
            try {
                ...
                const uBuffer = device.createBuffer({ size: 4, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
                ...
                const mBuffer = device.createBuffer({ size: maxNodes * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST });
                ...
                setError(err.message || "Entropia detectada na ponte WebGPU");
            }
        }
        ...
        return () => {
            isCancelled = true;
            if (uniformBufferRef.current) uniformBufferRef.current.destroy();
            if (matrixBufferRef.current) matrixBufferRef.current.destroy();
            if (deviceRef.current) deviceRef.current.destroy();
        };
    }, [nodes]);

    useEffect(() => {
        if (!isReady || !matrix || !deviceRef.current || !pipelineRef.current || !matrixBufferRef.current || !bindGroupRef.current) return;
        ...
        device.queue.writeBuffer(matrixBufferRef.current, 0, matrix);
        ...
        const renderPass = commandEncoder.beginRenderPass({
            colorAttachments: [{
                view: textureView,
                clearValue: { r: 0.0, g: 0.0, b: 0.0, a: 0.0 }, // Fundo transparente SOTA (Glassmorphism integration)
                loadOp: 'clear',
                storeOp: 'store',
            }],
        });
