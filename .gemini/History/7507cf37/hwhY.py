...
class VITOIWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        src_path = os.fsdecode(event.src_path) if isinstance(event.src_path, bytes) else str(event.src_path)
        if event.is_directory or not src_path.endswith(('.py', '.js', '.ts', '.tsx')): return
        if any(ign in Path(src_path).parts for ign in self.ignores): return
        if src_path in self.last and time.time() - self.last[src_path] < 5: return
        if os.path.getsize(src_path) // 4 > 8000:
            self.last[src_path] = time.time()
            console.print(f"\n[bold red]⚠️ ENTROPIA DETECTADA:[/] {os.path.basename(src_path)} excedeu limite de tokens!")
