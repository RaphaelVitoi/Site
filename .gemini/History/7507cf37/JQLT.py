...
def _remove_single_sanitizer_target(path: Path) -> Tuple[bool, float, str]:
    """Remove um unico alvo e retorna sucesso, espaco salvo e status."""
    def force_remove_readonly(func, p, exc_info):
        try:
        ...
        if path.exists():
            if dry_run:
                try:
                    ...
                    table.add_row(str(path.relative_to(BASE_DIR)), "N/A", f"[red]ERRO LENDO TAMANHO: {str(e)[:15]}[/]")
            else:
                success, size_mb, status = _remove_single_sanitizer_target(path)
                table.add_row(str(path.relative_to(BASE_DIR)), f"{size_mb:.2f} MB" if success else "N/A", status)
                if success:
                    removed_count += 1
                    saved_space += size_mb
...
def optimize_vscode():
    ...
    if config_path is None or not config_path.exists():
        console.print("[bold red][ERRO CRITICO] O nucleo de settings.json nao foi encontrado.[/]")
        return
...
class VITOIWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        src_path_str = os.fsdecode(event.src_path) if isinstance(event.src_path, bytes) else str(event.src_path)
        if event.is_directory or not src_path_str.endswith(('.py', '.js', '.ts', '.tsx')): return
        if any(ign in Path(src_path_str).parts for ign in self.ignores): return
        if src_path_str in self.last and time.time() - self.last[src_path_str] < 5: return
            ...
            try:
                with open(env_path, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        ...
                        if match:
                            _add_key_to_collection(all_keys, match.group(1), match.group(2), f"File ({file_name})")
            except Exception: pass
...
def _get_api_rate_limiter_status() -> dict:
    ...
    try:
        ...
        return {"used": api_budget_data.get('used', 0), "total": api_budget_data.get('total', 0)}
    except Exception:
...
def main(args=None):
    ...
    if command in COMMAND_MAP:
        ...
        console.print("[yellow]Use 'nexus-cli dashboard' ou 'nexus-cli' para ver as opcoes.[/]")
