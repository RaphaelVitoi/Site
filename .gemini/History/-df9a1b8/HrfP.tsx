            ...
            <div className="overflow-x-auto scrollbar-hide relative z-10">
              <table className="w-full text-left text-[0.7rem] font-mono tabular-nums">
                <thead className="bg-accent-sky/5 text-accent-sky-light uppercase tracking-[0.2em] border-b border-white/5">
                    <tr>
                      <th className="p-4 pl-8 font-black text-[0.65rem]">Street</th>
                      <th className="p-4 font-black text-[0.65rem]">Pote (BB)</th>
                      <th className="p-4 font-black text-center text-[0.65rem]">Stack Res.</th>
                      <th className="p-4 font-black text-center text-[0.65rem]">Fator SPR</th>
                      <th className="p-4 font-black text-right pr-8 text-[0.65rem]">RP Residual</th>
                    </tr>
                </thead>
                    ...
                    { activeSprData.map( ( stage: SprStage ) =>
                    {
                      ...
                      return (
                        <tr key={ stage.name } className={ `hover:bg-white/5 transition-all duration-300 ${ isDeath ? 'bg-accent-danger/5' : '' }` }>
                          <td className="p-4 pl-8 font-black text-accent-sky uppercase tracking-widest">{ stage.name }</td>
                          <td className="p-4 font-black text-white text-base">{ stage.potSize.toFixed( 1 ) }</td>
                          <td className="p-4 text-center font-bold text-text-dim">{ residual.toFixed( 1 ) }</td>
                          <td className={ `p-4 text-center font-black text-base ${ sprValue >= 1 ? 'text-accent-emerald' : 'text-accent-rose' }` }>
                            { sprText }
                          </td>
                          <td className={ `p-4 text-right pr-8 font-black text-base ${ isDeath ? 'text-accent-danger' : 'text-accent-amber' }` }>
            ...
            <div className="bg-linear-to-br from-slate-900/60 to-black/80 border border-white/10 p-8 rounded-4xl shadow-3xl relative overflow-hidden group/audit-final">
                ...
                <div className="space-y-4">
                   <div className="flex justify-between items-center py-3 border-b border-white/5 group/row">
                      <span className="text-[0.6rem] text-text-muted font-bold uppercase tracking-widest group-hover/row:text-text-main transition-colors">Δ Risk Premium</span>
                      <span className="text-[0.75rem] font-mono font-black text-accent-indigo tabular-nums">+{Math.abs(effectiveIpRp - effectiveOopRp).toFixed(2)}%</span>
                   </div>
                   <div className="flex justify-between items-center py-3 border-b border-white/5 group/row">
                      <span className="text-[0.6rem] text-text-muted font-bold uppercase tracking-widest group-hover/row:text-text-main transition-colors">Fator de Risco (BF)</span>
                      <span className="text-[0.75rem] font-mono font-black text-accent-emerald tabular-nums">{((100 / (100 - Math.max(effectiveIpRp, effectiveOopRp))) || 1).toFixed(2)}x</span>
                   </div>
                   <div className="flex justify-between items-center py-3 border-b border-white/5 group/row">
                      <span className="text-[0.6rem] text-text-muted font-bold uppercase tracking-widest group-hover/row:text-text-main transition-colors">Erosão Temporal (FGS)</span>
                      <span className="text-[0.75rem] font-mono font-black text-accent-rose tabular-nums">-0.15bb</span>
                   </div>
                   <div className="flex justify-between items-center py-3 group/row">
                      <span className="text-[0.6rem] text-text-muted font-bold uppercase tracking-widest group-hover/row:text-text-main transition-colors">Equity WASM-SIMD</span>
                      <span className="text-[0.75rem] font-mono font-black text-white tabular-nums">{equity.toFixed(1)}%</span>
                   </div>
                </div>
...
function AuditCard({ icon, color, label, value, sub }: Readonly<{ icon: string, color: string, label: string, value: string, sub: string }>) {
    const colorClasses = {
    ...
    return (
        <div className="bg-slate-950/40 border border-white/5 p-6 rounded-3xl shadow-inner group transition-all duration-500 hover:-translate-y-1 hover:bg-slate-900/60">
          <div className="flex items-center gap-3 mb-4">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${colorClasses}`}>
              <i className={`fa-solid ${icon} text-base`} />
            </div>
            <span className="text-[0.6rem] font-black text-text-darker uppercase tracking-[0.2em] group-hover:text-text-muted transition-colors">{label}</span>
          </div>
          <div className="text-2xl font-mono font-black text-white mb-1 tabular-nums tracking-tighter">{value}</div>
          <div className="text-[0.5rem] text-text-darker uppercase tracking-[0.2em] font-black group-hover:text-text-muted transition-colors">{sub}</div>
        </div>
    );
}
