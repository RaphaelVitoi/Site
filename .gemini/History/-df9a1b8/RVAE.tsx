...
import DegradationChart from '@/app/laboratorio-icm/DegradationChart';
...
export default function TheoryPanel ( { scenario, effectiveSprData, effectiveIpRp, effectiveOopRp }: Readonly<TheoryPanelProps> )
{
  ...
  useEffect( () =>
  {
    // SOTA: O motor Rust é imutável na assinatura. Desestruturamos a função para evitar que re-renders do contexto engatilhem floods.
    const dispatch = wasmContext?.dispatchInsolvencyMatrix;
    if ( activeTab === 'ranges' && dispatch )
    {
      ...
      dispatch( villainRange, board, rpFactor, heroInvested, currentPot, activePlayers );
    }
  ...
  return (
    <div className="glass-panel border border-white/5 rounded-2xl shadow-2xl shadow-slate-900/50 bg-(--bg-deep)/80 backdrop-blur-md flex flex-col transition-all duration-500">
      ...
      {/* Conteúdo por tab */ }
      <div className="flex-1 p-6 flex flex-col min-h-[21.875rem] transition-all duration-500">
