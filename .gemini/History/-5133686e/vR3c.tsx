...
export async function GET(req: Request) {
  try {
    ...
    const po = parseFloat(searchParams.get('po') || '0.20'); // Pot Odds Baseline (ex: 20%)
    const pm = parseFloat(searchParams.get('pm') || '0.05'); // Perspectiva Matemática
    const mw = parseInt(searchParams.get('mw') || '2', 10); // Entropia Multiway
    const ci = parseFloat(searchParams.get('ci') || '1.5'); // Coeficiente de Insolvência
    const evFold = parseFloat(searchParams.get('evFold') || '-1.12');
    ...
    return new ImageResponse(
      (
        <div tw="flex flex-col w-full h-full bg-slate-950 items-center justify-center p-12" style={{ fontFamily: 'monospace' }}>
          ...
          <svg viewBox="0 0 1200 400" tw="w-full flex-1">
