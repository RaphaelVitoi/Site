...
export function solveIcmDistortion(
  ...
  // SOTA v4.3: Risco de Ressurreição (Assimetria FGS)
  // Dobrar um IP short-stack é sistemicamente mais punitivo do que perder um pote comum.
  // A penalidade aplica um peso 20% maior na contração do Call quando o IP está sob risco.
  const resurrectionPenalty = deltaRp > 0 ? 1.2 : 1.0;
  ...
  const deltaCall =
    -Math.pow(absDelta / 10, bExponent) * k_oop_call * resurrectionPenalty;
