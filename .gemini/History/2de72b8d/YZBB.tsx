...
function StreetCard ( {
  ...
  const isEvFoldPositive = evFold >= 0;
  const evFoldColor = isEvFoldPositive ? 'var(--accent-emerald)' : 'var(--accent-danger)';
  ...
  const isDeltaPositive = deltaRp >= 0;
  const deltaRpText = `${isDeltaPositive ? '+' : ''}${deltaRp.toFixed( 1 )}%`;
  const deltaRpColor = deltaRp > 0 ? 'var(--accent-amber)' : 'var(--text-muted)';

  const isPmPositive = result.pmStreet >= 0;
  const pmText = `${isPmPositive ? '+' : ''}${result.pmStreet.toFixed( 2 )}`;
  const pmColor = isPmPositive ? 'var(--accent-emerald)' : 'var(--accent-danger)';

  const isCiSolvent = result.ciStreet >= 1;
  const ciColor = isCiSolvent ? 'var(--accent-emerald)' : 'var(--accent-danger)';
  const ciHint = isCiSolvent ? 'solvente' : 'insolvente';
  ...
  return (
    <div style={ {
      ...
      <MetricRow
        label="EV_fold"
        value={ `${isEvFoldPositive ? '+' : ''}${evFold.toFixed( 2 )} bb` }
        color={ evFoldColor }
