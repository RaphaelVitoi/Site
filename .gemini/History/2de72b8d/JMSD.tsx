...
function StreetCard ( {
  ...
  const heroRp = isBaseline ? 0 : ( heroIsIp ? result.ipRp : result.oopRp );
  const villainRp = isBaseline ? 0 : ( heroIsIp ? result.oopRp : result.ipRp );
  const deltaRp = isBaseline ? 0 : result.deltaRp;
  const heroLabel = heroIsIp ? ipLabel : oopLabel;
  const villainLabel = heroIsIp ? oopLabel : ipLabel;

  const evFold = result.evFoldStreet;
  const evFoldColor = evFold >= 0 ? 'var(--accent-emerald)' : 'var(--accent-danger)';
  ...
  return (
    <div style={ {
      ...
      <MetricRow
        label="ΔRP"
        value={ `${deltaRp >= 0 ? '+' : ''}${deltaRp.toFixed( 1 )}%` }
        color={ deltaRp > 0 ? 'var(--accent-amber)' : 'var(--text-muted)' }
        hint="IP − OOP"
      />
      ...
      <div style={ { marginTop: '0.4rem', borderTop: '1px solid rgba(99,102,241,0.15)', paddingTop: '0.4rem' } }>
        <MetricRow
          label="PM street"
          value={ `${result.pmStreet >= 0 ? '+' : ''}${result.pmStreet.toFixed( 2 )}` }
          color={ result.pmStreet >= 0 ? 'var(--accent-emerald)' : 'var(--accent-danger)' }
          hint="(Eq×R×Val×gain)−(loss+RIO)"
        />
        <MetricRow
          label="Ci street"
          value={ result.ciStreet.toFixed( 3 ) }
          color={ result.ciStreet >= 1 ? 'var(--accent-emerald)' : 'var(--accent-danger)' }
          hint={ result.ciStreet >= 1 ? 'solvente' : 'insolvente' }
        />
        <MetricRow
          label="Valuation ICM"
          value={ result.valuationStreet.toFixed( 3 ) }
          color={ result.valuationStreet < 1 ? 'var(--accent-amber)' : 'var(--accent-emerald)' }
          hint="gain÷loss"
        />
