...
# Carregamento robusto do Kernel (SOTA) - Tentativa #3
$KernelPath = $null
...
if ([string]::IsNullOrEmpty($KernelPath)) {
    ...
    Write-Host "[INFO] Kernel path defaulting to: $KernelPath" -ForegroundColor DarkYellow
}


try {
    # Add the directory containing the module to PSModulePath
    $ModuleDir = Split-Path -Path $KernelPath -Parent
    $env:PSModulePath = $ModuleDir + ";" + $env:PSModulePath
    
    # Now, try importing the module
    Write-Host "[INFO] Attempting to import module from: $KernelPath" -ForegroundColor DarkYellow
