...
function Initialize-Kernel {
    ...
    # Garantir que as variaveis globais de path existam para evitar falha interna no .psm1
    ...
    try {
        $ModuleDir = Split-Path -Path $KernelPath -Parent
        $env:PSModulePath = $ModuleDir + ";" + $env:PSModulePath
        Write-Host "[INFO] Carregando Kernel SOTA: $KernelPath" -ForegroundColor DarkYellow
        # Uso explicito do parametro -Path para evitar ambiguidade no Import-Module
        Import-Module -Path $KernelPath -Force -DisableNameChecking -ErrorAction Stop
    }
    catch {
...
function New-ReflectionTask {
    ...
    $AgentMemoryPath = Join-Path $script:ProjectRoot ".claude" "agent-memory" $AgentName "MEMORY.md"
