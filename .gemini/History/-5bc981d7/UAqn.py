# pylint: disable=missing-module-docstring, logging-fstring-interpolation, broad-exception-caught, line-too-long
...
def _analisar_arquivo(path: str) -> tuple[int, int, int, int]:
    ...
    try:
        ...
        logger.exception(f"Erro de Sintaxe em {path}")
        print(
            f" [!] Pulando {os.path.basename(path)}: Erro de sintaxe (possível corrupção)."
        )
        return 0, 0, 0, 0
    except Exception:  # noqa: BLE001
        logger.exception(f"Falha ao ler {path}")
        return 0, 0, 0, 0
