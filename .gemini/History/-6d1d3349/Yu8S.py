...
def _normalize_gemini_model(model: str) -> str:
    ...
    # Bloqueia a serie 2.5 e 3.x experimental, forçando o downgrade para 2.0-flash
    if any(v in model_l for v in ("3.1", "3.0", "2.5", "1.0")):
        return "gemini-2.0-flash"
    # Garante que Pro/Flash 1.5 tambem usem o motor 2.0 se disponivel
    elif "1.5" in model_l:
        return "gemini-2.0-flash"
    return model
...
async def call_gemini(session: aiohttp.ClientSession, model: str, system_prompt: str, user_prompt: str, api_key: str, timeout: Optional[aiohttp.ClientTimeout] = None, require_json: bool = False, **kwargs) -> Tuple[str, dict]:
    ...
    if global_rate_limiter.tokens < 1:
        logging.info(f"[GEMINI] Cota de 15 RPM atingida. Aguardando janela de tokens para o modelo {model}...")
    ...
    try:
        async with get_api_semaphore():
            try:
                ...
                # SOTA: Impede que erros semânticos HTTP (400, 403, 404) vazem para o fallback nativo de rede.
                raise
