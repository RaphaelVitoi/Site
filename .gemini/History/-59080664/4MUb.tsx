...
export function UniversalLabShell ( { initialSpot }: UniversalLabShellProps ) {
    ...
    const [ spot, setSpot ] = useState<SpotState>( initialSpot ?? { heroInvested: 0, pot: 0, activePlayers: 0 } );
    const [ metrics, setMetrics ] = useState<MetricsState>( { monopolyVector: 0, riskPremium: 0, perspective: 0 } );
            ...
            workerRef.current.onmessage = ( event: MessageEvent<unknown> ) => {
                ...
                if ( typeof data === 'object' && data !== null && 'type' in data )
                {
                    ...
                    if ( payload.type === 'QUANTUM_SYNC' )
                    {
                        ...
                        // Aqui injetaremos o parse das matrizes brutas e update do SotaMetricsContext
                    }
                }
            };
    ...
    return (
        <SotaWasmContext.Provider value={ { worker: workerRef.current } }>
            <SotaSpotContext.Provider value={ spot }>
                <SotaMetricsContext.Provider value={ metrics }>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 w-full items-start">
                        ...
                        <section className="w-full sm:w-[400px] flex flex-col gap-6">
