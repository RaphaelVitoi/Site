...
export type SpotState = Readonly<{ heroInvested: number; pot: number; activePlayers: number; stacks: readonly number[] }>;
...
export function UniversalLabShell ( { initialSpot }: UniversalLabShellProps ) {
    ...
    const [ spot, setSpot ] = useState<SpotState>( initialSpot ?? { heroInvested: 0, pot: 0, activePlayers: 2, stacks: [ 100, 100 ] } );
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col gap-3 mt-4 mb-6 border-t border-zinc-800/50 pt-4">
                                    <div className="flex justify-between items-center">
                                        <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Distribuição da Mesa (bb)</label>
                                        <select
                                    ...
                                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                        { spot.stacks.map( ( stack, idx ) => (
                                            <div key={ idx } className="flex flex-col gap-1">
                                                <span className="text-[0.65rem] text-zinc-500 font-mono pl-1">{ idx === 0 ? 'Hero' : `Vilão ${idx}` }</span>
                                                <input
                                                    type="number"
                                                    min="0"
                                                    step="0.1"
                                ...
                                <button
                                    ...
                                    onClick={ () => {
                                        if ( workerRef.current )
                                        {
                                            ...
                                            workerRef.current.postMessage( { type: 'EVALUATE_MATRIX', payload: buffer } );
                                        }
                                    } }
                                >
