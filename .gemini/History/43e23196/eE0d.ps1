...
$MaintenanceTasks = @(
    ...
    @{
        TaskName    = 'Nexus - Limpeza de Artefatos SOTA'
        Description = 'Aciona o @skillmaster para limpar o historico de task_results e do banco de dados (>15 dias).'
        ScriptPath  = Join-Path $ProjectRoot 'scripts\maintenance\run_cleanup_sota.ps1'
        Trigger     = 'WEEKLY'
        Day         = 'SUN' # Domingo
        StartTime   = '02:30' # Logo após o Vacuum
    },
    @{
        TaskName    = 'Nexus - Limpeza de Backups SOTA'
        Description = 'Aciona o @skillmaster para expurgar backups antigos (.zip > 7 dias).'
        ScriptPath  = Join-Path $ProjectRoot 'scripts\maintenance\invoke_backup_cleanup.ps1'
        Trigger     = 'WEEKLY'
        Day         = 'SUN' # Domingo
        StartTime   = '02:45' # Logo após a limpeza de artefatos
    },
    @{
        TaskName    = 'Nexus - Health Check Sistêmico SOTA'
        Description = 'Aciona o @skillmaster para auditar as memórias dos agentes e gerar o relatório de saúde consolidado.'
        ScriptPath  = Join-Path $ProjectRoot 'scripts\routines\Invoke-SystemHealthCheck.ps1'
        Trigger     = 'WEEKLY'
        Day         = 'FRI' # Sexta-feira
        StartTime   = '18:00' # Final da tarde, antes do fim de semana
    },
    @{
        TaskName    = 'Nexus - Expurgo de Dependencias Orfas SOTA'
        Description = 'Aciona a rotina para aniquilar tarefas failed presas a dependencias fantasmas.'
        ScriptPath  = Join-Path $ProjectRoot 'scripts\maintenance\invoke_orphan_purge.ps1'
        Trigger     = 'WEEKLY'
        Day         = 'MON' # Segunda-feira
        StartTime   = '02:00' # 2 da manha
    }
