...
# 2. Enfileirar tarefa de síntese para @chico (Kernel)
Write-Host '[2/4] Delegando síntese e registro para @chico (Kernel)...' -ForegroundColor Cyan
...
$task = @{ id = $handoffTaskId; description = $handoffTaskDesc; status = 'pending'; timestamp = (Get-Date -Format 'o'); agent = '@chico'; metadata = @{ priority = 'high' } }
...
<toast>
    <visual>
        <binding template="ToastText02">
            <text id="1">Handoff SOTA Concluído</text>
            <text id="2">Síntese registrada e prompt copiado para o clipboard!</text>
        </binding>
    </visual>
</toast>
'@
