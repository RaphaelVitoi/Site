...
async def _create_watchdog_task(
    ...
    try:
        ...
        logger.exception(f"[WATCHDOG] Falha ao criar tarefa de alerta '{task_id}': {e}")
...
def _calculate_failure_rate(
    ...
    if last_timestamp_str and last_failed is not None:
        try:
            ...
            # SOTA: Garantia de Coerencia Temporal (Blindagem contra mismatch naive/aware)
...
async def system_watchdog(manager: QueueManager):
    ...
    while True:
        try:
            ...
            logger.exception(f"[WATCHDOG] Falha no monitoramento 24/7: {e}")
