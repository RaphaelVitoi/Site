...
MAX_CALLS_PER_MINUTE = int(os.environ.get("MAX_CALLS_PER_MINUTE", "30"))
...
class AsyncTokenBucket:
    ...
    async def consume(self, tokens: int = 1):
        ...
        async with self.lock:
            ...
            # Forca espacamento estrito entre tarefas acordando simultaneamente
            now = time.monotonic()
            if now - self.last_consume < 2.0:
                await asyncio.sleep(2.0 - (now - self.last_consume))
