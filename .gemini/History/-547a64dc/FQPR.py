...
class UniversalArbitrator:
    ...
    def _calculate_base_weight(cls, task: Task) -> float:
        ...
        try:
            ...
            # Harmonização SOTA para tz-aware (ex: PowerShell) e tz-naive (ex: Python local)
    ...
    def extract_optimal_task(cls, pending_tasks: List[Task]) -> Optional[Task]:
        """
        Orquestra a fila priorizada. Complexidade: O(V \\log V) dominada pela ordenação do Heap.
        """
        ...
        for t_id, data in dag_map.items():
            if data["in_degree"] == 0:
                # Usa o ID da tarefa como tie-breaker determinístico para evitar TypeError
                heapq.heappush(execution_heap, (-data["total_utility"], data["task"].id, data["task"]))

        if not execution_heap:
            logging.warning("[NEXUS ORCHESTRATOR] Deadlock Operacional: Nenhuma tarefa possui in_degree=0.")
            return None

        _, _, optimal_task = heapq.heappop(execution_heap)
        return optimal_task
