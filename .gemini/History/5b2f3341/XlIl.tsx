...
export default function MasterSimulator() {
    ...
    const dispatchInsolvencyMatrix = useCallback( ( _villainRange: string, _board: string, _rpFactor: number, _heroInvested: number, _currentPot: number, _activePlayers: number ) => {
    ...
    const quantumConfig = useMemo( () => ( {
        scenario, pkoValue, isNearPayjump, blindsRisingSoon, streetFreqs, aggressionFactor, heroIsIp: heroPosition === 'IP', heroPosition, anteSize, heroInvestedBb: heroInvested, currentPotBb: currentPot, activePlayers
    ...
    }, [heroInvested, currentPot, bfForDash, rpForDash, quantumPerspectiva, heroRawStack, heroPosition, baseFgsErosion, apiQuantumMetrics, activePlayers] );
    ...
    }, [spotData, actionMetrics, finalIpRp, finalOopRp, activePlayers, heroInvested, scenario.id, scenario.name] );
    ...
    const dispatchIcmPerspectiva = useCallback( ( _input: any, _onResult: ( res: any ) => void ) => {
