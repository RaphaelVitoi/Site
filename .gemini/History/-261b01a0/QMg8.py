    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if "429" in error_msg and "retry_after=" in error_msg:
                try:
                    delay = int(error_msg.split("retry_after=")[1].rstrip("s").split()[0])
                except (IndexError, ValueError):
                    delay = 0
                if 0 < delay <= 60 and attempt < retries - 1:
                    logging.info(f"[[{_c(task.agent)}]{task.agent}[/]] 429 rate-limit em {strategy.name} ({model}, Chave {key_rank}). retry_after={delay}s. Aguardando...")
