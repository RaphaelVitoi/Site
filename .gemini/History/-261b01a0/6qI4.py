    ...
    **kwargs
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] Timeout na conexao com {strategy.name} ({model}, Chave {key_rank}). Rotacionando...")
            latency_ms = int((time.monotonic() - start) * 1000)
            await manager.record_key_usage_metric(
            ...
            if await _is_route_blocked(route_key):
                logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] Rota {provider}:{model} entrou em cooldown apos timeout. Avancando para proximo modelo.")
            ...
            if is_5xx and attempt < retries - 1:
                logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] HTTP {e.status} transiente em {strategy.name} ({model}, Chave {key_rank}). Retentando...")
                await asyncio.sleep(2.0)
                continue

            # 429 com retry-after: sleep e retry na mesma chave, ou rotacionar imediatamente
            if "429" in error_msg and "retry_after=" in error_msg:
                try:
                    # SOTA: Extrai apenas os digitos, imune a pontuacao ou formatacao ('42s.')
                    match = re.search(r"retry_after=(\d+)", error_msg)
                    delay = int(match.group(1)) if match else 0
                except Exception:
                    delay = 0
                if 0 < delay <= 60 and attempt < retries - 1:
                    # SOTA: Injeta Jitter para evitar o Efeito Manada (Thundering Herd)
                    jitter = random.uniform(0.5, 3.0)
                    sleep_time = float(delay) + jitter
                    logging.info(f"[[{_c(task.agent)}]{task.agent}[/]] 429 rate-limit em {strategy.name} ({model}, Chave {key_rank}). retry_after={delay}s. Aguardando {sleep_time:.2f}s com jitter...")
                    await asyncio.sleep(sleep_time)
                    continue
                logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] 429 quota esgotada em {strategy.name} ({model}, Chave {key_rank}). Rotacionando...")
                # SOTA: Penalidade de Rotacao para nao queimar a proxima chave no burst-limit do IP
