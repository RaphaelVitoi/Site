...
async def _execute_single_key(
    ...
    **kwargs
    ...
    # SOTA: Tolerância estendida para absorver os gargalos de 2 RPM do Google Free Tier
    rate_limit_allowance = 2
    for attempt in range(retries + rate_limit_allowance):
        ...
        try:
            ...
            if "429" in error_msg:
                ...
                if delay <= 65 and attempt < (retries + rate_limit_allowance) - 1:
                    # SOTA: Injeta Jitter para evitar o Efeito Manada
                    jitter = random.uniform(1.0, 3.0)
                    sleep_time = float(delay) + jitter
                    logging.info(f"[[{_c(task.agent)}]{task.agent}[/]] 429 rate-limit em {strategy.name} ({model}, Chave {key_rank}). Aguardando {sleep_time:.2f}s com jitter...")
