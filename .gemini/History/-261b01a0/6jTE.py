    ...
    **kwargs
    ...
    while True: # O motor agora é um loop persistente que só quebra ao esgotar retries.
        ...
        try:
            ...
            # =================================================================
            # SOTA CHAOS ENGINEERING: Injecao Estocastica de Entropia
            # =================================================================
                ...
                try:
                    ...
                    if random.random() < chaos_lambda:
                        ...
                        if chaos_type == "LATENCY":
                            logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] [bold red]CHAOS INJECTED:[/] Latencia artificial")
                            await asyncio.sleep(random.uniform(3.0, 7.0))
                        elif chaos_type == "TIMEOUT":
                            logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] [bold red]CHAOS INJECTED:[/] Timeout simulado")
                            raise asyncio.TimeoutError("ChaosCore: Timeout Injected")
                        elif chaos_type == "503":
                            logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] [bold red]CHAOS INJECTED:[/] HTTP 503 Service Unavailable")
                            raise Exception("ChaosCore: 503 Service Unavailable")
                except ValueError:
                    pass
            # =================================================================
            ...
            if (isinstance(e, aiohttp.ClientResponseError) and getattr(e, 'status', 0) >= 500) or "chaoscore: 503" in error_msg.lower():
                general_retries_left -= 1
                if general_retries_left < 0:
