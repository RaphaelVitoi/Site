...
# =====================================================================
def _extract_task_file_mentions(description: str, slug: str | None) -> list[Path]:
...
def _inject_task_docs(task: Task) -> str:
    ...
    paths_to_check = _extract_task_file_mentions(task.description or "", slug)
...
def _needs_web_search(agent: str, normalized_desc: str) -> bool:
    ...
    is_pesq = agent == "@pesquisador" and _calculate_heuristic_score(normalized_desc, research_terms) > te.HEURISTIC_THRESHOLD
    is_mav = agent == AGENT_MAVERICK and _calculate_heuristic_score(normalized_desc, strategic_terms) > te.HEURISTIC_THRESHOLD
    is_bib = agent == "@bibliotecario" and (_calculate_heuristic_score(normalized_desc, research_terms) > te.HEURISTIC_THRESHOLD or _calculate_heuristic_score(normalized_desc, web_infra_terms) > te.HEURISTIC_THRESHOLD)
    is_chico = agent == "@chico" and _calculate_heuristic_score(normalized_desc, orchestration_terms) > te.HEURISTIC_THRESHOLD
    is_val = agent in ["@validador", "@verifier", "@auditor"] and (_calculate_heuristic_score(normalized_desc, domain_terms) > te.HEURISTIC_THRESHOLD or _calculate_heuristic_score(normalized_desc, research_terms) > te.HEURISTIC_THRESHOLD)
    has_research = _calculate_heuristic_score(normalized_desc, research_terms) > te.HEURISTIC_THRESHOLD

    return is_pesq or is_mav or is_chico or is_bib or is_val or has_research
...
async def _execute_web_search(task: Task) -> tuple[str, int]:
    ...
    if (TAVILY_KEYS or PERPLEXITY_KEYS) and _needs_web_search(task.agent, normalized_description):
        try:
...
def _add_context_sections(user_prompt: str, project_context: str, web_context: str, collective_memory: str, agent_memory: str, task: Task, task_docs: str) -> str:
...
def _add_autonomy_and_guidelines(user_prompt: str, task: Task, agent_clean: str, autonomy_mode: str) -> str:
    ...
    if task.agent not in [AGENT_DISPATCHER, "@bibliotecario"]:
        ...
        if autonomy_mode == "stop":
            prompt_parts.append("Foco: O Observador Passivo. Isolamento absoluto de escrita. Use este modo para discussoes teoricas de GTO, insights filosoficos e analise exploratoria sem alterar o estado do sistema.\n")
        elif autonomy_mode == "default":
            prompt_parts.append("Foco: A Homeostase Estatica. Atue como Sentinela de Estabilidade. Execute auto-debugging, fixes de sintaxe e security patches reativamente para manter a integridade (Minimizacao de Entropia). Nao altere a logica de negocio sem aval.\n")
        elif autonomy_mode == "partial":
            prompt_parts.append("Foco: O Estrategista de Impacto (Equilibrio Bayesiano). Alem da homeostase, aplique refatoracoes preventivas de BAIXO impacto autonomamente. Para alteracoes Core/GTO (alto impacto), gere uma Decision Proposal para validacao humana.\n")
        elif autonomy_mode == "full":
            prompt_parts.append("Foco: O Agente Autonomo Total. Maximizacao da Potencia de Inovacao. Refatoracao estrutural, criacao de novos modulos, execucao de terminal complexa e deployment proativo ativados em grau maximo.\n")

        if autonomy_mode != "stop":
            derived = "PARTIAL" if autonomy_mode == "full" else "DEFAULT" if autonomy_mode == "partial" else "STOP"
...
def _build_subtask(task: Task, st: DispatcherSubtask, i: int, created_ids: list, agents_list: list, reason: str) -> Task:
    ...
    return Task(id=sub_id, description=st.description, agent=st.agent, timestamp=datetime.now().isoformat(), metadata=meta)
...
async def _process_dispatcher_output(task: Task, manager: QueueManager, response_text: str) -> None:
    ...
    try:
        ...
        created_ids = []
        agents_list = [sub.agent for sub in parsed_subtasks]
        for i, st in enumerate(parsed_subtasks):
            ...
            new_task = _build_subtask(task, st, i, created_ids, agents_list, "dispatcher_json_validated")
        ...
        if retry_subtasks:
            created_ids = []
            agents_list = [sub.agent for sub in retry_subtasks]
            for i, st in enumerate(retry_subtasks):
                ...
                if st.metadata:
                    meta.update(st.metadata)
                new_task = Task(id=sub_id, description=st.description, agent=st.agent, timestamp=datetime.now().isoformat(), metadata=meta)
                new_task = _build_subtask(task, st, i, created_ids, agents_list, "dispatcher_schema_retry_success")
                await manager.add_task(new_task)
...
async def _process_observers_and_handoff(task: Task, manager: QueueManager) -> None:
    ...
    autonomy_mode = await get_autonomy_mode(manager)
    if autonomy_mode != "off" and not task.id.startswith("AUTOFIX") and task.agent != AGENT_DISPATCHER:
