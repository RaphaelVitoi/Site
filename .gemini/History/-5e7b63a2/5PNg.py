...
def _needs_web_search(agent: str, normalized_desc: str) -> bool:
    return (
        _calculate_heuristic_score(normalized_desc, te._heuristic_terms("research_terms")) > te.HEURISTIC_THRESHOLD or
        (agent == AGENT_MAVERICK and _calculate_heuristic_score(normalized_desc, te._heuristic_terms("strategic_terms")) > te.HEURISTIC_THRESHOLD) or
        (agent == "@bibliotecario" and _calculate_heuristic_score(normalized_desc, te._heuristic_terms("web_infra_terms")) > te.HEURISTIC_THRESHOLD) or
        (agent == "@chico" and _calculate_heuristic_score(normalized_desc, te._heuristic_terms("orchestration_terms")) > te.HEURISTIC_THRESHOLD) or
        (agent in ("@validador", "@verifier", "@auditor") and _calculate_heuristic_score(normalized_desc, te._heuristic_terms("domain_terms")) > te.HEURISTIC_THRESHOLD)
    )
...
async def process_agent_task(task: Task, manager: QueueManager, timing_metrics: dict) -> str:
    ...
    if web_ms > 0: timing_metrics["web_search_ms"] = web_ms

    collective_memory, rag_ms = await _query_collective_memory(task, n_rag_results)
    if rag_ms > 0: timing_metrics["rag_query_ms"] = rag_ms
    ...
    if comp_ms > 0: timing_metrics["context_compression_ms"] = comp_ms
