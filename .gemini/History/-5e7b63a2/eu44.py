...
async def _fetch_web_search(task: Task, manager: QueueManager, session) -> str:
    ...
    # SOTA: Strategy Polimorfica para Web Search (Economia Generalizada de V(G))
    search_strategies = [
    ...
    for provider, keys, func, kwargs in search_strategies:
        ...
        for key in ranked_keys:
            ...
            logger.info(f"[[{te._c(task.agent)}]{task.agent}[/]] [dim]Tentando busca via {provider.capitalize()}...[/]")
            t0 = time.monotonic()
        ...
        logger.info(
            f"[[{te._c(task.agent)}]{task.agent}[/]] [dim]Tentando busca via Tavily...[/]"
        )
        t0 = time.monotonic()
        web_context = await call_tavily_search(
            session, key, task.description, max_results=3
        )
    ...
    for key in perplexity_keys:
        ...
        logger.info(
            f"[[{te._c(task.agent)}]{task.agent}[/]] [dim]Fallback para Perplexity...[/]"
        )
        t0 = time.monotonic()
