    ...
    return (
        ...
        or (
            ...
            and _calculate_heuristic_score(
                normalized_desc, te._heuristic_terms("strategic_terms")
            )
            > te.HEURISTIC_THRESHOLD
        )
        or (
...
async def _execute_web_search(task: Task) -> tuple[str, int]:
    ...
    if task.agent == "@pesquisador":
    if task.agent == AGENT_PESQUISADOR:
        task.description += "\n\n[DIRETRIZ BATEDOR AVANCADO SOTA] Analise os resultados da busca externa anexados (Tavily/Perplexity). Extraia a inteligencia bruta, identifique assimetrias de mercado e destile o Estado da Arte absoluto (OSINT)."
...
async def _apply_context_compression(
    ...
    if len(project_context) > 6000:
        logger.info(
            f"[[{te._c(task.agent)}]{task.agent}[/]] Contexto do projeto longo ({len(project_context)} chars). Acionando compressao..."
        )
    ...
    if len(agent_memory) > 4000:
        logger.info(
            f"[[{te._c(task.agent)}]{task.agent}[/]] Memoria do agente longa ({len(agent_memory)} chars). Acionando compressao..."
        )
...
async def process_agent_task(
    ...
    strategic_agents = (AGENT_MAVERICK, AGENT_PESQUISADOR, "@architect")
    ...
    project_context, agent_memory, comp_ms = await _apply_context_compression(
...
async def _handle_api_budget_exhaustion(
    e: Exception, task: Task, manager: QueueManager
):
    logger.error(
