...
export default function RangeMatrix({ ipRp, oopRp }: Readonly<RangeMatrixProps>) {
    ...
    // Carrega as edições persistidas assim que o componente é montado no cliente
    useEffect(() => {
        try {
            const saved = localStorage.getItem('rangeMatrixOverrides');
            if (saved) {
    ...
    const handleCellClick = (hand: string, currentStatus: string) => {
        ...
        setOverrides((prev) => {
            ...
            localStorage.setItem('rangeMatrixOverrides', JSON.stringify(newOverrides));
            return newOverrides;
        });
    };
    ...
    const resetOverrides = () => {
        setOverrides({});
        localStorage.removeItem('rangeMatrixOverrides');
    };
