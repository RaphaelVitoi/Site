...
export default function RangeMatrix({ ipRp, oopRp }: Readonly<RangeMatrixProps>) {
    ...
    const getHandStatus = (row: number, col: number) => {
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1rem' }}>
                ...
                {/* Legenda */}
                <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.55rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>
            ...
            <div style={{
                ...
                {RANKS.map((r1, i) =>
                    RANKS.map((r2, j) => {
                        ...
                        const status = getHandStatus(i, j);
            ...
            <p style={{ fontSize: '0.55rem', color: '#475569', fontStyle: 'italic', margin: '0.75rem 0 0 0', textAlign: 'center' }}>
                A matriz acima reage ao Risk Premium. Note como os ranges de flat e blefe colapsam na Death Zone (RP ≥ 40%).
            </p>
        </div>
    );
}