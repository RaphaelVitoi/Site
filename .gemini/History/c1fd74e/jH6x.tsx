...
export default function RangeMatrix ( { ipRp, oopRp, scenarioId }: Readonly<RangeMatrixProps> ) {
    ...
    const getHandStatus = ( row: number, col: number, hand: string ) => {
        ...
        // SOTA: Heurística de Força (Power Heuristic). Erradica o bug do rank linear (onde 72o > 22).
        const v1 = 14 - row;
        const v2 = 14 - col;
        ...
        if ( isPair ) power = 40 + ( high * 3 ); // Pares dominam a escala estrutural

        const rpPenalty = activeRp * 0.6;
        const effectivePower = power - rpPenalty;

        if ( effectivePower >= 60 ) return 'core';
        if ( effectivePower >= 48 ) return 'marginal';
        if ( effectivePower >= 38 ) return 'bluff';
        if ( activeRp >= 30 && effectivePower < 45 && effectivePower >= 35 ) return 'death';
