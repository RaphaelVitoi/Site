...
.DESCRIPTION
    Aciona o @organizador para cruzar os dados do agents_manifest.json,
...
1. `data/agents_manifest.json` (Fonte da verdade de roteamento - deve refletir os 18 Agentes IA).
2. `.claude/project-context.md` (Verificar a Secao 'Ecossistema de agentes').
...
- Verifique se TODOS os 18 agentes de IA estao descritos corretamente nas listas de pipeline no `project-context.md` e `GLOBAL_INSTRUCTIONS.md`.
...
- Ao final, ateste o conserto declarando a simetria na sua resposta final (que sera auditada via tasks.db).
"@
