...
#[wasm_bindgen]
pub struct MultiwayRioProfiler {
    base_tension: f32,
}

#[wasm_bindgen]
impl MultiwayRioProfiler {
    #[wasm_bindgen(constructor)]
    pub fn new(base_tension: f32) -> Self {
        Self { base_tension }
    }

    #[wasm_bindgen]
    pub fn compute_profile(&self, max_players: usize, spr_levels: usize) -> Float32Array {
        let buffer_size = max_players * spr_levels;
        let mut profile_matrix = vec![0.0f32; buffer_size];

        // SOTA: O(1) Iteration para Mapeamento de Tensão (Teorema D2: RIO MW escala N^2)
        for p in 2..=max_players {
            for s in 1..=spr_levels {
                let n = p as f32;
                let spr = s as f32;
                // A Tensão RIO escala quadraticamente com N, mas é mitigada pela diluição do SPR
                let rio = ((n - 1.0) * (n - 1.0) * self.base_tension) / spr.sqrt();
                let idx = (p - 1) * spr_levels + (s - 1);
                if idx < buffer_size {
                    profile_matrix[idx] = rio;
                }
            }
        }

        let js_array = Float32Array::new_with_length(buffer_size as u32);
        js_array.copy_from(&profile_matrix);
        js_array
    }
}
