...
export default function ScenarioStage( { scenario, effectiveIpRp = scenario.ipRp, effectiveOopRp = scenario.oopRp, dynamicDeathZone }: Readonly<ScenarioStageProps> ) {
  ...
  return (
    <div className="glass-panel p-10 sm:p-12 lg:p-16 animate-sota-in rounded-4xl bg-bg-panel/80 backdrop-blur-3xl border border-white/10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.7)] relative transition-all duration-700 hover:border-white/20 group/stage">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-10 border-b border-white/5 pb-12 relative z-10">
        <div className="space-y-5">
          ...
          <div className="px-4 py-1.5 rounded-lg bg-white/5 border border-white/10 inline-flex items-center gap-3">
            ...
            <span className="text-[0.65rem] font-black uppercase tracking-[0.3em] text-text-muted">{ scenario.narrativeSubtitle }</span>
          </div>
        </div>

        <div className="flex flex-col items-end gap-3">
            <span className="text-[0.55rem] font-black text-text-darker uppercase tracking-[0.4em]">Diagnóstico SOTA</span>
      ...
      <div className="my-12 relative group/theory">
          ...
          <div className={ `relative p-10 rounded-3xl border transition-all duration-700 shadow-inner ${isNodelockB20 ? 'bg-accent-indigo/10 border-accent-indigo/30 shadow-accent-indigo/5' : 'bg-slate-950/40 border-white/5 hover:bg-slate-950/60 hover:border-white/10'} text-[0.95rem] leading-loose` }>
            { isNodelockB20 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 pt-6 relative z-10">
        ...
        <div className="flex flex-col items-center gap-10 group/ip transition-all duration-500">
          ...
          <div className="bg-slate-900/60 px-10 py-5 rounded-2xl border border-white/5 text-center group-hover/ip:border-accent-indigo/40 group-hover/ip:bg-slate-900/80 transition-all shadow-2xl relative overflow-hidden">
            ...
            <span className="text-[0.6rem] font-black text-text-darker uppercase tracking-[0.4em] block mb-3 group-hover/ip:text-accent-indigo-light transition-colors relative z-10">Impacto Posicional</span>
            <div className="flex items-center justify-center gap-4 relative z-10">
                <span className="text-[0.9rem] font-mono font-black text-white tracking-tighter">{ effectiveIpRp.toFixed( 1 ) }% <span className="text-text-darker text-[0.6rem] ml-1">RP</span></span>
                <div className="w-px h-6 bg-white/10" />
                <span className="text-[0.9rem] font-mono font-black text-white tracking-tighter">{ calcBF( effectiveIpRp ).toFixed(2) }x <span className="text-text-darker text-[0.6rem] ml-1">BF</span></span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-10 group/oop transition-all duration-500">
          ...
          <div className="bg-slate-900/60 px-10 py-5 rounded-2xl border border-white/5 text-center group-hover/oop:border-accent-rose/40 group-hover/oop:bg-slate-900/80 transition-all shadow-2xl relative overflow-hidden">
            ...
            <span className="text-[0.6rem] font-black text-text-darker uppercase tracking-[0.4em] block mb-3 group-hover/oop:text-accent-rose-light transition-colors relative z-10">Vulnerabilidade</span>
            <div className="flex items-center justify-center gap-4 relative z-10">
                <span className="text-[0.9rem] font-mono font-black text-white tracking-tighter">{ effectiveOopRp.toFixed( 1 ) }% <span className="text-text-darker text-[0.6rem] ml-1">RP</span></span>
                <div className="w-px h-6 bg-white/10" />
                <span className="text-[0.9rem] font-mono font-black text-white tracking-tighter">{ calcBF( effectiveOopRp ).toFixed(2) }x <span className="text-text-darker text-[0.6rem] ml-1">BF</span></span>
            </div>
          </div>
        </div>
