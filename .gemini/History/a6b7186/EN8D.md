...
|--------|-----------|------------|
| Definicoes | `.claude/agents/*.md` | 17 arquivos |
| Memorias | `.claude/agent-memory/*/MEMORY.md` | 17 diretorios |
| Manifest | `data/agents_manifest.json` | 1 arquivo (17 agentes) |
...
- **17/17 agentes** tinham `model:` invalido (`anthropic/claude-3.5-sonnet` ou `google/gemini-flash-1.5`). Corrigido para `sonnet`.
...
**Problema:** 11/17 memorias sao genericas (conteudo de template, sem dados de execucao real). Sao filosofia, nao inteligencia operacional.
...
17 agentes bem definidos com papeis claros, invocaveis via Claude Code.
...
### O que deve ser mantido (Navalha SOTA aplicada)

- Os 17 `.claude/agents/*.md` (todos funcionam agora)
- As 17 memorias (mesmo as genericas, podem ser enriquecidas com uso)
