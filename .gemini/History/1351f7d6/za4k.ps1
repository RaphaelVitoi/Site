...
Write-Host '=== ANCORANDO DIRETRIZES SOTA NA IDE GLOBAL ===' -ForegroundColor Cyan
...
foreach ($SettingsPath in $VsCodeSettingsPaths) {
    if (Test-Path $SettingsPath) {
        try {
            ...
            Write-Host "[OK] Lei Marcial gravada com sucesso em: $SettingsPath" -ForegroundColor Green
        }
        catch {
            Write-Host "[ALERTA] Falha ao injetar em $SettingsPath (Possível erro de parsing do JSON). Detalhe: $_" -ForegroundColor Yellow
        }
    }
}

Write-Host "`nOperação independente concluída. O comportamento do Gemini está travado no SO." -ForegroundColor Magenta