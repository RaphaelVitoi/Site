...
try {
    $CustomInstructions = Get-Content -Path (Join-Path $PSScriptRoot '..\.vscode\gemini-codeassist-custom-instructions.md') -Raw -ErrorAction Stop
}
catch {
    if (-not $Quiet) { Write-Host '[ALERTA] Instrucoes nao encontradas. Abortando injecao.' -ForegroundColor Yellow }
    exit 0
}
