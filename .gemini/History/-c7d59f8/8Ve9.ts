...
export function useBayesianRange(initialRangeStr: string = "100%") {
  ...
  const currentBelief =
    history.length > 0 ? history[history.length - 1] : baseBelief;
  ...
  const applyAction = useCallback(
    (likelihood: ActionLikelihood) => {
      setHistory((prev) => {
        const prior = prev.length > 0 ? prev[prev.length - 1] : baseBelief;
