    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "historian-reports":
        ...
        async def _generate_historian_reports():
            ...
            try:
                ...
                telemetry = [
                    {"evLoss": 1.2, "isCorrect": False, "createdAt": datetime.now(timezone.utc).isoformat()},
                    {"evLoss": 0, "isCorrect": True, "createdAt": datetime.now(timezone.utc).isoformat()}
                ]

                if perf_history:
                    telemetry = [{"evLoss": 0, "isCorrect": True, "createdAt": f"{item['day']}T12:00:00Z"} for item in perf_history[-5:]]
