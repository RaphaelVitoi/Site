...
def _resolve_tasks_db_path() -> Path | None:
    """SOTA: Resolve o caminho do banco de dados expurgando arquivos fantasmas."""
    import sqlite3

    for candidate in ["tasks.db", DB_PATH_CLAUDE, DB_PATH_QUEUE]:
        p = Path(candidate)
    ...
    async def apply_yield(self, task: Task, manager: QueueManager) -> float:
        ...
        for dep_id in deps:
            ...
            if hasattr(manager, "get_task"):
                ...
                dep_exists = dep_task is not None
            else:
                # Fallback seguro caso get_task não esteja exposto publicamente no DAO
                all_tasks = await manager.get_tasks(None)
                dep_exists = any(t.id == dep_id for t in all_tasks)

            if not dep_exists:
                logger.error(
                    f"[[{_c(task.agent)}]{task.agent}[/]] [ENTROPIA FATAL] Tarefa {task.id} engatilhada com dependência fantasma: '{dep_id}'. Abortando imediatamente (status failed)."
                )
            ...
            if hasattr(manager, "get_task"):
                for dep_id in deps:
                    ...
                    if dep_task:
                        ...
                        # SOTA: Dependencia falhou ou estagnou em pending? Deadlock verdadeiro.
                        if dep_task.status in ["failed", "pending"]:
                            is_deadlock = True

            # Só alarma o Tier 1 se for um deadlock real ou se não conseguirmos determinar (fantasma)
            if is_deadlock or not deps_status_details:
                deps_info = (
                ...
                alert_task = Task(
                    ...
                    description=f"A tarefa {task.id} (Agente: {task.agent}) atingiu limite de starvation. Diagnóstico de dependências: {deps_info}. Requer intervenção cirúrgica (God Mode) para purgar ou re-enfileirar os nós falhos da DAG.",
                    agent=AGENT_CHICO,
                    status="pending",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "historian-reports":
        import asyncio
        from database.queue_manager import QueueManager
        from predictive_forest import PredictiveForestEngine

        async def _generate_historian_reports():
            qm = QueueManager()
            try:
                ...
                profile = {
                    ...
                    "Passivo Estrutural (RIO)": pred_profile.get(
        ...
        asyncio.run(_generate_historian_reports())
        sys.exit(0)

    if len(sys.argv) >= 2 and sys.argv[1] == "daily-stats":
        import asyncio
        from database.queue_manager import QueueManager
            ...
            try:
                ...
                stats = {
                    ...
                    "metrics": {
                        ...
                        "completed": sum(
                            1 for t in today_tasks if t.status == "completed"
                        ),
                    ...
                    "recent_activity": [
                        ...
                        for t in today_tasks[
                            -20:
                        ]  # Ultimas 20 para contexto de padroes
                    ],
                }
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "db-audit-dag":
        import sqlite3
        from pathlib import Path
        ...
        try:
            ...
            # SOTA: Abordagem agnóstica de schema lendo a lista de dependências no JSON do campo metadata
            ...
            logger.error(f"[FALHA] Erro ao auditar DAL: {e}")
            sys.exit(1)
        finally:
            if conn:
                conn.close()

    if len(sys.argv) >= 2 and sys.argv[1] == "db-purge-orphans":
        import sqlite3
        from pathlib import Path
        ...
        try:
                            ...
                            if isinstance(deps, list):
                                for dep in deps:
                                    if dep not in task_ids:
                                        ...
                                        purged_count += 1
                                        logger.info(
                                            f"[EXPURGO] Tarefa failed aniquilada: {t_id} (Dependia do id fantasma: {dep})"
                                        )
                                        break
                    except json.JSONDecodeError:
                        pass
            ...
            logger.error(f"[FALHA] Erro ao limpar DAL: {e}")
            sys.exit(1)
        finally:
            if conn:
                conn.close()

    if len(sys.argv) >= 2 and sys.argv[1] == "db-vacuum":
        import sqlite3
        from pathlib import Path
        ...
        try:
            ...
            logger.error(f"[FALHA] Erro ao executar VACUUM: {e}")
            sys.exit(1)
        finally:
            if conn:
                conn.close()

    # SOTA: Roteamento hibrido para o novo Nexus CLI
    nexus_cmds = {
        "dashboard",
        "start-worker",
        "stop-worker",
    ...
    if len(sys.argv) >= 2 and sys.argv[1].lower() in nexus_cmds:
        from scripts.cli.nexus import main as run_nexus
