...
if not AGENTS_MANIFEST:
    logger.warning(f"[RESILIENCIA] {PATH_AGENTS_MANIFEST} nao encontrado ou vazio. Acionando fallback via {PATH_INTENTMAP}.")
...
if AGENTS_MANIFEST and _intentmap_path.exists():
    ...
    if _intentmap_disk and set(AGENTS_MANIFEST.keys()) != {k.lstrip('@') for k in _intentmap_disk}:
        logger.critical(f"[CRITICAL] FAIL-FAST SOTA: Assimetria detectada entre {PATH_AGENTS_MANIFEST} e {PATH_INTENTMAP}. Risco de entropia no Roteamento Holografico. Abortando inicializacao.")
        sys.exit(1)
    ...
    try:
        ...
        if _CONFIG_MTIME.get("agents_manifest") != mtime:
            ...
            if new_manifest:
                ...
                logger.info(f"[HOT-RELOAD] {PATH_AGENTS_MANIFEST}. {len(VALID_AGENTS)} agentes.")
            else:
                logger.warning(f"[RESILIENCIA] Hot-reload detectou {PATH_AGENTS_MANIFEST} vazio. Acionando fallback via {PATH_INTENTMAP}.")
                INTENT_MAP = load_json_config(Path(PATH_INTENTMAP), {})
                AGENT_SOURCE = PATH_INTENTMAP
                AGENT_ROUTING_MAP = dict.fromkeys(INTENT_MAP.keys(), "fast_operations")
                AGENT_COLOR_MAP = dict.fromkeys(INTENT_MAP.keys(), "white")
...
if not VALID_AGENTS:
    logger.critical("CRITICAL: agents_manifest.json nao encontrado ou vazio. O sistema nao tem consciencia de seus agentes.")
    sys.exit(1)
