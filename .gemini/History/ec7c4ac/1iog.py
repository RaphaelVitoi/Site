...
if not logger.handlers:
    ...
    class SotaLogFormatter(logging.Formatter):
        """SOTA: Erradicação de CRLF Injection em disco preservando a árvore de Tracebacks."""
...
async def _read_telemetry_dump() -> list[dict]:
    ...
    dump_path = _core_config.PATH_TELEMETRY_DUMP.resolve()

    # SOTA SEC: Blindagem contra Deleção Arbitrária via Symlink / Path Traversal
    project_root = Path(__file__).parent.resolve()
    if not dump_path.is_relative_to(project_root):
        logger.error(
            f"[SEC CRITICO] O caminho de telemetria transborda a raiz segura. Abortando exclusão de arquivo: {dump_path}"
        )
        return []
