...
from llm.budget import (
    ...
    KEY_BLOCK_DURATION, GEMINI_MODEL_KEY_BLOCK_DURATION,
    ...
    _key_fingerprint, _score_key_from_stats, _rank_keys_by_health, AsyncTokenBucket
)
...
PATH_AGENTS_MANIFEST = "data/agents_manifest.json"
PATH_INTENTMAP = "data/intentmap.json"
...
AGENTS_MANIFEST = load_json_config(Path(PATH_AGENTS_MANIFEST), {})
AGENT_SOURCE = PATH_AGENTS_MANIFEST

# SOTA: Implementacao do protocolo de resiliencia (HOLOGRAPHIC ROUTING PROTOCOL, Secao 8)
if not AGENTS_MANIFEST:
    logging.warning(f"[RESILIENCIA] {PATH_AGENTS_MANIFEST} nao encontrado ou vazio. Acionando fallback via {PATH_INTENTMAP}.")
    INTENT_MAP = load_json_config(Path(PATH_INTENTMAP), {})
    AGENT_SOURCE = PATH_INTENTMAP
    # Em modo fallback, nao temos preferencias de modelo ou cores, usamos defaults.
    AGENT_ROUTING_MAP = dict.fromkeys(INTENT_MAP.keys(), "fast_operations")
    AGENT_COLOR_MAP = dict.fromkeys(INTENT_MAP.keys(), "white")
...
_intentmap_path = Path(PATH_INTENTMAP)
if AGENTS_MANIFEST and _intentmap_path.exists():
    _intentmap_disk = load_json_config(_intentmap_path, {})
    if _intentmap_disk and set(AGENTS_MANIFEST.keys()) != {k.lstrip('@') for k in _intentmap_disk.keys()}:
        logging.critical(f"[CRITICAL] FAIL-FAST SOTA: Assimetria detectada entre {PATH_AGENTS_MANIFEST} e {PATH_INTENTMAP}. Risco de entropia no Roteamento Holografico. Abortando inicializacao.")
        sys.exit(1)
...
PROTECTED_AGENTS_FROM_CLEANUP = tuple(SYSTEM_CONFIG.get("protected_agents_from_cleanup", (AGENT_MAVERICK, AGENT_CHICO)))
...
TECHNICAL_AGENTS = tuple(SYSTEM_HEURISTICS.get("technical_agents", [AGENT_IMPLEMENTOR, "@verifier", "@skillmaster", "@organizador", "@sequenciador"]))
...
DEFAULT_MODEL_HEALTH_GATE = {
    ...
    "protect_models": [MODEL_GEMINI_FLASH],
}
...
def _reload_agents_manifest() -> bool:
    ...
    manifest_path = Path(PATH_AGENTS_MANIFEST)
    try:
        ...
        if _CONFIG_MTIME.get("agents_manifest") != mtime:
            ...
            if new_manifest:
                ...
                logging.info(f"[HOT-RELOAD] {PATH_AGENTS_MANIFEST}. {len(VALID_AGENTS)} agentes.")
            else:
                logging.warning(f"[RESILIENCIA] Hot-reload detectou {PATH_AGENTS_MANIFEST} vazio. Acionando fallback via {PATH_INTENTMAP}.")
                INTENT_MAP = load_json_config(Path(PATH_INTENTMAP), {})
                AGENT_SOURCE = PATH_INTENTMAP
                AGENT_ROUTING_MAP = dict.fromkeys(INTENT_MAP.keys(), "fast_operations")
                AGENT_COLOR_MAP = dict.fromkeys(INTENT_MAP.keys(), "white")
...
def _apply_routing_heuristics(desc_lower: str) -> Tuple[Optional[str], int]:
...
def _intelligent_route_task(description: str, explicit_agent: Optional[str] = None) -> Tuple[str, Dict[str, Any]]:
    ...
    if explicit_agent not in [AGENT_DISPATCHER, "@architect", AGENT_MAVERICK]:
        ...
        if complexity_score > 150 or is_epic:
            logging.warning(f"[ROUTING SOTA] Tarefa muito complexa detectada ({complexity_score} palavras). Interceptando e roteando para {AGENT_DISPATCHER} com {AGENT_MAVERICK} de observador.")
...
class DynamicYieldManager:
    ...
    def apply_exhaustion_yield(self, task: Task) -> float:
        ...
        yield_time = 60.0
        logging.warning(f"[[{_c(task.agent)}]{task.agent}[/]] Exaustao de Chaves detectada para {task.id}. Aplicando yield tatico de {yield_time}s.")
        return yield_time
