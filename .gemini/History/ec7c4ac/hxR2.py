...
def _intelligent_route_task(
    ...
    if explicit_agent not in [AGENT_DISPATCHER, AGENT_ARCHITECT, AGENT_MAVERICK, AGENT_CHICO]:
...
def _cli_historian_reports() -> None:
    ...
    async def _read_telemetry_dump() -> list[dict]:
        ...
        if dump_path.exists() and dump_path.stat().st_size > 0:
            try:
                ...
                async with aiofiles.open(processing_path, "r", encoding="ascii", errors="backslashreplace") as f:
                    async for line in f:
                        if line.strip():
    ...
    def _build_profile(fail_rate: float, engine: PredictiveForestEngine) -> dict:
        try:
            pred_profile = engine.get_predictive_profile()
        except Exception:
            pred_profile = {}
        return {
            "Aversão ao Risco": pred_profile.get("Aversão ao Risco", round(0.85 - (fail_rate * 0.1), 2)),
            ...
            "Excesso de Agressão": pred_profile.get("Excesso de Agressão", round(0.30 + (fail_rate * 0.15), 2)),
    ...
    async def _generate_historian_reports():
        ...
        try:
            ...
            # SOTA: Consumo Atômico do Buffer WASM Telemetry (Homeostase de I/O)
            telemetry = []
            dump_path = _core_config.PATH_TELEMETRY_DUMP
            if dump_path.exists() and dump_path.stat().st_size > 0:
                try:
                    import shutil
                    processing_path = dump_path.with_suffix(".jsonl.processing")
                    # Deslocamento atômico para blindar contra race conditions com a Daemon Thread
                    shutil.move(str(dump_path), str(processing_path))
                    with open(processing_path, "r", encoding="ascii", errors="backslashreplace") as f:
                        for line in f:
                            if line.strip():
                                telemetry.append(json.loads(line))
                    processing_path.unlink(missing_ok=True)
            ...
            # Fallback legado para manter coerência visual caso o buffer de telemetria esteja vazio
