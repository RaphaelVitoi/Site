...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "db-audit-dag":
        ...
        if not db_path.exists() and Path(".claude/tasks.db").exists():
            db_path = Path(".claude/tasks.db")
        if not db_path.exists() and Path("queue/tasks.db").exists():
            db_path = Path("queue/tasks.db")

        # Previne o SQLite de criar um banco fantasma (o que gera a falha "no such table")
        if not db_path.exists():
            logger.error(
                "[ENTROPIA] Banco de dados de tarefas não encontrado na raiz, .claude/ ou queue/."
            )
            sys.exit(1)
