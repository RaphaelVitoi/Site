...
class UniversalArbitrator:
    ...
    def _compute_utilities(cls, graph: dict[str, dict[str, Any]]) -> None:
        ...
        def dfs_utility(node_id: str) -> float:
            ...
            if node_id in recursion_stack:
                logger.critical(
                    f"[SISTEMA] Entropia Detectada: Ciclo infinito no DAG envolvendo {node_id}"
                )
                raise CyclicDependencyError(
                    f"Ciclo topológico detectado na tarefa {node_id}"
                )
    ...
    def extract_optimal_task(cls, pending_tasks: list[Task]) -> Task | None:
        ...
        if not optimal_task:
            logger.warning(
                "[NEXUS ORCHESTRATOR] Deadlock Operacional: Nenhuma tarefa possui in_degree=0."
            )
            return None
