    ...
    try:
        ...
        try:
            ...
            if "```json" in clean_answer:
                clean_answer = clean_answer.split("```json")[1].split("```").strip()
            elif "```" in clean_answer:
                clean_answer = clean_answer.split("```").split("```")[0].strip()

            parsed_data = json.loads(clean_answer)
            profile_data = {
                "profile": parsed_data
                if "profile" not in parsed_data
                else parsed_data["profile"]
            }

        except (asyncio.TimeoutError, json.JSONDecodeError) as e:
            logger.warning(
                f"RAG indisponível ou parse falhou ({e}). Acionando Fallback Base."
            )
            profile_data = {
                "profile": {
