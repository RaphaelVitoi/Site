...
async def handle_ask_oracle(request):
    ...
    try:
        ...
        logger.exception(f"Falha na consulta ao Oraculo: {e}")
        return web.json_response({"error": str(e)}, status=500)
...
async def handle_get_db_summary(request: web.Request) -> web.Response:
    ...
    try:
        ...
        logger.exception(f"Falha ao obter sumario do DB: {e}")
        return web.json_response({"status": "error", "error": str(e)}, status=500)
...
async def handle_get_system_status(_request: web.Request) -> web.Response:  # NOSONAR
    ...
    try:
        ...
        logger.exception(f"Falha ao coletar status do sistema: {e}")
        return web.json_response(
...
async def handle_rag_ingest(request) -> web.Response:
    ...
    try:
        ...
        logger.exception(f"Falha ao disparar ingestao RAG: {e}")
        return web.json_response({"error": str(e)}, status=500)
...
async def handle_frontend_logs(request) -> web.Response:
    ...
    try:
        ...
        logger.exception(f"Falha ao processar logs do frontend: {e}")
        return web.json_response({"error": str(e)}, status=500)
...
async def handle_predictive_profile(_request: web.Request) -> web.Response:  # NOSONAR
    ...
    try:
        ...
        prompt = (
            ...
            "as tendências populacionais. Retorne APENAS um JSON válido contendo as chaves: "
            "'Aversão ao Risco', 'Pot Entrapment', 'Miopia de Payjump', "
            "'Excesso de Agressão', 'Passivo Estrutural (RIO)' e 'Desvio de Nash'. "
        ...
        try:
            ...
            # Normalização de Chaves para o contrato do Frontend
            profile = parsed_data.get("profile", parsed_data)

        except (asyncio.TimeoutError, json.JSONDecodeError, IndexError) as e:
            logger.warning(
                f"[PREDICTIVE] RAG indisponível ou parse falhou ({type(e).__name__}). Acionando Fallback Populacional."
            )
            profile = {
        ...
        logger.exception(f"[SYSTEM] Falha crítica no endpoint preditivo: {e}")
        return web.json_response({"error": str(e)}, status=500)
