...
async def handle_get_db_summary(request) -> web.Response:
...
async def handle_get_system_status(request) -> web.Response:
...
async def handle_rag_ingest(request) -> web.Response:
    ...
    try:
        ...
        if not hasattr(request.app, 'bg_tasks'):
            request.app['bg_tasks'] = set()
        task = asyncio.create_task(rag.ingest_all_memories())
        request.app['bg_tasks'].add(task)
        task.add_done_callback(request.app['bg_tasks'].discard)
