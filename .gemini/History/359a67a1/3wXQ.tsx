...
function getEsperancaMatConfig(
  isVacuum: boolean,
  isDeathZone: boolean,
  avgRp: number,
) {
  ...
  if (isDeathZone)
    return { value: "Estrangulada", color: "text-accent-danger" };
...
export default function QuantumSynthesis({
  ...
  const narrativeText = getContextualNarrative(
    isVacuum,
    isDeathZone,
    isNearPayjump,
    avgRp,
  ...
  return (
    <div
      className={`mt-6 p-6 glass-panel border overflow-hidden relative ${wrapperClass}`}
    >
      <div className="flex justify-between items-start mb-5">
        ...
        <div className="text-right">
          ...
          <span
            className={`text-3xl font-black font-mono tracking-tighter ${rpColor}`}
          >
            {avgRp.toFixed(1)}%
          </span>
        </div>
