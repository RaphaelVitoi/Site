    ...
    for i, key in enumerate(keys):
        ...
        for attempt in range(max_retries):
            try:
                ...
                manager.record_api_usage(task.id, task.agent, model, provider_name, prompt_tokens, completion_tokens)
...
def call_llm_api(task: Task, system_prompt: str, user_prompt: str, manager: QueueManager) -> str:
    ...
    for model in models_to_try:
        ...
        if "/" in model or "deepseek" in model or "llama" in model:
            response = _try_provider("openrouter", call_openrouter, model, system_prompt, user_prompt, openrouter_keys, task, manager,
                                     max_retries=3, usage_keys=('prompt_tokens', 'completion_tokens'))
        elif "gemini" in model:
            response = _try_provider("gemini", call_gemini, model, system_prompt, user_prompt, gemini_keys, task, manager,
                                     max_retries=2, usage_keys=('promptTokenCount', 'candidatesTokenCount'))
        elif "claude" in model:
            response = _try_provider("anthropic", call_anthropic, model, system_prompt, user_prompt, anthropic_keys, task, manager,
