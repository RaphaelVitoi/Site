...
export const QuizEngine: React.FC<QuizEngineProps> = ( { questions, onQuizRestart } ) => {
    ...
    const handleSelectOption = ( optionId: string ) => {
        ...
        logTelemetryEvent( {
            category: ( currentQ.category as any ) || "Fundamentos SOTA",
    ...
    return (
        <div className="bg-bg-panel/40 border border-white/5 rounded-xl overflow-hidden max-w-4xl mx-auto shadow-[0_10px_40px_-10px_rgba(0,0,0,0.4)] backdrop-blur-md">
            { isFinished ? (
                ...
                <>
                    ...
                    <div className="px-8 pb-8 flex justify-end min-h-[80px]">
                        { answers[safeQuestions[currentIndex].id] && (
                            <button
                                onClick={ handleNext }
                                className="px-6 py-2.5 bg-accent-indigo/15 hover:bg-accent-indigo/25 text-accent-indigo-light border border-accent-indigo/30 rounded-md font-bold transition-all uppercase tracking-widest text-xs flex items-center gap-2"
                            >
