...
Describe 'do.ps1 - A Membrana Inteligente SOTA' -Tags 'Unit' {
    ...
    Context 'Roteamento de Comandos de Sistema e Entropia' {
        It 'Deve invocar o motor de Chaos Engineering com -Chaos' {
            & $scriptFile -Chaos -Intensity 'high' -Target 'frontend'
            Assert-MockCalled 'npx.cmd', 'npx' -Times 1 -Exactly -Scope It -ParameterFilter {
                $Arguments -match 'chaos-core.ts' -and $Arguments -match '--intensity high' -and $Arguments -match '--target frontend'
            }
        }

        It 'Deve invocar o protocolo de obliteracao com -Obliterate' {
            Mock Test-Path { return $true }
            & $scriptFile -Obliterate "./temp_dir"
            Assert-MockCalled Remove-Item -Times 1 -Exactly -Scope It -ParameterFilter { $LiteralPath -like "*temp_dir" -and $Recurse -and $Force }
        }

        It 'Deve invocar o protocolo Anti-EPERM com -FixEPERM' {
            & $scriptFile -FixEPERM "npm install"
            Assert-MockCalled Stop-Process -Times 1 -Exactly -Scope It -ParameterFilter { $Name -eq "OneDrive" }
        }
    }

    Context 'Roteamento de Comandos de Manutencao' {
        It 'Deve invocar o script de backup com -Backup' {
            & $scriptFile -Backup
            Assert-MockCalled 'safeguard_system.ps1' -Times 1 -Exactly -Scope It
        }

        It 'Deve invocar o script de setup com -Setup' {
            & $scriptFile -Setup
            Assert-MockCalled 'Setup-NexusProfile.ps1' -Times 1 -Exactly -Scope It
        }
    }
}