...
def scan_file(filepath: str, chunk: int = 1, size: int = 50):
    """Executa a leitura paginada de um arquivo e extrai metadados de ontologia."""
    path = Path(filepath)
    if not path.exists():
    ...
    print(
        f"=== [SOTA INSIGHT] Lendo {path.name} "
        f"(Linhas {start + 1} a {min(end, total_lines)} de {total_lines}) ==="
    )
