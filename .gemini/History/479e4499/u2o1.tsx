...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1, currentPot = 2.5, activePlayers = 2 }: Readonly<PmLensPanelProps> ) {
  ...
  // SOTA: Estado do Web Worker (Motor Bruto)
  const [ heroRange, setHeroRange ] = useState( '' );
  const [ villainRange, setVillainRange ] = useState( '' );
  const [ isCalculatingEq, setIsCalculatingEq ] = useState( false );
  ...
  // --- SOTA: Módulo Assíncrono de Input Bruto ---
  const handleCalculateEquity = () => {
    setIsCalculatingEq( true );
    const worker = new Worker( new URL( '../workers/equity.worker.ts', import.meta.url ) );

    worker.onmessage = ( e ) => {
      setEquity( e.data.equity );
      setIsCalculatingEq( false );
      worker.terminate(); // SOTA: Aniquilação termodinâmica para evitar Memory Leaks do Web Worker
    };

    worker.onerror = ( error ) => {
      console.error( "[PmLens Worker] Falha catastrófica no motor WASM:", error );
      setIsCalculatingEq( false );
      worker.terminate();
    };
            ...
            <div>
              ...
              <div style={ { marginTop: '0.6rem', padding: '0.5rem', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '6px', width: '160px' } }>
                ...
                <div style={ { display: 'flex', flexDirection: 'column', gap: '0.3rem', marginBottom: '0.4rem' } }>
                  <input type="text" placeholder="Hero (ex: AKs)" value={ heroRange } onChange={ e => setHeroRange( e.target.value ) } style={ { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-main)', fontSize: '0.55rem', padding: '0.2rem 0.4rem', borderRadius: '4px', outline: 'none' } } />
                  <input type="text" placeholder="Vilão (ex: 15%)" value={ villainRange } onChange={ e => setVillainRange( e.target.value ) } style={ { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-main)', fontSize: '0.55rem', padding: '0.2rem 0.4rem', borderRadius: '4px', outline: 'none' } } />
                </div>
                <button
