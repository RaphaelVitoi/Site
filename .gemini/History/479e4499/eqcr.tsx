...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1, currentPot = 2.5, activePlayers = 2 }: Readonly<PmLensPanelProps> ) {
  ...
  const [ deltaHabilidade, setDeltaHabilidade ] = useState( 50 ); // Edge Bruta (0-100)
  const hintTimerRef = useRef<ReturnType<typeof setTimeout> | null>( null );

  // SOTA: Persistência da Amortização de Edge (Fricção Zero)
  // Garante que a identidade técnica do usuário sobreviva a recarregamentos.
  useEffect( () => {
    try
    {
      const saved = localStorage.getItem( 'vitoi_pm_delta_habilidade' );
      if ( saved !== null ) setDeltaHabilidade( Number( saved ) );
    } catch ( e ) { /* Fricção Zero */ }
  }, [] );

  const handleDeltaChange = ( val: number ) => {
    setDeltaHabilidade( val );
    try { localStorage.setItem( 'vitoi_pm_delta_habilidade', String( val ) ); } catch ( e ) { /* Fricção Zero */ }
  };
