    ...
    <div style={ { marginTop: '2rem', padding: '1.2rem 1.4rem', background: 'rgba(10,15,30,0.7)', border: '1px solid rgba(99,102,241,0.25)', borderRadius: '14px' } }>
      ...
      <div style={ { display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem', marginBottom: '1.1rem' } }>
        <div style={ { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' } }>
          <div>
            <div style={ { fontSize: '0.52rem', color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' } }>Hero</div>
          ...
          <div>
            <div style={ { fontSize: '0.52rem', color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' } }>Villain</div>
            <div style={ { display: 'flex', flexWrap: 'wrap', gap: '0.3rem' } }>
        ...
        <div style={ { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', background: 'rgba(255,255,255,0.02)', padding: '1.25rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' } }>
          <div style={ { display: 'flex', flexDirection: 'column', gap: '1rem' } }>
            <div>
              <div style={ { fontSize: '0.52rem', color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' } }>Posição</div>
          ...
          <div key={ s.name } style={ { background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' } }>
            <div style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.75rem' } }>
              ...
              <span style={ { padding: '3px 8px', borderRadius: '4px', fontSize: '0.55rem', fontWeight: 800, background: s.PM > 0 ? 'rgba(16,185,129,0.15)' : 'rgba(244,63,94,0.15)', color: s.PM > 0 ? 'var(--accent-emerald)' : 'var(--accent-danger)' } }>
                { s.loading ? '...' : ( s.PM > 0 ? 'SOBERANO' : 'INSOLVENTE' ) }
              </span>
            </div>
            <div style={ { display: 'flex', flexDirection: 'column', gap: '0.5rem' } }>
              <MetricRow label="Sunk Cost" value={ `-${s.heroCost.toFixed( 2 )}bb` } color="var(--text-dim)" loading={ s.loading } />
              <MetricRow label="Piso (EV_fold)" value={ `${s.evFold.toFixed( 2 )}%` } color="var(--accent-danger)" loading={ s.loading } />
              <MetricRow label="Passivo (RIO)" value={ `${s.rio.toFixed( 2 )}%` } color="var(--accent-amber)" loading={ s.loading } />
              <MetricRow label="FGS Health" value={ `${s.fgsHealth.toFixed( 2 )}x` } color="var(--accent-violet)" loading={ s.loading } />
              <MetricRow label="Insolvência (Cᵢ)" value={ renderCi( s.loading, s.ci ) } color={ s.ci !== null && s.ci < 1 ? 'var(--accent-danger)' : 'var(--accent-emerald)' } loading={ s.loading } />
            </div>
            <div style={ { marginTop: 'auto', paddingTop: '0.75rem', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } }>
              <span style={ { fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.05em' } }>Perspectiva</span>
