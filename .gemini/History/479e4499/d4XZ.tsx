...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1.125, currentPot = 2.5, activePlayers = 2 }: Readonly<PmLensPanelProps> ) {
  const [ heroIdx, setHeroIdx ] = useState( 8 );
  const [ villainIdx, setVillainIdx ] = useState( 6 );
  const [ heroIsIp, setHeroIsIp ] = useState( false );
  const [ pkoValue, setPkoValue ] = useState( 0 );
  const [ kappa, setKappa ] = useState( 0.5 );
  const [ deltaHabilidade, setDeltaHabilidade ] = useDebouncedLocalStorage<number>( 'vitoi_pm_delta_habilidade', 50 );
  ...
  const streetProgression = useMemo( () => {
    return STREETS.map( ( s, i ) => {
      let cumulative = Math.abs( heroInvested );
      if ( i > 0 ) cumulative += STREETS[ i - 1 ].potSize * 0.33;
      if ( i > 1 ) cumulative += STREETS[ i - 1 ].potSize * 0.4;
      return { ...s, cumulative };
    } );
  }, [ heroInvested ] );
  ...
  useEffect( () => {
    if ( !ecosystem?.dispatchIcmPerspectiva ) return;

    streetProgression.forEach( street => {
      const calculateForStreet = ( street: typeof streetProgression[ 0 ] ) => {
        const input = {
          stacks: STACKS,
          prizes: PRIZES,
          heroIdx,
          ...
          winProb: equity / 100,
          realizationFactor: heroIsIp ? 1 : 0.85,
        ...
        ecosystem.dispatchIcmPerspectiva?.( input, ( res ) => {
          setAsyncResults( prev => ( { ...prev, [ street.name ]: res } ) );
        } );
      } );
  };

  streetProgression.forEach( calculateForStreet );
}, [ heroIdx, villainIdx, equity, pkoValue, kappa, heroIsIp, anteSize, activePlayers, deltaHabilidade, streetProgression, ecosystem?.dispatchIcmPerspectiva ] );
        ...
        <div style={ { display: 'flex', flexDirection: 'column', gap: '0.6rem' } }>
          <div>
            ...
            <div style={ { display: 'flex', gap: '0.3rem', alignItems: 'center' } }>
              <SelectBtn label="IP" active={ heroIsIp } onClick={ () => setHeroIsIp( true ) } />
              <SelectBtn label="OOP" active={ !heroIsIp } onClick={ () => setHeroIsIp( false ) } />
            </div>
          </div>
          <div>
