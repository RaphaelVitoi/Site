...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1, currentPot = 2.5, activePlayers = 2 }: Readonly<PmLensPanelProps> ) {
  ...
  useEffect( () => {
    ...
    } catch ( e ) { console.debug( '[PmLens] Falha ao recuperar persistencia:', e ); }
  }, [] );

  const handleDeltaChange = ( val: number ) => {
    setDeltaHabilidade( val );
    try { localStorage.setItem( 'vitoi_pm_delta_habilidade', String( val ) ); } catch ( e ) { console.debug( '[PmLens] Falha ao salvar persistencia:', e ); }
  };
