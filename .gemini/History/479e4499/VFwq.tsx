...
function MetricRow ( { label, value, color, loading, tooltipTitle, tooltipDesc, theme = 'zinc' }: Readonly<{ label: string; value: string; color: string; loading?: boolean; tooltipTitle?: string; tooltipDesc?: string; theme?: any }> ) {
  ...
  return (
    <div style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } }>
      { tooltipTitle && tooltipDesc ? (
        <SotaTooltip title={ tooltipTitle } content={ tooltipDesc } align="left" theme={ theme }>
          { labelEl }
        </SotaTooltip>
      ) : labelEl }
...
// SOTA: Funções puras extraídas para erradicar a Complexidade Ciclomática (SonarLint S3776)
function getPostFlopOrder ( idx: number ) {
  if ( idx === 7 ) return 0;
  if ( idx === 8 ) return 1;
  return idx + 2;
}

function resolveHeroPosition ( pos: HeroPosition, length: number ) {
  if ( pos === 'BB' ) return Math.min( 8, length - 1 );
  if ( pos === 'SB' ) return Math.min( 7, Math.max( 0, length - 2 ) );
  return Math.min( 6, Math.max( 0, length - 3 ) );
}

function resolveAbsolutePos ( idx: number ): HeroPosition {
  if ( idx === 8 ) return 'BB';
  if ( idx === 7 ) return 'SB';
  return 'IP';
}

function buildStreetMetric ( street: { name: string; potSize: number; cumulative: number }, res: Record<string, unknown> | undefined ) {
  ...
  return {
    ...
    ci: res.ci == null ? null : Number( res.ci ),
...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1.125, currentPot = 2.5, activePlayers = 2, heroPosition = 'BB', blindsRisingSoon = false, initialStacks = STACKS, initialPrizes = PRIZES }: Readonly<PmLensPanelProps> ) {
  ...
  useEffect( () => {
    setHeroIdx( resolveHeroPosition( heroPosition, initialStacks.length ) );
  }, [ heroPosition, initialStacks.length ] );

  const absoluteHeroPos = resolveAbsolutePos( heroIdx );
  ...
  }, [ heroIdx, villainIdx, equity, pkoValue, kappa, anteSize, activePlayers, blindsRisingSoon, deltaHabilidade, streetProgression, initialStacks, initialPrizes, wasmActions, handleIcmResult, absoluteHeroPos, realizationFactor ] );
    ...
    <div style={ { marginTop: '2rem', padding: '1.2rem 1.4rem', background: 'rgba(10,15,30,0.7)', border: '1px solid rgba(99,102,241,0.25)', borderRadius: '14px' } }>
      ...
      <div style={ { display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem', marginBottom: '1.1rem' } }>
        <div style={ { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' } }>
          <div>
            <SotaTooltip title={ isMonopolist ? "Hero (Monopolista)" : "Agressor (Hero)" } content={ isMonopolist ? "Você controla 35%+ do ecossistema. RIO decai abruptamente e as rotações de blinds dos oponentes geram FGS positivo passivo." : "O agente ativo da equação. Seu stack atual define a margem de manobra (SPR) e a resistência estrutural à pressão do ICM." } align="left" theme={ isMonopolist ? 'symmetry' : 'zinc' }>
          ...
          <div>
            <SotaTooltip title="Defensor (Villain)" content="O passivo da colisão. O stack efetivo entre ambos dita o teto de perda e o risco instantâneo de eliminação (Bubble Factor)." align="left" theme="zinc">
        ...
        <div style={ { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', background: 'rgba(255,255,255,0.02)', padding: '1.25rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' } }>
          <div style={ { display: 'flex', flexDirection: 'column', gap: '1rem' } }>
            ...
            <div>
              <div style={ { display: 'flex', justifyContent: 'space-between', fontSize: '0.52rem', color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' } }>
                <SotaTooltip title="Equidade Bruta (Raw Equity)" content="Probabilidade isolada de vencer no Showdown (Vácuo). Totalmente cega para o FGS, RIO e Pressão de ICM." align="left" theme="machine">
          ...
          <div style={ { display: 'flex', flexDirection: 'column', gap: '1rem' } }>
            <div>
              <div style={ { display: 'flex', justifyContent: 'space-between', fontSize: '0.52rem', color: 'var(--accent-amber)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' } }>
                <SotaTooltip title="Poder de Bounty (PKO)" content="Distorção matemática progressiva. Afrouxa a pressão de sobrevivência do ICM quando cobrimos a stack do oponente." align="left" theme="symmetry">
            ...
            <div>
              <div style={ { display: 'flex', justifyContent: 'space-between', fontSize: '0.52rem', color: 'var(--accent-pink)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' } }>
                <SotaTooltip title="Taxa de Maluquice (Kappa κ)" content="Frequência estimada de que o vilão executará as linhas teóricas do Nash sem ceder ao Tilt ou Bobagem Humana." align="left" theme="entropy">
          ...
          <div style={ { display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' } }>
            <div style={ { padding: '0.75rem', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '8px' } }>
              ...
              <SotaTooltip title="Motor Quântico O(1)" content="Invoca o Web Worker nativo em Rust para triturar as matrizes combinatórias via Monte Carlo (10.000 iterações em <50ms)." align="center" theme="machine">
        ...
        { streetMetrics.map( s => (
          <div key={ s.name } style={ { background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' } }>
            ...
            <div style={ { display: 'flex', flexDirection: 'column', gap: '0.5rem' } }>
              <MetricRow label="Sunk Cost" value={ `-${s.heroCost.toFixed( 2 )}bb` } color="var(--text-dim)" loading={ s.loading } tooltipTitle="O Abismo do Fold (Sunk Cost)" tooltipDesc="Fichas investidas não lhe pertencem mais. Elas ditam a profundidade do custo irrecuperável de desistir." theme="entropy" />
              <MetricRow label="Piso (EV_fold)" value={ `${s.evFold.toFixed( 2 )}%` } color="var(--accent-danger)" loading={ s.loading } tooltipTitle="Baseline Dinâmico (EV_Fold)" tooltipDesc="A Esperança de simplesmente desistir e ceder o pote. Qualquer ação deve superar matematicamente esta âncora." theme="entropy" />
              <MetricRow label="Passivo (RIO)" value={ `${s.rio.toFixed( 2 )}%` } color="var(--accent-amber)" loading={ s.loading } tooltipTitle="Catástrofe Silenciosa (RIO)" tooltipDesc="O custo passivo de 'acertar e continuar perdendo'. Infla geometricamente (x²) em cenários Multiway." theme="entropy" />
              <MetricRow label="FGS Health" value={ `${s.fgsHealth.toFixed( 2 )}x` } color="var(--accent-violet)" loading={ s.loading } tooltipTitle="Erosão Antecipada (FGS t-3)" tooltipDesc="Punição gravitacional na órbita. Antecipa o dano do Big Blind iminente, forçando agressão para não morrer cego." theme="machine" />
              <MetricRow label="Insolvência (Cᵢ)" value={ renderCi( s.loading, s.ci ) } color={ s.ci == null || s.ci >= 1 ? 'var(--accent-emerald)' : 'var(--accent-danger)' } loading={ s.loading } tooltipTitle="Coeficiente de Insolvência (Ci)" tooltipDesc="Se Cᵢ < 1, as Pot Odds mentem. A mão não possui equidade suficiente para superar o passivo do RIO e do ICM." theme="machine" />
            </div>
            <div style={ { marginTop: 'auto', paddingTop: '0.75rem', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } }>
              <SotaTooltip title="A Métrica Soberana (PM)" content="A verdadeira utilidade da ação: (Expectativa - RIO) - EV_Fold. Exige que a ação rompa a inércia destrutiva do fold." align="left" theme="machine">
