...
function pmColor ( v: number ): string {
  if ( v > 0.5 ) return 'var(--accent-emerald)';
  if ( v > 0 ) return 'var(--accent-emerald-light)';
...
function _computeMatchupDeltas ( heroIdx: number, villainIdx: number, pkoValue: number, foldEvBb: number, currentPot: number, anteSize: number ): StreetDelta[] {
  return STREETS.map( ( { name, potSize: defaultPot } ) => {
    ...
    let valuation = 1, rioLiability = 0, amortizedEdge = 1;
    try
    {
      ...
      ( { deltaWinPct, deltaLosePct, dynamicEvFold: evFoldPct, fgsHealth, survivalPressure, bountyPower = 0, valuation, rioLiability, amortizedEdge } = result );
    } catch { /* silencioso */ }
    return { name, potSize, heroCost, evFoldPct, deltaWinPct, deltaLosePct, fgsHealth, survivalPressure, bountyPower, valuation, rioLiability, amortizedEdge };
  } );
}
...
function _getOrComputeDeltas ( h: number, v: number, pko: number, foldEvBb: number, currentPot: number, anteSize: number ): StreetDelta[] {
  ...
  if ( _deltasCache.size >= MAX_CACHE_SIZE ) _deltasCache.clear();
...
function _getOrComputeRp ( ip: number, oop: number, pko: number ) {
  ...
  if ( _rpCache.size >= MAX_CACHE_SIZE ) _rpCache.clear();

  let result: ReturnType<typeof deriveRps> = null;
  try { result = deriveRps( STACKS, PRIZES, ip, oop, pko * 100 ); } catch { /* silencioso */ }
...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1, currentPot = 2.5 }: Readonly<PmLensPanelProps> ) {
  ...
  const [ equity, setEquity ] = useState( 50 );    // 0-100 (inteiro)
  ...
  const streetMetrics = useMemo( () => {
    ...
    return icmDeltas.map( ( { name, potSize, heroCost, evFoldPct, deltaWinPct, deltaLosePct, fgsHealth, survivalPressure, bountyPower } ) => {
      ...
      // A Esperança (E) agora inclui o impacto do bounty em DeltaWin (ja embutido no icmDeltas)
      const E = eq * deltaWinPct + ( 1 - eq ) * deltaLosePct;

      // A Expectativa (P) aplica o FGS Health dinâmico e o R posicional
      // O fgsHealth aqui representa o multiplicador de 'longo prazo' calculado pelo motor
      const P = eq * deltaWinPct * R * fgsHealth + ( 1 - eq ) * deltaLosePct;
      const PM = P - evFoldPct;

      // Break-even equity: PM(eq_t) = 0
      const denom = deltaWinPct * R * fgsHealth - deltaLosePct;
      const threshEq = Math.abs( denom ) > 1e-6
        ? Math.max( 0, Math.min( 1, ( evFoldPct - deltaLosePct ) / denom ) )
        : null;

      return { name, potSize, heroCost, evFoldPct, evFoldChips: -heroCost, E, P, R, PM, threshEq, fgsHealth, survivalPressure, bountyPower };
    } );
  }, [ icmDeltas, equity, kappa, heroIsIp ] );

  // --- RP motor vs HRC ---
  const ipIndex = heroIsIp ? heroIdx : villainIdx;
  const oopIndex = heroIsIp ? villainIdx : heroIdx;

  // O(1) via cache de módulo — deriveRps (3× M-H) já pré-computado no pre-warm
  ...
  const hrcHeroRp = RP_HRC[ heroIdx ]?.[ villainIdx ] ?? null;
  const rpDelta = motorHeroRp !== null && hrcHeroRp !== null ? motorHeroRp - hrcHeroRp : null;
      ...
      <div style={ { marginTop: '0.75rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' } }>
        { streetMetrics.map( s => (
          <div key={ s.name } style={ {
            ...
            background: s.PM > 0 ? 'rgba(16,185,129,0.1)' : 'rgba(244,63,94,0.1)',
            border: `1px solid ${s.PM > 0 ? 'rgba(16,185,129,0.25)' : 'rgba(244,63,94,0.25)'}`,
            fontSize: '0.58rem', fontWeight: 700,
            color: s.PM > 0 ? 'var(--accent-emerald)' : 'var(--accent-danger)',
            ...MONO,
          } }>
