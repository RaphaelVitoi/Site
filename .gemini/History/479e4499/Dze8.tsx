...
export default function PmLensPanel ( { anteSize = 12.5, heroInvested = 1.125, currentPot = 2.5, activePlayers = 2, heroPosition = 'BB', blindsRisingSoon = false, initialStacks = STACKS, initialPrizes = PRIZES }: Readonly<PmLensPanelProps> ) {
  ...
  const realizationFactor = isHeroIP ? 1 : 0.85;
  const absoluteHeroPos = heroIdx === 8 ? 'BB' : ( heroIdx === 7 ? 'SB' : 'IP' );
  ...
  }, [ heroIdx, villainIdx, equity, pkoValue, kappa, anteSize, activePlayers, blindsRisingSoon, deltaHabilidade, streetProgression, initialStacks, initialPrizes, ecosystem?.dispatchIcmPerspectiva, handleIcmResult ] );
      ...
      <div style={ { display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem', marginBottom: '1.1rem' } }>
        ...
        <div style={ { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', background: 'rgba(255,255,255,0.02)', padding: '1.25rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' } }>
          <div style={ { display: 'flex', flexDirection: 'column', gap: '1rem' } }>
            <div>
              <PmLensTooltip title="Realização de Equidade (Fator R)" desc={ `Calculado para a colisão: ${PLAYERS[ heroIdx ]} vs ${PLAYERS[ villainIdx ]}. Quando o Hero joga Fora de Posição (OOP) no pós-flop, sofre penalidade estrutural e realiza apenas 85% da equidade. Em Posição (IP), realiza 100%.` } align="left">
                <div style={ { fontSize: '0.52rem', color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem', cursor: 'help' } } className="hover:text-indigo-400 transition-colors">Posição Pós-Flop</div>
              </PmLensTooltip>
              <div style={ { display: 'flex', gap: '0.4rem', alignItems: 'center' } }>
                <span style={ { fontSize: '0.75rem', fontWeight: 800, color: isHeroIP ? 'var(--accent-indigo)' : 'var(--accent-danger)', background: 'rgba(0,0,0,0.3)', padding: '0.3rem 0.7rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.1)' } }>{ isHeroIP ? 'IP (100%)' : 'OOP (85%)' }</span>
              </div>
            </div>
            <div>
