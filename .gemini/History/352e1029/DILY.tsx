        ...
        choices: [
            {
                ...
                metricImpact: { chipEv: '+0.12bb', icmEv: '-1.5% (Estático)', perspective: '-4.8% FGS (Catastrófico)' }
            },
            {
                id: 'fold',
                label: 'FOLD (Assumir o EV de Fold)',
                isCorrect: true,
                feedback: 'ESTADO DA ARTE. O EV do fold NUNCA é zero (aqui é -1.125bb, cravando blind + ante), mas evitar o cenário de RIO extremo preserva seu FGS. Você reconheceu a pseudo-densidade das pot odds. Sobrevivência e evasão de armadilhas geram o lucro real.',
                metricImpact: { chipEv: '-1.125bb', icmEv: '-0.9% (Custo + Ante)', perspective: '+0.5% (Preservação)' }
            },
            {
                id: 'squeeze',
                ...
                metricImpact: { chipEv: '-3.50bb', icmEv: '-5.0% (Alto Risco)', perspective: '-12.0% (Colapso Total)' }
            }
        ]
    },
    {
        ...
        context: 'Bolha de FT. Ante de 12.5%. Você é o Chip Leader com 60bb no BB. Um Short Stack (10bb) no SB vai all-in (shove). Você tem A8o.',
        ...
        choices: [
            {
                ...
                metricImpact: { chipEv: '+0.05bb', icmEv: '-1.2% (Desconto RP)', perspective: '-8.5% FGS (Risco de Ressurreição)' }
            },
            {
                id: 'fold',
                label: 'FOLD (Silêncio Estratégico)',
                isCorrect: true,
                feedback: 'ESTADO DA ARTE. O silêncio estratégico. Foldar A8o (ao custo de 1.125bb do blind e ante) protege seu valuation contra a variância amortizadora e mantém a vantagem sistêmica de ter um "morto-vivo" na mesa. Você reconheceu a poda da árvore de decisão.',
                metricImpact: { chipEv: '-1.125bb', icmEv: '-0.6% (Desconto + Ante)', perspective: '+2.1% (Manutenção de Pressão)' }
            }
        ]
    },
    {
        ...
        context: 'Você defendeu o BB (ante 12.5%) com 35bb de stack inicial e pagou Flop e Turn. No River, o pote tem 20bb, e você tem 15bb para trás. O agressor shova. As pot odds são irrecusáveis.',
        ...
        choices: [
            {
                ...
                metricImpact: { chipEv: '+0.50bb', icmEv: '-15.0% (Insolvência)', perspective: '-100% FGS (Eliminação Direta)' }
            },
            {
                id: 'fold',
                label: 'FOLD (Aceitar o Sunk Cost)',
                isCorrect: true,
                feedback: 'PRESERVAÇÃO DO SISTEMA. O EV do fold NUNCA é zero (aqui é exatamente a dor do seu Sunk Cost acumulado, que engloba as antes iniciais). Aceitar a perda estanca a sangria e salva os 15bb restantes (suas ferramentas de Edge futuras). Pot odds em potes grandes são frequentemente armadilhas lineares.',
                metricImpact: { chipEv: '-20.00bb', icmEv: '-18.0% (Sunk Cost)', perspective: '+0.0% (Sobrevivência Pura)' }
            }
        ]
    }
];
...
export default function IcmQuizVisceral () {
    ...
    const resolveMetric = ( type: 'chipEv' | 'icmEv' | 'perspective', choiceId: string, staticValue: string ) => {
        ...
        if ( type === 'icmEv' )
        {
            return ecosystem.actionMetrics[ choiceId ] ? `${ecosystem.actionMetrics[ choiceId ].perspectiva.toFixed( 2 )}% (Engine)` : staticValue;
        }
                    ...
                    <div style={ { display: 'flex', gap: '2rem', fontFamily: 'monospace' } }>
                        ...
                        <div>
                            ...
                            <span style={ { fontSize: '1rem', fontWeight: 800, color: 'var(--accent-amber)' } }>
                                { resolveMetric( 'icmEv', choiceData.id, choiceData.metricImpact.icmEv ) }
                            </span>
                        </div>
                        <div>
