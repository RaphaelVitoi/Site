...
function ActiveScenarioStep ( {
    ...
    readonly setAnswers: React.Dispatch<React.SetStateAction<Record<number, string>>>;
} ) {
    return (
        <motion.div
            key={ currentStep }
...
export default function IcmQuizVisceral () {
    ...
    useEffect( () => {
        fetchVisceralScenarios().then( ( data: Scenario[] ) => {
        ...
        <div style={ { maxWidth: '800px', margin: '3rem auto', padding: '2rem', background: 'rgba(15,23,42,0.6)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' } }>
            <div style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '1rem' } }>
                ...
                <span style={ { fontSize: '0.85rem', color: 'var(--text-dim)', fontFamily: 'monospace' } }>
                    { isFinished ? 'Síntese Final' : `Cenário ${currentStep + 1} de ${scenarios.length}` }
                </span>
            </div>

            <AnimatePresence mode="wait">
                { isFinished ? (
                    <QuizSynthesis onRestart={ () => { setAnswers( {} ); setCurrentStep( 0 ); } } />
                ) : (
                    <ActiveScenarioStep
                        scenario={ scenario }
