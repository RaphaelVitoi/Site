    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md">
            <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl shadow-[0_0_50px_-12px_rgba(99,102,241,0.2)] p-8 relative animate-fade-up">
                <button onClick={onClose} className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center rounded-full bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors">✕</button>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2">Motor de Narração (TTS)</label>
                        <select
                            value={audioProvider}
                            onChange={(e) => setAudioProvider(e.target.value as AudioProvider)}
                            className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl p-3.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all"
                        >
                            <option value="browser-native">Voz Nativa do Sistema (Offline)</option>
                            <option value="gemini-tts">Oráculo Premium (Google TTS)</option>
                        </select>
                    </div>
                        ...
                        <div className="bg-slate-950/50 p-5 rounded-xl border border-slate-800 space-y-4">
                            <div className="bg-emerald-900/10 border border-emerald-500/20 p-4 rounded-lg flex items-start gap-3">
                                ...
                                <p className="text-[13px] text-emerald-100/70 leading-relaxed">
                                    ...
                                    A sua chave é encriptada no navegador (Local Storage).
                                </p>
                            </div>
                            <div>
                            ...
                            <div className="flex justify-end pt-2">
                                <button onClick={handleClear} className="text-[10px] text-rose-400 hover:text-rose-300 font-bold uppercase tracking-widest transition-colors flex items-center gap-1.5"><span className="text-rose-500">⚠</span> Purgar Credenciais (Logout)</button>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-800 flex justify-end gap-3">
                    <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-[12px] font-bold uppercase tracking-widest text-slate-400 hover:text-white transition-colors">Cancelar</button>
                    <button onClick={handleSave} className="px-6 py-2.5 rounded-xl text-[12px] font-bold uppercase tracking-widest text-white bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-500/20 transition-all">Guardar Matriz</button>
                </div>
            </div>
        </div>
    );
