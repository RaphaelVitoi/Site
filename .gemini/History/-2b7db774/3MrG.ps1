...
# SOTA: Resolucao absoluta transversal corrigida para 3 niveis de profundidade (scripts/utils/tests)
...
$FallbackPrismaSchema = Join-Path $ProjectRoot 'prisma\schema.prisma'
$NextConfig = Join-Path $FrontendDir 'next.config.ts'

if (-not (Test-Path $PrismaSchema) -and (Test-Path $FallbackPrismaSchema)) {
    $PrismaSchema = $FallbackPrismaSchema
}
...
#Test Prisma
if (Test-Path $PrismaSchema -and (Select-String -Path $PrismaSchema -Pattern 'provider\s*=\s*"(sqlite|postgresql)"' -Quiet)) { Write-Host '  [OK] DB Provider SOTA (SQLite/PostgreSQL) detectado.' -ForegroundColor Green }
if ((Test-Path -LiteralPath $PrismaSchema) -and (Select-String -Path $PrismaSchema -Pattern 'provider\s*=\s*"(sqlite|postgresql)"' -Quiet)) { Write-Host '  [OK] DB Provider SOTA (SQLite/PostgreSQL) detectado.' -ForegroundColor Green }
else { Write-Host '  [WARN] Provider de Banco de Dados ausente ou divergente.' -ForegroundColor DarkYellow }
