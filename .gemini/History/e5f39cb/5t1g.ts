...
import type { HeroPosition } from './types';
...
export function calculateBaseFgsErosion(quantumPerspectiva: Record<string, any> | null, blindsRisingSoon: boolean, anteSize: number, heroPosition: HeroPosition): number {
...
export function performTourScrollAndHighlight(step: TourStep, setTourSpotlight: (rect: DOMRect | null) => void): ReturnType<typeof setTimeout> {
    if (step.openDetails) {
...
export function classifyRp(rp: number): { label: string; color: string; badge: string; } {
...
export function getRpCellStyle(val: number, isDiag: boolean, isHighlighted: boolean) {
...
// SOTA v4.2: Helpers para RangeMatrix
export function getHandStatus(row: number, col: number, hand: string, activeRp: number, overrides: Record<string, string>) {
