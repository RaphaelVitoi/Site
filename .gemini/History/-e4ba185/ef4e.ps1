...
Write-Host " 🏆 NEXUS STATUS :: Vitórias da Semana" -ForegroundColor Yellow
...
if ($completedThisWeek.Count -eq 0) {
    Write-Host " Nenhuma sinapse foi concluída nos últimos 7 dias." -ForegroundColor DarkGray
}
else {
    Write-Host " [ TAREFAS CONCLUÍDAS: $($completedThisWeek.Count) ]`n" -ForegroundColor White
    
    # Ordenar das mais recentes para as mais antigas
    $completedThisWeek = $completedThisWeek | Sort-Object -Property @{Expression = { datetime { $_.completedAt } else { $_.timestamp }) }; Descending=$true }
...
foreach ($t in $completedThisWeek) {
    ...
    Write-Host " ✅ [$dateFormatted] $agent | $desc" -ForegroundColor Green
}
}
