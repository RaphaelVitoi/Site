...
export default async function DashboardPage ()
{
    ...
    const quizEvents = events.filter( ( e: any ) => e.category === 'quiz' || e.category === 'Fundamentos SOTA' || e.category === 'Bolha' || e.category === 'Risk Premium' || e.category === 'Pós-Flop' );
    ...
    const quizAccuracy = quizEvents.length > 0
        ? ( quizEvents.filter( ( e: any ) => e.isCorrect ).length / quizEvents.length ) * 100
        ? ( quizEvents.filter( ( e: TelemetryEvent ) => e.isCorrect ).length / quizEvents.length ) * 100
        : 0;

    const simEvents = events.filter( ( e: any ) => e.category === 'simulator' || e.evLoss > 0 );
    const simEvents = events.filter( ( e: TelemetryEvent ) => e.category === 'simulator' || e.evLoss > 0 );
    const avgEvLoss = simEvents.length > 0
        ? simEvents.reduce( ( acc: number, e: any ) => acc + e.evLoss, 0 ) / simEvents.length
        ? simEvents.reduce( ( acc: number, e: TelemetryEvent ) => acc + e.evLoss, 0 ) / simEvents.length
        : 0;

    const totalLatencyMs = events.reduce( ( acc: number, e: any ) => acc + ( e.latency || 0 ), 0 );
