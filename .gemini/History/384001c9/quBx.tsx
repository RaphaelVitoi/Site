...
export default async function DashboardPage ()
{
    ...
    const quizEvents = events.filter( ( e: Prisma.TelemetryEvent ) => e.category === 'quiz' || e.category === 'Fundamentos SOTA' || e.category === 'Bolha' || e.category === 'Risk Premium' || e.category === 'Pós-Flop' );
    const quizAccuracy = quizEvents.length > 0
        ? ( quizEvents.filter( ( e: Prisma.TelemetryEvent ) => e.isCorrect ).length / quizEvents.length ) * 100
        : 0;

    const simEvents = events.filter( ( e: Prisma.TelemetryEvent ) => e.category === 'simulator' || e.evLoss > 0 );
    const avgEvLoss = simEvents.length > 0
        ? simEvents.reduce( ( acc: number, e: Prisma.TelemetryEvent ) => acc + e.evLoss, 0 ) / simEvents.length
        : 0;

    const totalLatencyMs = events.reduce( ( acc: number, e: Prisma.TelemetryEvent ) => acc + ( e.latency || 0 ), 0 );
