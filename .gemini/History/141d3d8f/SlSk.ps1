...
.DESCRIPTION
    Lê e valida a sintaxe e a estrutura lógica (Schema) dos arquivos JSON 
    que compõem a base de conhecimento do ecossistema de agentes.
#>
...
Write-Host "Iniciando validação de sintaxe e schema dos arquivos JSON...`n" -ForegroundColor DarkGray

$AllPassed = $true

foreach ($file in $CortexFiles) {
    $FilePath = Join-Path $PSScriptRoot $file.Name
    ...
    try {
        ...
        if ($null -eq $jsonParsed) { throw "Arquivo vazio ou JSON inválido (null)." }
        
        # 2. Validação de Schema (Estrutura)
        if ($file.Type -eq "Dictionary") {
            foreach ($property in $jsonParsed.psobject.properties) {
                ...
                if (-not $key.StartsWith($file.KeyPrefix)) {
                    throw "Chave inválida '$key'. Deve obrigatoriamente começar com '$($file.KeyPrefix)'."
                }
...
if ($AllPassed) {
    Write-Host "`n[SUCCESS] Todos os arquivos do Cortex estão íntegros e corretos." -ForegroundColor Green
}
else {
    Write-Host "`n[ERROR] Corrupção ou erro de formatação detectado no Cortex. Corrija os arquivos e tente novamente." -ForegroundColor Red
}