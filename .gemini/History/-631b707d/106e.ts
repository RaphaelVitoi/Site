...
// @ts-expect-error SOTA: O bundler resolve o modulo FFI/WASM diretamente na esteira de build do Next.js
import initWasm, { QuantumCfrEngine } from '../panels/pkg/vitoi_equity_engine.js';
...
type WorkerPayload = MatrixJobPayload | DistortionJobPayload | NashProfilerJobPayload | CfrHeatmapJobPayload;
...
async function runGpuMatrix(
    ...
    const trueInsolvencyEv = (winRate * futurePot) - (loseRate * futureCost);
...
async function handleCfrHeatmap(payload: CfrHeatmapJobPayload) {
    ...
    const regretMatrix = quantumCfrEngine.compute_cfr_heatmap(payload.nodes, payload.iterations, kappa);

    ( globalThis as unknown as Worker ).postMessage( { type: 'CFR_HEATMAP', matrix: regretMatrix, id: payload.id }, [ regretMatrix.buffer ] );
}
