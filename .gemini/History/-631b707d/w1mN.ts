...
async function runGpuMatrix(
    ...
    const { heroRange = "AhKd", villainRange = "100%", board = "", rpFactor = 0, heroInvested = 0, currentPot = 0, activePlayers = 2, kappa = 1, betSizing = 0.5, humanNoiseFactor = 0.0 } = p;
    ...
    const mwNoiseMultiplier = Math.pow(opponents, 1.0 + humanNoiseFactor);
...
function runJitMatrix(p: MatrixJobPayload): number[] {
    const { rpFactor = 0, heroInvested = 0, currentPot = 0, betSizing = 0.5, activePlayers = 2, humanNoiseFactor = 0.0 } = p;
    let wins = 0;
...
function handleDistortionJob(payload: DistortionJobPayload, id: string | number) {
    const hnf = payload.humanNoiseFactor ?? 0.0;
    const nashResults = {
        flop: solveIcmDistortion( payload.ipRpFlop, payload.oopRpFlop, payload.freqFlop, payload.topologicAggression, hnf ),
        turn: solveIcmDistortion( payload.ipRpTurn, payload.oopRpTurn, payload.freqTurn, payload.topologicAggression, hnf ),
        river: solveIcmDistortion( payload.ipRpRiver, payload.oopRpRiver, payload.freqRiver, payload.topologicAggression, hnf ),
    };
