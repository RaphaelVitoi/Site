...
async function loadHandRanksLUT(device: GPUDevice): Promise<GPUBuffer | null> {
    ...
    try {
        ...
        console.warn("[SOTA WebGPU] Tabela HandRanks.dat indisponível. Fallback estocástico ativo.", err);
        return null;
    }
}
...
globalThis.onmessage = async ( e: MessageEvent ) => // NOSONAR
{
    ...
    try
    {
        ...
        } else
        {
            ...
            if (isGpuActive && gpuDevice && computePipeline) {
                ...
                const lutResource = handRanksBuffer ?? dummyLut;
