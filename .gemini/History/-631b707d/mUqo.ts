...
import initWasm, { QuantumCfrEngine, MultiwayRioProfiler, solve_icm_distortion_v2 } from '../panels/pkg/vitoi_equity_engine.js';
...
async function handleCfrHeatmap(payload: CfrHeatmapJobPayload) {
    ...
    ( globalThis as unknown as Worker ).postMessage( { type: 'CFR_HEATMAP', matrix: safeMatrix, path, id: payload.id }, [ safeMatrix.buffer ] );
}
...
async function handleMultiwayRio(payload: MultiwayRioJobPayload) {
    ...
    ( globalThis as unknown as Worker ).postMessage( { type: 'MULTIWAY_RIO_RESULT', matrix: safeRioMatrix, id: payload.id }, [ safeRioMatrix.buffer ] );
}
...
function handleDistortionJob(payload: DistortionJobPayload, id: string | number) {
    ...
    const solveStreet = (ipRp: number, oopRp: number, freqs: ChipEvFreqs, pot: number, sIdx: number) => {
        try {
            // Tentativa via WASM v2 (O(1) Rust)
            if (solve_icm_distortion_v2) {
                const res = solve_icm_distortion_v2(
                    ipRp, oopRp, agg, ap, pot, sIdx,
                    freqs.oop_fold || 0, freqs.oop_call || 0, freqs.oop_raise || 0
                );
                ...
                const formatMetric = (val: number, oldVal: number) => ({ center: val * 100, spread: Math.max(3, Math.abs(ipRp-oopRp)*0.25), delta: (val*100) - oldVal });
            ...
            console.warn("[SOTA Worker] Fallback para solver TS devido a erro WASM v2:", e);
        }
        return solveIcmDistortion(ipRp, oopRp, freqs, agg);
    };

    const nashResults = {
        flop: solveStreet( payload.ipRpFlop, payload.oopRpFlop, payload.freqFlop, pots[0], 0 ),
        turn: solveStreet( payload.ipRpTurn, payload.oopRpTurn, payload.freqTurn, pots[1], 1 ),
        river: solveStreet( payload.ipRpRiver, payload.oopRpRiver, payload.freqRiver, pots[2], 2 ),
    };
