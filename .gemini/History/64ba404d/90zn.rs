...
fn precompute_combos(mask: &[u8], dead_cards: u64) -> Vec<(u8, u8)> {
    ...
    for c1 in 1..52 {
        if (dead_cards & (1u64 << c1)) != 0 { continue; }
        for c2 in 0..c1 {
            if (dead_cards & (1u64 << c2)) != 0 { continue; }
    ...
    if combos.is_empty() { combos.push((1, 0)); }
    combos
}
...
#[wasm_bindgen]
pub fn solve_icm_distortion_zerocopy(payload: &js_sys::Float64Array) -> js_sys::Float64Array {
    let p = payload.to_vec();
    let ip_rp = p[0];
    let oop_rp = p[1];
    let _kappa = p[2];
    let topologic_aggression = p[3];
    let active_players = p[4] as u32;
    let fold = p[5];
    let raise = p[7];
