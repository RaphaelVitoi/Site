...
// --- 1. COMPONENTE DE ESTATÍSTICAS SOTA ---
class IcmStatsPanel extends HTMLElement {
    update(sol, agressionFactor, isMuted) {
        this.innerHTML = this._generateHTML(sol, agressionFactor, isMuted);
    }

    _generateHTML(sol, agressionFactor, isMuted) {
        const getStatus = (v) => {
            if (v > 0) return 'status-positive';
            if (v < 0) return 'status-negative';
            return 'status-neutral';
        };

        const chartHtml = this._renderChart(sol);

        const muteStateClass = isMuted
            ? 'bg-rose-500/10 border-rose-500/30 text-rose-400 hover:bg-rose-500/20'
            : 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700 hover:text-white';
        const muteIcon = isMuted ? 'fa-volume-xmark' : 'fa-volume-high';
        const muteText = isMuted ? 'Mutado' : 'Som Ativo';
        ...
        return `
            ...
            <div class="flex items-center justify-between mb-6 bg-slate-900/30 p-3 rounded-lg border border-white/5">
                <div class="flex items-center gap-3">
                    ...
                    <input type="range" id="agression-slider" min="0.5" max="1.5" step="0.1" value="${agressionFactor}" class="w-24 h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer">
                    <span id="agression-value" class="text-xs font-mono font-bold text-indigo-400">${agressionFactor.toFixed(1)}x</span>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div class="ev-diff-panel col-span-1 md:col-span-2 bg-slate-900/50 p-4 rounded-xl border border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div class="w-full md:w-1/2 flex flex-col gap-1">
                        ...
                        <div class="w-full bg-slate-800 h-2 rounded-full overflow-hidden flex"><div class="bg-slate-600 h-full" style="width: 33%"></div><div class="bg-pink-500 h-full shadow-[0_0_10px_rgba(236,72,153,0.5)]" style="width: ${Math.min(67, Number.parseFloat(sol.evDiff.value))}%"></div></div>
                    </div>
                </div>
    ...
    _renderChart(sol) {
        const bBase = 33.3;
        const dBase = 50;
        const bIcm = Number.parseFloat(sol.bluff.value);
        const dIcm = Number.parseFloat(sol.defense.value);
        ...
        return `
            ...
            <div class="flex gap-4 mt-2 text-[9px] uppercase font-bold text-slate-500">
                <div class="flex items-center gap-1"><div class="w-2 h-2 bg-slate-700 rounded-sm"></div> Base GTO</div>
                <div class="flex items-center gap-1"><div class="w-2 h-2 bg-indigo-500 rounded-sm"></div> ICM Ajustado</div>
            </div>`;
    }
}
customElements.define('icm-stats-panel', IcmStatsPanel);

// --- 2. COMPONENTE MOTOR SOTA ---
class IcmGameEngine extends HTMLElement {
    constructor() {
        super();
        this.ui = {
        };
        ...
        this.agressionFactor = 1;
        this.charts = {}; // Armazena instâncias do ApexCharts
        this.isMuted = true;

        // Cache de referências imutáveis para erradicação de Dangling Listeners
        this._handleSelectBound = (e) => this.loadScenario(e.detail?.id || e.detail);
        this._toggleCompareModeBound = () => this._toggleCompareMode();
        this._handleInputBound = (e) => this._handleInput(e);
        this._handleChangeBound = (e) => {
            if (e.target.classList.contains('scenario-selector')) this.updateComparisonChart();
        };
        this._handleClickBound = (e) => this._handleClick(e);
    }

    connectedCallback() {
        // DOM Scoping: Erradicação total do getElementById global
        this.ui = {
            list: this.querySelector('#main-scenario-list'),
            gaugeIP: this.querySelector('#gauge-ip'),
            gaugeOOP: this.querySelector('#gauge-oop'),
            title: this.querySelector('#stage-title'),
            contextBadge: this.querySelector('#stage-context'),
            context: this.querySelector('#stage-context'),
            singleView: this.querySelector('#single-scenario-view'),
            content: this.querySelector('#content-area'),
            compareArea: this.querySelector('#compare-area'),
            btnCompare: this.querySelector('#btn-mode-compare')
        };
        this._bindEvents();
        this._initialLoad();
    }
    ...
    _bindEvents() {
        if (this.ui.list) {
            ...
            this.ui.list.addEventListener('select', this._handleSelectBound);
            this.ui.list.addEventListener('scenario-select', this._handleSelectBound);
        }

        if (this.ui.btnCompare) {
            this.ui.btnCompare.addEventListener('click', this._toggleCompareModeBound);
        }

        this.addEventListener('input', this._handleInputBound);

        if (this.ui.compareArea) {
            this.addEventListener('change', this._handleChangeBound);
        }

        this.addEventListener('click', this._handleClickBound);
    }

    disconnectedCallback() {
        // Desacoplamento cirúrgico de Listeners (Prevenção de Memory Leaks)
        if (this.ui.list) {
            this.ui.list.removeEventListener('select', this._handleSelectBound);
            this.ui.list.removeEventListener('scenario-select', this._handleSelectBound);
        }
        if (this.ui.btnCompare) {
            this.ui.btnCompare.removeEventListener('click', this._toggleCompareModeBound);
        }
        this.removeEventListener('input', this._handleInputBound);
        if (this.ui.compareArea) {
            this.removeEventListener('change', this._handleChangeBound);
        }
        this.removeEventListener('click', this._handleClickBound);

        // Desacoplamento da Engine de Charts (Libera Eventos Globais de Window Resize)
        if (this.charts.main) {
    ...
    _toggleCompareMode() {
        ...
        if (isHidden) {
            if (globalThis.Analytics) globalThis.Analytics.trackEvent('simulator_compare_mode', { action: 'enter' });
            ...
            if (this.ui.title) this.ui.title.innerText = "Comparação Tática";
            if (this.ui.contextBadge) this.ui.contextBadge.innerText = "Modo Análise";
    ...
    _handleInput(e) {
        ...
        if (e.target.id === 'sim-hand-select') {
            ...
            if (equity) {
                const input = this.querySelector('#sim-equity-input');
                if (input) input.value = equity;
                this.runSimulation(equity);
            }
        }
        if (e.target.id === 'agression-slider') {
            this.agressionFactor = Number.parseFloat(e.target.value);
            const el = this.querySelector('#agression-value');
            if (el) el.innerText = this.agressionFactor.toFixed(1) + 'x';
            this.recalcScenario();
            if (globalThis.Analytics) globalThis.Analytics.trackEvent('simulator_agression_change', { factor: this.agressionFactor });
        }
    }
    ...
    _handleClick(e) {
        if (e.target.closest('#btn-export-pdf')) {
            this.exportPDF();
            if (globalThis.Analytics) globalThis.Analytics.trackEvent('simulator_export_pdf', { scenario: this.currentScenarioData?.id });
        }
        ...
        if (btn) {
            ...
            if (this.currentScenarioData) {
                ...
                this.updateGauge(this.ui.gaugeIP, d.ip, d.oop.rp, this.currentSolution?.dynamicDeathZone);
                this.updateGauge(this.ui.gaugeOOP, d.oop, d.ip.rp, this.currentSolution?.dynamicDeathZone);
            }
        }
    }
    ...
    _initialLoad() {
        try {
            if (ALL_SCENARIOS.length > 0) this.loadScenario(ALL_SCENARIOS[0].id);
        } catch (err) {
            this._handleScenarioError(err);
        }
    }

    _handleScenarioError(err) {
        if (this.ui.title) this.ui.title.innerText = "Erro no Motor";
        if (this.ui.content) this.ui.content.innerHTML = `<div class="bg-rose-500/10 border border-rose-500/30 p-4 rounded text-rose-400 font-mono text-xs">${err.message || err.stack}</div>`;
    }
    ...
    loadScenario(id) {
        try {
            ...
            this._renderScenarioState();
        } catch (err) {
            this._handleScenarioError(err);
        }
    }

    _applyScenarioData(data) {
        if (globalThis.Analytics) globalThis.Analytics.trackEvent('simulator_load_scenario', { id: data.id, label: data.label });
    ...
    _renderScenarioState() {
        ...
        const ipStackNum = Number.parseFloat(data.data?.ip?.stack) || 25;
        const oopStackNum = Number.parseFloat(data.data?.oop?.stack) || 25;
        const effStack = Math.min(ipStackNum, oopStackNum);

        this.currentSolution = this.solver.solve(data.data.ip.rp, data.data.oop.rp, this.agressionFactor, effStack);

        if (data.data?.ip && data.data?.oop) {
            this.updateGauge(this.ui.gaugeIP, data.data.ip, data.data.oop.rp, this.currentSolution.dynamicDeathZone);
            this.updateGauge(this.ui.gaugeOOP, data.data.oop, data.data.ip.rp, this.currentSolution.dynamicDeathZone);
        }

        if (this.ui.content) {
            this.ui.content.innerHTML = `
                <icm-stats-panel id="stats-panel-container"></icm-stats-panel>
                <div id="main-chart" class="mt-4 min-h-37.5 w-full"></div>
                <div class="mt-8 border-t border-slate-800 pt-6 text-slate-300 leading-relaxed space-y-4">
                    ${contentHtml}
                </div>
            `;
            const panel = this.ui.content.querySelector('icm-stats-panel');
            if (panel) panel.update(this.currentSolution, this.agressionFactor, this.isMuted);
        }
        setTimeout(() => this._renderMainChart(this.currentSolution), 10);
    }
    ...
    recalcScenario() {
        ...
        const panel = this.querySelector('icm-stats-panel');
        if (panel) panel.update(this.currentSolution, this.agressionFactor, this.isMuted);
    ...
    updateGauge(el, d, opponentRp, dynamicDeathZone) {
        if (!el) return;
        ...
        if (dynamicDeathZone) el.setAttribute('dynamic-death-zone', dynamicDeathZone);
    ...
    runSimulation(val) {
        const equity = Number.parseFloat(val);
        const output = this.querySelector('#sim-result');

        if (Number.isNaN(equity) || !this.currentSolution) {
            output.innerHTML = '<span class="text-slate-500 text-xs">Aguardando input...</span>';
            return;
        }

        const req = Number.parseFloat(this.currentSolution.evDiff.totalRequired);
    ...
    _renderMainChart(sol) {
        ...
        // Mutação O(1) para instâncias vivas (Fricção Zero)
        if (this.charts.main) {
            this.charts.main.updateSeries([{
        ...
        const el = this.querySelector("#main-chart");
        if (el) {
    ...
    renderComparisonInterface() {
        ...
        this.ui.compareArea.innerHTML = `
            ...
            <div id="comparison-chart" class="min-h-75"></div>
        ...
        const sel2 = this.querySelector('#sel-sc2');
    ...
    updateComparisonChart() {
        const id1 = this.querySelector('#sel-sc1').value;
        const id2 = this.querySelector('#sel-sc2').value;
        ...
        if (this.charts.compare) {
            this.charts.compare.updateSeries(comparison.series);
            const verdictEl = this.querySelector('#comparison-verdict');
        ...
        this.charts.compare = new ApexCharts(this.querySelector("#comparison-chart"), options);
    ...
    _compareScenarios(s1, s2) {
        const ipStackNum1 = Number.parseFloat(s1.data?.ip?.stack) || 25;
        const oopStackNum1 = Number.parseFloat(s1.data?.oop?.stack) || 25;
        const effStack1 = Math.min(ipStackNum1, oopStackNum1);

        const ipStackNum2 = Number.parseFloat(s2.data?.ip?.stack) || 25;
        const oopStackNum2 = Number.parseFloat(s2.data?.oop?.stack) || 25;
        const effStack2 = Math.min(ipStackNum2, oopStackNum2);

        const sol1 = this.solver.solve(s1.data.ip.rp, s1.data.oop.rp, this.agressionFactor, effStack1);
        const sol2 = this.solver.solve(s2.data.ip.rp, s2.data.oop.rp, this.agressionFactor, effStack2);

        return {
            series: [
                { name: s1.label, data: [s1.data.ip.rp, s1.data.oop.rp, Number.parseFloat(sol1.bluff.value), Number.parseFloat(sol1.defense.value), Number.parseFloat(sol1.evDiff.value)] },
                { name: s2.label, data: [s2.data.ip.rp, s2.data.oop.rp, Number.parseFloat(sol2.bluff.value), Number.parseFloat(sol2.defense.value), Number.parseFloat(sol2.evDiff.value)] }
            ],
    ...
    _generatePDF() {
        ...
        html2pdf().set({
            margin: 10,
            filename: `Analise_ICM_${data.id}_${Date.now()}.pdf`,
...
customElements.define('icm-game-engine', IcmGameEngine);

globalThis.gameEngineInstance = null; // A engine agora é orgânica via DOM Element.
