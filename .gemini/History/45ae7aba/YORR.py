...
def call_gemma_local(
    ...
    _ = model  # NOSONAR - Preserva a assinatura SOTA da Factory
...
async def _try_provider(
    ...
    for i, key in enumerate(keys):
        ...
        for attempt in range(max_retries):
            ...
            logger.warning(
                f"[{task.agent}] Falha em {provider_name} com {model} (Chave {i + 1}): {error_msg}"
            )
            ...
            if action == "abort_key":
                break
            if action == "retry":
                # SOTA Rate Limiter: Backoff Exponencial na mitigacao do 429
...
async def _dispatch_provider_call(
    ...
    if "gemma" in model_l and ("google/" in model_l or model_l.startswith("gemma")):
        ...
        try:
            res, _ = call_gemma_local(model, system_prompt, user_prompt)
            return res
        except Exception as e:
            logger.warning(
