...
def _evaluate_api_error(
    ...
    if any(err in error_msg for err in ["500", "502", "503", "504", "timeout"]):
        ...
        if fail_count >= MAX_PROVIDER_FAILURES:
            ...
            logger.critical(
                f"[CIRCUIT BREAKER] Provedor '{provider_name}' caiu ({fail_count} falhas). Bloqueando por 5 min."
                "[CIRCUIT BREAKER] Provedor '%s' caiu (%d falhas). Bloqueando por 5 min.",
                provider_name, fail_count
            )
...
async def _execute_provider_attempt(
    ...
    try:
        logger.info(
            f"[{task.agent}] Acionando {model} via {provider_name} (Chave {key_index}, Tentativa {attempt})..."
            "[%s] Acionando %s via %s (Chave %d, Tentativa %d)...",
            task.agent, model, provider_name, key_index, attempt
        )
        ...
        logger.info(
            f"[{task.agent}] [SOTA TELEMETRY] {model} via {provider_name} consolidou resposta em {latency:.2f}s"
            "[%s] [SOTA TELEMETRY] %s via %s consolidou resposta em %.2fs",
            task.agent, model, provider_name, latency
        )
...
async def _try_single_key(
    ...
    """Processa a tentativa de uma unica chave, aplicando retry e backoff SOTA. Retorna (response_text, action)."""
    for attempt in range(max_retries):
        ...
        logger.warning(
            f"[{task.agent}] Falha em {provider_name} com {model} (Chave {key_index}): {error_msg}"
            "[%s] Falha em %s com %s (Chave %d): %s",
            task.agent, provider_name, model, key_index, error_msg
        )
        ...
        if action == "retry":
            ...
            logger.warning(
                f"[{task.agent}] [RATE LIMITER SOTA] Throttle/Timeout detectado. Backoff acionado: aguardando {backoff_time}s antes do retry..."
                "[%s] [RATE LIMITER SOTA] Throttle/Timeout detectado. Backoff acionado: aguardando %ds antes do retry...",
                task.agent, backoff_time
            )
...
async def _try_provider(
    ...
    if PROVIDER_BLOCK_UNTIL.get(provider_name, 0) > current_time:
        logger.warning(
            f"[CIRCUIT BREAKER] Provedor '{provider_name}' temporariamente em quarentena. Acionando Fallback..."
...
async def _dispatch_provider_call(
    ...
    if "gemma" in model_l and ("google/" in model_l or model_l.startswith("gemma")):
        ...
        try:
            ...
            logger.warning(
                f"Falha no Motor Gemma Local: {e}. Tentando roteamento externo..."
    ...
    logger.warning(f"Modelo desconhecido '{model}' no pipeline, pulando.")
...
async def call_llm_api(
    ...
    logger.warning(f"[{task.agent}] Modo Simulacao Ativado.")
