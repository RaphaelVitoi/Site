...
def _get_dir_size_safe(path: Path) -> int:
    """Calcula o tamanho de um diretorio de forma segura e performatica usando os.scandir."""
    total_size = 0
    try:
        for entry in os.scandir(path):
            if entry.is_dir(follow_symlinks=False):
                total_size += _get_dir_size_safe(Path(entry.path))
            elif entry.is_file(follow_symlinks=False):
                try:
                    total_size += entry.stat().st_size
                except OSError:
                    pass  # Ignora arquivos inacessiveis
    except OSError:
        pass  # Ignora diretorios inacessiveis
    return total_size
...
def run_sanitizer(dry_run=True):
    ...
    targets = [
        ...
        BASE_DIR / "frontend" / ".next-build", # SOTA: Adicionado alvo de entropia de build dev
    ]
    ...
    for path in targets:
        if path.exists():
            try:
                size = _get_dir_size_safe(path) if path.is_dir() else path.stat().st_size
...
def audit_api_keys():
    ...
    def test_google_api(key):
        """Valida uma chave Google/Gemini fazendo uma chamada leve."""
    ...
    def test_openrouter_api(key):
        """Valida uma chave OpenRouter."""
        url = "https://openrouter.ai/api/v1/auth/key"
        req = urllib.request.Request(url, headers={"Authorization": f"Bearer {key}"})
        try:
            with urllib.request.urlopen(req, timeout=10) as response:
                # OpenRouter retorna 200 para chaves validas, 401 para invalidas.
                return response.status == 200
        except (urllib.error.HTTPError, urllib.error.URLError):
            return False
...
def run_dashboard():
    ...
    while True:
        ...
        if choice == "q":
            ...
            os.system('cls' if os.name == 'nt' else 'clear')
            # Refatoracao SOTA: Invocar o comando diretamente via dispatcher map, evitando recursao em main().
            if subcommand in COMMAND_MAP:
                COMMAND_MAP[subcommand](*args)
            elif subcommand in COMMAND_MAP_WITH_ARGS:
                COMMAND_MAP_WITH_ARGS[subcommand](*args)
...
COMMAND_MAP_WITH_ARGS = {
    "autonomy": set_autonomy,
    "refactor": run_refactor_engine,
    "visualize-map": lambda: _run_powershell_script(ROUTINES_DIR / "invoke_routing_map_visualization.ps1"),
}
...
def main(args=None):
    ...
    # Roteador de Comandos SOTA (Command Dispatcher Pattern)
    if command in COMMAND_MAP:
        COMMAND_MAP[command]()
    elif command in COMMAND_MAP_WITH_ARGS:
        if not cli_args:
            console.print(f"[red]O comando '{command}' requer argumentos.[/]")
            return
        try:
            COMMAND_MAP_WITH_ARGS[command](*cli_args)
        except TypeError:
             console.print(f"[red]Argumentos invalidos para o comando '{command}'.[/]")
    else:
        console.print(f"[red]Comando desconhecido: '{command}'[/]")
        console.print("[yellow]Use 'nexus-cli dashboard' para ver as opcoes.[/]")
