...
export default function RiskGeometryMasterclass() {
    ...
    const glassPanelClass = "bg-slate-900/75 backdrop-blur-[24px] border border-white/5 rounded-[24px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.05)] relative overflow-hidden transition-all duration-300";

    return (
        <div className="min-h-screen text-slate-50 selection:bg-indigo-500/30 pb-20 pt-10" style={{
        < div className="min-h-screen text-slate-50 selection:bg-indigo-500/30 pb-20 pt-10 font-inter" style={{
                backgroundImage: `radial-gradient(circle at 0% 0%, rgba(99, 102, 241, 0.05), transparent 40%), radial-gradient(circle at 100% 100%, rgba(14, 165, 233, 0.05), transparent 40%), linear-gradient(rgba(255, 255, 255, 0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255, 255, 255, 0.02) 1px, transparent 1px)`,
                backgroundSize: '100% 100%, 100% 100%, 40px 40px, 40px 40px',
                backgroundColor: '#020617'
            }}>
            <style>{`
                @import url('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css');
                @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;700;800&family=Great+Vibes&display=swap');
                @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;700;800&family=Montserrat:wght@700;800;900&family=Great+Vibes&display=swap');
                ...
                .ticker-wrap { width: 100%; overflow: hidden; background: rgba(15, 23, 42, 0.8); border-bottom: 1px solid rgba(255,255,255,0.05); padding: 8px 0; }
                .ticker { display: inline-block; white-space: nowrap; animation: ticker 30s linear infinite; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.2em; color: #818cf8; }
                ...
                @keyframes pulseGlow {
                    0%, 100% { filter: drop-shadow(0 0 10px rgba(99, 102, 241, 0.4)); }
                ...
                .scenario-btn {
                    ...
                    cursor: pointer; color: #94a3b8; font-weight: 600; position: relative; overflow: hidden;
                }
                .scenario-btn:hover { background: rgba(30, 41, 59, 0.8); transform: translateX(6px); color: #f8fafc; }
                .scenario-btn.active {
                    background: linear-gradient(90deg, rgba(99, 102, 241, 0.2), rgba(99, 102, 241, 0.05));
                    border-left: 3px solid #818cf8; border-top: 1px solid rgba(99, 102, 241, 0.2); border-bottom: 1px solid rgba(99, 102, 241, 0.2);
                    color: #fff; box-shadow: inset 0 0 20px rgba(99, 102, 241, 0.05);
                    transform: translateX(6px);
                }
                .scenario-btn .icon-box {
                    width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center;
                    background: rgba(15, 23, 42, 0.6); font-size: 18px; border: 1px solid rgba(255,255,255,0.05);
                    transition: all 0.3s ease; z-index: 2; position: relative;
                }
                .scenario-btn.active .icon-box { background: #6366f1; color: white; border-color: #818cf8; box-shadow: 0 0 15px rgba(99, 102, 241, 0.5); }

                /* ACTION TABS */
                .action-tab {
                    flex: 1; padding: 18px 0; text-align: center; font-weight: 800; text-transform: uppercase;
                    letter-spacing: 0.15em; font-size: 11px; color: #64748b; border-bottom: 2px solid transparent;
                    cursor: pointer; transition: all 0.3s ease; background: rgba(15, 23, 42, 0.4);
                }
                .action-tab:hover { color: #cbd5e1; background: rgba(30, 41, 59, 0.4); }
                .action-tab.active { color: #fff; background: transparent; }
                .action-tab[data-target="theory"].active { border-color: #0ea5e9; color: #0ea5e9; }
                .action-tab[data-target="exploit"].active { border-color: #6366f1; color: #6366f1; }
                .action-tab[data-target="quiz"].active { border-color: #f59e0b; color: #f59e0b; }
                .action-tab[data-target="dilution"].active { border-color: #e11d48; color: #e11d48; }

                /* PIPELINE DILUTION */
                .pipeline { display: flex; align-items: center; justify-content: space-between; position: relative; margin: 40px 0; padding: 0 20px; }
                .pipeline::before { content: ''; position: absolute; top: 50%; left: 40px; right: 40px; height: 2px; background: rgba(255,255,255,0.1); z-index: 1; transform: translateY(-50%); }
                .pipeline-node { position: relative; z-index: 2; display: flex; flex-direction: column; align-items: center; justify-content: center; width: 60px; height: 60px; border-radius: 50%; background: #0f172a; border: 2px solid #334155; transition: all 0.5s ease; }
                .pipeline-node.active { border-color: #f43f5e; box-shadow: 0 0 20px rgba(225, 29, 72, 0.4); background: rgba(225, 29, 72, 0.1); }
                
                /* QUIZ */
                .quiz-option {
                    background: rgba(30, 41, 59, 0.3); border: 1px solid rgba(255,255,255,0.05);
                    padding: 18px 24px; border-radius: 14px; cursor: pointer; transition: all 0.2s;
                    display: flex; gap: 16px; align-items: flex-start;
                }
                .quiz-option:hover { background: rgba(30, 41, 59, 0.8); transform: translateY(-2px); box-shadow: 0 10px 20px rgba(0,0,0,0.2); }
                .quiz-option.correct { background: rgba(16, 185, 129, 0.15); border-color: rgba(16, 185, 129, 0.5); box-shadow: inset 0 0 20px rgba(16, 185, 129, 0.1); }
                .quiz-option.wrong { background: rgba(225, 29, 72, 0.15); border-color: rgba(225, 29, 72, 0.5); }
                .quiz-option.disabled { pointer-events: none; opacity: 0.4; filter: grayscale(1); transform: none; box-shadow: none;}

                .signature-text {
                    font-family: 'Great Vibes', cursive; font-size: 3rem; color: #a5b4fc; 
                    background: linear-gradient(120deg, #e0e7ff 0%, #93c5fd 30%, #6366f1 70%, #4338ca 100%);
                    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
                    transform: rotate(-3deg); display: inline-block; text-shadow: 0 4px 20px rgba(99,102,241,0.3);
                }
            `}</style>

            {/* Axiom Ticker */}
            <div className="w-full overflow-hidden bg-slate-900/80 border-b border-white/5 py-2 fixed top-0 z-50">
                <div className="animate-ticker text-[11px] font-bold uppercase tracking-widest text-indigo-400">
                    <div className="ticker-wrap fixed top-0 left-0 z-50">
                    ...
                    <div className="w-full max-w-[1400px] mx-auto flex flex-col gap-8 mt-16 px-4 md:px-8">
                        <header className="text-center pt-4 mb-4">
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 mb-4">
                                ...
                                <div className="w-2 h-2 rounded-full bg-indigo-400 pulse-glow"></div>
                            ...
                            <p className="text-slate-400 text-sm font-bold uppercase tracking-[0.3em]">Masterclass de Teoria dos Jogos</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                            ...
                            <aside className="lg:col-span-4 flex flex-col gap-6">
                                <div className={`${glassPanelClass} p-6 max-h-[850px] flex flex-col`}>
                                    <div className="glass-panel p-6 max-h-[850px] flex flex-col">
                                        ...
                                        <div className="space-y-3 overflow-y-auto pr-2 pb-4">
                                            {DATABASE.map(sc => {
                                                ...
                                                return (
                                                    <button key={sc.id} onClick={() => setCurrentId(sc.id)} className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-300 flex items-center gap-4 cursor-pointer relative overflow-hidden font-semibold ${isActive ? 'bg-gradient-to-r from-indigo-500/20 to-indigo-500/5 border-l-4 border-l-indigo-400 border-y border-white/5 text-white shadow-[inset_0_0_20px_rgba(99,102,241,0.05)] transform translate-x-1.5' : 'bg-slate-800/30 border border-white/5 text-slate-400 hover:bg-slate-800/80 hover:text-slate-100 hover:translate-x-1.5'}`}>
                                                        <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center text-lg border transition-all z-10 ${isActive ? 'bg-indigo-500 text-white border-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.5)]' : 'bg-slate-900/60 border-white/5'}`}>{sc.icon}</div>
                                                        <div className="flex flex-col pr-2 z-10 w-full">
                                                            <button key={sc.id} onClick={() => setCurrentId(sc.id)} className={`scenario-btn ${isActive ? 'active' : ''}`}>
                                                                <div className="icon-box shadow-md">{sc.icon}</div>
                                                                <div className="flex flex-col pr-2 z-10 w-full text-left">
                                                                    ...
                                                                    <span className="text-[13px] font-bold tracking-wide">{sc.title}</span>
                                                                    <span className="text-[13px] font-bold text-white tracking-wide">{sc.title}</span>
                                                                </div>
                                                                <i className="fa-solid fa-chevron-right text-slate-600 opacity-50 text-xs ml-auto z-10"></i>
                                        ...
                                        <main className="lg:col-span-8 flex flex-col gap-6">
                                            {/* THE STAGE */}
                                            <div className={`${glassPanelClass} p-8 md:p-10 relative`}>
                                                <div className="glass-panel p-8 md:p-10 relative">
                                                    ...
                                                    <div className="flex justify-between items-start border-b border-slate-800 pb-6 mb-8 relative z-10">
                                                        <div>
                                                            ...
                                                            <h2 className="text-3xl md:text-4xl font-black text-white mt-4 tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">{currentData.title}</h2>
                                                            <h2 className="text-3xl md:text-4xl font-black text-white mt-4 tracking-tight gradient-text">{currentData.title}</h2>
                                                        </div>
                                                        <div className="text-right hidden sm:flex flex-col items-end">
                                                            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2">Veredito do Solver</span>
                                                            <div className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest border transition-all duration-500 bg-slate-900 ${currentData.verdict.class}`}>
                                                        ...
                                                        <div className="flex items-center justify-between px-2 md:px-12 relative z-10">
                                                            ...
                                                            <div className="flex flex-col items-center flex-1">
                                                                <div className="text-center mb-4">
                                                                    ...
                                                                    <span className="data-mono text-sm text-slate-400 font-medium bg-slate-900/50 px-2 py-0.5 rounded border border-slate-800">{currentData.ip.stack}</span>
                                                                </div>
                                                                <div className="relative w-28 h-28 md:w-36 md:h-36">
                                                                    <svg viewBox="0 0 36 36" className="block mx-auto max-w-[140px] max-h-[140px]">
                                                                        ...
                                                                        <path fill="none" strokeWidth="2.5" strokeLinecap="round" className="transition-all duration-1000 ease-out" stroke="#0ea5e9"
                                                                            style={{ strokeDasharray: `${Math.min(100, (currentData.ip.rp / 26) * 100)}, 100`, filter: 'drop-shadow(0 0 8px rgba(14, 165, 233, 0.6))' }}
                                                                            d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                                                    </svg>
                                                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                                        <span className="data-mono text-2xl md:text-3xl font-black text-white"><AnimatedNumber value={currentData.ip.rp} /></span>
                                                            ...
                                                            <div className="flex flex-col items-center flex-1">
                                                                <div className="text-center mb-4">
                                                                    ...
                                                                    <span className="data-mono text-sm text-slate-400 font-medium bg-slate-900/50 px-2 py-0.5 rounded border border-slate-800">{currentData.oop.stack}</span>
                                                                </div>
                                                                <div className="relative w-28 h-28 md:w-36 md:h-36">
                                                                    <svg viewBox="0 0 36 36" className="block mx-auto max-w-[140px] max-h-[140px]">
                                                                        <path fill="none" stroke="rgba(30, 41, 59, 0.8)" strokeWidth="2.5" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                                                        <path fill="none" strokeWidth="2.5" strokeLinecap="round" className="transition-all duration-1000 ease-out" stroke="#e11d48"
                                                                            style={{ strokeDasharray: `${Math.min(100, (currentData.oop.rp / 26) * 100)}, 100`, filter: 'drop-shadow(0 0 8px rgba(225, 29, 72, 0.6))' }}
                                                                            d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                                                    </svg>
                                                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                                        <span className="data-mono text-2xl md:text-3xl font-black text-white"><AnimatedNumber value={currentData.oop.rp} /></span>
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                        <div className={`${glassPanelClass} p-6 border-t-4 border-t-sky-500`}>
                                                            <div className="glass-panel p-6 border-t-4 border-t-sky-500 hover:border-t-sky-400 transition-colors">
                                                                <div className="flex justify-between items-start mb-6">
                                                                    ...
                                                                    <span className={`data-mono text-xs font-bold px-2 py-1 rounded border ${(bluff - 33.3) > 0 ? 'text-sky-400 bg-sky-500/10 border-sky-500/20' : ((bluff - 33.3) < 0 ? 'text-rose-400 bg-rose-500/10 border-rose-500/20' : 'bg-slate-800 border-slate-700 text-slate-400')}`}>
                                                                        {bluff - 33.3 > 0 ? '+' : ''}{(bluff - 33.3).toFixed(1)}% vs cEV
                                                                    </span>
                                                                </div>
                                                                <div>
                                                                    <div className="flex items-baseline gap-2 mb-2">
                                                                        <span className="data-mono text-5xl font-black text-white tracking-tighter"><AnimatedNumber value={bluff} /></span>
                                                                    </div>
                                                                    <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden mt-4 shadow-inner">
                                                                        <div className="h-full bg-sky-500 transition-all duration-1000 ease-out" style={{ width: `${bluff}%` }}></div>
                                                                    </div>
                                                                </div>
                                                            </div>

                                                            <div className={`${glassPanelClass} p-6 border-t-4 border-t-rose-500`}>
                                                                <div className="glass-panel p-6 border-t-4 border-t-rose-500 hover:border-t-rose-400 transition-colors">
                                                                    <div className="flex justify-between items-start mb-6">
                                                                        <div>
                                                                        ...
                                                                        <span className={`data-mono text-xs font-bold px-2 py-1 rounded border ${(defense - 50.0) > 0 ? 'text-sky-400 bg-sky-500/10 border-sky-500/20' : ((defense - 50.0) < 0 ? 'text-rose-400 bg-rose-500/10 border-rose-500/20' : 'bg-slate-800 border-slate-700 text-slate-400')}`}>
                                                                            {defense - 50.0 > 0 ? '+' : ''}{(defense - 50.0).toFixed(1)}% vs cEV
                                                                        </span>
                                                                    </div>
                                                                    <div>
                                                                        <div className="flex items-baseline gap-2 mb-2">
                                                                            <span className="data-mono text-5xl font-black text-white tracking-tighter"><AnimatedNumber value={defense} /></span>
                                                                        </div>
                                                                        <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden mt-4 shadow-inner">
                                                                            <div className="h-full bg-rose-500 transition-all duration-1000 ease-out" style={{ width: `${defense}%` }}></div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            {/* INTERACTIVE PANELS */}
                                                            <div className={`${glassPanelClass} flex flex-col min-h-[400px]`}>
                                                                <div className="flex border-b border-slate-800 overflow-x-auto hide-scroll bg-slate-900/40">
                                                                    <div className="glass-panel flex flex-col min-h-[400px]">
                                                                        <div className="flex border-b border-slate-800 overflow-x-auto hide-scroll">
                                                                            {[
                                                                                { id: 'theory', icon: 'fa-book-open', label: 'Fundamento Teórico', color: 'border-sky-500 text-sky-400' },
                                                                                { id: 'dilution', icon: 'fa-water', label: 'Diluição SPR', color: 'border-rose-500 text-rose-400' },
                                                                                { id: 'exploit', icon: 'fa-crosshairs', label: 'Vetor de Exploit', color: 'border-indigo-500 text-indigo-400' },
                                                                                { id: 'quiz', icon: 'fa-brain', label: 'Prova Analítica', color: 'border-amber-500 text-amber-400' }
                                    ...
                                    <div key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 px-4 py-5 text-center font-extrabold uppercase tracking-[0.15em] text-[11px] cursor-pointer transition-all duration-300 border-b-2 whitespace-nowrap ${activeTab === tab.id ? `bg-transparent ${tab.color}` : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-800/40'}`}>
                                    <div key={tab.id} onClick={() => setActiveTab(tab.id)} data-target={tab.target} className={`action-tab ${activeTab === tab.id ? 'active' : ''}`}>
                                        <i className={`fa-solid ${tab.icon} mr-2 opacity-70`}></i> {tab.label}
                                    </div>
                                ))}
                            </div>

                            <div className="p-8 animate-fade-up" key={activeTab + currentId}>
                                ...
                                {activeTab === 'dilution' && (
                                    <div>
                                        ...
                                        <div className="bg-slate-900/60 border border-slate-800 p-8 rounded-2xl relative">
                                            ...
                                            <div className="flex items-center justify-between relative my-10 px-2 sm:px-10">
                                                <div className="absolute top-1/2 left-10 right-10 h-[2px] bg-white/10 -translate-y-1/2 z-0 hidden sm:block"></div>
                                            <div className="pipeline mt-4">
                                                {[
                                                    ...
                                                    <div key={node.label} className={`relative z-10 flex flex-col items-center justify-center w-16 h-16 rounded-full border-2 transition-all duration-500 ${node.active ? 'bg-rose-500/10 border-rose-500 shadow-[0_0_20px_rgba(225,29,72,0.4)]' : 'bg-slate-900 border-slate-700'}`}>
                                                        <span className={`text-[10px] font-bold ${node.active ? 'text-slate-300' : 'text-slate-500'} mb-0.5`}>{node.label}</span>
                                                    <div key={node.label} className={`pipeline-node ${node.active ? 'active' : ''}`}>
                                                        <span className={`text-xs font-bold ${node.active ? 'text-slate-300' : 'text-slate-500'} mb-1`}>{node.label}</span>
                                                        <span className={`data-mono text-xs font-black text-rose-400 ${node.o}`}><AnimatedNumber value={node.val} /></span>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                {activeTab === 'quiz' && (
                                    <div>
                                            ...
                                            {quizOptions.map((opt, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} onClick={() => { if (!quizAnswered) { setSelectedOpt(idx); setQuizAnswered(true); } }}
                                                        className={`p-4 md:px-6 md:py-4 rounded-xl cursor-pointer transition-all duration-300 flex gap-4 items-start border ${showSuccess ? 'bg-emerald-500/15 border-emerald-500/50 shadow-[inset_0_0_20px_rgba(16,185,129,0.1)]' : showFail ? 'bg-rose-500/15 border-rose-500/50' : isDisabled ? 'opacity-40 grayscale pointer-events-none bg-slate-800/30 border-white/5' : 'bg-slate-800/30 border-white/5 hover:bg-slate-800/80 hover:-translate-y-0.5 hover:shadow-lg'}`}
                                                        className={`quiz-option ${stateClass}`}
                                                    >
                                                        <div className={`w-6 h-6 shrink-0 rounded border flex items-center justify-center mt-0.5 text-xs shadow-inner ${showSuccess ? 'border-emerald-500 bg-emerald-500/20 text-emerald-400' : showFail ? 'border-rose-500 bg-rose-500/20 text-rose-400' : 'border-slate-600 bg-slate-900 text-transparent'}`}>
                                                            {showSuccess && <i className="fa-solid fa-check"></i>}{showFail && <i className="fa-solid fa-xmark"></i>}
                                                        <div className="w-6 h-6 shrink-0 rounded border border-slate-600 flex items-center justify-center mt-0.5 icon-box text-xs bg-slate-900 shadow-inner">
                                        ...
                                        {quizAnswered && (
                                            <div className="mt-8 bg-indigo-950/40 border border-indigo-500/30 p-6 rounded-xl relative overflow-hidden animate-fade-up">
                ...
                <footer className="text-center pb-12 pt-8 flex flex-col items-center">
                    ...
                    <div className="font-[Great_Vibes] text-4xl text-transparent bg-clip-text bg-gradient-to-r from-indigo-200 to-indigo-600 -rotate-3 drop-shadow-lg">Raphael Vitoi</div>
                    <div className="signature-text">Raphael Vitoi</div>
