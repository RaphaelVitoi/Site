...
export default function RiskGeometryMasterclass() {
    ...
    const [aiChatHistory, setAiChatHistory] = useState<{ speaker: string, text: string, type: 'user' | 'ai' | 'system' }[]>([
    ...
    const handleAskAI = async () => {
        ...
        try {
            const systemInstruction = `Você é Raphael Vitoi, especialista em ICM e Teoria dos Jogos. Contexto: O cenário atual é "${currentData.title}". Agressor tem RP de ${currentData.ip.rp}% e Defensor tem RP de ${currentData.oop.rp}%. Responda de forma analítica e incisiva em português de Portugal.`;
            ...
            setAiChatHistory([...newHistory, { speaker: 'Oráculo AI ✨', text: data.text, type: 'ai' }]);
        } catch (err: any) {
                                        ...
                                        <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2 border border-slate-800/50 bg-slate-900/30 rounded-xl p-4 shadow-inner max-h-[350px] custom-scrollbar">
                                            {aiChatHistory.map((msg, i) => (
                                                <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                                                    <div className={`p-4 max-w-[85%] rounded-2xl shadow-md ${msg.type === 'user' ? 'bg-indigo-600/30 border border-indigo-500/40 rounded-tr-none' : msg.type === 'ai' ? 'bg-slate-800 border border-slate-700 rounded-tl-none' : 'bg-rose-900/30 border border-rose-500/40 rounded-tl-none'}`}>
                                                        <p className="text-[13.5px] text-slate-200 leading-relaxed">
                                                            {msg.type !== 'user' && <strong className={`block mb-1 text-xs ${msg.type === 'ai' ? 'text-fuchsia-400' : 'text-rose-400'}`}>{msg.speaker}</strong>}
                                                            {msg.text}
                                                        </p>
                                                    </div>
                                                </div>
