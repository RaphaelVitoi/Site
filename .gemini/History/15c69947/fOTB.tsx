    ...
    <div className="min-h-screen bg-bg-base text-text-bright overflow-x-hidden font-body">
      ...
      <section id="hero" className="relative pt-40 pb-24 px-6 overflow-hidden">
        <div className="sota-container relative z-10 flex flex-col items-center text-center max-w-4xl mx-auto animate-sota-in">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-indigo/10 border border-accent-indigo/20 text-[0.6rem] font-black text-accent-indigo-light uppercase tracking-[0.2em] mb-8 shadow-[0_0_15px_rgba(99,102,241,0.2)]">
            <span className="w-1.5 h-1.5 rounded-full bg-accent-indigo animate-pulse" />
            { ' ' }SOTA Intelligence Active
          </div>

          <h1 className="text-[clamp(2.5rem,8vw,5rem)] font-black mb-8 tracking-tighter leading-[0.9] bg-linear-to-b from-text-bright via-text-muted to-text-darker bg-clip-text text-transparent font-heading drop-shadow-2xl">
            O Edge Mudou <br /> de Lugar
          </h1>

          <p className="text-xl md:text-2xl text-text-muted max-w-2xl leading-relaxed mb-12 font-medium">
            Descubra como a elite do poker usa a <strong className="text-text-bright drop-shadow-md">Perspectiva Matemática</strong> para dominar o pós-flop em um field saturado de solvers.
          </p>

          <div className="flex flex-wrap justify-center gap-4">
            <Link href="#metodo" className="btn-primary px-12 py-5 text-lg font-black tracking-tighter rounded-2xl shadow-xl shadow-indigo-900/40 hover:shadow-indigo-500/40 transition-all">
              EXPLORAR O NEXUS <i className="fa-solid fa-arrow-right ml-3" />
            </Link>
            <Link href="/artigos/estado-da-arte" className="btn-secondary px-12 py-5 text-lg font-black tracking-tighter rounded-2xl border-white/10 hover:bg-white/5 transition-all backdrop-blur-md">
              WHITEPAPER
            </Link>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-16 items-center">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                { [
                  ...
                  <div key={ item.title } className="p-6 rounded-3xl bg-white/2 border border-white/5 hover:border-accent-indigo/30 transition-all group">
      ...
      <div className="bg-bg-deep border-y border-white/5 py-24 mb-24">
        ...
        <div className="sota-container">
          <GlassPanel className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-12 p-8 sm:p-12 items-center">
            <div className="relative aspect-square w-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-bg-deep flex items-center justify-center group">
              <i className="fa-solid fa-network-wired text-5xl text-accent-indigo/20 absolute animate-pulse z-0" />
              <video autoPlay muted playsInline loop preload="metadata" poster="/fallback-nexus.jpg" className="absolute inset-0 w-full h-full object-cover grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-1000 z-10">
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-wrap gap-3">
                { ['Embaixador Deepsolver', 'Afiliado GTO Wizard', 'Criador trueICM'].map( ( tag ) => (
                  <span key={ tag } className="px-4 py-2 rounded-xl bg-white/3 border border-white/10 text-[0.65rem] font-black text-text-muted uppercase tracking-widest">{ tag }</span>
                ) ) }
              </div>
            </div>
          </GlassPanel>
      ...
      {/* O DIÁRIO DO ARQUITETO (BLOG) */ }
      <div className="sota-container pb-32 mt-12">
        <div className="bg-linear-to-br from-bg-elevated/60 to-bg-base p-12 rounded-3xl border border-accent-indigo/20 text-center relative overflow-hidden group">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(99,102,241,0.1),transparent_50%)] pointer-events-none group-hover:scale-110 transition-transform duration-1000" />
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-indigo/10 border border-accent-indigo/20 text-[0.6rem] font-black text-accent-indigo-light uppercase tracking-[0.2em] mb-6 shadow-[0_0_12px_rgba(99,102,241,0.2)]">
            <span className="w-1.5 h-1.5 rounded-full bg-accent-indigo animate-pulse" />
            { ' ' }Live Feed
          </div>
          <h3 className="text-3xl sm:text-4xl font-black text-text-bright mb-4 font-heading tracking-tighter">O Diário do Arquiteto</h3>
          <p className="text-text-muted mb-10 max-w-2xl mx-auto leading-relaxed">
            Acompanhe os registros imutáveis de Engenharia de Software, Teoria dos Jogos e os bastidores do desenvolvimento do Nexus em tempo real.
          </p>
          <Link href="/blog" className="btn-primary pulse-glow px-12 py-5 text-lg font-black tracking-widest rounded-2xl">
            ACESSAR O CÓRTEX <i className="fa-solid fa-brain ml-3" />
          </Link>
        </div>
      </div>
      ...
      <div className="sota-container pb-32">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Link href="/artigos/estado-da-arte" className="hub-card border-accent-indigo/20">
