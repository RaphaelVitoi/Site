...
export default function PerformanceChart({ data }: { data: DailyStat[] }) {
    ...
    if (!data || data.length === 0) return <div className="text-zinc-600 italic">Aguardando dados de autopoiese...</div>;
        ...
        <div className="bg-black/40 border border-green-900/30 p-6 rounded-xl">
                ...
                {data.map((day, i) => {
                    ...
                    return (
                        <div key={day.day} className="flex-1 flex flex-col items-center group relative">
                            ...
                            {/* Bar */}
                            <div
                                className={`w-full bg-gradient-to-t from-green-900 to-green-400 rounded-t-sm transition-all duration-1000 ease-out transform origin-bottom ${mounted ? 'scale-y-100' : 'scale-y-0'}`}
            ...
            <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-4">
                ...
                <div className="text-xs text-green-400 font-mono">
                    {data.reduce((acc, curr) => acc + curr.count, 0).toLocaleString('pt-BR')} Vitórias Totais
                </div>
            </div>
        </div>
    );
