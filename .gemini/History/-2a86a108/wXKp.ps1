...
foreach ($AgentDir in $AgentsFolders) {
    ...
    if (Test-Path $MemoryFile) {
        ...
        if ($DaysSinceUpdate -le 30) { $HealthyAgents++ } else { $AmnesicAgents++ }
...
if ($AmnesicAgents -gt 0) {
    Write-Host "[ALERTA] $AmnesicAgents agentes com amnésia detectados. Disparando notificação visual." -ForegroundColor Red
...
<toast>
    <visual>
        <binding template="ToastText02">
            <text id="1">Nexus System Health</text>
            <text id="2">ALERTA: $AmnesicAgents agentes com amnésia detectados!</text>
        </binding>
    </visual>
</toast>
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('NexusHealthCheck').Show($Toast)
}
