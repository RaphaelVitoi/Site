...
@pytest.mark.parametrize(
    "description, expected_agent, expected_gate",
    [
        (
            "Validar a matemática do ICM para um novo simulador.",
            "@validador",
            "gate_domain_validation",
        ),
        (
            "Desenvolver um novo componente UI em Next.js para o frontend.",
            "@implementor",
            "gate_web_infra",
        ),
        (
            "Otimizar o fluxo de tarefas e a fila no task_executor.py.",
            "@chico",
            "gate_orchestration",
        ),
        (
            "Analisar a segurança e autenticação do novo login.",
            "@securitychief",
            "gate_security",
        ),
        (
            "Pesquisar o mercado de concorrentes para a nova feature.",
            "@pesquisador",
            "gate_research",
        ),
        (
            "Definir a visão de produto e estratégia para o Q3.",
            "@maverick",
            "gate_strategy",
        ),
    ],
)
async def test_dispatcher_fallback_with_heuristic_gates(
    mock_queue_manager, description, expected_agent, expected_gate
):
