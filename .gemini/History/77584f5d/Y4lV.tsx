...
const RiskVisualizer: React.FC<RiskVisualizerProps> = ({ chipEV, riskPremium, color }) => {
  ...
  // Dynamic PKO Value Bar
  const pkoFactor = Math.min(1, riskPremium + (riskPremium * (pkoValue / 50)))
  ...
  return (<div className="relative w-full h-48"> {/* Container com altura fixa, ajustavel via CSS/Tailwind */}
    <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full">
      ...
      {/* PKO Overlay (If > 0) */}
      {riskPremium > 0 && (<motion.rect width="100" height={normalizedChipEV * pkoFactor} x="0" y={100 - (normalizedChipEV * pkoFactor)} fill={COLOR_MAP['orange-500']} transition={transition} style={{
      ...
      {/*
               ...
               zIndex: -1 -> force it to the very back
               */}
      <motion.path
        d={chipEVPath}
      ...
      <text
        ...
        className="fill-white opacity-70 text-xxs"
