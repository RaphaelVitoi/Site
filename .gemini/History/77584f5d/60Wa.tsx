...
const RiskVisualizer: React.FC<RiskVisualizerProps> = ({ chipEV, riskPremium, color }) => {
  ...
  const pkoFactor = Math.min(1, pkoValue / 50)
  ...
  return (
    <div className="relative w-full h-48"> {/* Container com altura fixa, ajustavel via CSS/Tailwind */}
      <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full">
        ...
        {/* Camada PKO - Incentivo para caca (Se > 0) */}
        {pkoValue > 0 && (
          <motion.rect
            width="100"
            height={normalizedChipEV * pkoFactor}
            x="0"
            y={100 - (normalizedChipEV * pkoFactor)}
            fill={COLOR_MAP['orange-500']}
