...
function useQuantizedDebounce ( callback: Function, delayMs: number )
{
...
export const NashMatrixProfiler: React.FC = () =>
{
    ...
    useEffect( () =>
    {
        ...
        workerRef.current.onmessage = ( e: MessageEvent ) =>
        {
            ...
            setMetrics( prev => [ ...prev, {
                id,
                t0, t1, t2, t3,
                totalLatencyMs: parseFloat( ( t3 - t0 ).toFixed( 2 ) ),
                workerFrictionMs: parseFloat( ( t1 - t0 ).toFixed( 2 ) ),
                wasmExecutionMs: parseFloat( ( t2 - t1 ).toFixed( 2 ) )
            } ] );
    ...
    const processDistortion = useCallback( ( payload: any ) =>
    {
        ...
        workerRef.current.postMessage( { id, payload, t0 } );
    }, [] );

    // Debouncer SOTA: Blindagem contra engarrafamento de UI (Intervalo de Quantum ~ 60fps)
    const dispatchQuantized = useQuantizedDebounce( processDistortion, 16 );

    const handleConcurrentStressTest = () =>
    {
        for ( let i = 0; i < 50; i++ )
        {
            dispatchQuantized( { ip_rp: Math.random(), oop_rp: Math.random(), kappa: 0.8 } );
        }
    };
