        ...
        <div className="font-mono space-y-12">
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
                ...
                <div className="p-6 border border-white/10 bg-black/40 rounded-2xl shadow-inner h-72 flex flex-col justify-center relative overflow-hidden">
                    ...
                    <span>Perspectiva Matemática</span>
                    </h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                    ...
                    <div className="h-87.5 w-full relative">
                        ...
                        <div className="mt-6 flex flex-col gap-3 border-t border-white/5 pt-6">
                            <h4 className="text-[0.65rem] font-black text-text-darker uppercase tracking-widest mb-2 flex items-center gap-2"><i className="fa-solid fa-microchip text-accent-indigo"></i> Top Vazamentos (Mente Preditiva)</h4>
                            {topLeaks.map(([name, value], idx) => (
                                <div key={name} className="flex flex-col gap-1">
                                    <div className="flex justify-between items-center text-[0.6rem] font-bold uppercase tracking-wider">
                                        <span className={idx === 0 ? 'text-accent-rose-light' : 'text-text-muted'}>{name}</span>
                                        <span className="font-mono text-white">{(value * 100).toFixed(1)}%</span>
                                    </div>
                                    <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden shadow-inner">
                                        <div className={`h-full rounded-full ${idx === 0 ? 'bg-accent-rose shadow-[0_0_8px_var(--accent-rose)]' : 'bg-accent-indigo'}`} style={{ width: `${value * 100}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
