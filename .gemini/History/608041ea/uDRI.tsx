...
export default function DashboardSOTA({
  ...
  const rawProfile =
    initialData?.profile || metricsContext?.predictiveProfile || defaultProfile;
  ...
  const radarData = Object.keys(activeProfile).map((key) => ({
    subject: key,
    Deficiencia: Number(
      (activeProfile[key as keyof typeof activeProfile] * 100).toFixed(1),
    ),
  }));
        ...
        <div className="glass-panel p-10 lg:p-14 rounded-4xl bg-bg-panel/40 border border-white/10 shadow-sota-glass flex flex-col justify-between relative transition-all duration-700 hover:border-white/20 group/pm-guide overflow-hidden">
          ...
          <div className="space-y-8 relative z-10">
            <div className="flex items-center gap-5 mb-6">
              ...
              <div>
                <h3 className="text-sm font-black text-white uppercase tracking-[0.25em] m-0 leading-none mb-2">
                  Perspectiva Matemática
                </h3>
                <p className="text-[0.65rem] text-text-darker font-black uppercase tracking-widest m-0">
                  Operador:{" "}
                  <span className="text-accent-indigo-light">{userName}</span>
                </p>
              </div>
            </div>
            <p className="text-[0.9rem] text-text-muted leading-relaxed font-medium italic border-l-2 border-accent-indigo/30 pl-8 py-4 bg-white/2 rounded-r-2xl">
              &quot;A Matriz de Insolvência ao lado não é apenas um cálculo de
              equidade, mas uma projeção A* Pathfinding de sobrevivência
              financeira.&quot;
            </p>
            ...
            {/* SOTA v4.5: Geometria do Risco & Estresse Topológico */}
      ...
      <section className="space-y-10 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12">
          <div className="glass-panel p-10 rounded-4xl bg-bg-panel/40 border border-white/5 shadow-2xl flex flex-col items-center justify-between group/vulnerabilities">
            <div className="text-center space-y-2 mb-8">
              <h3 className="text-[0.7rem] font-black text-accent-indigo uppercase tracking-[0.3em] m-0">
                Vulnerabilidades
              </h3>
                ...
                {topLeaks.map(([name, value], idx) => (
                  <div key={name} className="flex flex-col gap-2 group/leak">
                    <div className="flex justify-between items-center px-1">
                      <span
                        className={`text-[0.65rem] font-black uppercase tracking-widest transition-colors ${idx === 0 ? "text-accent-rose" : "text-text-muted group-hover/leak:text-white"}`}
                      >
                        {name}
                      </span>
                    ...
                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden shadow-inner">
                      {/* @ts-expect-error Framer Motion typings conflict in React 19 */}
                      <motion.div
                        initial={{ width: 0 }}
          ...
          <div className="glass-panel p-10 rounded-4xl bg-bg-panel/40 border border-white/5 shadow-2xl flex flex-col relative overflow-hidden group/telemetry">
            ...
            <div className="flex items-center justify-between mb-10 border-b border-white/5 pb-6 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-[0.75rem] font-black text-white uppercase tracking-[0.4em] m-0">
        ...
        <div className="mt-12 animate-sota-in">
          ...
          <GemmaAnalysisPanel
            ...
            potSize={potSize}
            heroStack={40} // Default baseline. A ser conectado via SotaSync em iterações futuras.
            villainStack={55}
          />
        </div>
      </section>
