...
type ChatMode = 'coach' | 'villain' | 'simulator';
...
export default function AICoachPanel({ scenario, nash }: Readonly<AICoachPanelProps>) {
  ...
  const handleSend = (text: string, selectedMode: ChatMode = mode) => {
    const handleSend = async (text: string, selectedMode: ChatMode = mode) => {
      ...
      setTimeout(() => {
        const responseText = formatResponse(selectedMode, trimmed, scenario, nash);
        try {
          let responseText = formatResponse(selectedMode, trimmed, scenario, nash);

          // Se o modo for Oráculo (coach), consulta a Mente Coletiva SOTA (RAG)
          if (selectedMode === 'coach') {
            ...
            if (res.ok) {
              ...
              console.warn('[RAG] Falha na consulta SOTA.');
            }
          } else {
            // Mantém o delay estético para os outros modos locais (Vilão, Simulador)
  ...
  return (
    <div className={styles.glassPanel} style={{ padding: '1.5rem', minHeight: '580px', display: 'flex', flexDirection: 'column' }}>
