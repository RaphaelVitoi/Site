...
fn best_rank(cards: &[Card]) -> i32 {
    ...
    // SOTA: Ordenação prévia. A biblioteca rs_poker assume que as cartas estão ordenadas (descendente)
    // para avaliar sequências (Straights) sem acionar o pânico de 'unreachable code'.
    ...
    for i in 0..n {
        for j in (i+1)..n {
            for k in (j+1)..n {
                for l in (k+1)..n {
                    for m in (l+1)..n {
                        let hand = Hand::new_with_cards(vec![sorted_cards[i], sorted_cards[j], sorted_cards[k], sorted_cards[l], sorted_cards[m]]);
