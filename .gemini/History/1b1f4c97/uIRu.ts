...
function processMatrix ( buffer: ValidBuffer ) {
    ...
    // 1. Vetor Monopólio: Escala com a dominância estrutural e o pote.
    // SOTA: Decaimento exponencial termodinâmico para simular o aprisionamento (Pot Entrapment).
    const commitmentDecay = Math.exp( -heroInvested / ( heroStack || 1 ) );
    const potLeverage = 1 + ( pot / totalChips );
    const monopolyVector = Math.max( 0.01, heroDominance * potLeverage * commitmentDecay );
