...
def apply_god_mode(text: str):
    ...
    for match in re.finditer(cmd_pattern, text, re.DOTALL | re.IGNORECASE):
        ...
        try:
            ...
            # Seguranca SOTA: shell=False e a unica opcao segura. shlex.split() garante suporte a argumentos com aspas.
            cmd_parts = shlex.split(cmd, posix=(os.name != 'nt'))
            result = subprocess.run(cmd_parts, shell=False, capture_output=True, text=True, timeout=300, check=False)
