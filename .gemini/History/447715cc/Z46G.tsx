...
function InfoTooltip ( { text }: Readonly<{ text: string }> ) {
  return (
    <div className="relative group inline-flex items-center ml-0.75 cursor-help">
...
function ActionRow ( {
  ...
  // SOTA: Desconstrução do Espectro de Decisão
  const gtoFreq = Math.max( 0, Math.min( 100, result.center - result.delta ) );
  const icmFreq = result.center;

  // SOTA: Perspectiva Vitoi (Edge Amortization). Amplifica ou restringe o desvio baseado no Fator Humano.
  let vitoiFreq = gtoFreq + ( result.delta * aggressionFactor );
  vitoiFreq = Math.max( 0, Math.min( 100, vitoiFreq ) );

  const isDeviating = Math.abs( vitoiFreq - gtoFreq ) > 1.5;
  const vitoiColor = isDeviating ? ( vitoiFreq > gtoFreq ? 'var(--accent-emerald)' : 'var(--accent-danger)' ) : 'var(--text-muted)';

  return (
    <div style={ {
      display: 'grid',
      gridTemplateColumns: '60px 1fr 1fr 1fr 64px',
      alignItems: 'center',
      gap: '0.4rem',
      padding: '0.45rem 0',
      borderBottom: '1px solid rgba(255,255,255,0.04)',
    } }>
      <span style={ { fontSize: '0.55rem', fontWeight: 800, color: accent, textTransform: 'uppercase', letterSpacing: '0.07em', display: 'flex', alignItems: 'center', gap: '2px' } }>
        { label } { labelTooltip && <InfoTooltip text={ labelTooltip } /> }
      </span>
      <span style={ { fontSize: '0.75rem', color: 'var(--text-dim)', textAlign: 'center', fontFamily: 'monospace' } }>
        { gtoFreq.toFixed( 0 ) }<span style={ { fontSize: '0.5rem', opacity: 0.5 } }>%</span>
      </span>
      <span style={ { fontSize: '0.75rem', color: 'var(--accent-amber)', textAlign: 'center', fontWeight: 700, fontFamily: 'monospace' } }>
        <AnimatedNumber value={ icmFreq } decimals={ 0 } suffix="" /><span style={ { fontSize: '0.5rem', opacity: 0.8 } }>%</span>
      </span>
      <span style={ { fontSize: '0.8rem', color: vitoiColor, textAlign: 'center', fontWeight: 900, fontFamily: 'monospace', textShadow: isDeviating ? `0 0 8px ${vitoiColor}40` : 'none' } }>
        <AnimatedNumber value={ vitoiFreq } decimals={ 0 } suffix="" /><span style={ { fontSize: '0.5rem', opacity: 0.8 } }>%</span>
      </span>
    ...
    <div className={ styles.glassPanel } style={ { padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' } }>
      ...
      <div style={ { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', padding: '0.75rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '0.5rem' } }>
        ...
        <div>
          ...
          <span style={ { fontSize: '0.8rem', fontWeight: 900, color: 'var(--accent-amber)' } }>{ current.rps.oop.toFixed( 1 ) }%</span>
        </div>
      </div>

      {/* Cabeçalho do Espectro de Decisão */ }
      <div style={ { display: 'grid', gridTemplateColumns: '60px 1fr 1fr 1fr 64px', gap: '0.4rem', paddingBottom: '0.2rem', borderBottom: '1px solid rgba(255,255,255,0.05)', marginTop: '0.5rem' } }>
        <span style={ { fontSize: '0.5rem', fontWeight: 800, color: 'var(--text-darker)', textTransform: 'uppercase' } }>Ação</span>
        <span style={ { fontSize: '0.5rem', fontWeight: 800, color: 'var(--text-dim)', textTransform: 'uppercase', textAlign: 'center' } }>GTO</span>
        <span style={ { fontSize: '0.5rem', fontWeight: 800, color: 'var(--accent-amber)', textTransform: 'uppercase', textAlign: 'center' } }>ICM</span>
        <span style={ { fontSize: '0.5rem', fontWeight: 900, color: 'var(--accent-emerald)', textTransform: 'uppercase', textAlign: 'center' } }>VITOI</span>
        <span style={ { fontSize: '0.5rem', fontWeight: 800, color: 'var(--text-darker)', textTransform: 'uppercase', textAlign: 'right' } }>Ajuste</span>
      </div>

      {/* Ações IP */ }
      <div style={ { marginBottom: '0.5rem' } }>
        <div style={ { fontSize: '0.55rem', fontWeight: 700, color: 'var(--accent-indigo-light)', textTransform: 'uppercase', letterSpacing: '0.12em', paddingTop: '0.5rem', paddingBottom: '0.2rem' } }>IP — Agressor</div>
        <ActionRow label="Check" chipEv={ current.freqs.ip_check } result={ current.nash.ip.check } field="ip_check" accent="var(--accent-indigo-light)" freqs={ current.freqs } aggressionFactor={ aggressionFactor } onChange={ ( f ) => onStreetFreqChange( activeStreet, f ) } />
        <ActionRow label="Bet S" chipEv={ current.freqs.ip_bet_small } result={ current.nash.ip.bet_small } field="ip_bet_small" accent="var(--accent-indigo-light)" freqs={ current.freqs } aggressionFactor={ aggressionFactor } onChange={ ( f ) => onStreetFreqChange( activeStreet, f ) } />
        <ActionRow label="Bet L" chipEv={ current.freqs.ip_bet_large } result={ current.nash.ip.bet_large } field="ip_bet_large" accent="var(--accent-indigo-light)" freqs={ current.freqs } aggressionFactor={ aggressionFactor } onChange={ ( f ) => onStreetFreqChange( activeStreet, f ) } />
      </div>

      {/* Ações OOP */ }
      <div style={ { marginBottom: '1.1rem' } }>
        <div style={ { fontSize: '0.55rem', fontWeight: 700, color: 'var(--accent-danger)', textTransform: 'uppercase', letterSpacing: '0.12em', paddingTop: '0.5rem', paddingBottom: '0.2rem' } }>OOP — Defensor</div>
        <ActionRow label="Call" chipEv={ current.freqs.oop_call } result={ current.nash.oop.call } field="oop_call" accent="var(--accent-danger)" freqs={ current.freqs } aggressionFactor={ aggressionFactor } onChange={ ( f ) => onStreetFreqChange( activeStreet, f ) } />
        <ActionRow label="Fold" chipEv={ current.freqs.oop_fold } result={ current.nash.oop.fold } field="oop_fold" accent="var(--accent-danger)" freqs={ current.freqs } aggressionFactor={ aggressionFactor } onChange={ ( f ) => onStreetFreqChange( activeStreet, f ) } />
        <ActionRow label="Raise" chipEv={ current.freqs.oop_raise } result={ current.nash.oop.raise } field="oop_raise" accent="var(--accent-danger)" freqs={ current.freqs } aggressionFactor={ aggressionFactor } onChange={ ( f ) => onStreetFreqChange( activeStreet, f ) } />
      </div>
      ...
      <MetricTooltip align="left" title="Agressividade Humana (Fator Ψ)" desc="Modulador bayesiano SOTA. Se o oponente real desvia do equilíbrio (ex: paga demais ou blefa de menos), a distribuição de Nash é forçada a se contrair ou expandir.">
        <div style={ { background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '10px', padding: '0.85rem 1.1rem' } }>
          <div style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.45rem' } }>
            ...
            <span className={ styles.dataMono } style={ { fontSize: '0.8rem', fontWeight: 800, color: 'var(--accent-emerald)' } }>{ aggressionFactor.toFixed( 1 ) }×</span>
          </div>
          <input type="range" min="0.5" max="1.5" step="0.1" value={ aggressionFactor } onChange={ ( e ) => onAggressionChange( Number.parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-indigo)' } } />
        </div>
      </MetricTooltip>
      ...
      <MetricTooltip align="left" title="Bounty Power" desc="Diluidor de Risk Premium. A recompensa imediata (bounty) infla a utilidade do Call, destruindo o Teto de Risco do ICM tradicional.">
        <div style={ { background: 'rgba(255,255,255,0.02)', border: pkoValue > 0 ? '1px solid rgba(251, 191, 36, 0.3)' : '1px solid rgba(255,255,255,0.05)', borderRadius: '10px', padding: '0.85rem 1.1rem' } }>
          <div style={ { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.45rem' } }>
            ...
            <span className={ styles.dataMono } style={ { fontSize: '0.8rem', fontWeight: 800, color: 'var(--accent-gold)' } }>{ pkoValue === 0 ? 'OFF' : `${Math.round( pkoValue * 100 )}%` }</span>
          </div>
          <input type="range" min="0" max="0.8" step="0.05" value={ pkoValue } onChange={ ( e ) => onPkoChange( Number.parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-amber)' } } />
        </div>
      </MetricTooltip>
