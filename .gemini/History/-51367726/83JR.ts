...
export function useQuantumEngine({
  ...
  const dispatchInsolvencyMatrix = useCallback(
    (
      ...
      const villainRange = args[0];
      const board = args[1];
      const rpFactor = args[2];
      const heroInvested = args[3];
      const currentPot = args[4];
      const activePlayers = args[5];
      const kappaOverride = args[6];
      const heroRange = args[7] ?? "AhKd";
      const betSizing = args[8] ?? 0.5;
