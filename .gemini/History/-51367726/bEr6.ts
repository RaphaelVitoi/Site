...
export interface DistortionPayload
{
    ...
    pots: [ number, number, number ]; // SOTA v4.2: Gravidade do Pote
    kappa?: number;
    isPredictive?: boolean;
}
...
export function useQuantumEngine ( { scenario, pkoValue = 0, isNearPayjump = false, blindsRisingSoon = false, streetFreqs = { flop: {} as ChipEvFreqs, turn: {} as ChipEvFreqs, river: {} as ChipEvFreqs }, aggressionFactor = 1, heroIsIp = false, heroPosition = 'IP', anteSize = 12.5, heroInvestedBb, currentPotBb, activePlayers = 2, kappaValue = 1, activeNodelock = null, isPredictive = true }: QuantumEngineParams )
{
    ...
    const dispatchInsolvencyMatrix = useCallback( ( villainRange: string, board: string, rpFactor: number, heroInvested: number, currentPot: number, activePlayers: number, kappaOverride?: number ) =>
    {
        ...
        insolvencyWorkerRef.current.postMessage( {
            type: 'MATRIX', villainRange, board, rpFactor, heroInvested, currentPot, activePlayers, kappa: kappaOverride ?? effectiveKappa, id: Date.now()
        } );
    }, [ effectiveKappa ] );
    ...
    const postFlopRps = useMemo( () =>
    {
        ...
        return { flop, turn, river, pots: [ potFlop, potTurn, potRiver ] as [ number, number, number ] };
    ...
    useEffect( () =>
    {
        ...
        if ( dispatchIcmDistortion )
        {
            dispatchIcmDistortion( {
                ipRpFlop, oopRpFlop, freqFlop: effectiveStreetFreqs.fold ? effectiveStreetFreqs.flop : { fold: 1, call: 0, raise: 0 },
                ipRpTurn, oopRpTurn, freqTurn: effectiveStreetFreqs.fold ? effectiveStreetFreqs.turn : { fold: 1, call: 0, raise: 0 },
                ipRpRiver, oopRpRiver, freqRiver: effectiveStreetFreqs.fold ? effectiveStreetFreqs.river : { fold: 1, call: 0, raise: 0 },
                ...
                pots: postFlopRps?.pots || [ 7.5, 22.5, 40 ],
                kappa: effectiveKappa,
                isPredictive
            } );
        }
    }, [ dispatchIcmDistortion, ipRpFlop, oopRpFlop, ipRpTurn, oopRpTurn, ipRpRiver, oopRpRiver, effectiveStreetFreqs, topologicAggression, activePlayers, effectiveKappa, isPredictive, postFlopRps?.pots ] );
    }, [ dispatchIcmDistortion, ipRpFlop, oopRpFlop, ipRpTurn, oopRpTurn, ipRpRiver, oopRpRiver, effectiveStreetFreqs, topologicAggression, activePlayers, effectiveKappa, isPredictive ] );

    const { nashFlop, nashTurn, nashRiver } = useMemo( () => ( {
        nashFlop: nashResults?.flop ?? solveIcmDistortion( ipRpFlop, oopRpFlop, effectiveStreetFreqs.flop, topologicAggression ),
        nashTurn: nashResults?.turn ?? solveIcmDistortion( ipRpTurn, oopRpTurn, effectiveStreetFreqs.turn, topologicAggression ),
        nashRiver: nashResults?.river ?? solveIcmDistortion( ipRpRiver, oopRpRiver, effectiveStreetFreqs.river, topologicAggression ),
