...
export function useQuantumEngine({
  ...
  const stableStacks = useMemo(
    () => scenario.stacks || [],
    [scenario.stacks?.join("|")],
  );
  const stablePrizes = useMemo(
    () => scenario.prizes || [],
    [scenario.prizes?.join("|")],
  );
  const stableSprData = useMemo(
    () => scenario.sprData || [],
    [scenario.sprData?.map((s) => `${s.name}:${s.potSize}`).join("|")],
  );
  ...
  const [insolvencyMatrixData, setInsolvencyMatrixData] =
    useState<InsolvencyMetrics | null>(null);
  const [isCalculatingInsolvency, setIsCalculatingInsolvency] = useState(false);
  const [nashResults, setNashResults] = useState<NashDistortionResults | null>(
    null,
  );
  ...
  const distortionTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    worker.onmessage = (e: MessageEvent<InsolvencyWorkerResponse>) => {
      ...
      if (e.data.error) {
        console.warn(
          "[SotaEcosystem] Entropia de Input (Insolvency WASM):",
          e.data.error,
        );
    ...
    return () => {
      ...
      if (distortionTimeoutRef.current)
  ...
  const dispatchInsolvencyMatrix = useCallback(
    ...
    (
      // NOSONAR
      villainRange: string, // NOSONAR
      board: string, // NOSONAR
      rpFactor: number, // NOSONAR
      heroInvested: number, // NOSONAR
      currentPot: number, // NOSONAR
      activePlayers: number, // NOSONAR
      kappaOverride?: number, // NOSONAR
      heroRange: string = "AhKd", // NOSONAR
      betSizing: number = 0.5, // NOSONAR
    ) => {
      // NOSONAR
  ...
  const quantumPerspectiva = useMemo(() => {
    ...
    const realizationFactor = heroIsIp
      ? 1
      : baseRealization + 0.15 * (1 - effectiveKappa);
    try {
  ...
  const rpAdjustment = useMemo(() => {
    ...
    const deadMoneyFactor =
      preflopDeadMoney > 0 ? standardDeadMoney / preflopDeadMoney : 1;
  ...
  const postFlopRps = useMemo(() => {
    ...
    const turnFutureRpInfluence = heroIsIp
      ? (river?.ipRp ?? 0)
      : (river?.oopRp ?? 0);
    const turn = derivePostFlopRps(
      stableStacks,
      resolvedPrizes,
      ipIndex,
      oopIndex,
    ...
    const flopFutureRpInfluence = heroIsIp
      ? (turn?.ipRp ?? 0)
      : (turn?.oopRp ?? 0);
    const flop = derivePostFlopRps(
      stableStacks,
      resolvedPrizes,
      ipIndex,
      oopIndex,
  ...
  const oopRpRiver =
    postFlopRps?.river?.oopRp ?? effectiveOopRp * Math.exp(-0.1);
    ...
    return stableSprData.map((stage: SprStage) => {
      ...
      if (stage.name === "FLOP")
        return {
          ...stage,
          rpValue: heroIsIp
            ? (postFlopRps.flop?.ipRp ?? stage.rpValue)
            : (postFlopRps.flop?.oopRp ?? stage.rpValue),
        };
      if (stage.name === "TURN")
        return {
          ...stage,
          rpValue: heroIsIp
            ? (postFlopRps.turn?.ipRp ?? stage.rpValue)
            : (postFlopRps.turn?.oopRp ?? stage.rpValue),
        };
      if (stage.name === "RIVER")
        return {
          ...stage,
          rpValue: heroIsIp
            ? (postFlopRps.river?.ipRp ?? stage.rpValue)
            : (postFlopRps.river?.oopRp ?? stage.rpValue),
        };
      return stage;
    });
    ...
    if (activeNodelock?.type === "block_bet") {
      ...
      return {
        flop: {
          ...
          ip_bet_small: heroIsIp
            ? override
            : stableStreetFreqs.flop.ip_bet_small,
        },
        turn: {
          ...stableStreetFreqs.turn,
          ip_bet_small: heroIsIp
            ? override
            : stableStreetFreqs.turn.ip_bet_small,
        },
        river: {
          ...stableStreetFreqs.river,
          ip_bet_small: heroIsIp
            ? override
            : stableStreetFreqs.river.ip_bet_small,
        },
      };
    }
  ...
  const formatSyncSolverResult = (
    ...
    if (
      "ip" in result &&
      "oop" in result &&
      result.ip?.check?.center !== undefined
    ) {
      if (result.deltaRp === undefined) result.deltaRp = ipRp - oopRp;
      return result;
    }
    ...
    return {
      ...
      oop: {
        fold: formatMetric(
          result?.oop?.fold?.center ?? result?.fold ?? freqs.oop_fold ?? 0,
        ),
        call: formatMetric(
          result?.oop?.call?.center ?? result?.call ?? freqs.oop_call ?? 0,
        ),
        raise: formatMetric(
          result?.oop?.raise?.center ?? result?.raise ?? freqs.oop_raise ?? 0,
        ),
      },
    };
  };
