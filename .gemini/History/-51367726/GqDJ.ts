...
export function useQuantumEngine( { scenario, pkoValue, isNearPayjump, blindsRisingSoon, streetFreqs, aggressionFactor, heroIsIp = false, heroPosition = 'IP', anteSize = 12.5, heroInvestedBb, currentPotBb, activePlayers = 2 }: QuantumEngineParams ) {
    ...
    useEffect( () => {
        if ( wasmContext?.dispatchIcmDistortion ) {
            wasmContext.dispatchIcmDistortion( {
    ...
    }, [wasmContext?.dispatchIcmDistortion, ipRpFlop, oopRpFlop, ipRpTurn, oopRpTurn, ipRpRiver, oopRpRiver, streetFreqs, topologicAggression, activePlayers] );

    const { nashFlop, nashTurn, nashRiver } = useMemo( () => ( {
        nashFlop: wasmContext?.nashResults?.flop ?? solveIcmDistortion( ipRpFlop, oopRpFlop, streetFreqs.flop, topologicAggression, activePlayers ),
        nashTurn: wasmContext?.nashResults?.turn ?? solveIcmDistortion( ipRpTurn, oopRpTurn, streetFreqs.turn, topologicAggression, activePlayers ),
        nashRiver: wasmContext?.nashResults?.river ?? solveIcmDistortion( ipRpRiver, oopRpRiver, streetFreqs.river, topologicAggression, activePlayers ),
    } ), [wasmContext?.nashResults, ipRpFlop, oopRpFlop, ipRpTurn, oopRpTurn, ipRpRiver, oopRpRiver, streetFreqs, topologicAggression, activePlayers] );
