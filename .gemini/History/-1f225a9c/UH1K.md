...
- \[HANDOFF-20260509-2\] - Erradicação do Cold-Start e Migração ONNX
  - Resultado: Motor RAG (ChromaDB) migrado integralmente de PyTorch para ONNX Runtime (`ONNXMiniLM_L6_V2`). Tempo de inicialização (Cold-Start) obliterado de 16s para 0.18s (183ms). RAG instanciado como Singleton assíncrono (eager-load) no boot do Orquestrador SOTA (`task_executor.py`).
  - Aprendizado: A importação do `torch` em instâncias CLI curtas causa "starvation" sistêmico devido ao carregamento repetitivo dos tensores na CPU. A exportação para ONNX (C++ puro) é a única via aceitável para inferências locais ultrarrápidas na API. O ONNX Runtime exige alinhamento rígido de ABI com o NumPy (v1.25.1 para compatibilidade com NumPy 2.x). O ChromaDB possui proteções estritas contra troca de "embedding functions" na mesma coleção, exigindo vaporização e recriação da base.
