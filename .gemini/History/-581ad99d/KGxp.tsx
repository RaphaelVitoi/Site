...
// SOTA: Tipagem estrita de matriz para inviabilizar o uso do "Type Casting" (as const) nos componentes de IU.
interface AggressiveAction {
    id: 'call' | 'raise';
...
export const ActionTooltip = ( { title, desc, align = 'center', children }: { title: string, desc: string, align?: 'left' | 'center' | 'right', children: React.ReactNode } ) => {
    const alignClasses = {
        left: 'left-0',
    ...
    return (
        <div className="relative group cursor-help flex-1 flex flex-col">
            { children }
            <div className={ `absolute bottom-full ${alignClasses[ align ]} mb-3 w-64 sm:w-72 max-w-[85vw] p-3 bg-[#0a0f1c] border border-neutral-700/50 rounded-lg shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-100 pointer-events-none` }>
...
const ActionPanelComponent: React.FC<ActionPanelProps> = ( { fold, call, raise, isBaseline = false, onActionSelect } ) => {

    // SOTA: Delegação de Eventos (Event Delegation). Erradica a alocação de funções inline na árvore JSX.
    const handleActionClick = useCallback( ( e: React.MouseEvent<HTMLButtonElement> ) => {
        const actionId = e.currentTarget.dataset.actionId;
        if ( ( actionId === 'fold' || actionId === 'call' || actionId === 'raise' ) && onActionSelect )
        {
    ...
    const aggressiveActions: AggressiveAction[] = useMemo( () => [
        {
            id: 'call',
            metrics: call,
            title: 'Call',
            ...
            cls: {
                ...
                tooltipDesc: 'Call raramente encerra a mão. Pagar insere sua stack em uma espiral de Reverse Implied Odds (RIO) nas streets subsequentes. O Coeficiente de Insolvência (Cᵢ) revela a verdade: se < 1, as Pot Odds são uma armadilha e a expectativa marginal desaba.',
        ...
        },
        {
            ...
            cls: {
                ...
                tooltipDesc: 'Agressão transfere a dor estrutural e alavanca o Bubble Factor do oponente contra ele mesmo. Retomar a iniciativa mitiga a Insolvência (Cᵢ), convertendo pressão passiva em Fold Equity não-linear e ganho futuro de FGS.',
    ...
    return (
        <div className="w-full flex flex-col md:flex-row gap-4 bg-neutral-950 p-4 border border-neutral-900 rounded-md font-mono">
            ...
            <ActionTooltip align="left" title="Baseline de Sobrevivência (Fold)" desc="Em ICM, o EV do Fold não é zero. É o Sunk Cost (Fichas já investidas). Este valor estabelece o piso absoluto da equação: toda ação ofensiva precisa render MAIS do que essa base para ser matematicamente viável.">
                <button
                    data-action-id="fold"
                    onClick={ handleActionClick }
                    className="w-full flex-1 relative overflow-hidden group bg-neutral-900 border border-neutral-800 hover:border-neutral-600 transition-all duration-300 rounded p-4 text-left flex flex-col justify-between min-h-25"
                >
                    ...
                    <div className="absolute inset-0 bg-linear-to-t from-neutral-950 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                </button>
            </ActionTooltip>

            {/* AÇÕES AGRESSIVAS: Despacho Estático de Matriz */ }
            { aggressiveActions.map( ( action, index ) => (
                <ActionTooltip key={ action.id } align={ index === 1 ? 'right' : 'center' } title={ action.cls.tooltipTitle } desc={ action.cls.tooltipDesc }>
                    <button
                        data-action-id={ action.id }
                        onClick={ handleActionClick }
                        className={ `w-full flex-1 relative overflow-hidden group bg-neutral-900 border ${action.cls.border} transition-all duration-300 rounded p-4 text-left flex flex-col justify-between min-h-25` }
                    >
                        <div
...
// SOTA: Fricção Zero na Reconciliação do React.
