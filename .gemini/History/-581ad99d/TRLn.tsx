...
export const ActionTooltip = ( { title, desc, align = 'center', children }: { title: string, desc: string, align?: 'left' | 'center' | 'right', children: React.ReactNode } ) => {
    const alignClasses = {
        left: '-left-2 sm:left-0',
        center: 'left-1/2 -translate-x-1/2',
        right: '-right-2 sm:right-0'
    };
    return (
...
const ActionPanelComponent: React.FC<ActionPanelProps> = ( { fold, call, raise, isBaseline = false, onActionSelect } ) => {
    ...
    const handleActionClick = useCallback( ( e: React.MouseEvent<HTMLButtonElement> ) => {
        const actionId = e.currentTarget.dataset.actionId as 'fold' | 'call' | 'raise' | undefined;
        if ( actionId && onActionSelect )
        {
    ...
    const aggressiveActions = useMemo( () => [
        {
            id: 'call' as const,
            metrics: call,
            title: 'Call',
