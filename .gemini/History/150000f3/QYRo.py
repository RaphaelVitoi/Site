...
def _intelligent_route_task(description: str, explicit_agent: Optional[str] = None) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        flash_health = _get_model_recent_health(DEFAULT_GEMINI_FAST_MODEL)
        keys_critical = flash_health < 0.25 # Abaixo de 25% indica exaustão iminente no pool
    except Exception:
        keys_critical = False
...
class DynamicYieldManager:
    ...
    async def apply_exhaustion_yield(self, task: Task) -> float:
