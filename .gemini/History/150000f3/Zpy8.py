...
def _verify_cortex_integrity(base_dir: Path):
    ...
    critical_files = ["GLOBAL_INSTRUCTIONS.md", ".claude/COSMOVISAO.md", ".claude/MODUS_OPERANDI.md"]
    for file_rel in critical_files:
        file_path = base_dir / file_rel
        if file_path.exists():
            content = file_path.read_bytes()
            file_hash = hashlib.sha256(content).hexdigest()
            logging.debug(f"[DIGITAL NOTARY] Integridade de {file_rel} atestada (SHA-256: {file_hash[:8]}).")
        else:
            logging.warning(f"[CORTEX SHIELD] Alerta de Amputação: Artefato legal ausente ({file_rel}).")
