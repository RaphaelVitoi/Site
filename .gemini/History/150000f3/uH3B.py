...
# SOTA: Isola logs no stderr para nao corromper pipes de JSON no stdout
...
# SOTA: Expurgar handlers pre-existentes (injetados por modulos como memory_rag)
...
_env_alts = [m.strip() for m in _alt_models_raw.split(",") if m.strip()]
_cfg_alts = [m for m in SYSTEM_CONFIG.get("openrouter_alternatives", []) if str(m).strip()]
OPENROUTER_ALTERNATIVE_MODELS = tuple(
    _env_alts + _cfg_alts if (_env_alts or _cfg_alts) else ["meta-llama/llama-3.3-70b-instruct:free", "meta-llama/llama-3.1-8b-instruct:free"]
)
...
RAG_IGNORE_PATTERNS = SYSTEM_HEURISTICS.get("rag_ignore_patterns", [".venv", ".git", ".chroma_db", "vector_store", "__pycache__", "node_modules", ".archive"])
    ...
    try:
        ...
        if _CONFIG_MTIME.get("system_config") != mtime:
            ...
            if new_config:
                ...
                _env_alts = [m.strip() for m in _alt_raw.split(",") if m.strip()]
                _cfg_alts = [m for m in SYSTEM_CONFIG.get("openrouter_alternatives", []) if str(m).strip()]
                OPENROUTER_ALTERNATIVE_MODELS = tuple(_env_alts + _cfg_alts if (_env_alts or _cfg_alts) else ["meta-llama/llama-3.3-70b-instruct:free", "meta-llama/llama-3.1-8b-instruct:free"])
...
# ==========================================
# GOVERNANÇA SOTA: DIGITAL NOTARY
# ==========================================
def _verify_cortex_integrity(base_dir: Path):
    """Digital Notary: Validação criptográfica dos artefatos de lei."""
    critical_files = [".claude/GLOBAL_INSTRUCTIONS.md", ".claude/COSMOVISAO.md", ".claude/MODUS_OPERANDI.md"]
    for file_rel in critical_files:
        file_path = base_dir / file_rel

        # SOTA: Auto-healing path resolution para GLOBAL_INSTRUCTIONS
        if not file_path.exists() and "GLOBAL_INSTRUCTIONS" in file_rel:
            if (base_dir / "GLOBAL_INSTRUCTIONS.md").exists():
                file_path = base_dir / "GLOBAL_INSTRUCTIONS.md"

        if file_path.exists():
            content = file_path.read_bytes()
            file_hash = hashlib.sha256(content).hexdigest()
            logging.debug(f"[DIGITAL NOTARY] Integridade de {file_path.name} atestada (SHA-256: {file_hash[:8]}).")
        else:
            logging.warning(f"[CORTEX SHIELD] Alerta de Amputação: Artefato legal ausente ({file_path.name}).")
...
def _sync_runtime():
    ...
    _verify_cortex_integrity(_BASE_DIR)
...
def _export_kg() -> str:
    try:
        # SOTA: Heurística Anti-Deadlock para extrair o KG sem inicializar o ChromaDB.
        # Evita o congelamento (file lock) causado pela concorrência com o Worker principal.
        # SOTA: Resolução Absoluta e Estrita (O(1)) do caminho para o Knowledge Graph.
        kg_db_path = Path(__file__).parent.resolve() / "database" / "knowledge_graph.db"
        ...
        # SOTA: Verifica se a tabela existe antes para prevenir OperationalError (no such table)
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='kg_nodes'")
        if not cursor.fetchone():
            conn.close()
            return json.dumps({"nodes": [], "edges": []})

        nodes = [dict(row) for row in cursor.execute("SELECT * FROM kg_nodes").fetchall()]
        edges = [dict(row) for row in cursor.execute("SELECT * FROM kg_edges").fetchall()]
        conn.close()
        return json.dumps({"nodes": nodes, "edges": edges})
    except Exception as e:
        return json.dumps({"nodes": [], "edges": [], "error": str(e)})

async def retrieve_sota_context(query: str, n_results: int = 3, local_only: bool = True) -> str:
    """
    SOTA: Abstração unificada no Orquestrador para consultar entidades do Knowledge Graph
    e integrá-las ao contexto RAG (Busca Semântica Híbrida).
    """
    try:
        rag = await get_rag_async()
        # CUSTO ZERO: Força a busca puramente matemática no ChromaDB (CPU local)
        return await rag.query_memory(query, n_results=n_results, local_only=local_only)
    except Exception as e:
        logging.error(f"[CORTEX SHIELD] Falha ao recuperar contexto da Mente Coletiva (KG+RAG): {e}")
        return ""
...
if __name__ == "__main__":
    try:
        ...
        # Exportacao do Knowledge Graph para o Frontend
        if len(sys.argv) >= 2 and sys.argv[1] == "kg-export":
            print(_export_kg())
            sys.exit(0)

        # SOTA: Exportacao de Telemetria Preditiva para a Interface de Friccao Zero
        if len(sys.argv) >= 2 and sys.argv[1] == "telemetry-predictive":
            try:
                from llm.budget import KEY_BLOCKLIST, GEMINI_ALL_KEYS_WITH_POOLS, ROUTE_FAILURE_COUNTS, ROUTE_BLOCKLIST, _RATE_LIMITERS
                total_keys = len(GEMINI_ALL_KEYS_WITH_POOLS)
                blocked_keys = len(KEY_BLOCKLIST)
                exhaustion_ratio = blocked_keys / total_keys if total_keys > 0 else 1.0

                starvation_events = sum(l.starvation_events for l in _RATE_LIMITERS.values()) if _RATE_LIMITERS else 0

                status = "STABLE"
                if exhaustion_ratio > 0.8 or starvation_events > 20:
                    status = "CRITICAL"
                elif exhaustion_ratio > 0.5 or starvation_events > 5:
                    status = "DEGRADED"
                ...
                telemetry = {
                    ...
                    "budget": {
                        ...
                        "exhaustion_ratio": round(exhaustion_ratio, 2)
                    },
                    "routes": {
                        "failed_routes_count": len(ROUTE_FAILURE_COUNTS),
                        "blocked_routes_count": len(ROUTE_BLOCKLIST)
                    },
                    "rate_limiters": {
                        "global_starvation_events": starvation_events
                    }
                }
                print(json.dumps(telemetry))
            except Exception as e:
                print(json.dumps({"error": str(e), "predictive_status": "UNKNOWN"}))
            sys.exit(0)

        # SOTA: Consulta Híbrida (KG + RAG) via CLI (Fallback do Oráculo do do.ps1)
        if len(sys.argv) >= 3 and sys.argv[1] == "query":
            question = " ".join(sys.argv[2:])
            try:
                result = asyncio.run(retrieve_sota_context(question))
                print(result)
            except Exception as e:
                print(f"Erro ao consultar a Mente Coletiva: {e}")
            sys.exit(0)
