...
export type LabState = {
    ...
    streets: number;
    fold: number;
    call: number; // Aposta Menor / Call
    raise: number; // Aposta Maior / Raise
};
...
export type GtoCfrContentProps = Readonly<{
    initialState?: LabState;
}>;

export function GtoCfrContent ( { initialState = DEFAULT_STATE }: GtoCfrContentProps )
{
    const [ history, setHistory ] = useState<LabState[]>( [ initialState ] );
    const [ historyIndex, setHistoryIndex ] = useState( 0 );
    const [ draft, setDraft ] = useState<LabState>( initialState );
    ...
    useEffect( () =>
    {
        const timer = setTimeout( () =>
        {
            ...
            {
                const newHistory = [ ...history.slice( 0, historyIndex + 1 ), draft ];
    ...
    // SOTA Performance: useMemo para evitar reprocessamento do Pathfinding A*
    ...
    // SOTA Performance: useMemo para a estratégia mista (Regret Matching)
                ...
                <GlassPanel className="p-8 border-accent-emerald/20">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <div className="flex flex-col gap-6">
                            ...
                            <div>
                                <label htmlFor="regretCall" className="text-label text-text-muted">Arrependimento (Aposta Pequena/Call)</label>
                            ...
                            <div>
                                <label htmlFor="regretRaise" className="text-label text-text-muted">Arrependimento (Aposta Grande/Raise)</label>
                        ...
                        <div className="flex flex-col justify-center space-y-6 bg-black/40 border border-white/5 rounded-2xl p-8 shadow-inner">
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <div className="h-2 bg-bg-deep rounded-full overflow-hidden relative"><div className="absolute inset-0 w-full h-full bg-text-darker transition-transform duration-500 ease-out origin-left" style={ { transform: `translateX(-${ 100 - (cfrStrategy.fold * 100) }%)` } }></div></div>
                                </div>
                                <div>
                                    ...
                                    <div className="h-2 bg-bg-deep rounded-full overflow-hidden relative"><div className="absolute inset-0 w-full h-full bg-accent-emerald transition-transform duration-500 ease-out shadow-[0_0_10px_rgba(52,211,153,0.5)] origin-left" style={ { transform: `translateX(-${ 100 - (cfrStrategy.call * 100) }%)` } }></div></div>
                                </div>
                                <div>
                                    ...
                                    <div className="h-2 bg-bg-deep rounded-full overflow-hidden relative"><div className="absolute inset-0 w-full h-full bg-accent-rose transition-transform duration-500 ease-out shadow-[0_0_10px_rgba(244,63,94,0.5)] origin-left" style={ { transform: `translateX(-${ 100 - (cfrStrategy.raise * 100) }%)` } }></div></div>
                                </div>
                            </div>
