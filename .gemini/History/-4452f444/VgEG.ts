...
globalThis.onmessage = (e: MessageEvent<IcmMessageData>) => {
  ...
  try {
    ...
    const results = calculateMalmuthHarville(
      players as ICMPlayer[],
      prizes as number[],
      totalPool as number | undefined,
    );
    const results = calculateMalmuthHarville(players, prizes, totalPool);
