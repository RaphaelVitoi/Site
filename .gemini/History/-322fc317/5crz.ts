 ...
 */
function computeRioMwStreet ( numPlayers: number, potTotal: number ): number {
  if ( numPlayers <= 2 ) return 0; // HU: sem RIO multiway
...
export function derivePostFlopRps (
  ...
  // D6: EV_fold por street = −investido acumulado pelo hero (1ª ordem, dominante)
  const evFoldStreet = -potAcumuladoHero;

  // SPR remanescente
  const stackHeroRemanescente = stacksRemanescentes[ heroIdx ];
  const sprRemanescente = potTotal > 0 ? stackHeroRemanescente / potTotal : Infinity;

  // D6: R por street (dinâmico)
  const rStreet = computeRStreet( street, heroIsIp, sprRemanescente );

  // D6 + D2: RIO multiway por street
  const rioMwStreet = computeRioMwStreet( numPlayers, potTotal );

  // D6: Valuation por street — razão ICM gain/loss com stacks remanescentes
  const totalPrizes = prizes.reduce( ( s, v ) => s + v, 0 );
  const baselineIcm = calculateMapaICM( stacksRemanescentes, prizes );
  const heroEquityBaseline = baselineIcm.equities[ heroIdx ];

  // Simular ganho (hero ganha pot) e perda (hero perde pot)
  // Ambos simétricos: a disputa é sobre potTotal, não sobre o stack inteiro
  const effStack = Math.min( stacksRemanescentes[ ipIndex ], stacksRemanescentes[ oopIndex ] );
  const disputeAmount = Math.min( potTotal, effStack ); // Capped pelo stack restante
  const stacksWin = stacksRemanescentes.map( ( s, i ) => {
    if ( i === heroIdx ) return s + disputeAmount;
    if ( i === ( heroIsIp ? oopIndex : ipIndex ) ) return Math.max( 0.1, s - disputeAmount );
    return s;
  } );
  const stacksLose = stacksRemanescentes.map( ( s, i ) => {
    if ( i === heroIdx ) return Math.max( 0.1, s - disputeAmount );
    if ( i === ( heroIsIp ? oopIndex : ipIndex ) ) return s + disputeAmount;
    return s;
  } );
  ...
  // D6: PM por street = (Equity × R × Valuation × gain) − (|EV_fold| + RIO)
  // Usando 50% equity como baseline (sem informação de mão específica)
  const equityProxy = 0.5;
  const pmStreet = ( equityProxy * rStreet * gainPct * valuationStreet )
    - ( ( 1 - equityProxy ) * lossPct )
    - rioMwStreet;

  // D3: Ci por street
  const potOddsPct = potTotal > 0 ? ( potTotal / ( potTotal + disputeAmount ) ) * 100 : 0;
  const ciStreet = potOddsPct > 0 ? pmStreet / potOddsPct : 0;

  // Pot entrapment: severidade do aprisionamento (quanto do stack do hero já está no pot)
  const potEntrapmentRatio = stacks[ heroIdx ] > 0 ? potAcumuladoHero / stacks[ heroIdx ] : 0;
