...
export function derivePostFlopRps (
  ...
  const stacksRemanescentes = stacks.map( ( s, i ) => {
    ...
    if ( i === ( heroIsIp ? oopIndex : ipIndex ) ) return Math.max( 0, s - ( potTotal - potAcumuladoHero ) );
    return s;
  } );
  ...
  const stacksLose = stacksRemanescentes.map( ( s, i ) => i === ( heroIsIp ? oopIndex : ipIndex ) ? s + potTotal : s );
