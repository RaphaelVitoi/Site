...
function getThemeConfig(isDeathZone: boolean) {
    return {
        wrapperBorder: isDeathZone ? 'rgba(244, 63, 94, 0.4)' : 'rgba(99, 102, 241, 0.2)',
        wrapperBoxShadow: isDeathZone ? '0 0 40px rgba(244, 63, 94, 0.15)' : '0 10px 40px rgba(0,0,0,0.3)',
        wrapperAnimation: isDeathZone ? 'jitter 0.2s infinite' : 'fadeIn 0.5s ease-out',
        gaugeBorderTop: isDeathZone ? 'var(--accent-danger)' : 'var(--accent-emerald)',
        badgeColor: isDeathZone ? 'var(--accent-danger)' : 'var(--accent-indigo-light)',
        badgeBg: isDeathZone ? 'rgba(244, 63, 94, 0.1)' : 'rgba(99, 102, 241, 0.1)',
        badgeText: isDeathZone ? '⚠️ ALERTA DE INSOLVÊNCIA' : 'Oráculo Pedagógico',
        rpColor: isDeathZone ? 'var(--accent-danger)' : 'var(--accent-emerald)',
        narrativeBorder: isDeathZone ? 'var(--accent-danger)' : 'var(--accent-indigo)',
        narrativeBg: isDeathZone ? 'linear-gradient(to right, rgba(244,63,94,0.05), transparent)' : 'transparent'
    };
}

function getGaugeRightColor(avgRp: number, isDeathZone: boolean) {
    if (avgRp <= 20) return 'transparent';
    return isDeathZone ? 'var(--accent-danger)' : 'var(--accent-gold)';
}
...
export default function QuantumSynthesis({
    ...
    const { wrapperBorder, wrapperBoxShadow, wrapperAnimation, gaugeBorderTop, badgeColor, badgeBg, badgeText, rpColor, narrativeBorder, narrativeBg } = getThemeConfig(isDeathZone);
