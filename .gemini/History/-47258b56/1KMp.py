...
async def handle_get_task_result(request):
    try:
        ...
        if not success:
            return web.json_response({"error": "Caminho invalido." if content == "INVALID_PATH" else "Resultado nao encontrado."}, status=400 if content == "INVALID_PATH" else 404)
...
async def handle_ask_oracle(request):
    ...
    try:
        ...
        answer = await rag.query_memory(question, n_results=n_results, local_only=False)
...
async def handle_get_system_status(request: web.Request) -> web.Response:
    ...
    try:
        ...
        status_data = {
            "rate_limiter": {
                "capacity": sum(l.capacity for l in _RATE_LIMITERS.values()),
                "current_tokens": round(sum(l.tokens for l in _RATE_LIMITERS.values()), 2),
                "starvation_events": sum(l.starvation_events for l in _RATE_LIMITERS.values()),
