function Invoke-SafeCommand {
    ...
    # [SEC] Bloqueio implacavel contra encadeamento de comandos e sub-expressoes
    if ($Command -match '[;\|&$\(\)]') {
        Write-Error '[SEC] Comando bloqueado. Operadores de encadeamento ou sub-expressoes nao sao permitidos na camada segura.'
        return $false
    }
    ...
    switch ($CommandName) {
        ...
        'npm' {
            $AllowedNpmSubCommands = @('install', 'i', 'uninstall', 'list', 'view', 'search', 'docs', 'outdated', 'ci')
