function Invoke-SafeCommand {
    ...
    # SOTA: Split O(N) seguro que erradica a vulnerabilidade de Catastrophic Backtracking do regex anterior
    $CommandParts = @([regex]::Matches($Command, '(?x) " (?:[^"]|"")* " | \S+ ') | ForEach-Object { $_.Value.Trim('"') })
    if ($CommandParts.Count -eq 0) { return $false }

    $CommandName = $CommandParts[0]
    $CommandArgs = @()
    if ($CommandParts.Count -gt 1) { $CommandArgs = @($CommandParts[1..($CommandParts.Count - 1)]) }
        ...
        default {
            if ($CommandName -match '\.ps1$') {
                ...
                if (Test-Path -LiteralPath $CommandName) {
                    ...
                    Write-Error "[SEC] O script '$CommandName' nao foi encontrado fisicamente no disco. Verifique o caminho absoluto ou relativo."
                    return $false
                }
            }
