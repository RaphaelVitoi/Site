...
# Função para adicionar uma menção recíproca
function Add-ReciprocalMention {
    param (
        [string]$AgentName,
    ...
    try {
        ...
        # Append the reciprocal mention if it's not already there
        if ($Content -notmatch "(?i)@$AgentName\b") {
            $Content += "`n- Menciona @$AgentName para manter a reciprocidade."

            # Write the modified content back to the file
            Set-Content $MemoryFile -Value $Content -Encoding UTF8

            Write-Host "  [CORRIGIDO] Adicionada menção recíproca de '$AgentName' para '$OtherAgentName' em '$MemoryFile'." -ForegroundColor Green
        }
    }
    catch {
        Write-Error "Erro ao adicionar menção recíproca no arquivo '$MemoryFile': $_"
    }
}
...
foreach ($Resultado1 in $ResultadosAnalise) {
    ...
    foreach ($AgenteMencionado in $Resultado1.AgentesMencionados) {
        ...
        if (-not $Encontrado) {
            ...
            # Adiciona a menção recíproca automaticamente
            Add-ReciprocalMention -AgentName $Agente1 -OtherAgentName $AgenteMencionado.Replace("@", "") -MemoryFile (Join-Path $MemoryDir $AgenteMencionado.Replace("@", "") "MEMORY.md")
        }
    }
}
