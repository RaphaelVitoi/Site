...
def start_worker():
    ...
    for proc in psutil.process_iter(["pid", "cmdline"]):
        if (
            proc.info["cmdline"]
            and "task_executor.py" in " ".join(proc.info["cmdline"])
...
def get_api_key():
    ...
    for k, v in os.environ.items():
        if (
            k.upper().startswith("GEMINI") or k.upper().startswith("GOOGLE")
