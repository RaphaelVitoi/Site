# pylint: disable=missing-module-docstring, missing-function-docstring, missing-class-docstring, broad-exception-caught, logging-fstring-interpolation, line-too-long, unused-argument, deprecated-argument, unused-variable, too-many-lines, invalid-name
# ruff: noqa: S404, S603, S605, S607, S310, E741, B007
import collections
import json
import os
import re
...
def _run_powershell_script(script_path: Path, *args):
    if not script_path.exists():
        console.print(f"[red][ERRO] Modulo nao encontrado: {script_path.name}[/]")
        time.sleep(2)
        return
...
def run_dashboard():
    ...
    while True:
        ...
        menu.add_row("[11]", "[bold red]Purge-RAG[/] (Expurgo Vetorial do ChromaDB)")
        menu.add_row("[12]", "[bold blue]Ingest-RAG[/] (Reidratar Mente Coletiva SOTA)")
        menu.add_row("", "")
...
COMMAND_MAP = {
    ...
    "purge-rag": lambda: _run_powershell_script(
        BASE_DIR / "scripts" / "maintenance" / "invoke_rag_purge.ps1"
    ),
    "ingest-rag": lambda: _run_script(BASE_DIR / "memory_rag.py", "ingest"),
    "start-gemma": lambda: _run_powershell_script(
        BASE_DIR / "scripts" / "gemma-service.ps1"
    ),
    "stop-gemma": lambda: _run_powershell_script(
        BASE_DIR / "scripts" / "gemma-service.ps1", ["stop"]
    ),
}
