...
const markdownComponents: Components = {
  ...
  p: ({ children, ...props }) => (
    <p
      className="mb-4 text-[0.95rem] text-text-muted leading-relaxed"
      {...props}
    >
      {children}
    </p>
  ...
  strong: ({ children, ...props }) => (
    <strong className="font-bold text-text-bright" {...props}>
      {children}
    </strong>
  ),
  blockquote: ({ children, ...props }) => (
    <blockquote
      className="border-l-4 border-accent-indigo/50 pl-4 py-1 italic bg-white/5 rounded-r-lg my-6 text-text-muted"
      {...props}
    >
      {children}
    </blockquote>
  ...
  a: ({ children, href, ...props }) => {
    if (
      ...
      return (
        <div className="my-8 rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 aspect-video bg-black/20">
  ...
  code: ({ className, children, ...props }) => {
    ...
    return isInline ? (
      ...
      <code
        className="block bg-[#0a0a0f] text-text-bright p-4 rounded-xl border border-white/10 font-mono text-[0.85rem] overflow-x-auto my-6 shadow-2xl tabular-nums"
        {...props}
      >
        {children}
      </code>
...
export function SotaMarkdown({ content }: Readonly<SotaMarkdownProps>) {
  ...
  return (
    <div className="text-text-main leading-relaxed font-sans sota-markdown-content">
      <ReactMarkdown
