...
import { calculateActionMetrics, calculateBaseFgsErosion, computeQuantumMetrics } from '@/components/simulator/engine/quantumMetrics';
...
const EquityCalculator = lazy( () => import( '@/components/simulator/panels/EquityCalculator' ) );
const ComparisonRadar = lazy( () => import( '@/components/simulator/panels/ComparisonRadar' ) );
const PerspectivePanel = lazy( () => import( '@/components/simulator/panels/PerspectivePanel' ) );
const PostFlopPanel = lazy( () => import( '@/components/simulator/panels/PostFlopPanel' ) );
const PmLensPanel = lazy( () => import( '@/components/simulator/panels/PmLensPanel' ) );
const ReferencialAula12 = lazy( () => import( '@/components/simulator/ReferencialAula12' ) );
const SimulatorQuizWidget = lazy( () => import( '@/components/quiz/SimulatorQuizWidget' ).then( m => ( { default: m.SimulatorQuizWidget as React.ComponentType<any> } ) ) );
const NashPanel = lazy( () => import( '@/components/simulator/panels/NashPanel' ) );
const TheoryPanel = lazy( () => import( '@/components/simulator/panels/TheoryPanel' ) );
const ScenarioStage = lazy( () => import( '@/components/simulator/panels/ScenarioStage' ) );
const MatchupSelector = lazy( () => import( '@/components/simulator/panels/MatchupSelector' ) );
const RangeMatrix = lazy( () => import( '@/components/simulator/panels/RangeMatrix' ) );
...
const SpatialControls = ( {
    ...
    return (
        <div className="glass-panel p-6 mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6 items-end relative">
            ...
            <div className="space-y-2">
                <SotaTooltip align="center" title="Fator Ψ (Axioma Lipe Piv)" desc="Taxa de Maluquice Humana. 100% = GTO puro. 0% = Apenas Pot Odds.">
                    <label htmlFor="sim-kappa" className="text-label cursor-help hover:text-accent-pink">Fator Ψ (κ)</label>
                </SotaTooltip>
                <div className="flex gap-2">
                    <input
                        id="sim-kappa"
                        type="range" min="0" max="1" step="0.05" value={ kappa }
                        onChange={ e => setKappa( Number( e.target.value ) ) }
                        className="flex-1 h-1.5 mt-4 rounded-full appearance-none bg-white/10 accent-accent-pink cursor-pointer"
                    />
                    <span className="text-[0.65rem] font-mono font-bold w-10 text-center text-accent-pink-light mt-2.5">
...
export default function MasterSimulator() {
    ...
    const [kappa, setKappa] = useState<number>( 0.5 );
    ...
    // SOTA: Antevisão Espacial - Variáveis içadas (hoisting) para alimentar o motor Quântico sem vazamento de escopo
    ...
    const apiQuantumMetrics = useMemo( () => (
        quantumPerspectiva ? computeQuantumMetrics( {
            quantumPerspectiva: quantumPerspectiva as any,
            activePlayers,
            heroInvested,
            currentPot,
            ...
            heroRawStack,
            totalTableChips,
            handEquity: nativeRangeMetric.equity,
            kappa
        } ) : null
    ), [quantumPerspectiva, activePlayers, heroInvested, currentPot, scenario.stacks, heroRawStack, totalTableChips, nativeRangeMetric.equity, kappa] );
    ...
    const baseFgsErosion = useMemo(
        () => calculateBaseFgsErosion( quantumPerspectiva as any, blindsRisingSoon, anteSize, heroPosition, heroRawStack, totalTableChips ),
    ...
    const spotData = useMemo( () => _createSpotData( {
        heroUpdatedStack, villainUpdatedStack, isIp, currentPot, bfForDash, rpForDash,
        quantumPerspectiva, isBaseline, baseFgsErosion, apiQuantumMetrics,
        street: 'PRE', board: '', heroRange: 'Any Two', villainRange: 'Any Two'
    } ),
    ...
    const actionMetrics = useMemo( () => {
        return calculateActionMetrics( { heroInvested, currentPot, bfForDash, rpForDash, quantumPerspectiva: quantumPerspectiva as any, heroRawStack, heroPosition, baseFgsErosion, apiQuantumMetrics: apiQuantumMetrics as any, activePlayers, totalTableChips } );
    ...
    const spotContextValue = useMemo( () => {
        ...
        return {
            ...
            heroInvested,
            kappa,
            setGlobalKappa: setKappa
        };
    }, [spotData, actionMetrics, finalIpRp, finalOopRp, activePlayers, heroInvested, kappa, scenario.id, scenario.name, setKappa] );
    ...
    const toolContents: Record<ActiveTool, React.ReactNode> = {
        scenario: (
            <Suspense fallback={ <LoadingFallback /> }>
                ...
                <div className="mb-8 glass-panel p-6 animate-sota-in border-accent-indigo/10">
