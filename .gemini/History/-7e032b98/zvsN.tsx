...
import { SotaEcosystemContext } from './SotaContext';
...
function computeQuantumMetrics ( quantumPerspectiva: Record<string, any>, activePlayers: number, heroInvested: number, currentPot: number, stacks: number[] ) {
    ...
    const perspectiva = expectativa - adjustedEvFold;

    const denom = ( adjustedDeltaWin * rFactor * fgsHealth ) - deltaLosePct;
    let threshEq = null;
    if ( Math.abs( denom ) > 1e-6 ) threshEq = Math.max( 0, Math.min( 1, ( adjustedEvFold - deltaLosePct ) / denom ) );
...
function _calculateBaseFgsErosion ( quantumPerspectiva: Record<string, any> | null, blindsRisingSoon: boolean, anteSize: number, heroPosition: HeroPosition ): number {
    ...
    if ( blindsRisingSoon )
    {
        ...
        const penaltyMap: Record<string, number> = { SB: -1.5, IP: -0.5, BB: 0 };
...
interface SotaContextParams {
    ...
    finalOopRp: number;
    apiQuantumMetrics: any;
    nativeRangeMetric: any;
    dispatchNativeEquity: any;
    setManualEquity: any;
}

function _createSotaEcosystemContextValue ( params: SotaContextParams ) {
    const pot = params.spotData.pot;
    const sunkCost = Math.abs( params.actionMetrics.fold.chipEv );
    const potOddsPct = ( pot + sunkCost ) > 0 ? ( pot / ( pot + sunkCost ) ) * 100 : 33;
    return {
        spotData: params.spotData,
        actionMetrics: params.actionMetrics,
        quantumPerspectiva: params.quantumPerspectiva,
        effectiveIpRp: params.finalIpRp,
        effectiveOopRp: params.finalOopRp,
        potOddsPct,
        apiQuantumMetrics: params.apiQuantumMetrics,
        nativeRangeMetric: params.nativeRangeMetric,
        dispatchNativeEquity: params.dispatchNativeEquity,
        setManualEquity: params.setManualEquity
    };
}
...
export default function MasterSimulator () {
    ...
    const sotaEcosystemContextValue = useMemo( () => {
        return _createSotaEcosystemContextValue( {
            spotData,
            actionMetrics,
            quantumPerspectiva,
            finalIpRp: finalIpRp,
            finalOopRp: finalOopRp,
            apiQuantumMetrics,
            nativeRangeMetric,
            dispatchNativeEquity,
            setManualEquity
        } );
    }, [ spotData, actionMetrics, quantumPerspectiva, finalIpRp, finalOopRp, apiQuantumMetrics, nativeRangeMetric, dispatchNativeEquity, setManualEquity ] );
    ...
    return (
        <SotaEcosystemContext.Provider value={ sotaEcosystemContextValue }>
            <div className={ styles.simRoot }>
                { tourSpotlight && (
                    <div className="tour-spotlight" style={ {
                        top: tourSpotlight.top - 8, left: tourSpotlight.left - 8,
                        width: tourSpotlight.width + 16, height: tourSpotlight.height + 16,
                    } } />
                ) }

                <SimulatorHeader
                    scenarioName={ scenario.id === 'nodelock' ? 'Sandbox de Exploração Livre' : scenario.name }
                ...
                <Suspense fallback={ <LoadingFallback /> }>
                    <div style={ { maxWidth: '1280px', margin: '0 auto', padding: '0 1.5rem' } }>
                        <PmLensPanel anteSize={ anteSize } heroInvested={ heroInvested } currentPot={ currentPot } activePlayers={ activePlayers } />
                        <IcmQuizVisceral />
                    </div>
                </Suspense>
