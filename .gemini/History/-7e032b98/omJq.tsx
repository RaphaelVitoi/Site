...
export default function MasterSimulator () {
    ...
    const [ nativeRangeMetric, setNativeRangeMetric ] = useState<{ equity: number; isCalculating: boolean; error?: string }>( { equity: 50, isCalculating: false } );
    ...
    // Erradica a desconexão visual resetando os dados quando a realidade do spot muda.
    useEffect( () => {
        setInsolvencyMatrixData( null );
    }, [ scenario.id, heroPosition, activePlayers ] );
    ...
    useEffect( () => {
        ...
        equityWorkerRef.current.onmessage = ( e ) => {
            ...
            if ( error )
            {
                ...
                setNativeRangeMetric( prev => ( { ...prev, isCalculating: false, error } ) );
            } else
            {
                setNativeRangeMetric( { equity, isCalculating: false, error: undefined } );
            }
        };
