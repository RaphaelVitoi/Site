...
const SimulatorHeader = ( { scenarioName, stacks, effectiveIpRp, effectiveOopRp, rpSource, sidebarOpen, onToggleSidebar }: any ) => (
    <header className="p-4 border-b border-white/10 flex justify-between items-center bg-bg-panel/50 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-4">
            <button onClick={ onToggleSidebar } className="w-8 h-8 flex items-center justify-center bg-white/5 rounded-md hover:bg-white/10 text-white transition-colors"><i className="fa-solid fa-bars" /></button>
            <h1 className="text-sm font-bold text-text-bright uppercase tracking-widest">{ scenarioName || 'Laboratório SOTA' }</h1>
        </div>
        <div className="flex gap-4 text-xs font-mono font-bold text-text-muted">
            <span><span className="text-indigo-400">IP RP</span> { effectiveIpRp?.toFixed( 1 ) }%</span>
            <span><span className="text-rose-400">OOP RP</span> { effectiveOopRp?.toFixed( 1 ) }%</span>
        </div>
    </header>
);

const SimulatorNavigation = ( { activeTool, onSelectTool }: any ) => (
    <div className="flex gap-2 p-2 bg-bg-panel/50 rounded-xl overflow-x-auto border border-white/5 scrollbar-hide">
        { [{ id: 'scenario', label: 'Cenário SOTA' }, { id: 'calculator', label: 'Calculadora' }, { id: 'matchup', label: 'Matchups' }, { id: 'comparar', label: 'Comparativo' }, { id: 'perspectiva', label: 'Perspectiva' }, { id: 'posflop', label: 'Pós-Flop' }].map( t => (
            <button key={ t.id } onClick={ () => onSelectTool( t.id ) } className={ `px-4 py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap uppercase tracking-widest ${activeTool === t.id ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'text-text-muted hover:bg-white/5'}` }>{ t.label }</button>
        ) ) }
    </div>
);
...
const GuideToolbar = () => (
    <div className="flex flex-wrap items-center gap-4 bg-bg-panel/40 border border-white/5 rounded-2xl p-4 shadow-inner mb-8 animate-sota-in">
        ...
        <SimControlTooltip align="left" title="Delta (Δ)" desc="Diferença matemática absoluta entre dois estados.">
        ...
        <SimControlTooltip align="center" title="Pontos Percentuais (p.p.)" desc="Diferença aritmética real entre porcentagens.">
            <span className="text-[0.7rem] font-bold text-text-muted cursor-help hover:text-accent-indigo flex items-center gap-1.5 transition-colors">
                <i className="fa-solid fa-percent text-[0.6rem]"></i> p.p.
            </span>
        </SimControlTooltip>
        <div className="w-px h-4 bg-white/10 mx-2"></div>
        <SimControlTooltip align="right" title="Física Sincronizada" desc="A mesa respira em tempo real. Cada ajuste de stack altera a gravidade de todos os simuladores.">
...
const SpatialControls = ( {
    ...
    isPredictive,
    setIsPredictive
}: SpatialControlsProps & { isPredictive: boolean, setIsPredictive: ( v: boolean ) => void } ) => {
    ...
    return (
        <div className="glass-panel p-6 mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 items-end relative">
            ...
            <div className="space-y-2">
                <SimControlTooltip align="left" title="Ponto Zero" desc="Sua desvantagem estrutural inicial.">
                    <label className="text-label cursor-help hover:text-accent-indigo">Posição (Ponto Zero)</label>
                </SimControlTooltip>
                <select
                    value={ heroPosition }
            ...
            <div className="space-y-2">
                <SimControlTooltip align="center" title="Investimento" desc="O abismo do seu EV de Fold.">
                    <label className="text-label cursor-help hover:text-accent-indigo">Sunk Cost (Investido)</label>
                </SimControlTooltip>
            ...
            <div className="space-y-2">
                <SimControlTooltip align="center" title="Dead Money" desc="O oxigênio do torneio.">
                    <label className="text-label cursor-help hover:text-accent-indigo">Pote Atual</label>
                </SimControlTooltip>
            ...
            <div className="space-y-2">
                <SimControlTooltip align="right" title="Entropia Multiway" desc="Ações escalam quadraticamente o RIO.">
                    <label className={ `text-label ${isMultiway ? 'text-accent-danger' : ''}` }>Jogadores</label>
                </SimControlTooltip>
                <input
            ...
            <div className="space-y-2">
                <SimControlTooltip align="right" title="FGS Control" desc={ isPredictive ? "Cálculo Automático via Motor SOTA." : "Ajuste manual da erosão de stack." }>
                    <label className="text-label">FGS / Erosão</label>
                </SimControlTooltip>
