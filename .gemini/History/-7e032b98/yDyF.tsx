...
function computeQuantumMetrics ( quantumPerspectiva: Record<string, any>, activePlayers: number, heroInvested: number, currentPot: number, stacks: number[] ) {
    ...
    // SOTA: Escalonamento Logarítmico para Multiway
    const mwFactor = 1 + Math.log2( opponents );
    ...
    const perspectiva = expectativa - evFoldPct - rioMw;
...
export default function MasterSimulator () {
    ...
    useEffect( () => {
        ...
        return () => icmWorkerRef.current?.terminate();
    }, [] );
    ...
    } ), [ nativeRangeMetric, dispatchNativeEquity, setManualEquity, dispatchIcmPerspectiva ] );
