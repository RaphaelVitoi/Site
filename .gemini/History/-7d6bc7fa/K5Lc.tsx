      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 relative z-10">
        <div className="max-w-2xl">
          ...
          <p className="text-[0.7rem] text-text-muted mt-1.5 mb-0 leading-relaxed font-medium">
            Mapeamento termodinâmico de EV_fold e Risk Premium. O descarte (<code className="text-accent-danger/80">Fold</code>) possui valor intrínseco de sobrevivência baseado no Payjump e na inércia da mesa.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="space-y-2">
            <span className="text-[0.55rem] font-black text-text-darker uppercase tracking-[0.2em] flex items-center gap-2 px-1"><i className="fa-solid fa-flask-vial text-accent-indigo" /> Presets de Laboratório</span>
            <div className="flex gap-1.5 flex-wrap">
              { PRESETS.map( ( p, i ) => (
                <button
                  key={ p.label }
                  onClick={ () => handlePresetChange( i ) }
                  aria-pressed={presetIdx === i ? 'true' : 'false'}
            ...
            <div className="flex gap-1.5 p-1 bg-black/40 border border-white/5 rounded-xl shadow-inner">
              { [true, false].map( isIp => (
                <button
                  ...
                  aria-pressed={heroIsIp === isIp ? 'true' : 'false'}
            ...
            <div className="flex gap-1 p-1 bg-black/40 border border-white/5 rounded-xl shadow-inner">
              { [2, 3, 4, 5].map( n => (
                <button
                  ...
                  aria-pressed={numPlayers === n ? 'true' : 'false'}
      ...
      <div className="bg-bg-deep/80 backdrop-blur-md border border-white/10 rounded-3xl p-5 lg:p-7 shadow-2xl relative overflow-hidden group mt-2">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-white/5 pb-4 mb-4 relative z-10 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {ci !== null && (
              <div className="flex items-center gap-2 bg-black/40 px-2.5 py-1 rounded-lg border border-white/5 shadow-inner">
                <span className="text-[0.55rem] font-black text-text-dim uppercase tracking-widest">Insolvência (Cᵢ):</span>
