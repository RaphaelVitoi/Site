...
export default function PostFlopPanel( { anteSize = 12.5, scenarioId, initialStacks, initialPrizes, heroIsIp: heroIsIpProp, activePlayers: activePlayersProp, effectiveSprData, pkoValue = 0, ipLabel, oopLabel }: Readonly<PostFlopPanelProps> ) {
  ...
  const [heroIsIp, setHeroIsIp] = useState( heroIsIpProp ?? true );
  const [numPlayers, setNumPlayers] = useState( activePlayersProp ?? 2 );
            ...
            <div className="flex gap-2 flex-wrap">
              { PRESETS.map( ( p, i ) => (
                <button
                  ...
                  disabled={ isMasterControlled }
                  className={ `px-3 py-1.5 rounded-lg text-[0.58rem] font-black border transition-all cursor-pointer ${presetIdx === i ? 'bg-accent-indigo border-accent-indigo text-text-main shadow-lg shadow-accent-indigo/20' : 'bg-slate-800/50 border-white/5 text-text-muted hover:bg-slate-700/50 hover:border-white/10'} disabled:opacity-50 disabled:cursor-not-allowed` }
                >
                  { p.label }
                </button>
              ) ) }
            ...
            <div className="flex gap-2">
              { [true, false].map( isIp => (
                <button
                  ...
                  disabled={ isMasterControlled }
                  className={ `flex-1 px-3 py-1.5 rounded-lg text-[0.58rem] font-black border transition-all cursor-pointer ${heroIsIp === isIp ? 'bg-accent-sky border-accent-sky text-text-main shadow-lg shadow-accent-sky/20' : 'bg-slate-800/50 border-white/5 text-text-muted hover:bg-slate-700/50'} disabled:opacity-50 disabled:cursor-not-allowed` }
                >
