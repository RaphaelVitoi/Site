...
function StackInput({ val, index, ipIdx, oopIdx, onUpdate }: { val: number; index: number; ipIdx: number; oopIdx: number; onUpdate: (idx: number, val: number) => void }) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 px-3 py-2 rounded-xl border transition-all ${isActive ? 'bg-accent-indigo/10 border-accent-indigo/30 ring-1 ring-accent-indigo/20' : 'bg-transparent border-white/5 opacity-40 hover:opacity-100'}`}>
      ...
      <div className="flex items-baseline gap-1">
        <input type="number" value={val} onChange={(e) => onUpdate(index, Number.parseFloat(e.target.value) || 0)} className={`w-full bg-transparent border-none text-[0.8rem] font-black outline-none font-mono tabular-nums ${isActive ? 'text-white' : 'text-text-dim'}`} />
...
function PotControl({ label, val, min, max, anteSize, onChange }: { label: string; val: number; min: number; max: number; anteSize: number; onChange: (v: number) => void }) {
  return (
  ...
  return (
    <div className="glass-panel flex flex-col gap-8 border border-white/10 rounded-3xl shadow-2xl bg-bg-panel/90 backdrop-blur-2xl p-8 lg:p-10 transition-all duration-500 hover:border-accent-indigo/40 relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 xl:grid-cols-9 gap-3 bg-black/50 border border-white/5 p-5 rounded-2xl shadow-inner relative z-10">
        { stacks.map( ( s, i ) => (
          <StackInput key={`stack-${i}`} val={s} index={i} ipIdx={ipIdx} oopIdx={oopIdx} onUpdate={updateStack} />
        ) ) }
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-8 bg-black/40 p-6 rounded-3xl border border-white/5 shadow-inner">
          <PotControl label="FLOP" val={potFlop} min={1} max={Math.min( stacks[ipIdx], stacks[oopIdx] ) * 0.4} anteSize={anteSize} onChange={v => { setPotFlop(v); setIsLocked(false); }} />
          <PotControl label="TURN" val={potTurn} min={1} max={Math.min( stacks[ipIdx], stacks[oopIdx] ) * 0.7} anteSize={anteSize} onChange={v => { setPotTurn(v); setIsLocked(false); }} />
          <PotControl label="RIVER" val={potRiver} min={1} max={Math.min( stacks[ipIdx], stacks[oopIdx] )} anteSize={anteSize} onChange={v => { setPotRiver(v); setIsLocked(false); }} />
        </div>
      </div>
      ...
      <div className="bg-bg-deep/80 backdrop-blur-md border border-white/10 rounded-3xl p-6 lg:p-8 shadow-2xl relative overflow-hidden group mt-4">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-white/5 pb-5 mb-5 relative z-10 gap-4">
          <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full ${isCalculatingWasm ? 'bg-accent-emerald animate-pulse shadow-[0_0_10px_var(--accent-emerald)]' : 'bg-accent-indigo shadow-[0_0_10px_var(--accent-indigo)]'}`}></div>
            <strong className="text-[0.75rem] font-black text-white uppercase tracking-widest">
              Inteligência Híbrida (WASM + PM Lens)
            </strong>
            <span className={`px-2.5 py-0.5 rounded border text-[0.6rem] font-mono ${isLocked ? 'bg-accent-indigo/10 border-accent-indigo/30 text-accent-indigo-light' : 'bg-white/5 border-white/10 text-text-muted'}`}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {pm !== null && (
              <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-lg border border-white/5 shadow-inner">
                <span className="text-[0.6rem] font-black text-text-dim uppercase tracking-widest">PM Global:</span>
                <span className={`text-[0.75rem] font-black font-mono ${getPmColorClass(pm)}`}>{formatPct(pm)}</span>
              </div>
            )}
            {ci !== null && (
              <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-lg border border-white/5 shadow-inner">
                <span className="text-[0.6rem] font-black text-text-dim uppercase tracking-widest">Insolvência (Cᵢ):</span>
                <span className={`text-[0.75rem] font-black font-mono ${ci < 1 ? 'text-accent-danger' : 'text-accent-emerald'}`}>{formatCi(false, ci)}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
          <div className="flex flex-col gap-3">
            <span className="text-[0.65rem] font-black text-accent-emerald-light uppercase tracking-widest flex items-center gap-2">
              <i className="fa-solid fa-microchip" /> Motor WebGPU
            </span>
            <div className="bg-black/50 border border-white/5 rounded-xl p-4 flex flex-col justify-center gap-4 h-full shadow-inner">
              <div className="flex justify-between items-center">
                <span className="text-[0.65rem] text-text-muted font-bold uppercase tracking-widest">Win Rate Real</span>
                <span className={`text-[0.85rem] font-mono font-black ${winRateClass}`}>
                  {winRateText}
                </span>
              </div>
              <div className="w-full h-px bg-white/5" />
              <div className="flex justify-between items-center">
                <span className="text-[0.65rem] text-text-muted font-bold uppercase tracking-widest">A* Risk Infl.</span>
                <span className={`text-[0.85rem] font-mono font-black ${riskIndexClass}`}>
                  {riskIndexText}
                </span>
              </div>
            </div>
          ...
          <div className="md:col-span-2 flex flex-col justify-center">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[0.65rem] font-black text-text-muted uppercase tracking-widest">Sinergia Sistêmica</span>
            </div>
            <p className="text-[0.75rem] text-text-dim leading-relaxed m-0 italic font-medium">
              A Perspectiva Matemática (PM) atua diretamente sobre as estimativas de <code className="text-accent-danger font-bold bg-accent-danger/10 px-1 rounded">EV_fold</code> e <code className="text-accent-indigo-light font-bold bg-accent-indigo/10 px-1 rounded">Risk Premium</code>. A <span className="text-accent-danger font-bold">Insolvência (Cᵢ)</span> desmascara a ilusão do ChipEV, enquanto o Motor WebGPU injeta a Equidade Absoluta e o Pathfinding A*, mitigando o viés do vácuo no mapeamento pós-flop.
            </p>
            {!isLocked && (
              <button
                onClick={() => setIsLocked(true)}
                className="mt-4 self-start px-4 py-2 bg-accent-emerald/10 text-accent-emerald-light border border-accent-emerald/30 rounded-lg text-[0.6rem] font-black uppercase tracking-widest hover:bg-accent-emerald/20 hover:shadow-[0_0_15px_rgba(16,185,129,0.2)] transition-all"
              >
                <i className="fa-solid fa-link mr-1" /> Reacoplar ao Orquestrador
              </button>
            )}
          </div>
        </div>
