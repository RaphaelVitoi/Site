...
Set-Alias -Name nexus-commit -Value "nexus-commit" -Scope Global -ErrorAction SilentlyContinue
...
function nexus-commit {
    Write-Host "[COMMIT] Materializando cognição do Hemisfério Esquerdo..." -ForegroundColor Yellow
    ...
    try {
        Get-Clipboard | Set-Content -Path $TempFile -Encoding UTF8
        if ((Get-Item $TempFile).Length -eq 0) {
            Write-Host "[FAIL] Área de transferência vazia. Operação abortada." -ForegroundColor Red
            return
        }
