...
# 🌌 MANUAL DO WORKFLOW DE AGENTES (ESTADO DA ARTE v6.2)
...
* **O Pulso (Execução):** `task_executor.py worker`. O daemon multithread que aciona os LLMs e aplica o *God Mode*.
* **O Córtex (Identidade):** Diretório `.claude/`. Contém a `COSMOVISAO.md`, as memórias ativas dos agentes e os prompts de sistema.

---

## 2. O CICLO DE VIDA DA MATÉRIA (WORKFLOW)
## 2. PROTOCOLOS DE EXCELÊNCIA (Ouro Conceitual)

Toda tarefa neste ecossistema é um microcosmo de aprendizado (como ditado no *Templo do Aprendizado Generativo*). O fluxo segue o princípio de **Fricção Zero**.
...
*Exemplo: `| @pesquisador | Concluído | 2026-03-16 | 5 fontes validadas sobre ICM |`*
...
*Ação Imediata:* Corrigir a SPEC.
*Ação Fractal:* Adicionar nota de "Atenção" no feedback para o agente que errou, evitando reincidência e forçando-o a atualizar sua `MEMORY.md`.
...
4. A cognição é disparada seguindo a Matriz de Roteamento de Economia (priorizando *Free Tiers* rápidos ou densos de acordo com a persona).
...
* Se o agente escreveu um bloco de código indicando o *Caminho* do arquivo, o Python cria/sobrescreve o arquivo no HD físico instantaneamente.
* Se o agente usou a tag `Comando:`, o Python executa o comando no terminal (ex: `npm install`).
* *Nota de Segurança:* Comandos destrutivos estão na *blacklist*. No modo de Autonomia Parcial, alterações de estado exigem confirmação.
...
* Se falhar, o **Núcleo de Autodebugging** (Auto-Cura) é acionado. Uma subtarefa é injetada relatando o erro de *stderr* ao mesmo agente para que ele se corrija e tente novamente, sem intervenção humana.

---

## 3. O ESQUADRÃO: AS 15 LÂMINAS ESPECIALIZADAS
## 4. WORKFLOW DETALHADO: AS 15 LÂMINAS ESPECIALIZADAS

A divisão de trabalho impede alucinações cognitivas e mantém o escopo rígido. O esquadrão atua de forma orquestrada:
O pipeline linear atua como uma linha de montagem de excelência. Agentes consultivos atuam em paralelo.

* **@pesquisador:** Busca de mercado e estado da arte.
* **@prompter:** Refinamento lógico de entradas.
* **@planner:** Arquitetura de software e elaboração de PRDs/SPECs.
* **@auditor:** Único bloqueador linear. Caça a bugs, regressões e quebras de `COSMOVISAO.md`.
* **@implementor:** A força bruta. Escrita de código pesado (JS, Next.js, CSS).
* **@verifier:** QA final, testes unitários.
* **@curator:** Ética, estética, copywriting (A voz do Poker Racional).
* **@validador:** Especialista em domínios específicos (matemática, direito, poker).
* **@securitychief:** Blindagem de dados e análise de vulnerabilidades.
* **@seo:** Otimização para motores de busca e ranqueamento (Novo).
* **@bibliotecario:** Compressão de memórias longas para evitar degradação da janela de tokens (Novo).
* **@organizador:** Manutenção de índices, diretórios e entropia do sistema.
* **@skillmaster:** Automações rotineiras, CRON e limpeza de logs.
* **@dispatcher:** Fatiador de Épicos. Quebra mega-tarefas em arrays JSON delegáveis.
* **@sequenciador:** Orquestrador de tráfego de tarefas paralelas.
...
## 4. AS TRÊS LEIS DE OURO (MANTENEDORES)
## 5. A CAMADA 24/7 E A TRÍADE DE GOVERNANÇA

1. **A Lei do Pure ASCII:** NUNCA utilize caracteres acentuados ou especiais em variáveis de inicialização (`init_*.ps1`). O PowerShell no Windows corrompe a passagem de Here-Strings com Encoding confuso. Use a injeção em SOTA Base64.
2. **A Lei da Autopoiese:** Nenhum agente morre sem refletir. Ao final de ciclos, os agentes atualizam seus próprios arquivos `MEMORY.md`, cristalizando sabedoria e evitando cometer os mesmos erros no futuro.
3. **A Lei da Assimetria de Confiança:** Você confia no `@auditor` para criticar o `@planner`, mas nunca confia no `@implementor` para auditar a si mesmo. O pipeline segue o fluxo contínuo.
...
2. **A Lei da Materialização Certa:** Agentes no *God Mode* nunca devem usar respostas do tipo "adicione isso na linha 15". Eles devem enviar o conteúdo **integral** do arquivo para o motor forjar a realidade sem atritos de merge.
...
*Assinado por CHICO | Córtex Administrativo SOTA.*
