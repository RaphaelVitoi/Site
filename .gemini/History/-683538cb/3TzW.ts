    ...
    try
    {
        ...
        try
        {
            ...
            if ( data?.nodes?.length > 0 )
            {
                ...
                return NextResponse.json( { nodes: data.nodes, edges: links, links }, { status: 200 } );
            }
        } catch ( pyErr )
        {
            console.warn( '[KG API] Base Ontologica (Python) indisponivel ou vazia. Acionando Fallback Holografico...', pyErr instanceof Error ? pyErr.message : String( pyErr ) );
        }
        ...
        for ( const [ source, target ] of Object.entries( handoff ) )
        {
            ...
            links.push( { id: `${source}_handoff_${targetStr}`, source_id: source, target_id: targetStr, source: source, target: targetStr, relation: 'handoff', weight: 2 } );
        }
