...
import { useFrictionlessWeb } from './useFrictionlessWeb';
...
const ExportAntevisaoButton: React.FC<ExportAntevisaoProps> = ( props ) => {
    ...
    const { share } = useFrictionlessWeb();
    ...
    const handleExport = async () => {
        try
        {
            ...
            try
            {
                if ( typeof File !== 'undefined' )
                {
                    ...
                    canShareFiles = typeof navigator.share === 'function' && !!navigator.canShare?.( shareData );
                }
            } catch ( e )
            {
                console.debug( '[CORTEX SHIELD] Compartilhamento nativo de arquivos incompatível no ambiente atual.', e );
            }
            ...
            if ( canShareFiles && shareData )
            {
                ...
                {
                    setStatus( 'idle' );
                    return; // Abortado pelo usuário, silenciamento (Fricção Zero)
                }
            } else
            {
        ...
        } catch ( error: unknown )
        {
            ...
            console.error( '[CORTEX SHIELD] Falha na exportação OG:', errorMessage );
            setStatus( 'error' );
