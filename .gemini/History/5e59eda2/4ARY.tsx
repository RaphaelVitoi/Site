...
export function DownwardDriftSimulator() {
  ...
  const [multiwayRioMatrix, setMultiwayRioMatrix] =
  ...
  const { potEntrapment, chartData, crossoverPoint } = useMemo(() => {
    ...
    const entrapment =
      (physics.heroInvested + betToCall) / Math.max(0.1, physics.heroStack);
    ...
    for (let invested = 0; invested <= physics.heroStack; invested += 2) {
      ...
      if (multiwayRioMatrix) {
        const spr = Math.max(
          ...
          Math.min(
            sprLevels,
            Math.round(
              (physics.heroStack - invested) /
                Math.max(0.1, physics.pot + invested),
            ),
          ),
        );
      ...
      const fakeUtilityEv = calculateUtilityEV(
        fakeChipEv,
      ...
      data.push({
        ...
        threshold: Math.min(
          50,
          20 + (invested / (physics.pot + invested)) * 30,
        ),
      });
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 relative z-10">
        <div>
          <h2 className="text-xl font-black text-text-bright uppercase tracking-tighter flex items-center gap-3">
            <i className="fa-solid fa-chart-line text-accent-indigo" /> Downward
            Drift &amp; Entrapment
          </h2>
