...
export function DownwardDriftSimulator() {
    ...
    const activeTension = isOop ? calculateRioTension( physics.heroInvested, physics.pot, physics.heroStack, 'OOP', baseRioLiability ) : calculateRioTension( physics.heroInvested, physics.pot, physics.heroStack, 'IP', baseRioLiability );
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 relative z-10">
                <div>
                    <h2 className="text-xl font-black text-text-bright uppercase tracking-tighter flex items-center gap-3">
                        ...
                        { ' ' }Downward Drift &amp; Entrapment
                    </h2>
                    <p className="text-[0.65rem] text-text-muted font-mono uppercase tracking-widest mt-1 opacity-60">Sincronia Quantum Ativa</p>
                </div>
                <div className="flex gap-4">
                    <div className="bg-bg-deep border border-white/5 px-4 py-2 rounded-xl">
                        <span className="text-label text-accent-emerald">Tensão Ativa: { ( activeTension * 100 ).toFixed( 1 ) }%</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div className="space-y-6 bg-black/20 p-6 rounded-2xl border border-white/5">
                    <div className="space-y-2">
                        <div className="flex justify-between items-end mb-1">
                            ...
                            <span className="text-sm font-black font-mono text-text-bright">{ physics.heroStack }bb</span>
                        </div>
                        <input aria-label="Stack do Hero" type="range" min="10" max="100" value={ physics.heroStack } onChange={ e => updatePhysics( { heroStack: Number( e.target.value ) } ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-indigo cursor-pointer" />
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between items-end mb-1">
                            <span className="text-label opacity-50">Sunk Cost (Investido)</span>
                            <span className="text-sm font-black font-mono text-accent-amber">{ physics.heroInvested }bb</span>
                        </div>
                        <input aria-label="Sunk Cost (Investido)" type="range" min="0" max={ physics.heroStack } value={ physics.heroInvested } onChange={ e => updatePhysics( { heroInvested: Number( e.target.value ) } ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-amber cursor-pointer" />
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between items-end mb-1">
                        ...
                        <input aria-label="Jogadores (Multiway N²)" type="range" min="2" max="9" value={ activePlayers } onChange={ e => setActivePlayers( Number( e.target.value ) ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-danger cursor-pointer" />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-bg-deep/40 p-6 rounded-2xl border border-white/5 text-center flex flex-col justify-center items-center">
                        <span className="text-[0.5rem] text-text-darker font-black uppercase tracking-widest mb-2">Entrapment Ratio</span>
                        <span className="text-3xl font-black text-accent-indigo font-heading tracking-tighter">{ ( potEntrapment * 100 ).toFixed( 1 ) }%</span>
                    </div>
                    <div className="bg-bg-deep/40 p-6 rounded-2xl border border-white/5 text-center flex flex-col justify-center items-center">
                        <span className="text-[0.5rem] text-text-darker font-black uppercase tracking-widest mb-2">Pot Size</span>
                        <span className="text-3xl font-black text-text-bright font-heading tracking-tighter">{ physics.pot }bb</span>
                    </div>
                </div>
            </div>
                ...
                <div className="relative h-72 w-full min-h-72">
                    <ResponsiveContainer width="100%" height="100%" minWidth={10} minHeight={10}>
                        <LineChart data={ chartData }>
                            ...
                            <RechartsTooltip
                                contentStyle={ { backgroundColor: 'var(--bg-panel)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px' } }
                            />
                            { crossoverPoint !== null && (
                                <ReferenceLine x={ crossoverPoint } stroke="var(--accent-danger)" strokeDasharray="4 4">
                                    <Label value="COLAPSO OOP" position="insideTopLeft" fill="var(--accent-danger)" fontSize={ 9 } fontWeight={ 900 } />
                                </ReferenceLine>
                            ) }
