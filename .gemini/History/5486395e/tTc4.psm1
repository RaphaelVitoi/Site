...
function Invoke-MemoryAudit {
    ...
    # Get all MEMORY.md files
    $MemoryFiles = Get-ChildItem -Path $MemoryDir -Filter "MEMORY.md" -Recurse

    # Check compliance for each MEMORY.md file
    $ComplianceResults = foreach ($File in $MemoryFiles) {
        $FilePath = $File.FullName
        Write-Host "Analisando: $FilePath" -ForegroundColor Cyan

        # Read the content of the MEMORY.md file
        try {
            $Content = Get-Content -Path $FilePath -Raw
        }
        catch {
            Write-Error "Não foi possível ler o arquivo: $FilePath"
            continue
        }
        ...
        if ($MissingSections.Count -gt 0) {
            Write-Warning "Arquivo faltando seções: $($MissingSections -join ', ')"
            Write-Warning "Arquivo em $($File.Directory.Name) faltando seções: $($MissingSections -join ', ')"
            Add-CortexValidationLog -Success $false -Details "MEMORY.md faltando seções: $($MissingSections -join ', ')"
        }

        # Check for correct tagging system
        if ($Content -notmatch "`#padrao") {
            if ($Content -notmatch "#padrao") {
                Write-Warning "Sistema de tagging ausente ou incorreto."
                Add-CortexValidationLog -Success $false -Details "Sistema de tagging ausente ou incorreto."
            }
        }

        Write-Host "`n=== AUDITORIA DE MEMÓRIA CONCLUÍDA ===" -ForegroundColor Green
    }
