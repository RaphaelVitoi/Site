    ...
    return [ordered]@{
            ...
            Invoke-WithMutex {
                ...
                if (Test-Path $Script:QueuePath) {
                    try {
                        ...
                        # AUTOPOIESE: Detecção de Entropia e Auto-Cura
                        ...
                        $msg = "[KERNEL][AUTO-HEALING] Corrupção detectada. Entropia isolada em '$corruptFile'. Sistema regenerado. Erro: $($_.Exception.Message) - $($_.Exception.StackTrace)"
                        $msg | Add-Content -Path $Script:LogPath
                        $queue = @()
                    }
                }
        
                # Lógica Upsert: Remove versão anterior da tarefa (se existir) para permitir atualização de status
        ...
        function Invoke-TaskCleanup {
            ...
            Invoke-WithMutex {
                ...
                # Priorizar a manutenção de tarefas de agentes críticos (ex: @maverick)
                ...
                if ($toArchive.Count -gt 0) {
                    ...
                    $queueWrapper = [ordered]@{ version = "4.0"; updatedAt = (Get-Date -Format "o"); checksum = $keepChecksum; tasks = $toKeep }
