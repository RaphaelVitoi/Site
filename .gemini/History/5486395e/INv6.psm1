...
# Inicializacao Segura de Caminhos (Fallback para evitar erro de Path Nulo)
$Script:QueuePath = if ($Global:AgentPaths.Queue) { $Global:AgentPaths.Queue } else { Join-Path $ModuleBase "queue\tasks.db" }
...
# Garante que o diretorio de log existe antes de tentar o Join-Path
if (-not (Test-Path $Script:LogPath)) { New-Item -ItemType Directory -Path $Script:LogPath -Force | Out-Null }
$Script:KernelLogPath = Join-Path $Script:LogPath "kernel.log"
...
function Test-TaskSchema {
    ...
    if ($TaskPayload.status -notmatch '^(pending|running|completed|failed|cancelled)$') { return $false }
    return $true
}
...
function Add-AgentTask {
    ...
    if (Test-Path $pyScript) {
        ...
        # OTIMIZACAO SOTA: Tentativa Zero-Latency via API Local
        $apiPort = 17042
        $apiUrl = "http://127.0.0.1:$apiPort/add"
        
        try {
            $response = Invoke-RestMethod -Uri $apiUrl -Method Post -Body $taskJson -ContentType "application/json" -ErrorAction Stop
            Invoke-WithMutex {
                $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
                try {
                    $logDir = Split-Path $Script:LogPath
                    if (-not (Test-Path $logDir)) { New-Item -ItemType Directory -Path $logDir -Force | Out-Null }
                    "[$timestamp][ENQUEUED] $($NewTask.id) - $($NewTask.status) (API Zero-Latency)" | Add-Content -Path $Script:KernelLogPath -ErrorAction Stop
                }
                catch {}
            }
            return
            ...
            # Fallback (Worker Offline): Executa Cold Start via CLI Python
            $pythonCmd = "python"
            ...
            Invoke-WithMutex {
                ...
                try {
                    $logDir = Split-Path $Script:LogPath
                    if (-not (Test-Path $logDir)) { New-Item -ItemType Directory -Path $logDir -Force | Out-Null }
                    "[$timestamp][ENQUEUED] $($NewTask.id) - $($NewTask.status) (CLI Fallback)" | Add-Content -Path $Script:KernelLogPath -ErrorAction Stop
                }
                catch {}
            }
        }
...
function Invoke-MemoryAudit {
    Write-Host "=== INICIANDO AUDITORIA DE MEMORIA DOS AGENTES ===" -ForegroundColor Green
    ...
    foreach ($File in $MemoryFiles) {
        ...
        try {
            ...
            Write-Warning "Nao foi possivel ler o arquivo: $FilePath"
            continue
        }
        ...
        if (-not $IsFileOk) {
            $NonCompliantCount++
            Add-CortexValidationLog -Success $false -Details "MEMORY.md nao conforme: $($File.Directory.Name)"
        }
    }
    Write-Host "=== AUDITORIA CONCLUIDA: $NonCompliantCount arquivos com problemas. ===" -ForegroundColor Green
}
...
function Add-CortexValidationLog {
    ...
    Invoke-WithMutex {
        ...
        try {
            $msg | Add-Content -Path $Script:KernelLogPath
        }
        catch {}
    }
}
