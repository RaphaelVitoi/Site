    ...
    if (Test-Path $pyScript) {
        ...
        Invoke-WithMutex {
            ...
            try {
                ...
                "[$timestamp][ENQUEUED] $($NewTask.id) - $($NewTask.status) (SQLite)" | Add-Content -Path $Script:KernelLogPath -ErrorAction Stop
            }
            catch {}
        }
        return
    ...
    if (Test-Path $pyScript) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($jsonOutput)) {
            try {
                ...
                return @($queue)
                $apiPort = 17042
                $apiUrl = "http://127.0.0.1:$apiPort/status?status=$arg"
