...
function Add-AgentTask {
    ...
    if (Test-Path $pyScript) {
        Invoke-WithMutex {
            ...
            # Preserva as aspas duplas ao transpor a barreira do PowerShell para o Python
            $taskJsonEscaped = $taskJson.Replace('"', '\"')
            
            $PythonCmd = "python"
            $VenvPython = Join-Path $PSScriptRoot ".venv\Scripts\python.exe"
            if (Test-Path $VenvPython) { $PythonCmd = $VenvPython }
            
            $output = & $PythonCmd $pyScript add $taskJsonEscaped
