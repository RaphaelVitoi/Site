  ...
  if (isCompleted) {
    return (
      <motion.div
        ...
        {...({
          className:
            "glass-panel p-8 lg:p-10 rounded-4xl bg-bg-panel/80 border border-white/10 shadow-2xl text-center relative overflow-hidden",
        } as any)}
      >
        <div className="absolute inset-0 bg-radial-[at_center] from-accent-indigo/10 to-transparent pointer-events-none" />
        <h2 className="text-xl font-black text-white tracking-widest uppercase mb-4 relative z-10">
          Diagnóstico Concluído
        </h2>
        <p className="text-xs text-text-muted mb-4 uppercase tracking-[0.2em] relative z-10">
          Penalidade Termodinâmica (EV Loss)
        </p>
        <div
          className={`text-5xl font-mono font-black relative z-10 ${evLossAccumulated > 0.5 ? "text-accent-danger drop-shadow-[0_0_15px_rgba(225,29,72,0.5)]" : "text-accent-emerald drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]"}`}
        >
        ...
        <p className="text-[0.65rem] text-text-darker mt-6 max-w-sm mx-auto leading-relaxed relative z-10">
          A sobrevivência no Poker exige a preservação do valuation da stack. Um
          EV Loss alto indica falhas na leitura das Reverse Implied Odds ou da
          aversão à perda de ICM.
        </p>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div
      ...
      {...({
        ...
        className:
          "glass-panel p-8 lg:p-10 rounded-4xl border border-white/5 shadow-2xl relative overflow-hidden group/quiz",
      } as any)}
    >
              ...
              <motion.button
                ...
                {...({
                  onClick: () => {
                    ...
                    setTimeout(
                      nextQuiz,
                      visceralFeedback === "shake_fatal" ? 2000 : 1500,
                    );
                  },
                  disabled: visceralFeedback !== "idle",
                  className:
                    "text-left px-6 py-4 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-all text-sm font-bold text-text-muted hover:text-white group/btn disabled:opacity-50 disabled:cursor-not-allowed",
                } as any)}
              >
      ...
      {visceralFeedback !== "idle" && (
        <motion.div
          ...
          {...({
            className:
              "mt-6 p-4 rounded-xl bg-black/40 border border-white/5 relative z-10",
          } as any)}
        >
