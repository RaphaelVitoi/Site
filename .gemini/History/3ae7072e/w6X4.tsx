...
{
  data?: any; // Mantido por retrocompatibilidade temporária
  scenario?: Scenario;
}

export default function PerspectivePanel( { scenario }: PerspectivePanelProps )
{
  ...
  // --- Sincronização com o Cenário SOTA ---
  useEffect( () =>
  {
    if ( scenario && scenario.stacks.length >= 2 )
    {
      const count = scenario.stacks.length;
      setPlayerCount( count );
      setStacks( [ ...scenario.stacks, ...Array( 8 - count ).fill( 0 ) ] );

      const newNames = scenario.stacks.map( ( _, i ) => i === 0 ? 'IP (Agressor)' : i === 1 ? 'OOP (Defensor)' : `Jogador ${ i + 1 }` );
      setNames( [ ...newNames, ...Array( 8 - count ).fill( '' ) ] );

      const rpPrizes = scenario.ftPrizes && scenario.ftPrizes.length > 0 ? scenario.ftPrizes : scenario.prizes;
      if ( rpPrizes && rpPrizes.length > 0 ) setPrizes( [ ...rpPrizes ] );

      setHeroIdx( 0 );
      setVillainIdx( 1 );

      // Estima um Pot e Custo lógicos baseados na menor stack para não começar zerado
      const effStack = Math.min( scenario.stacks[ 0 ], scenario.stacks[ 1 ] );
      const newPot = Math.max( 2, Math.floor( effStack * 0.25 ) );
      setPotSize( newPot );
      setHeroCost( Math.max( 1, Math.floor( newPot / 2 ) ) );
    }
  }, [ scenario ] );
