...
function PerspectiveBlock ( { label, probs, color, prizes }: Readonly<{ label: string; probs: number[]; color: string; prizes: number[] }> ) {
  return (
    <div style={ { background: 'rgba(15,23,42,0.6)', border: `1px solid ${color}33`, borderRadius: '10px', padding: '0.65rem' } }>
      <div style={ { fontSize: '0.55rem', fontWeight: 900, color, textTransform: 'uppercase', marginBottom: '0.5rem' } }>{ label }</div>
      { prizes.slice( 0, 5 ).map( ( _, i ) => (
        <div key={ i } style={ { display: 'flex', alignItems: 'center', gap: '0.3rem', marginBottom: '0.2rem' } }>
          <span style={ { fontSize: '0.5rem', color: '#475569', width: '12px' } }>{ i + 1 }</span>
          <div style={ { flex: 1, height: '4px', background: '#1e293b', borderRadius: '2px' } }>
            <div style={ { height: '100%', width: `${( probs[ i ] || 0 ) * 100}%`, background: color, borderRadius: '2px' } } />
          </div>
          <span style={ { fontSize: '0.55rem', color, width: '30px', textAlign: 'right' } }>{ ( ( probs[ i ] || 0 ) * 100 ).toFixed( 0 ) }%</span>
        </div>
      ) ) }
    </div>
  );
...
export default function PerspectivePanel ( { initialStacks, initialPrizes, initialNames }: Readonly<PerspectivePanelProps> ) {
  const stacks = useMemo( () => initialStacks ?? [ 9.4, 52.4, 22.2, 7.0, 44.3, 24.3, 40.0, 13.4, 55.0 ], [ initialStacks ] );
  ...
  const [ realization, setRealization ] = useState( 1.0 );
  const [ edgeBase, setEdgeBase ] = useState( 1.2 );
  const [ potSize, setPotSize ] = useState( 7.5 );
  const [ heroCost, setHeroCost ] = useState( 3.75 );
  ...
  }, [ stacks, prizes, potSize, heroCost, winProb, realization, edgeBase, bountyValue, numPlayers, isNearPayjump, blindsRising ] );
  ...
  const chartData = useMemo( () => {
    ...
    for ( let p = 0; p <= 100; p += 5 )
    {
      ...
      const r = calculatePerspectivaVitoi( {
        ...
        realizationFactor: realization, edgeBase, bountyValue,
    ...
    return points;
  }, [ stacks, prizes, potSize, heroCost, realization, edgeBase, bountyValue, numPlayers, isNearPayjump, blindsRising ] );
    ...
    <div className={ styles.glassPanel } style={ PANEL_STYLE }>
      ...
      <div style={ { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem', background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '12px' } }>
        <div>
          ...
          <input type="range" min="2" max="5" step="1" value={ numPlayers } onChange={ ( e ) => setNumPlayers( parseInt( e.target.value ) ) } style={ { width: '100%', accentColor: '#f43f5e' } } />
        </div>
        ...
        <div>
          ...
          <input type="range" min="0" max="0.1" step="0.005" value={ bountyValue } onChange={ ( e ) => setBountyValue( parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: '#fbbf24' } } />
        </div>
        ...
        <div>
          ...
          <input type="range" min="0" max="1" step="0.01" value={ winProb } onChange={ ( e ) => setWinProb( parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: '#6366f1' } } />
        </div>
        <div>
          <label style={ { fontSize: '0.45rem', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 900 } }>Pot ({ potSize }bb)</label>
          <input type="range" min="2" max="100" step="0.5" value={ potSize } onChange={ ( e ) => setPotSize( parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: '#f59e0b' } } />
        </div>
        <div>
          <label style={ { fontSize: '0.45rem', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 900 } }>Realização R ({ realization }x)</label>
          <input type="range" min="0.5" max="1.5" step="0.05" value={ realization } onChange={ ( e ) => setRealization( parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: '#10b981' } } />
        </div>
      </div>
      ...
      <div style={ { display: 'flex', flexDirection: 'column', gap: '0.4rem' } }>
        ...
        <div style={ { ...LAYER_STEP, borderLeftColor: '#f59e0b' } }>
          <div>
            <span style={ { fontSize: '0.5rem', fontWeight: 900, color: '#f59e0b', display: 'block' } }>LAYER 2: LÓGICA (VALUATION & RIO)</span>
        ...
        <div style={ { ...LAYER_STEP, borderLeftColor: '#10b981' } }>
          <div>
            <span style={ { fontSize: '0.5rem', fontWeight: 900, color: '#10b981', display: 'block' } }>LAYER 3: EXPECTATIVA (PISO DINÂMICO)</span>
