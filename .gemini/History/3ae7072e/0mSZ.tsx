...
export default function PerspectivePanel ( { initialStacks, initialPrizes, anteSize = 12.5, heroInvestedBb = 1.0, currentPotBb = 2.5 }: Readonly<PerspectivePanelProps> ) {
  ...
  return (
    <div className={ styles.glassPanel } style={ PANEL_STYLE }>
      ...
      <div style={ { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem', background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '12px' } }>
        <div>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900 } }>Oponentes ({ numPlayers })</label>
          <input type="range" min="2" max="5" step="1" value={ numPlayers } onChange={ ( e ) => setNumPlayers( Number.parseInt( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-danger)' } } />
        </div>
        <div style={ { display: 'flex', flexDirection: 'column', gap: '0.2rem', justifyContent: 'center' } }>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '0.4rem' } }>
        ...
        <div>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900 } }>PKO Bounty ({ Math.round( bountyValue * 100 ) }%)</label>
          <input type="range" min="0" max="0.1" step="0.005" value={ bountyValue } onChange={ ( e ) => setBountyValue( Number.parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-gold)' } } />
        </div>

        <div>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900 } }>Credibilidade κ ({ Math.round( kappa * 100 ) }%)</label>
          <input type="range" min="0" max="1" step="0.05" value={ kappa } onChange={ ( e ) => setKappa( Number.parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-pink)' } } />
        </div>

        <div style={ { gridColumn: 'span 3', height: '1px', background: 'rgba(255,255,255,0.05)', margin: '0.2rem 0' } } />

        <div>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900 } }>Equity ({ Math.round( winProb * 100 ) }%)</label>
          <input type="range" min="0" max="1" step="0.01" value={ winProb } onChange={ ( e ) => setWinProb( Number.parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-indigo)' } } />
        </div>
        <div>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900 } }>Sunk Cost / Pot</label>
        ...
        <div>
          <label style={ { fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 900 } }>Realização R ({ realization }x)</label>
          <input type="range" min="0.5" max="1.5" step="0.05" value={ realization } onChange={ ( e ) => setRealization( Number.parseFloat( e.target.value ) ) } style={ { width: '100%', accentColor: 'var(--accent-emerald)' } } />
        </div>
      </div>
