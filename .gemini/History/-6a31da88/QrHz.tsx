  ...
  if (isDeathZone) {
    return (
      <>
        ...
        />
        <span
          className={`text-[0.5rem] font-black uppercase tracking-[0.15em] mt-1 ${strokeClass}`}
        >
          CRITICAL
        </span>
      </>
  ...
  return (
    <>
      <div className="flex items-baseline leading-none">
        ...
        <span
          className={`font-mono text-[0.85rem] font-extrabold leading-none ml-0.5 ${isCritical ? "text-accent-danger" : "text-text-muted"}`}
        >
          %
        </span>
      </div>
      <span
        className={`text-[0.6rem] font-extrabold uppercase tracking-[0.15em] mt-0.5 ${strokeClass}`}
      >
        RP
      </span>
    </>
...
export default function RiskGauge({
  ...
  const { startColor, endColor, strokeClass, textShadowClass, glowColor } =
    getGaugeColors(isDeathZone, isPredatorZone, isCritical, rpDiff);
  ...
  if (isDeathZone) {
    gaugeFilter = [
      ...
      `drop-shadow(0 0 30px ${startColor})`,
      `drop-shadow(0 0 12px ${glowColor})`,
    ];
  } else if (isPredatorZone) {
    gaugeFilter = [
      `drop-shadow(0 0 10px ${glowColor})`,
      `drop-shadow(0 0 20px ${startColor})`,
      `drop-shadow(0 0 10px ${glowColor})`,
    ];
  }
  ...
  return (
    <div className="flex flex-col items-center font-sans group/gauge">
      ...
      <div className="text-center z-10 relative bg-slate-900/40 p-4 rounded-3xl border border-white/5 shadow-inner backdrop-blur-xl group-hover/gauge:bg-slate-900/60 transition-colors w-full">
        <div className="text-[10px] font-black uppercase tracking-[0.25em] text-text-muted mb-1">
          {label}
        </div>
        <div className="font-serif text-3xl font-black text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)] tracking-tight">
          {pos}
        </div>
        <button
