...
from core.config import PROTECTED_AGENTS_FROM_CLEANUP, VALID_AGENTS
...
class QueueManager:
    ...
    def __init__(self, queue_path: str = None):
        ...
        if not self._is_memory:
            ...
            try:
                # self.db_path is already an absolute path from _ensure_writable_db_path
                self.db_path.relative_to(base_path)
            except ValueError:
    ...
    def _ensure_writable_db_path(self, desired_path: Path) -> Path:
        ...
        try:
            # SOTA: Pathlib's relative_to is the canonical and safest way to check for subpaths.
            # It correctly handles case-insensitivity on Windows and other path nuances.
            desired_path.resolve(strict=False).relative_to(self.base_path.resolve())
        except ValueError:
            # This error means desired_path is not within the project's base path.
    ...
    async def update_task_metadata(self, task_id: str, metadata_patch: Dict[str, Any], merge: bool = True):
    ...
    async def cleanup(self, days: int = 30):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            deleted_rows = await cursor.rowcount
            await db.commit()
            logging.info(f"[CLEANUP] {deleted_rows} tarefas antigas arquivadas e removidas do banco de dados.")

        logging.info(f"[CLEANUP] Iniciando varredura de arquivos obsoletos (mais de {days} dias)...")
