    ...
    <GlassPanel className="flex flex-col gap-4 p-8">
      <div className="flex justify-between items-center pb-3 border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[0.55rem] font-mono text-accent-indigo-light uppercase tracking-widest bg-accent-indigo/10 px-2 py-0.5 rounded border border-accent-indigo/20 shadow-inner">
                ...
                >
                    ...
                    { isCustom ? (
                      <div className="flex justify-end items-center gap-1.5">
                        <input
                          ...
                          aria-label={`Prêmio da Posição ${p.place}`}
                          title={`Prêmio da Posição ${p.place}`}
                          placeholder="%"
                        />
                  ...
                  <td className="px-4 py-3">
                    <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden w-full max-w-30 ml-auto relative">
                      <div
                        className={ `absolute inset-0 w-full h-full transition-transform duration-1000 ease-out origin-left ${ p.place === 1
                          ? 'bg-linear-to-r from-accent-amber to-accent-gold'
                          : 'bg-linear-to-r from-accent-indigo to-accent-indigo-light'
                          }` }
                        style={ { transform: `translateX(-${ 100 - Math.min( p.percent * 2, 100 ) }%)` } }
                      />
                    </div>
                  </td>
                </tr>
