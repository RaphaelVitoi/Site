"use client";
/**
 * IDENTITY: Visualizador de Estrutura de Payouts SOTA v4.6 GOLD
  ...
  if (active && payload && payload.length > 0) {
    ...
    return (
      <div className="p-3 bg-slate-950/90 border border-white/10 rounded-xl shadow-2xl backdrop-blur-md">
        <p className="text-[0.6rem] font-black text-text-muted uppercase tracking-widest mb-1">
          {data.pos}º Lugar
        </p>
        <p className="text-[0.8rem] font-mono font-black text-accent-indigo-light">
    ...
    <div className="glass-panel p-6 sm:p-8 lg:p-12 animate-sota-in flex flex-col gap-10 rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl relative overflow-hidden transition-all duration-300">
      ...
      <div className="flex justify-between items-start border-b border-white/5 pb-6">
        <div>
          ...
          <p className="m-0 mt-1.5 text-[0.6rem] text-text-dim font-medium uppercase tracking-wider">
            Distribuição do Pool de Premiação
          </p>
        </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 overflow-x-auto scrollbar-hide pb-2">
          {payouts.map((p, i) => (
            <div
              key={`payout-${i}` /* NOSONAR */}
              className="p-4 bg-black/40 border border-white/5 rounded-2xl flex flex-col gap-1 transition-all hover:bg-black/60 hover:border-accent-indigo/20 group"
            >
              <span className="text-[0.5rem] font-black text-text-darker uppercase tracking-widest group-hover:text-text-muted transition-colors">
                {i + 1}º LUGAR
              </span>
              <div className="flex items-baseline gap-1">
                <span className="text-[0.9rem] font-mono font-black text-white">
                  {p.toFixed(1)}
                </span>
                <span className="text-[0.55rem] font-black text-text-muted">
                  %
                </span>
              </div>
            </div>
