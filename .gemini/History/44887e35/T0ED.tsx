...
export default function GtoCfrLabPage() {
    ...
    const geometricFraction = useMemo( () => {
        ...
        const base = Math.pow( growthFactor, 1.0 / streets );
        return ( base - 1.0 ) / 2.0;
                    ...
                    <GlassPanel className="p-8 border-accent-indigo/20">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                <div>
                                    <label className="text-label text-text-muted">Pote Atual (BB)</label>
                                    <input type="number" value={ currentPot } onChange={ ( e ) => setCurrentPot( Number( e.target.value ) ) } className="w-full bg-bg-deep border border-white/5 rounded-lg p-3 text-text-bright font-mono mt-2 focus:ring-1 focus:ring-accent-indigo outline-none" />
                                </div>
                                <div>
                                    <label className="text-label text-text-muted">Pote Alvo / SPR Alvo (BB)</label>
                                    <input type="number" value={ targetPot } onChange={ ( e ) => setTargetPot( Number( e.target.value ) ) } className="w-full bg-bg-deep border border-white/5 rounded-lg p-3 text-text-bright font-mono mt-2 focus:ring-1 focus:ring-accent-indigo outline-none" />
                                </div>
                                <div>
                                    <label className="text-label text-text-muted">Streets Restantes (N)</label>
                                    <input type="number" min="1" max="3" value={ streets } onChange={ ( e ) => setStreets( Number( e.target.value ) ) } className="w-full bg-bg-deep border border-white/5 rounded-lg p-3 text-text-bright font-mono mt-2 focus:ring-1 focus:ring-accent-indigo outline-none" />
                                </div>
                            </div>
                ...
                <section>
                    ...
                    <GlassPanel className="p-8 border-accent-emerald/20">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                <div>
                                    <label className="text-label text-text-muted">Arrependimento (Fold)</label>
                                    <input type="number" value={ regrets.fold } onChange={ ( e ) => setRegrets( { ...regrets, fold: Number( e.target.value ) } ) } className="w-full bg-bg-deep border border-white/5 rounded-lg p-3 text-text-bright font-mono mt-2 focus:ring-1 focus:ring-accent-emerald outline-none" />
                                </div>
                                <div>
                                    <label className="text-label text-text-muted">Arrependimento (Call)</label>
                                    <input type="number" value={ regrets.call } onChange={ ( e ) => setRegrets( { ...regrets, call: Number( e.target.value ) } ) } className="w-full bg-bg-deep border border-white/5 rounded-lg p-3 text-text-bright font-mono mt-2 focus:ring-1 focus:ring-accent-emerald outline-none" />
                                </div>
                                <div>
                                    <label className="text-label text-text-muted">Arrependimento (Raise)</label>
                                    <input type="number" value={ regrets.raise } onChange={ ( e ) => setRegrets( { ...regrets, raise: Number( e.target.value ) } ) } className="w-full bg-bg-deep border border-white/5 rounded-lg p-3 text-text-bright font-mono mt-2 focus:ring-1 focus:ring-accent-emerald outline-none" />
                                </div>
                            </div>
