...
export function BayesianRangeGrid() {
  ...
  return (
    <div className="flex flex-col items-center gap-2 p-4 bg-bg-panel/80 rounded-2xl border border-white/10">
      ...
      <div className="grid grid-cols-13 gap-[1px] bg-black/50 p-1 border border-white/5 rounded-lg w-full max-w-sm">
          ...
          row.map((freq: number, c: number) => {
            ...
            return (
              <motion.div
                ...
                className={`aspect-square flex items-center justify-center text-[0.45rem] font-bold rounded-[2px]
                  ${isPair ? 'bg-accent-emerald text-black' : isSuited ? 'bg-accent-indigo text-white' : 'bg-white/20 text-white'}`}
