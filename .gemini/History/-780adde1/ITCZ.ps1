...
try {
    ...
    $recentActivityJson = & $PythonCmd $Executor db-get all --since 24h --json
    $recentTasks = if ($recentActivityJson) { $recentActivityJson | ConvertFrom-Json } else { @() }

    if ($recentTasks.Count -eq 0) {
        Write-Host '[ECONOMIA GENERALIZADA] Nenhuma atividade de tarefas detectada nas últimas 24 horas. O backup diário não é necessário.' -ForegroundColor Green
        exit 0
    }
...
$taskId = "BACKUP-DAILY-$(Get-Date -Format 'yyyyMMdd')"
$taskDescription = @"
DIRETRIZ DE MANUTENCAO SOTA:
Execute um backup online do banco de dados para garantir a integridade dos dados e a longevidade do ecossistema.
