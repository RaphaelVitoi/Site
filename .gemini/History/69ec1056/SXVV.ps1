...
$QueuePath = Join-Path $ProjectRoot "queue\tasks.db" # Apontando para o banco de dados SQLite
...
# Queue Status
$pending = 0; $running = 0
# A logica de contagem agora chama o task_executor.py para obter dados do SQLite
$PythonExe = if (Test-Path "$ProjectRoot\.venv\Scripts\python.exe") { "$ProjectRoot\.venv\Scripts\python.exe" } else { "python.exe" }
$TaskExecutorPath = Join-Path $ProjectRoot "task_executor.py"

try {
    $countsJson = & $PythonExe $TaskExecutorPath db-get counts
