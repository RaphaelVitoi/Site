...
7.  `@dispatcher` — Fase ENTRADA (Triagem de Backlog)
8.  `@architect` — Fase 0.5 (Visão Arquitetural de Alto Nível)
9.  `@pesquisador` — Fase 1 (Exploração especializada de dados/terreno)
10. `@prompter` — Fase 2 (Estruturação do prompt ótimo)
11. `@planner` — Fase 3 (Investigação e criação cirúrgica de PRD/SPEC)
12. `@auditor` — Fase 4 (Paranoia técnica. Único com poder de bloqueio de SPEC)
13. `@implementor` — Fase 5 (Forja de código de produção via God Mode)
14. `@verifier` — Fase 6 (QA final, detecção de bugs, integridade do ecossistema)
