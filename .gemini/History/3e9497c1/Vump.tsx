...
const MOCK_QUESTIONS: QuizQuestion[] = [
  {
    ...
    options: [
      {
        id: "opt1",
        label: "Iterar sobre um array usando map() repetidas vezes.",
      },
      { id: "opt2", label: "Buscar um item no array usando a função find()." },
      {
        id: "opt3",
        label:
          "Acessar a resposta diretamente via chave num Dicionário (Record).",
      },
      {
        id: "opt4",
        label: "Usar múltiplos loops aninhados para calcular o placar.",
      },
    ],
  ...
  },
  {
    ...
    options: [
      { id: "opt1", label: "Para deixar as fontes com cores dinâmicas." },
      {
        id: "opt2",
        label:
          'Evita que os números "pulem" ou tremam durante mudanças de estado.',
      },
      {
        id: "opt3",
        label: "Para aumentar o tamanho da fonte automaticamente.",
      },
...
export default function QuizPage() {
  const { track } = useSotaTelemetry("StandaloneQuiz");
    ...
    <div className="min-h-screen bg-bg-base text-text-bright overflow-x-hidden font-body pb-24">
      ...
      <div className="sota-container mt-8">
        <QuizEngine
          ...
          onAnswer={(isCorrect, evLoss, category) => {
            track(category || "quiz", {
              isCorrect,
              evLoss,
            });
