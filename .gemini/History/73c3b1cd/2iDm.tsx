...
export default function DegradationChart ( { sEff, riskAversion }: DegradationChartProps ) {
    ...
    const data = useMemo( () => {
        return [ 2, 3, 4, 5, 6 ].map( ( players ) => {
            ...
            const rio = players < 3 ? 0.0 : 0.5 * Math.pow( players, 1.5 ) * riskAversion;

            // 3. Amortizacao da Edge (Colapso da arvore mecanica)
            const edge = sEff <= 15 ? 0.0 : sEff >= 100 ? 1.0 : Math.log( sEff - 14 ) / Math.log( 86 );

            // 4. Utilidade Bruta e Perspectiva Matematica VITOI v3.2
            const rawUtility = 1.0 + evFold - rio;
            const perspectiva = rawUtility * ( 0.5 + 0.5 * edge );
