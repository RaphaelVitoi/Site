<#
.SYNOPSIS
    Sincroniza a realidade das pastas com os 17 agentes listados no Manifesto SOTA.
.DESCRIPTION
...
Write-Host "=== VERIFICANDO ALINHAMENTO FRACTAL DOS 17 AGENTES ===" -ForegroundColor Cyan

foreach ($Agent in $AgentNames) {
    $AgentProps = $Manifest.$Agent
    $Color = if ($AgentProps.color) { $AgentProps.color } else { "white" }
    $Model = if ($AgentProps.gemini_model) { $AgentProps.gemini_model } else { "gemini-2.5-flash" }
    ...
    if (-not (Test-Path $AgentDocPath)) {
        ...
        Write-Host "[+] Arquivo de base criado para @$Agent" -ForegroundColor Green
    }
