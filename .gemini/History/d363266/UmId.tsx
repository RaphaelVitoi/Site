...
export const metadata = {
  ...
  description:
    "Como o Risk Premium se dissipa conforme o SPR decresce. A física do ICM aplicada por street.",
};
      ...
      <div className="sota-container mb-24">
        <GlassPanel className="p-8 md:p-16 border-white/5">
          <div className="prose prose-invert prose-lg max-w-none text-text-muted">
            <p className="text-xl">
              O erro sistêmico do jogador comum é carregar a paralisia do ICM
              para o River. O{" "}
              <strong className="text-text-bright">Motor de Diluição</strong>{" "}
              prova que, conforme o SPR cai, a capacidade do ICM de distorcer as
              pot odds evapora.
            </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 my-16">
              {[
                ...
                },
                {
                  ...
                  rp: "<10%",
                  widthCls: "w-[10%]",
                  status: "Foco: Equidade Bruta",
                  color: "bg-accent-emerald",
                },
              ].map((s) => (
                <div
                  key={s.street}
                  className="relative p-8 rounded-3xl bg-white/3 border border-white/10 overflow-hidden group"
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 ${s.color} opacity-5 blur-3xl -mr-16 -mt-16 group-hover:opacity-20 transition-opacity`}
                  />
                  <span className="text-[0.5rem] font-black text-text-darker uppercase tracking-[0.3em] mb-2 block">
                    {s.street}
                  </span>
                  <h4 className="text-2xl font-black text-text-bright mb-4">
                    {s.spr}
                  </h4>
                  <div className="flex items-center gap-3 mb-6">
                    <div className="flex-1 h-1.5 rounded-full bg-white/5">
                      <div
                        className={`h-full rounded-full ${s.color} ${s.widthCls}`}
                      />
                    </div>
                    <span className="text-xs font-mono font-bold text-text-bright">
                      {s.rp} RP
                    </span>
                  </div>
            ...
            <p>
              Conforme as fichas saem do seu stack e entram no centro da mesa, o
              seu <strong>Valuation</strong> aumenta. O River é a zona de
              libertação: com um SPR de 0.5, o custo existencial já foi
              amortizado e a decisão converge para o equilíbrio de ChipEV.
            </p>

            <blockquote className="text-2xl font-light italic text-text-bright border-l-2 border-accent-indigo/30 pl-10 py-4 my-16">
              &quot;Quem tem medo de ICM no River está, na verdade, cego para a
              diluição do risco. A maestria exige saber quando parar de proteger
              e começar a realizar.&quot;
            </blockquote>
          </div>
        </GlassPanel>
      </div>
