...
def _read_global_context() -> str:
    ...
    if global_file.exists():
        with open(global_file, "r", encoding="ascii", errors="ignore") as f:
            return f.read() + "\n\n"
    return ""
...
def _build_infra_ctx(task: Task | None, task_files: list | None) -> str:
    ...
    for doc_name, doc_paths in docs_to_read:
        for doc_path in doc_paths:
            ...
            if file_obj.exists():
                with open(file_obj, "r", encoding="ascii", errors="ignore") as f:
...
async def _process_dispatcher_result(
    ...
    try:
        ...
        logger.exception(f"[{task.id}] Falha ao interpretar matriz do Dispatcher: {e}")
...
async def process_agent_task(task: Task, manager: QueueManager):
    ...
    try:
        ...
        logger.warning(f"Erro na Memoria Coletiva (RAG): {e}")
