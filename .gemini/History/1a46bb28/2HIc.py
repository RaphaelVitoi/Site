...
def _inject_task_docs_engine(task: Task) -> tuple[str, list[str]]:
        ...
        if task_dir.exists() and task_dir.is_dir():
            for doc_file in task_dir.glob("*.*"):
                if doc_file.suffix.lower() in [".md", ".json", ".txt"]:
                    task_docs = _read_and_append_doc(doc_file, "ARTEFATO", task_docs, successfully_read_task_files)
    ...
    for p in paths_to_check:
        ...
        if p.suffix.lower() not in valid_suffixes:
            continue
        task_docs = _read_and_append_doc(p, "ARTEFATO REFERENCIADO", task_docs, successfully_read_task_files)
