...
self.onmessage = (e) => {
    ...
    if (!regretSum || regretSum.length !== totalNodes * ACTIONS) {
    ...
    for (let i = 0; i < totalNodes; i++) {
        ...
        for (let a = 0; a < ACTIONS; a++) {
            currentStrategy[offset + a] = Math.max(regretSum[offset + a], 0);
        ...
        for (let a = 0; a < ACTIONS; a++) {
            if (normalizingSum > 0) {
                ...
                currentStrategy[offset + a] = 1.0 / ACTIONS; // Distribuição uniforme se arrependimento for negativo
            }
            strategySum[offset + a] += currentStrategy[offset + a];
        }
        ...
        for (let a = 0; a < ACTIONS; a++) {
            ...
            regretSum[offset + a] = (regretSum[offset + a] + regret) * kappa;
    ...
    self.postMessage({ id, matrix: renderMatrix });
};
