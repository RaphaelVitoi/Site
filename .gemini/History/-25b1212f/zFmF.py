...
def _parse_dispatcher_subtasks_strict(response_text: str) -> List[DispatcherSubtask]:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Falha irrevogavel no parser JSON estrutural: {str(e)}")
...
async def _retry_dispatcher_schema_once(
...
) -> Optional[List[DispatcherSubtask]]:
    ...
    logger.warning(f"[{task.id}] Retry unico do dispatcher ativado para corrigir schema JSON.")
    ...
    try:
        ...
        logger.error(f"[{task.id}] Retry do dispatcher nao normalizou schema: {retry_error}")
        return None
