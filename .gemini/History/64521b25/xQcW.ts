...
// SOTA: Conversor O(N) para transpilar a máscara de bits JS (BigInt) para o vetor de bytes (Uint8Array)
// exigido pela FFI do WebAssembly (Rust) em formato Little-Endian (166 bytes cobrem 1326 bits).
...
globalThis.onmessage = async ( e: MessageEvent ) => {
    ...
    try {
        if ( !initialized ) {
            ...
            await init( `/wasm/vitoi_equity_engine_bg.wasm${cacheBuster}` as any );
            initialized = true;
        }
