import { rangeToBitmask } from './rangeParser';
...
globalThis.onmessage = async ( e: MessageEvent ) => {
    ...
    try
    {
        if ( !wasmEngine )
        {
            ...
            // Garante que o `onmessage` seja registrado instantaneamente.
        ...
        const heroMask = rangeToBitmask( cleanHero );
        const villainMask = rangeToBitmask( cleanVillain );
        ...
        const equity = wasmEngine.calculate_equity_monte_carlo_binary( heroMask, villainMask, cleanBoard, iterations, seed );
