...
function _expandSingleRange ( r: string ): string {
    ...
    if ( base.length === 4 && !base.includes( '+' ) ) return base; // Exato (Ex: AhKh)
...
globalThis.onmessage = async ( e: MessageEvent ) => {
    ...
    try
    {
        if ( !initialized )
        {
            ...
            await init( { module_or_path: `/wasm/vitoi_equity_engine_bg.wasm${cacheBuster}` } );
            initialized = true;
        }
        ...
        // SOTA: TextEncoder para forjar o Uint8Array exigido pelo Rust/WASM (Fricção Zero)
        const encoder = new TextEncoder();
        const heroBytes = encoder.encode( cleanHero );
        const villainBytes = encoder.encode( cleanVillain );
        const boardBytes = encoder.encode( cleanBoard );
        ...
        const equity = calculate_equity_monte_carlo_binary( heroBytes, villainBytes, boardBytes, iterations, seed );
    ...
    } catch ( error: any )
    {
        ...
        globalThis.postMessage( { error: error.message || String( error ), id } ); // SOTA: Não sobrescreve a equidade em caso de falha matemática
    }
};
