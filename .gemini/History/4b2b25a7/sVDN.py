"""Módulo de configuração central SOTA."""
    ...
    try:
        ...
        if _CONFIG_MTIME.get("agents_manifest") != mtime:
            ...
            if new_manifest:
                ...
                logger.warning(
                    "[RESILIENCIA] %s vazio. Fallback via %s.",
            ...
            logger.info("[HOT-RELOAD] Agents Manifest. %d agentes.", len(VALID_AGENTS))
            return True
    except OSError:
        pass
    return False
...
def maybe_reload_config() -> bool:
    """Aciona hot-reload de manifesto e configurações se houver alteração."""
    with _CONFIG_LOCK:
...
def feature_enabled(flag_name: str) -> bool:
    """Verifica se uma flag de funcionalidade está ativa."""
...
def heuristic_terms(group_name: str) -> dict[str, int]:
    """Retorna os termos de roteamento heurístico associados ao grupo."""
...
def health_gate_value(key: str, default: Any) -> Any:
    """Extrai a heurística do health gate para roteamento dinâmico."""
...
def _flush_telemetry_buffer() -> None:
    with _TELEMETRY_LOCK:
        ...
        try:
            ...
            logger.exception(
                "[SOTA TELEMETRY] Falha catastrófica ao persistir telemetria: %s", e
            )
