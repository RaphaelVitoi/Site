...
try:
    ...
    try:
        ...
        # [SOTA Bypass] O Windows PT-BR corrompe a string do DXGI (ex: com 'ã').
        # O erro confirma que o backend nativo carregou com sucesso.
        DML_AVAILABLE = True
...
atefrom transformers import AutoModelForCausalLM, AutoTokenizer

from engine.math_sota import compute_quantum_metrics

MODEL_ID = "google/gemma-2-9b-it"  # Use "google/gemma-2-2b-it" caso falte VRAM
...
if torch.cuda.is_available():
    ...
    try:
        ...
        DEVICE = "privateuseone:0"  # Alias de baixo nível do plugin DirectML
    DTYPE = torch.float16  # DirectML tem melhor suporte para float16
    print("1. Carregando modelo e motor SOTA (Alvo: AMD DIRECTML)...")
else:
    DEVICE = "cpu"
    DTYPE = torch.bfloat16  # bfloat16 é mais rápido em CPUs modernas
