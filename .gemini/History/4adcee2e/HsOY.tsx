...
export default function LeituraICM () {
        ...
        <main className="container" style={ { maxWidth: '800px', margin: '0 auto', padding: '4rem 1.5rem' } }>
            ...
            <section style={ { marginBottom: '4rem' } }>
                ...
                { ( [
                    ...
                    { num: '06', id: 'modulo-5', label: 'SOTA', title: 'O Paradigma Vitoi', desc: 'EV do Fold Dinâmico, Insolvência das Pot Odds em Multiway e Amortização de Edge.' },
            ...
            <article>
                ...
                {/* ==================== MÓDULO 5 ==================== */ }
                <h2 id="modulo-5" style={ { scrollMarginTop: '5rem' } }>Módulo 5: O Paradigma Vitoi (A Perspectiva Matemática)</h2>

                <h3>5.1 O Axioma do EV do Fold Dinâmico</h3>
                <p>O EV do fold <strong>NUNCA é zero</strong> em torneios. Matematicamente, o fold é uma transação de capital onde você aceita uma perda garantida (Sunk Cost + Antes) para evitar uma perda incerta e potencialmente maior. O verdadeiro <em>baseline</em> de qualquer decisão é o custo de desistência.</p>
                <p>O threshold correto para um call ou raise não é "ter EV positivo", mas sim: <code style={ { color: 'var(--accent-indigo-light)' } }>Esperança(ação) &gt; EV_fold</code>.</p>

                <div className="callout callout-emerald">
                    <h4 style={ { color: 'var(--accent-emerald)', marginTop: 0 } }>Antevisão e Erosão (FGS t-3)</h4>
                    <p style={ { marginBottom: 0 } }>O modelo de FGS tradicional falha por ser cego ao relógio. Se os blinds sobem em 3 minutos, a sua stack atual perderá poder de compra no futuro imediato. O <strong>EV do fold torna-se violentamente mais negativo</strong>. A <em>Perspectiva Matemática</em> dita que, sob iminência de salto de blinds, você deve ser mais agressivo hoje, comprando potes "baratos" antes que a erosão destrua o seu valuation.</p>
                </div>

                <h3>5.2 O Colapso das Pot Odds em Potes Multiway</h3>
                <p>As Pot Odds são uma métrica linear importada de finanças básicas. Para o jogador de elite, elas são uma armadilha. Em cenários Multiway (3+ jogadores), a atratividade ilusória do pote mascara a explosão das <strong>Reverse Implied Odds (RIO)</strong>.</p>
                <p>Em potes MW, o risco de construir a <em>segunda melhor mão</em> cresce quadraticamente. As pot odds mentem ao oferecer um "preço barato" para entrar na mão, ignorando que o custo de saída nas streets finais sob pressão de ICM será catastrófico.</p>

                <h3>5.3 Amortização da Edge e Complexidade de Árvore</h3>
                <p>A vantagem técnica sobre um oponente (Edge Relativa) não é uma constante linear; ela é uma função dependente da profundidade da stack e da complexidade da árvore de decisão. <strong>A complexidade é a arma do forte; a simplicidade é o escudo do fraco.</strong></p>
                <ul style={ { marginLeft: '1.5rem', color: '#cbd5e1' } }>
                    <li><strong>100bb (Alta Resolução Sistêmica):</strong> A árvore de decisões é fractal. Há espaço para overbets, blockers específicos para triple barrels e linhas de check-raise polarizadas. A Oportunidade de Erro (Oe) do oponente é máxima, permitindo a plena realização da sua Edge.</li>
                    <li><strong>10bb (Colapso Binário):</strong> A árvore é podada para Push/Fold. O jogo se torna uma Matemática de Colisão Pura. A limitação de opções "protege" o amador da própria inabilidade pós-flop, neutralizando a vantagem de processamento do profissional.</li>
                </ul>

                <div style={ { background: 'rgba(15,23,42,0.5)', padding: '1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.08)', margin: '2rem 0' } }>
                    <h4 style={ { color: 'var(--accent-indigo-light)', marginTop: 0, marginBottom: '1rem', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.08em' } }>Topologia da Amortização da Edge</h4>
                    <pre className="mermaid" style={ { background: 'transparent', padding: 0, margin: 0, display: 'flex', justifyContent: 'center' } }>
                        { `graph TD
    A[Stack Efetivo] --> B{Ferramentas Disponíveis}
    B -->|100bb| C[3bet / Multi-barrel / Overbet]
    B -->|25bb| D[Open / Cbet / Shove]
    B -->|10bb| E[Push ou Fold]

    C --> F[Alta Oportunidade de Erro - Oe]
    D --> G[Média Oe]
    E --> H[Mínima Oe]

    F --> I[Máxima Realização de Edge]
    H --> J[Amortização de Edge pela Variância]
    ...
    style J fill:#7f1d1d,stroke:#ef4444,stroke-width:2px,color:#fff`}
                ...
                <p>
                    O verdadeiro erro de um Chip Leader na bolha não é o call matemático com EV marginal positivo; é o call que devolve a complexidade da árvore de decisão ao oponente que estava confinado à simplicidade. Manter um Short Stack vivo (dando fold) possui um
                    { ' ' }
                    <strong>EV Sistêmico Absurdo</strong>
                    , pois você preserva a pressão da mesa (Risk Premium alto para os intermediários) e não transfere &quot;ferramentas de erro&quot; para a mão de um amador ao dobrá-lo.
                </p>
            </article>
