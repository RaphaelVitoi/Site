...
const getIcmEquity = (
  ...
  const stacks = [
    Math.max(0, myStack),
    Math.max(0, otherStack1),
    Math.max(0, otherStack2),
  ];
...
const getAmortizedEdge = (
  ...
  const amortizedEdgeMultiplier =
    (varianceSigma / baseEdge) * Math.log10(sEff) * 1.5;
...
export default function ResurrectionRiskSimulator() {
  ...
  const results = useMemo(() => {
    ...
    const clEdgeLoss =
      getAmortizedEdge(clStackLoss, ssStackWin, edgeFactor) *
      psychologicalPenalty;
    const clEdgeFold = getAmortizedEdge(clStack - bb, msStack, edgeFactor);

    const futureEvWinVal =
      getIcmEquity(clStackWin, msStack, 0, prizes) * clEdgeWin;
    const futureEvLossVal =
      getIcmEquity(clStackLoss, ssStackWin, msStack, prizes) * clEdgeLoss;

    const msEliminatesSsProb = 0.35;
    const foldEvIfMsWins = getIcmEquity(
      clStack - bb,
      0,
      msStack + ssStack + bb,
      prizes,
    ...
    const foldFutureEv =
      (msEliminatesSsProb * foldEvIfMsWins +
        (1 - msEliminatesSsProb) * foldEvIfMsFolds) *
      clEdgeFold;
    const netFutureEv =
      (equity / 100) * futureEvWinVal +
      ((100 - equity) / 100) * futureEvLossVal;
    ...
    for (let i = 40; i <= 80; i++) {
      const currentNetEv =
        (i / 100) * futureEvWinVal + ((100 - i) / 100) * futureEvLossVal;
  ...
  }, [physics, equity, isHydrated, applyBubbleAxiom]);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-white/5 pb-8">
        <div className="space-y-2">
          ...
          <p className="text-xs text-text-muted leading-relaxed max-w-2xl font-medium">
            ...
            <span className="text-accent-indigo font-bold">EV Futuro</span>.
            Ressuscitar um oponente destrói a{" "}
            <strong className="text-white">Fear Equity</strong> da mesa. Sob o
            Axioma Psicológico, a sua Edge Relativa despenca se você sangrar o
            status de predador absoluto.
          </p>
        </div>
        <div
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-label opacity-50">
                ...
                <span>{physics.edgeFactor.toFixed(2)}x</span>
              </div>
              <input
                title="Edge Factor"
