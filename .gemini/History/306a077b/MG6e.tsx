...
export default function ResurrectionRiskSimulator ()
{
    ...
    const results = useMemo( () =>
    {
        ...
        const psychologicalPenalty = applyBubbleAxiom ? 0.8 : 1; // Punição de 20% na edge se o CL perder a Fear Equity
                ...
                <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/20 p-6 rounded-3xl border border-white/5">
                    <div className="space-y-4">
                        <div className="space-y-1">
                            ...
                            <input title="Chip Leader Stack" aria-label="Chip Leader Stack" type="range" min="50" max="200" value={ physics.heroStack } onChange={ e => updatePhysics( { heroStack: Number( e.target.value ) } ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-indigo cursor-pointer" />
                        </div>
                        <div className="space-y-1">
                            <div className="flex justify-between text-label opacity-50"><span>Short Stack (SS)</span><span>{ physics.villain1Stack }bb</span></div>
                            <input title="Short Stack" aria-label="Short Stack" type="range" min="5" max="30" value={ physics.villain1Stack } onChange={ e => updatePhysics( { villain1Stack: Number( e.target.value ) } ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-danger cursor-pointer" />
                        </div>
                    </div>
                    <div className="space-y-4">
                        <div className="space-y-1">
                            <div className="flex justify-between text-label opacity-50"><span>Equidade vs Range</span><span>{ equity }%</span></div>
                            <input title="Equidade vs Range" aria-label="Equidade vs Range" type="range" min="40" max="80" value={ equity } onChange={ e => setEquity( Number( e.target.value ) ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-emerald cursor-pointer" />
                        </div>
                        <div className="space-y-1">
                            ...
                            <input title="Edge Factor" aria-label="Edge Factor" type="range" min="0.9" max="1.5" step="0.05" value={ physics.edgeFactor } onChange={ e => updatePhysics( { edgeFactor: Number( e.target.value ) } ) } className="w-full h-1 bg-white/10 rounded-full appearance-none accent-accent-amber cursor-pointer" />
                        </div>
