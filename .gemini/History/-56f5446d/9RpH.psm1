...
if ($Script:AgentStats -eq $null) {
    ...
    function Invoke-AgentMaverick {
        ...
        if ($myTask) {
            ...
            # Placeholder implementations - refine based on actual data
            ...
            Write-Warning "Erro ao calcular o intervalo dinâmico"
        }
...
function Invoke-AgentPlanner {
    # Marca o tempo de inicio da execução do agente
    $startTime = Get-Date
...
function Start-OrganismPulse {
    ...
    while ($true) {
        try {
            ...
            $agentName = if ($_.InvocationInfo.MyCommand.Name -match "Maverick") { "@maverick" } elseif ($_.InvocationInfo.MyCommand.Name -match "Planner") { "@planner" } elseif ($_.InvocationInfo.MyCommand.Name -match "Auditor") { "@auditor" } else { "unknown" }
            $maxRetries = 3
            $retryDelaySeconds = 5
            ...
            for ($retries = 1; $retries -le $maxRetries; $retries++) {
                ...
                try {
                    switch ($agentName) {
                        "@maverick" { Invoke-AgentMaverick }
                        "@planner" { Invoke-AgentPlanner }
                        "@auditor" { Invoke-AgentAuditor }
                        default { Write-Warning "Não foi possível identificar o agente para reiniciar." }
                    }
                    Write-Host "$($agentName) reiniciado com sucesso." -ForegroundColor Green
                    break # Sai do loop se o agente for reiniciado
                }
                catch {
                    Write-Error "Falha ao reiniciar $($agentName): $_"
            ...
            if ($retries -gt $maxRetries) {
                Write-Error "Falha crítica: Não foi possível reiniciar $($agentName) após $($maxRetries) tentativas. Pode ser necessária intervenção manual."
