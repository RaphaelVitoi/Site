...
if ($Script:AgentStats -eq $null) {
    ...
    $AllAgents = @(
        ...
        "dispatcher", "architect", "chico"
    )
    
    foreach ($name in $AllAgents) {
        $Script:AgentStats["@$name"] = @{ executions = 0; totalTime = 0 }
    }
}
...
function Invoke-AgentOrganizador {
    ...
    $contextFile = Join-Path $Global:AgentPaths.Root ".claude\project-context.md"
    $needsSync = $false
...
function Save-AgentStats {
    $StatsPath = Join-Path $Global:AgentPaths.Root ".claude\stats"
    ...
    while ($true) {
        ...
        foreach ($agentName in $Global:AutopoiesisConfig.ActiveAgents) {
            switch ($agentName) {
                ...
                "@architect" { Invoke-AgentWithRetry -AgentAction { Invoke-AgentArchitect } -AgentName "@architect" }
                "@organizador" { Invoke-AgentWithRetry -AgentAction { Invoke-AgentOrganizador } -AgentName "@organizador" }
                Default { Write-Host "[PULSO] Agente $agentName detectado na config, mas sem ciclo de autopoiese implementado." -ForegroundColor DarkGray }
            }
        }
