...
class MemoryRAG:
    ...
    def _robust_json_load(self, text: str) -> list:
        """Extracao cirurgica de matrizes JSON envelopadas em Markdown ou corrompidas por verbosidade."""
        # 1. Limpeza de artefatos Markdown e ruidos laterais
        cleaned_output = re.sub(r'```(?:json)?|```', '', text, flags=re.IGNORECASE).strip()

        # 2. Busca pelo primeiro e ultimo caractere estrutural (Array)
        ...
        if match:
            ...
            try:
                ...
                logging.warning(f"[RAG] Falha na desserializacao ({e.msg}). Acionando fix de trailing commas...")
                # 3. Tratamento de virgulas pendentes (Trailing Commas)
                json_str_fixed = re.sub(r',\s*\]', ']', json_str)
                try:
                    parsed = json.loads(json_str_fixed)
                except json.JSONDecodeError:
                    logging.warning("[RAG] Falha irrecuperavel no parser JSON do fallback.")
                    return []

            # 4. Sanitizacao estrita de tipo (ChromaDB requer lista de strings)
            ...
            if isinstance(parsed, list):
                ...
                for item in parsed:
                    ...
                    elif isinstance(item, dict) and item: # Se a IA mandar [{"pergunta": "algo"}]
                        clean_list.append(str(list(item.values())[0]))
                return clean_list
        logging.warning(f"[RAG] Nenhum array JSON encontrado no output do LLM para expansao de query.")
        return []
        ...
        try:
            ...
            if OPENROUTER_KEYS and (gemini_failed or not GEMINI_ALL_KEYS or "gemini-2.5-flash" not in FAST_OPERATIONS_MODELS):
                try:
                    ...
                    response, _ = await call_openrouter(session, or_model, system_prompt, user_prompt, OPENROUTER_KEYS[0], dyn_timeout, require_json=True)
                    expanded_queries = self._robust_json_load(response)
                    return [question] + expanded_queries
                except Exception as e:
        ...
        try: # Importa do task_executor para centralizar a logica de chamada de API
            ...
            if OPENROUTER_KEYS and (gemini_failed or not GEMINI_ALL_KEYS or "gemini-2.5-flash" not in FAST_OPERATIONS_MODELS):
                try:
                    ...
                    dyn_timeout = aiohttp.ClientTimeout(total=60.0, connect=15.0, sock_read=45.0)
                    response, _ = await call_openrouter(session, or_model, system_prompt, user_prompt, OPENROUTER_KEYS[0], dyn_timeout)
                    return enforce_pure_ascii(response.strip())
                except Exception as e:
                    logging.warning(f"[ORACULO] Falha na sintese via OpenRouter: {e}")
        except Exception as e:
    ...
    async def query_memory(self, question: str, n_results: int = 3, local_only: bool = False, synthesize: bool = False) -> str:
        try:
            ...
            if not synthesize:
                ...
                # SOTA: Compressao Cognitiva dos resultados da WebSearch para economia de tokens
                from task_executor import _compress_context # Importa a funcao de compressao
                compressed_context = await _compress_context(raw_context, "@bibliotecario") # @bibliotecario como agente para contexto
                return await self._synthesize_answer(question, compressed_context)
        except Exception as e:
