...
# SOTA: Tokenizer adaptado para preservar o Fator Ψ e símbolos matemáticos (EV, RP)
TOKEN_PATTERN = re.compile(r'\w+|[Ψψ]', re.UNICODE)
...
class MemoryRAG:
    def __init__(self, memory_dir: str = ".claude/agent-memory"):
        ...
        kg_dir = self.base_dir / "database"
        kg_dir.mkdir(parents=True, exist_ok=True)
        self.kg_db_path = str(kg_dir / "knowledge_graph.db")
        self.schema_path = str(kg_dir / "schema_graph.sql")

        db_path_obj = self.memory_dir / "vector_store"
        ...
        try:
            ...
            if "code: 14" in str(e) or "unable to open database file" in str(e):
                logging.critical(f"\n[RAG FATAL] Bloqueio de I/O no SQLite (ChromaDB) detectado em: {db_path}")
                logging.critical("Causa SOTA (Windows): O Orquestrador (start-worker) esta ativo e detem o lock exclusivo do banco, ou o OneDrive esta retendo o arquivo.")
                logging.critical(">>> ACAO: Execute 'stop-worker' no terminal para liberar o lock antes de efetuar a ingestao manual profunda.\n")
                sys.exit(1)
            raise
    ...
    async def _extract_and_persist_kg(self, chunks: list[str], source_name: str) -> None:
        try:
            from task_executor import call_gemini, GEMINI_KEYS
            import aiohttp
            import aiosqlite
        except ImportError:
            return

        prompt_path = self.base_dir / ".claude" / "prompts" / "kg_extraction_zero_shot.md"
        if not GEMINI_KEYS:
            return
        ...
        if prompt_path.exists():
            ...
            # SOTA: Fallback resiliente caso o template Zero-Shot falte na topologia documental
            system_prompt = 'Extraia entidades e relacoes ontologicas do texto. Retorne APENAS um JSON valido no formato estrito: {"nodes": [{"id": "string", "label": "string", "type": "string", "description": "string"}], "edges": [{"source_id": "string", "target_id": "string", "relation": "string", "weight": 1.0}]}. Nenhuma formatacao markdown, apenas o JSON bruto.'

        async with aiohttp.ClientSession() as session:
            async with aiosqlite.connect(self.kg_db_path) as db:
                for chunk in chunks:
                    try:
                        user_prompt = f"Fonte: {source_name}\n\nTexto:\n{chunk}"
                        response, _ = await call_gemini(session, "gemini-1.5-flash-latest", system_prompt, user_prompt, GEMINI_KEYS[0])

                        cleaned = response.strip('`').strip('json\n').strip()
                        data = json.loads(cleaned)

                        for node in data.get("nodes", []):
                            await db.execute("""
                                INSERT OR IGNORE INTO kg_nodes (id, label, type, description)
                                VALUES (?, ?, ?, ?)
                            """, (node["id"], node["label"], node["type"], node.get("description", "")))

                        for edge in data.get("edges", []):
                            edge_id = f"{edge['source_id']}_{edge['relation']}_{edge['target_id']}"
                            await db.execute("""
                                INSERT OR IGNORE INTO kg_edges (id, source_id, target_id, relation, weight, context_ref)
                                VALUES (?, ?, ?, ?, ?, ?)
                            """, (edge_id, edge["source_id"], edge["target_id"], edge["relation"], edge.get("weight", 1.0), source_name))

                        await db.commit()
                    except Exception as e:
                        logging.warning(f"[KG] Falha na extracao causal ontologica para chunk de {source_name}: {e}")

    async def _process_single_file(self, file_path: Path) -> list[str]:
        if ".chroma_db" in str(file_path) or "vector_store" in str(file_path):
            return []
        ...
        if chunks:
            ...
            # SOTA: Extracao Assincrona do Knowledge Graph
            await self._extract_and_persist_kg(chunks, source_name)
    ...
    async def ingest_all_memories(self):
        ...
        # SOTA: Erradicação da falha silenciosa de schemas ausentes.
        # O RAG forja sua própria base de dados (Autopoiese).
        import aiosqlite
        async with aiosqlite.connect(self.kg_db_path) as db:
            await db.execute("""
    ...
    def _query_and_rerank_sync(self, expanded_queries: list[str], question: str, n_results: int) -> list[dict]:
        """Executa a I/O do ChromaDB e o calculo intensivo (CPU-bound) do Lexical Score BM25 fora do Event Loop."""
        ...
        query_terms = [t for t in TOKEN_PATTERN.findall(question.lower())]

        # SOTA: Extracao de Frequencias para BM25 Lexical Scoring
        doc_token_lists = []
        doc_lens = []
        df = {}
        ...
        # IDF com smoothing (Okapi BM25)
        idf_map = {}
        for term, count in df.items():
            idf_map[term] = math.log(((N - count + 0.5) / (count + 0.5)) + 1.0)

        k1 = 1.5
        b = 0.75

        scored_docs = []
        max_possible_bm25 = sum(idf_map.get(qt, 0) for qt in set(query_terms))
        ...
        for i, doc in enumerate(documents):
            ...
            for qt in set(query_terms):
                if qt in df:
                    ...
                    bm25_score += idf_map[qt] * (numerator / denominator)

            # Normalizacao heuristica do BM25 (max teorico aproxima-se de sum(idf))
            lexical_score = bm25_score / max_possible_bm25 if max_possible_bm25 > 0 else 0.0

            # Similaridade Semantica (Distancia Vetorial L2/Cosine Invertida)
            semantic_score = 1.0 / (1.0 + distances[i])

            # Hibridizacao SOTA: Balanceamento BM25 vs Vetorial
    ...
    def _query_kg_sync(self, question: str) -> str:
        """SOTA: Consulta as relacoes causais no Knowledge Graph baseada em overlap de tokens."""
        ...
        try:
            ...
            query = "SELECT n1.label, e.relation, n2.label, e.weight, e.context_ref FROM kg_edges e JOIN kg_nodes n1 ON e.source_id = n1.id JOIN kg_nodes n2 ON e.target_id = n2.id"
            cursor.execute(query)
            rows = cursor.fetchall()
            conn.close()

            # Filtragem em memoria para Friccao Zero (evita SQL injection em loops)
            relevant_edges = []
            for r in rows:
                row_text = f"{r[0]} {r[1]} {r[2]}".lower()
                if any(t in row_text for t in terms):
                    relevant_edges.append(r)

            if not relevant_edges:
                return ""

            output = ["\n=== REDE DE CAUSALIDADE ONTOLOGICA (KNOWLEDGE GRAPH) ==="]
            for r in sorted(relevant_edges, key=lambda x: x[3], reverse=True)[:10]:
                output.append(f"[{r[0]}] --({r[1]})--> [{r[2]}] (Peso: {r[3]}) | Fonte: {r[4]}")
            return "\n".join(output) + "\n"
        except Exception as e:
            logging.error(f"[KG] Erro ao consultar o Grafo de Conhecimento: {e}")
            return ""
    ...
    async def query_memory(self, question: str, n_results: int = 3, local_only: bool = False) -> str:
        try:
            ...
            # Integracao do Knowledge Graph
            kg_context = await asyncio.to_thread(self._query_kg_sync, question)

            if not top_docs:
                return kg_context if kg_context else ""

            output_parts = [kg_context] if kg_context else []
