...
from llm.budget import PERPLEXITY_KEYS, TAVILY_KEYS
...
class MemoryRAG:
    ...
    async def ingest_all_memories(self):
        ...
        def is_ignored(path: Path) -> bool:
            ...
            return any(p in str(path) for p in te.RAG_IGNORE_PATTERNS)
        ...
        for file_path in files_to_process:
            ...
            if chunks:
                sources_to_delete.append(str(file_path))
                batch_chunks.extend(chunks)
                batch_metadatas.extend(metadatas)
                batch_ids.extend(ids)
                logging.info(f"[RAG INGEST] Mapeados {len(chunks):02d} fragmentos de: {source_name}")

        if sources_to_delete:
            for i in range(0, len(sources_to_delete), 50):
                batch_sources = sources_to_delete[i:i+50]
                await asyncio.to_thread(self.collection.delete, where={"source": {"$in": batch_sources}})
        ...
        if batch_chunks:
            ...
            for i in range(0, len(batch_chunks), batch_size):
                await asyncio.to_thread(
                    ...
                    documents=batch_chunks[i:i+batch_size],
                    metadatas=batch_metadatas[i:i+batch_size],
                    ids=batch_ids[i:i+batch_size]
                )
            logging.info(f"[RAG INGEST] Batch upsert concluido: {len(batch_chunks)} fragmentos ingeridos no ChromaDB.")
        ...
        try:
            ...
            if ids_to_delete:
                batch_size = 50
                logging.info(f"[RAG] Iniciando analise de {len(ids_to_delete)} fragmentos ausentes para Quarentena (Soft-Delete)...")
                ...
                for i in range(0, len(ids_to_delete), batch_size):
                    ...
                    results = await asyncio.to_thread(self.collection.get, ids=batch, include=["metadatas"])
                    metadatas = results.get('metadatas', [])

                    for j, meta in enumerate(metadatas):
                        if meta and meta.get("status") == "pending_purge":
                            if current_time - float(meta.get("purge_detected_at", current_time)) > 172800: # 48h
                                real_deletes.append(batch[j])
                        else:
                            if meta is None: meta = {}
                            meta["status"] = "pending_purge"
                            meta["purge_detected_at"] = current_time
                            soft_updates_ids.append(batch[j])
                            soft_updates_metas.append(meta)

                if real_deletes:
                    for i in range(0, len(real_deletes), batch_size):
                        await asyncio.to_thread(self.collection.delete, ids=real_deletes[i:i+batch_size])
                    logging.info(f"[RAG PURGE] Expurgados definitivamente {len(real_deletes)} fragmentos obsoletos.")

                if soft_updates_ids:
                    for i in range(0, len(soft_updates_ids), batch_size):
                        await asyncio.to_thread(self.collection.update, ids=soft_updates_ids[i:i+batch_size], metadatas=soft_updates_metas[i:i+batch_size])
                    logging.info(f"[RAG PURGE] {len(soft_updates_ids)} fragmentos colocados em Quarentena (Soft-Delete 48h).")
            else:
    ...
    async def _expand_query(self, question: str) -> list[str]:
        ...
        try:
            ...
            from llm.budget import GEMINI_ALL_KEYS, OPENROUTER_KEYS
            from llm.session import get_global_http_session
            import aiohttp
            ...
            timeout_val = float(te._agent_sla_value("@bibliotecario", "llm_timeout_seconds", 60))

            gemini_failed = True
            if GEMINI_ALL_KEYS and "gemini-2.5-flash" in te.FAST_OPERATIONS_MODELS: # Prioriza Gemini Flash se estiver no pool
                fast_model = "gemini-2.5-flash"
                dyn_timeout = aiohttp.ClientTimeout(total=timeout_val, connect=10.0, sock_read=timeout_val - 10.0)
            ...
            if OPENROUTER_KEYS and (gemini_failed or not GEMINI_ALL_KEYS or "gemini-2.5-flash" not in te.FAST_OPERATIONS_MODELS):
                # SOTA: O Llama 8B e muito leve para json/sinteses complexas. Priorizamos modelos >= 24B
                valid_or = [m for m in te.FAST_OPERATIONS_MODELS if "/" in m]
                fallback = valid_or[0] if valid_or else "meta-llama/llama-3.3-70b-instruct:free"
                or_model = next((m for m in OPENROUTER_ALTERNATIVE_MODELS if "r1" not in str(m).lower() and "8b" not in str(m).lower()), fallback)
                or_model = next((m for m in te.OPENROUTER_ALTERNATIVE_MODELS if "r1" not in str(m).lower() and "8b" not in str(m).lower()), fallback)
                logging.info(f"[RAG] Tentando expansao de query via OpenRouter ({or_model})...")
                for i, key in enumerate(OPENROUTER_KEYS):
                    try:
                        dyn_timeout = aiohttp.ClientTimeout(total=timeout_val, connect=15.0, sock_read=timeout_val - 15.0)
