...
export default async function Home ()
{
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 animate-fade-up">
      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border mb-8 transition-colors ${pulse !== null ? 'bg-accent-emerald/10 border-accent-emerald/30' : 'bg-rose-500/10 border-rose-500/30'}`}>
        <span className={`w-2 h-2 rounded-full ${pulse !== null ? 'bg-accent-emerald animate-pulse' : 'bg-rose-500'}`}></span>
        <span className={`text-[0.65rem] font-black uppercase tracking-widest ${pulse !== null ? 'text-accent-emerald-light' : 'text-rose-400'}`}>
          {pulse !== null ? `Mente Coletiva SOTA · ${pulse.active} Tarefas Ativas` : 'Orquestrador Offline'}
        </span>
      </div>
