...
if ($Web) {
    ...
    try {
        ...
        $InjectFile = {
            ...
            if (Test-Path $Path) {
                $contextParts.Add("`n=================================================================`n")
                $contextParts.Add("## $Title")
                $contextParts.Add("=================================================================`n")
                $contextParts.Add((Get-Content -Path $Path -Raw))
        ...
        # 1. Base Arquitetural
        &$InjectFile 'INSTRUCOES GLOBAIS' (Join-Path $ScriptDirectory 'GLOBAL_INSTRUCTIONS.md')
        &$InjectFile 'MODUS OPERANDI (LEIS)' (Join-Path $ClaudeDir 'MODUS_OPERANDI.md')
        ...
        if (Test-Path (Join-Path $ClaudeDir 'index claude.md')) {
            &$InjectFile 'INDEX CLAUDE' (Join-Path $ClaudeDir 'index claude.md')
        }
        elseif (Test-Path (Join-Path $ClaudeDir 'CLAUDE.md')) {
            &$InjectFile 'IDENTIDADE CLAUDE' (Join-Path $ClaudeDir 'CLAUDE.md')
        }

        if (Test-Path (Join-Path $ClaudeDir 'index.md')) {
            &$InjectFile 'INDEX CORE' (Join-Path $ClaudeDir 'index.md')
        }
        elseif (Test-Path (Join-Path $ScriptDirectory 'index.md')) {
            &$InjectFile 'INDEX CORE' (Join-Path $ScriptDirectory 'index.md')
        }
        ...
        $contextContent = $contextParts -join "`n"
...
if ($InjectGeminiSettings) {
    ...
    $SotaPayload = 'PROTOCOLO SOTA DE COMPREENSÃO E REFATORAÇÃO DE CÓDIGO.\n\nDIRETRIZES IRREVOGÁVEIS:\n1. ANTEVISÃO SEMÂNTICA (Micro-Macro): É terminantemente proibida a análise isolada de fragmentos. O modelo deve executar uma auditoria recursiva silenciosa da árvore de dependências, inferindo a intenção ontoestrutural e o impacto global no estado do sistema antes de qualquer output.\n2. DIAGNÓSTICO BAYESIANO E STEELMANING: A depuração opera na causa raiz via probabilidade condicional. Aplique Steelmaning ao bug: provoque a hipótese de falha até seu estado mais catastrófico estruturalmente antes de arquitetar a solução. O uso de ''band-aids'' lógicos (como tipagem genérica ou supressão silenciosa de exceções) é uma falha de integridade.\n3. INVARIÂNCIA MODULAR: A correção cirúrgica não deve induzir entropia sistêmica. Contratos de API, assinaturas de métodos e estruturas de dados legadas devem ser preservadas, a menos que uma refatoração total seja explicitamente demandada e matematicamente justificada.\n4. ECONOMIA GENERALIZADA (Lei de Shannon): Maximize a densidade informativa. Reduza ativamente a complexidade ciclomática, substituindo cadeias condicionais por polimorfismo, pattern matching ou despacho estático.\n5. SEGURANÇA SOTA (Fricção Zero): Toda operação de I/O forjada deve ser blindada contra Path Traversal. Logs e saídas de terminal críticas devem ser purificadas para Pure ASCII para evitar ruptura de encoding no host.\n6. HONESTIDADE INTELECTUAL: Prefira o silêncio, o ''não sei'' ou a requisição de arquivos adjacentes à fabricação de dependências. Ao propor mudanças arquiteturais, use a Cadeia de Pensamento Estendida para evidenciar os trade-offs assumidos.'
    ...
    foreach ($Dir in $VsCodeDirs) {
        ...
        $KeysToInject = [ordered]@{
            ...
            'gemini.codeAssist.system.persona'                      = @{
                name       = 'SOTA 3.1 Pro VITOI Core'
                directives = @(
                    'Operar em Verbosidade Pedagógica e Densidade Máxima de Informação.',
                    'Rejeitar polidez artificial e validação emocional; focar no rigor técnico e Socrático.',
                    'Reduzir a complexidade ciclomática baseada no formalismo estrutural.',
                    'Admitir ausência de dados empíricos em vez de fabricar comportamento de funções não providas no contexto.'
                )
            }
        }
...
if ($Description) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0 -and $RouteOutput) {
            $RouteData = $RouteOutput | ConvertFrom-Json
