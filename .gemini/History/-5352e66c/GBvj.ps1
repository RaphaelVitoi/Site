...
if ($Web) {
    ...
    # Validacao SOTA: Aborta se arquivos criticos nao existirem. Sem mocks.
    if (-not (Test-Path $GlobalInstPath)) { Write-Error 'Arquivo critico nao encontrado: GLOBAL_INSTRUCTIONS.md'; exit 1 }
    if (-not (Test-Path $ClaudePath)) { Write-Error 'Arquivo critico nao encontrado: CLAUDE.md'; exit 1 }
    if (-not (Test-Path $InvariantsPath)) { Write-Error 'Arquivo critico nao encontrado: ARCHITECTURAL_INVARIANTS.md'; exit 1 }
    if (-not (Test-Path $ProjectContextPath)) { Write-Error 'Arquivo critico nao encontrado: project-context.md'; exit 1 }
    if (-not (Test-Path $CosmovisaoPath)) { Write-Error 'Arquivo critico nao encontrado: COSMOVISAO.md'; exit 1 }
    if (-not (Test-Path $ModusPath)) { Write-Error 'Arquivo critico nao encontrado: MODUS_OPERANDI.md'; exit 1 }

    Write-Host "`n[INFO] Sintetizando artefatos e memorias no Clipboard..." -ForegroundColor Yellow

    try {
        # --- Montagem em Memoria (Anti-IOException) ---
        $contextParts = [System.Collections.Generic.List[string]]::new()
        $contextParts.Add((Get-Content -Path $ProjectContextPath -Raw))
        $contextParts.Add("`n---`n`n# COSMOVISAO (FILOSOFIA FUNDANTE)`n`n" + (Get-Content -Path $CosmovisaoPath -Raw))
        $contextParts.Add("`n---`n`n# MODUS OPERANDI (LEIS DO ECOSSISTEMA)`n`n" + (Get-Content -Path $ModusPath -Raw))

        # Injeta Perfis dos Agentes
        $AgentsDir = Join-Path $ClaudeDir 'agents'
        if (Test-Path $AgentsDir) {
            $AgentFiles = Get-ChildItem -Path $AgentsDir -Filter *.md
            if ($AgentFiles) {
                $contextParts.Add("`n---`n`n# PERFIS DOS AGENTES (IDENTIDADES)`n")
                foreach ($File in $AgentFiles) {
                    $contextParts.Add("`n## PERFIL: $($File.Name)`n`n" + (Get-Content -Path $File.FullName -Raw))
                }
            }
        }

        # Injeta Memorias Individuais
        ...
        if (Test-Path $AgentMemoryDir) {
            ...
            if ($MemoryFiles) {
                $contextParts.Add("`n---`n`n# MEMORIAS DOS AGENTES (ESTADO ATUAL E APRENDIZADOS)`n")
                foreach ($File in $MemoryFiles) {
                    ...
                    $contextParts.Add($Header + $MemoryContent)
                }
            }
        }

        # Escrita atomica no arquivo temporario
        $finalTempContent = $contextParts -join "`n"
        Set-Content -Path $TempContextPath -Value $finalTempContent -Encoding UTF8

        # Chama o Assembler para condensar tudo
