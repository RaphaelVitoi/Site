...
if ($TestFrontend) {
    Write-Host '=== [SISTEMA] INICIANDO AUDITORIA E2E SOTA (FRONTEND) ===' -ForegroundColor Cyan
    $FrontendDir = Join-Path $ScriptDirectory 'frontend'
    Push-Location $FrontendDir
    $NpxCmd = if (Get-Command 'npx.cmd' -ErrorAction SilentlyContinue) { 'npx.cmd' } else { 'npx' }
    Write-Host '[INFO] Executando Vitest (Friccao Zero)...' -ForegroundColor Yellow
    & $NpxCmd vitest run
    $ExitCode = $LASTEXITCODE
    Pop-Location
    if ($ExitCode -ne 0) { Write-Error 'Falha nos testes do Frontend (WASM/React). Entropia detectada.'; exit $ExitCode }
    Write-Host '[VITORIA] Topologia Fluida SOTA e integracao do Motor Quantico validadas com sucesso no Frontend.' -ForegroundColor Green
    exit 0
}

if ($TestBackend) {
    Write-Host '=== [SISTEMA] INICIANDO AUDITORIA E2E SOTA (BACKEND) ===' -ForegroundColor Cyan
    $TestScript1 = Join-Path $ScriptDirectory 'scripts\tests\test_task_executor.py'
    $TestScript2 = Join-Path $ScriptDirectory 'frontend\src\components\laboratorio\test_task_executor.py'
    $TestScript = if (Test-Path $TestScript1) { $TestScript1 } elseif (Test-Path $TestScript2) { $TestScript2 } else { $null }

    if (-not $TestScript) { Write-Error 'Arquivo de teste do backend nao encontrado.'; exit 1 }
    Write-Host "[INFO] Invocando Unittest em: $TestScript" -ForegroundColor Yellow
    & $PythonCmd -m unittest $TestScript
    if ($LASTEXITCODE -ne 0) { Write-Error 'Falha na Homeostase do Orquestrador (Backend).'; exit $LASTEXITCODE }
    Write-Host '[VITORIA] Roteamento Semantico e Dynamic Yield Manager validados com sucesso no Backend.' -ForegroundColor Green
    exit 0
}

if ($TestWasm) {
    Write-Host '=== [SISTEMA] INICIANDO AUDITORIA E2E SOTA (WASM/RUST) ===' -ForegroundColor Cyan
    $WasmDir = Join-Path $ScriptDirectory 'wasm-equity'
    Push-Location $WasmDir
    Write-Host '[INFO] Executando wasm-pack test (Node.js SOTA)...' -ForegroundColor Yellow
    wasm-pack test --node
    $ExitCode = $LASTEXITCODE
    Pop-Location
    if ($ExitCode -ne 0) { Write-Error 'Falha nos testes do motor WASM. Entropia detectada no ranqueador Bitwise.'; exit $ExitCode }
    Write-Host '[VITORIA] Motor Quantico O(1) e Serializacao Binaria validados com sucesso via Node.js (Friccao Zero).' -ForegroundColor Green
    exit 0
}

if ($BuildWasm) {
    Write-Host '=== [SISTEMA] FORJANDO BINARIOS SOTA (WASM) ===' -ForegroundColor Cyan
    $WasmDir = Join-Path $ScriptDirectory 'wasm-equity'
    $PublicWasmDir = Join-Path $ScriptDirectory 'frontend\public\wasm'

    Push-Location $WasmDir
    Write-Host '[INFO] Compilando motor Rust (Target Web)...' -ForegroundColor Yellow
    wasm-pack build --target web
    $ExitCode = $LASTEXITCODE

    if ($ExitCode -eq 0) {
        Write-Host '[INFO] Roteando artefatos para a topologia do Next.js...' -ForegroundColor Yellow
        if (-not (Test-Path -LiteralPath $PublicWasmDir)) { New-Item -ItemType Directory -Path $PublicWasmDir -Force | Out-Null }
        Copy-Item -Path 'pkg\vitoi_equity_engine_bg.wasm' -Destination $PublicWasmDir -Force
        Write-Host '[VITORIA] Motor Quântico compilado e ancorado no Frontend com sucesso.' -ForegroundColor Green
    }
    else {
        Write-Error 'Falha na compilação estrutural do WASM.'
    }
    Pop-Location
    exit $ExitCode
}
