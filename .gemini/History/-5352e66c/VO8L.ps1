...
if ($TestWasm) {
    ...
    Write-Host '[INFO] Executando wasm-pack test (Firefox Headless)...' -ForegroundColor Yellow
    wasm-pack test --headless --firefox
    $ExitCode = $LASTEXITCODE
    Pop-Location
    if ($ExitCode -ne 0) { Write-Error 'Falha nos testes do motor WASM. Entropia detectada no ranqueador Bitwise.'; exit $ExitCode }
    Write-Host '[VITORIA] Motor Quantico O(1) e Serializacao Binaria validados com sucesso no ambiente real do navegador.' -ForegroundColor Green
    exit 0
}
