...
function Write-CryptoAuditSOTA {
    ...
    "$RawData|$HashString" | Out-File -FilePath $AuditLogPath -Append -Encoding UTF8
}
...
if ($Watch) {
    ...
    $IngestionManifestPath = Join-Path $PSScriptRoot 'data\rag_ingestion_manifest.json'
    $WatchPatterns = @('*.md', '*.docx', '*.py', '*.ts', '*.tsx', '*.ps1') 
    if (Test-Path $IngestionManifestPath) {
        try {
            $ManifestData = Get-Content $IngestionManifestPath -Raw | ConvertFrom-Json
            $Exts = @()
    ...
    foreach ($event in $events) {
        Register-ObjectEvent -InputObject $watcher -EventName $event -Action $action -SourceIdentifier "NexusFileWatcher.$event" -MessageData $WatchPatterns | Out-Null
    }
    ...
    try {
        while ($true) {
            ...
            if ($Global:NexusPendingIngestion) {
                $Global:NexusPendingIngestion = $false
                Write-Host '[ACAO] Disparando re-ingestao da memoria RAG em background (Lote Debounced)...' -ForegroundColor Cyan
...
if ($PSBoundParameters.ContainsKey('FixEPERM')) {
    ...
    try {
        ...
        $NodeProcs = Get-WmiObject Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -match $CurrentPathEscaped }
        if ($NodeProcs) {
...
if ($Backup) {
    Invoke-NexusScript -ScriptName 'scripts\utils\safeguard_system.ps1' -Message 'INICIANDO PROTOCOLO DE SALVAGUARDA'
}
...
if ($Web) {
    ...
    Write-Host '2. Gemini 2.5 / Pro / Advanced (Contexto Massivo, RAG e Visao Holistica)'
    Write-Host '3. Modelos Abertos / DeepSeek (Raciocinio Bruto Step-by-Step)'

    $MenuChoice = if ($Force) { '1' } else { Read-Host 'Selecione o motor cognitivo alvo [1/2/3]' }
    ...
    $GlobalInstPath = Join-Path $ScriptDirectory 'GLOBAL_INSTRUCTIONS.md'
    $ProjectContextPath = Join-Path $ClaudeDir 'project-context.md'
    $TempContextPath = Join-Path $ClaudeDir "temp_project_context_$(Get-Random).md"

    if (-not $TestMode) {
        if (-not (Test-Path $GlobalInstPath)) { Write-Error 'Arquivo GLOBAL_INSTRUCTIONS.md nao encontrado.'; exit 1 }
        if (-not (Test-Path (Join-Path $ClaudeDir 'CLAUDE.md'))) { Write-Error 'Arquivo CLAUDE.md nao encontrado.'; exit 1 }
    }
    # Resiliencia SOTA: Substitui o 'exit 1' por fallback harmonioso.
    # Se o arquivo nao existir, cria um mock vazio para evitar crash no Assembler.
    $ClaudePath = Join-Path $ClaudeDir 'CLAUDE.md'
    $InvariantsPath = Join-Path $ClaudeDir 'ARCHITECTURAL_INVARIANTS.md'
    
    if (-not (Test-Path $GlobalInstPath)) { New-Item -ItemType File -Path $GlobalInstPath -Force | Out-Null; Write-Warning '[AVISO] GLOBAL_INSTRUCTIONS.md ausente. Gerando mock vazio para evitar falha no Assembler.' }
    if (-not (Test-Path $ClaudePath)) { New-Item -ItemType File -Path $ClaudePath -Force | Out-Null; Write-Warning '[AVISO] CLAUDE.md ausente. Gerando mock vazio.' }
    if (-not (Test-Path $InvariantsPath)) { New-Item -ItemType File -Path $InvariantsPath -Force | Out-Null; Write-Warning '[AVISO] ARCHITECTURAL_INVARIANTS.md ausente. Gerando mock vazio.' }

    try {
        # --- Montagem do Contexto Holistico Dinamico SOTA ---
        # 1. Comeca com o contexto base do projeto
        if (Test-Path $ProjectContextPath) {
            Get-Content -Path $ProjectContextPath -Raw | Set-Content -Path $TempContextPath -Encoding UTF8
        }
        else {
            Set-Content -Path $TempContextPath -Value "# CONTEXTO DO PROJETO`n" -Encoding UTF8
        }

        # 2. Injeta Cosmovisao (Filosofia Fundante)
        $CosmovisaoPath = Join-Path $ClaudeDir 'COSMOVISAO.md'
        if (Test-Path $CosmovisaoPath) {
            Add-Content -Path $TempContextPath -Value "`n`n---`n`n# COSMOVISAO (FILOSOFIA FUNDANTE)`n"
            Add-Content -Path $TempContextPath -Value (Get-Content -Path $CosmovisaoPath -Raw)
        }

        # 3. Injeta Modus Operandi (Leis do Ecossistema)
        $ModusPath = Join-Path $ClaudeDir 'MODUS_OPERANDI.md'
        if (Test-Path $ModusPath) {
            Add-Content -Path $TempContextPath -Value "`n`n---`n`n# MODUS OPERANDI (LEIS DO ECOSSISTEMA)`n"
            Add-Content -Path $TempContextPath -Value (Get-Content -Path $ModusPath -Raw)
        }

        # 4. Injeta Perfis dos Agentes (.claude/agents/*.md)
        $AgentsDir = Join-Path $ClaudeDir 'agents'
        if (Test-Path $AgentsDir) {
            $AgentFiles = Get-ChildItem -Path $AgentsDir -Filter *.md
            if ($AgentFiles) {
                Add-Content -Path $TempContextPath -Value "`n`n---`n`n# PERFIS DOS AGENTES (IDENTIDADES)`n"
                foreach ($File in $AgentFiles) {
                    Add-Content -Path $TempContextPath -Value "`n`n## PERFIL: $($File.Name)`n"
                    Add-Content -Path $TempContextPath -Value (Get-Content -Path $File.FullName -Raw)
                }
            }
        }

        # 5. Injeta as Memorias Individuais de cada agente (.claude/agent-memory/*/MEMORY.md)
        ...
        if (Test-Path $AgentMemoryDir) {
            ...
            if ($MemoryFiles) {
                Add-Content -Path $TempContextPath -Value "`n`n---`n`n# MEMORIAS DOS AGENTES (ESTADO ATUAL E APRENDIZADOS)`n"
                foreach ($File in $MemoryFiles) {
                    $AgentName = $File.Directory.Name
                    $MemoryContent = Get-Content -Path $File.FullName -Raw
                    $Header = "`n`n## MEMORIA DO AGENTE: @$AgentName`n`n"
                    Add-Content -Path $TempContextPath -Value ($Header + $MemoryContent)
                }
            }
        }

        # 6. Chama o Assembler para condensar tudo (incluindo GLOBAL_INSTRUCTIONS nativamente)
        $contextContent = Invoke-ContextAssembler `
            -GlobalInstructionsPath $GlobalInstPath `
            -ClaudeIdentityPath (Join-Path $ClaudeDir 'CLAUDE.md') `
            -ProjectContextPath $TempContextPath `
            -ArchitecturalInvariantsPath (Join-Path $ClaudeDir 'ARCHITECTURAL_INVARIANTS.md') `
            -LLMRecommendation $RecomendacaoLLM
        ...
        # Remove o contexto temporario apos copiado, preservando todos os arquivos originais
        if (Test-Path $TempContextPath) {
            Remove-Item -Path $TempContextPath -Force
        }
    }
    exit 0
}
...
if ($PSBoundParameters.ContainsKey('Autonomy')) {
    $Labels = @{
        ...
        'default' = 'Homeostase (auto-fix, sem comandos de terminal)'
...
if ($InjectGeminiSettings) {
    ...
    $SotaPayload = 'PROTOCOLO SOTA DE COMPREENSAO E REFATORACAO DE CODIGO.\n\nDIRETRIZES IRREVOGAVEIS:\n1. ANTEVISAO SEMANTICA (Micro-Macro): E terminantemente proibida a analise isolada de fragmentos. Audite recursivamente a arvore de dependencias antes de inferir a intencao ontoestrutural.\n2. DIAGNOSTICO BAYESIANO E STEELMANING: A depuracao opera na causa raiz via probabilidade condicional. Aplique Steelmaning ao bug.\n3. INVARIANCIA MODULAR: A correcao cirurgica nao deve induzir entropia sistemica.\n4. ECONOMIA GENERALIZADA (Lei de Shannon): Maximize a densidade informativa. Reduza ativamente a complexidade ciclomatica.\n5. SEGURANCA SOTA: Blindagem absoluta contra Path Traversal e vazamento de memoria.\n6. HONESTIDADE INTELECTUAL: Prefira o silencio ao inves de fabricacao. Ative a Cadeia de Pensamento Estendida.'
    ...
    foreach ($Dir in $VsCodeDirs) {
        ...
        $KeyName = 'gemini.codeAssist.customSystemInstructions'
        $PropertyExists = $null -ne $JsonObj.PSObject.Properties[$KeyName]

        if ($PropertyExists -and $JsonObj.$KeyName -eq $SotaPayload -and -not $Force) {
            Write-Host "[OK] Simetria ja alcancada em: $RepoName" -ForegroundColor DarkGray
            continue
        }

        if ($PropertyExists) {
            $JsonObj.$KeyName = $SotaPayload
        }
        else {
            $JsonObj | Add-Member -MemberType NoteProperty -Name $KeyName -Value $SotaPayload
        }
...
if ($Description) {
    ...
    try {
        ...
        Invoke-WebRequest -Uri $apiUrl -Method Post -Body $taskJson -Headers $headers -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop | Out-Null
