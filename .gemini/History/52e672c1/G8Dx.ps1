...
# 4. Acao Corretiva Autonoma (Fortalecimento do Sistema Imunologico)
Write-Host "[SENTINELA] Iniciando protocolo de auto-correcao..." -ForegroundColor DarkYellow
...
$taskId = "MAINT-DBCLEANUP-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
$taskDesc = "Executar a rotina de limpeza 'db-cleanup 30' para mitigar o risco de degradacao de performance do banco de dados, conforme detectado pela sentinela @maverick."
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "[SENTINELA] Falha ao enfileirar tarefa de auto-correcao para @skillmaster: $output"
}
else {
    Write-Host "[SENTINELA] [SISTEMA IMUNOLOGICO ATIVADO] Tarefa de limpeza ($taskId) enfileirada para @skillmaster." -ForegroundColor Magenta
}
