...
class QueueManager:
    ...
    def add_health_check_result(self, cpu_usage: float, memory_usage: float, disk_usage: float):
        timestamp = datetime.now().isoformat()
        with self._get_conn() as conn:
            conn.execute("""
            ...
            """)
            conn.execute("""
                INSERT INTO health_checks (timestamp, cpu_usage, memory_usage, disk_usage, is_alert)
                VALUES (?, ?, ?, ?, ?)
            """, (timestamp, cpu_usage, memory_usage, disk_usage, self._is_alert(cpu_usage, memory_usage, disk_usage)))
            conn.commit()

    def _is_alert(self, cpu_usage: float, memory_usage: float, disk_usage: float) -&gt; bool:
        CPUThreshold = 80
        MemoryThreshold = 80
        DiskThreshold = 90
        return cpu_usage &gt; CPUThreshold or memory_usage &gt; MemoryThreshold or disk_usage &gt; DiskThreshold

    def get_latest_health_check(self) -&gt; Optional[Dict[str, Any]]:
        with self._get_conn() as conn:
            row = conn.execute("""
...
class QueueManager:
    ...
    Args:
      task (Task): A tarefa a ser adicionada
    """
    ...
    def get_god_mode_audit_logs(self, limit: int = 50) -> List[Dict]:
        """Fetches the latest God Mode audit logs."""
        with self._get_conn() as conn:
            rows = conn.execute("""
                SELECT * FROM god_mode_audit
                ORDER BY timestamp DESC
                LIMIT ?
            """, (limit,)).fetchall()
            logs = []
    ...
    def log_god_mode_action(self, task_id: str, agent: str, action_type: str, details: Dict, status: str, error_message: Optional[str] = None):
        """Logs an action performed by God Mode."""
        timestamp = datetime.now().isoformat()
        with self._get_conn() as conn:
            conn.execute("""
                INSERT INTO god_mode_audit (timestamp, task_id, agent, action_type, details, status, error_message)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                timestamp,
                task_id,
...
def log_god_mode_action(action_type: str, **kwargs):

    logging.info("Implementar sistema de auditoria")
    return "off"

def apply_god_mode(text: str, task: Task, manager: QueueManager):
    ...
    for match in re.finditer(pattern, text, re.DOTALL | re.IGNORECASE):
        ...
        try:
            ...
            manager.log_god_mode_action(
                task_id=task.id,
                agent=task.agent,
                action_type='file_forged',
            ...
            logging.error(f"[FAIL] Erro ao forjar {filepath}: {e}")
    ...
    for match in re.finditer(cmd_pattern, text, re.DOTALL | re.IGNORECASE):
        ...
        if "rm -rf" in cmd.lower() or "del /s" in cmd.lower():
            ...
            continue
        # Adicione esta verificação para restringir o God Mode apenas a usuários autorizados.
        authorized_users = ["Raphael"]  # Substitua com os usuários autorizados
        if os.environ.get("USERNAME") not in authorized_users:
            logging.error(f"[SEC] God Mode bloqueado para usuário não autorizado: {os.environ.get('USERNAME')}")
            continue
        try:
            ...
            logging.error(f"[FAIL] Erro no comando {cmd}: {e}")
...
def process_agent_task(task: Task, manager: QueueManager):
    ...
    apply_god_mode(response_text, task, manager)
...
def run_health_check(manager: QueueManager):
    """Collects system health metrics and adds them to the database."""
    try:
        ...
        manager.add_health_check_result(cpu_usage, memory_usage, disk_usage)

        logging.info(f"Health Check: CPU={cpu_usage}%, Memory={memory_usage}%, Disk={disk_usage}%")

        latest_health_check = manager.get_latest_health_check()
        if latest_health_check and latest_health_check['is_alert']:
            logging.warning("System is in alert state!")
    except Exception as e: logging.error(f"Health check failed: {e}")
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "add":
            ...
            try:
                ...
                logs = manager.get_god_mode_audit_logs(limit=limit)
                if not logs:
                    print("Nenhum log de auditoria do God Mode encontrado.")
                else:
                    print(f"--- Exibindo os últimos {len(logs)} logs de auditoria do God Mode ---")
                    for log in logs:
                        status_color = "\033[92m" if log['status'] == 'success' else "\033[91m"
                        reset_color = "\033[0m"
                        print(f"\nID: {log['id']} | Timestamp: {log['timestamp']}")
                        print(f"Task ID: {log['task_id']} | Agente: {log['agent']}")
                        print(f"Ação: {log['action_type']} | Status: {status_color}{log['status'].upper()}{reset_color}")
                        print("Detalhes:")
                        print(json.dumps(log['details'], indent=2, ensure_ascii=False))
                        if log['error_message']:
                            print(f"Erro: {log['error_message']}")
            except Exception as e:
                print(f"ERROR: Falha ao buscar logs de auditoria: {e}")
    else: start_worker()