...
def _update_terminal_status(counts: dict, running_tasks_count: int, status_line) -> None:
    ...
    if _SetConsoleTitleW:
        ...
        _SetConsoleTitleW(f"NEXUS WORKER | Pendentes: {pending_count} | Rodando: {running_tasks_count} | Pulso: {current_time}")
...
async def _dispatch_optimal_task(manager: QueueManager, semaphore: asyncio.Semaphore, running_tasks: set) -> None:
    ...
    if task:
        ...
        async def task_wrapper(task, manager, sem):
            ...
            try:
            ...
            except BaseException as e: # Catch BaseException to handle KeyboardInterrupt
                error_str = str(e).lower()
                error_class = type(e).__name__

                # SOTA: Intercepta a Exaustao Absoluta (APIKeysExhaustedError) e aciona a Hibernacao Tatica
                if error_class == "APIKeysExhaustedError" or "exhaust" in error_str:
                    import task_executor as task_exec
                    yield_time = await task_exec.global_yield_manager.apply_exhaustion_yield(task, manager)
                    sem.release()
                    released = True
                ...
                if any(k in error_str for k in ["depend", "lock", "starvation", "wait", "file not found", "no such file"]):
                    import task_executor as task_exec
                    ...
                    await manager.update_task_status(task.id, "pending")
                    return
                logger.error(f"[[{te._c(task.agent)}]{task.agent}[/]] Falha catastrófica: {e}")
            finally:
                if not released:
                    sem.release()
        ...
        if pending_tasks:
            # Fato: Tarefas pendentes existem, mas nenhuma possui in_degree == 0. Deadlock Topologico.
            # O DynamicYieldManager e cego a este cenario pois as tarefas sequer entram em 'running'.
            if not any(t.agent == "@chico" and "DEADLOCK-DAG" in t.id for t in pending_tasks):
                alert_task = Task(
...
async def _cleanup_worker(manager: QueueManager, running_tasks: set) -> None:
    ...
    if running_tasks:
        ...
        # SOTA: Isolamento de escopo (list) para prevenir RuntimeError se callbacks dispararem
        for task_future in list(running_tasks):
            task_future.cancel()
        try:
