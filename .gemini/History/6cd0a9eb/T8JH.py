...
async def start_worker(manager: Optional[QueueManager] = None):
    ...
    # 1.5 SOTA: Crash Recovery (Resgate de zumbis apos morte abrupta do processo)
    try:
    ...
    try:
        while True:
            ...
            try:
                ...
                # Atualiza a linha de status dinamica no terminal
                status_line.update(f"[bold]Vigilia de Guardiao Absoluta SOTA[/] | Em Suspensao: [yellow]{pending_count}[/] | Corpos Ativos: [magenta]{len(running_tasks)}[/] | Completas: [green]{counts.get('completed',0)}[/]")

                await semaphore.acquire()

                # SOTA 8.0: Implementacao do "Critical Path Utility" (CPU) via Grafo Topologico
                ...
                # A operacao massiva foi reduzida de O(N^2 log N) para O(V + E + V log V)
                # PROATIVIDADE SOTA: Offload para Thread Pool para nao congelar o micro-servidor da API durante o calculo do DAG
                loop = asyncio.get_running_loop()
                task = await loop.run_in_executor(None, UniversalArbitrator.extract_optimal_task, pending_tasks)

                if task:
                    logger.info(f"[bold magenta][>] ESPACO DE ENTRADA VITAL REQUISITADO:[/] [{te._c(task.agent)}]{task.agent}[/] (ID: {task.id})")
                    ...
                    logger.info(f"[bold magenta][>] ESPACO DE ENTRADA VITAL REQUISITADO:[/] [{te._c(task.agent)}]{task.agent}[/] (ID: {display_id})")
                    ...
                    semaphore.release()
                    await asyncio.sleep(0.5)  # Friccao Zero: Pulso acelerado para latencia indetectavel
        ...
        # SOTA (Correcao 4): Mitigacao de Orfanizacao e Tarefas Zumbis
        if running_tasks:
        ...
        # SOTA: Garante a limpeza do arquivo PID ao encerrar
        if te.PID_FILE.exists():
            try:
                te.PID_FILE.unlink()
            except OSError:
                pass

        # SOTA: Encerrar sessao global de forma graciosa
