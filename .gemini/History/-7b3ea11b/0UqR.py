    ...
    try:
        async with session.post(url, json=payload, timeout=10) as resp:
            if resp.status == 200:
                ...
                print(f"[!] Chave {idx:02d} ({mask}) - Rate Limit (HTTP 429)")
...
async def main():
    ...
    # Captura qualquer chave que inicie com $env:GEMINI_
    keys = re.findall(r'\$env:GEMINI_[A-Z0-9_]+_KEY(?:_\d+)?\s*=\s*[\'"]([^\'"]+)[\'"]', content, re.IGNORECASE)
