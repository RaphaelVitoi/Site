...
db_path = None
for p in [project_root / "frontend" / "prisma" / "dev.db", project_root / "dev.db"]: # Prioriza o banco do frontend
    if p.exists():
        db_path = p
        break

if not db_path:
    print(f"[ERRO FATAL] Banco SQLite (dev.db) não encontrado na infraestrutura. Buscado em: {project_root}")
    exit(1)
