...
export const InsolvencyMatrix: React.FC = () =>
{
    ...
    const colorProps = useMemo( () => ( { style: { color: radarColor } } ), [ radarColor ] );
    ...
    const chartData = result ? [
        ...
        { metric: 'A* Proj. EV', value: Number( Math.max( 0, result.trueInsolvencyEv ).toFixed( 2 ) ) },
        { metric: 'A* Risk Infl.', value: Number( ( result.riskIndex * 100 ).toFixed( 2 ) ) }
    ] : [];

    return (
        <div className="bg-[#121212] border border-[#00ffcc]/30 p-6 rounded-lg font-mono text-white shadow-[0_0_15px_rgba(0,255,204,0.1)]">
            <h2 className="text-xl font-bold text-[#00ffcc] mb-4 tracking-tighter uppercase">MATRIZ DE INSOLVÊNCIA & PATHFINDING (WEBGPU O(1))</h2>
            ...
            <div className="bg-black/50 p-4 border-l-4 border-[#00ffcc] relative overflow-hidden">
                { ( isCalculating || isRenderingDeferred ) && <div className="absolute top-0 right-0 p-2 text-[0.65rem] font-bold tracking-widest text-[#00ffcc] uppercase animate-pulse">Triturando VRAM...</div> }

                <h3 className="text-sm text-gray-500 uppercase tracking-widest mb-2">Termodinâmica & Pathfinding</h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 text-lg items-center">
                    <div>
                        <div><span className="text-gray-500 text-sm block">Win Rate (VRAM)</span> { ( ( result?.winRate ?? 0 ) * 100 ).toFixed( 2 ) }%</div>
                        <div className="mt-2"><span className="text-gray-500 text-sm block">A* Projected EV</span> { result?.trueInsolvencyEv?.toFixed( 2 ) ?? '0.00' } bb</div>
                        <div className="mt-4 pt-2 border-t border-gray-800 text-2xl font-bold" {...colorProps}>
                            <span className="text-gray-500 text-sm block">Risk Inflation (A* Sizing)</span>
                            { ( ( result?.riskIndex ?? 0 ) * 100 ).toFixed( 1 ) }%
                        </div>
                    </div>
