...
const CustomTooltip = ({ active, payload }: { active?: boolean, payload?: any[] }) => {
    if (active && payload && payload.length > 0) {
        ...
        return (
            <div className="px-6 py-5 bg-slate-950/95 backdrop-blur-3xl border border-white/10 rounded-2xl shadow-2xl relative overflow-hidden transition-all">
                <div className="absolute inset-0 bg-radial-[at_top_right] from-accent-indigo/10 to-transparent pointer-events-none" />
                <p className="text-[0.65rem] font-black text-text-darker uppercase tracking-[0.25em] mb-2 relative z-10">{payloadData.subject}</p>
                <p className="text-xl font-black text-white font-mono tracking-tighter tabular-nums relative z-10">
                    {(payloadData.value * 100).toFixed(1)}
