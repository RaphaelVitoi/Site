...
async def _compress_context(text: str, task_agent: str, prefer_local_fallback: bool = False) -> str:
    ...
    try:
        ...
        gemini_failed = False
        if gemini_compression_keys and not prefer_local_fallback and not await _is_route_blocked(route_gemini):
            logging.info(f"[[{_c(task_agent)}]{task_agent}[/]] Acionando compressao cognitiva via Gemini Nativo ({gemini_model})...")
            last_gemini_error = None
            for i, key in enumerate(gemini_compression_keys):
                try:
        ...
        if not prefer_local_fallback:
            ...
            await _register_route_failure(route_gemini)
            if last_gemini_error and ("connection closed" in str(last_gemini_error).lower() or "cannot connect" in str(last_gemini_error).lower()):
                # Consistente com _try_provider: banimento imediato atingindo o threshold de uma so vez
                extra = max(0, ROUTE_FAILURE_THRESHOLD - 1)
                for _ in range(extra):
                    await _register_route_failure(route_gemini)

        OPENROUTER_ALTERNATIVE_MODELS = te.OPENROUTER_ALTERNATIVE_MODELS
        if OPENROUTER_KEYS and (prefer_local_fallback or gemini_failed or not gemini_compression_keys or await _is_route_blocked(route_gemini)):
            compression_model = next(
                (m for m in OPENROUTER_ALTERNATIVE_MODELS if "r1" not in str(m).lower()),
                OPENROUTER_ALTERNATIVE_MODELS[0] if OPENROUTER_ALTERNATIVE_MODELS else "mistralai/mistral-small-3.1-24b-instruct:free"
            )
        ...
        # Nenhum provedor retornou resultado - sem chaves ou todos falharam silenciosamente.
...
async def call_llm_api(task: Task, system_prompt: str, user_prompt: str, manager: QueueManager, require_json: bool = False, **kwargs) -> str:
    ...
    _c = te._c
    AGENT_ROUTING_MAP = te.AGENT_ROUTING_MAP
    DEEP_THINKING_MODELS = te.DEEP_THINKING_MODELS
    FAST_OPERATIONS_MODELS = te.FAST_OPERATIONS_MODELS
    AGENTS_MANIFEST = te.AGENTS_MANIFEST
    ...
    agent_clean = task.agent.replace("@", "")
    # model_override no metadata tem prioridade maxima (ex: escalada @securitychief critical)
    designated_model = (task.metadata or {}).get("model_override") or AGENTS_MANIFEST.get(agent_clean, {}).get("primary_model")
    if designated_model:
        if designated_model in models_to_try:
            models_to_try.remove(designated_model)
        models_to_try.insert(0, designated_model)

    # Analisa saude da compressao (Circuit Breaker) para chavear para modo local
