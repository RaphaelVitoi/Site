...
async def call_llm_api(task: Task, system_prompt: str, user_prompt: str, manager: QueueManager, require_json: bool = False, **kwargs) -> str:
    ...
    if len(user_prompt) > 10000:
        logging.info(f"[[{_c(task.agent)}]{task.agent}[/]] Contexto do usuario muito longo ({len(user_prompt)} chars). Acionando compressao...")
        user_prompt = await _compress_context(user_prompt, task.agent, manager)
