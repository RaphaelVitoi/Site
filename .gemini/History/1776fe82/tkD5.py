...
# from llm.gemini import call_gemini # Nao mais necessario, _try_provider encapsula
...
async def _try_compress_gemini(session: aiohttp.ClientSession, system_prompt: str, text: str, task_agent: str, _c) -> tuple[Optional[str], bool]:
async def _try_compress_gemini(session: aiohttp.ClientSession, system_prompt: str, text: str, task_agent: str, _c, manager: QueueManager) -> tuple[Optional[str], bool]:
    ...
    for i, key in enumerate(gemini_compression_keys):
        try:
            dyn_timeout = aiohttp.ClientTimeout(total=60.0, connect=10.0, sock_read=50.0)
            response, _ = await call_gemini(session, gemini_model, system_prompt, text, key, dyn_timeout)
            COMPRESSION_CIRCUIT_BREAKER["consecutive_failures"] = 0
            return response, False
        except Exception as gemini_error:
            last_gemini_error = gemini_error
            logging.warning(f"[[{_c(task_agent)}]{task_agent}[/]] Gemini (Chave {i+1}) indisponivel para compressao: {gemini_error}.")
            if "429" in str(gemini_error):
                import random
                import re
                try:
                    m = re.search(r"retry_after=(\d+)", str(gemini_error))
                    delay = int(m.group(1)) if m else 15
                except:
                    delay = 15
                await asyncio.sleep(max(15, delay) + random.uniform(1.0, 3.0))
            continue

    # Cria uma tarefa dummy para _try_provider
    dummy_task = Task(
    ...
    from llm.budget import _register_route_failure, ROUTE_FAILURE_THRESHOLD
    await _register_route_failure(route_gemini)
    if last_gemini_error and ("connection closed" in str(last_gemini_error).lower() or "cannot connect" in str(last_gemini_error).lower()):
        extra = max(0, ROUTE_FAILURE_THRESHOLD - 1)
        for _ in range(extra):
            await _register_route_failure(route_gemini)
    return None, True
    # Usa _try_provider para compressao
    result = await _try_provider(
        ...
        task=dummy_task,
        manager=manager,
        max_retries=1, # Compressao deve ser rapida, poucos retries para erros gerais
        timeout=aiohttp.ClientTimeout(total=60.0, connect=10.0, sock_read=50.0),
        require_json=False,
        _c=_c
    )

    if result and result != "SKIP_KEY" and result != "ROUTE_BLOCKED":
        COMPRESSION_CIRCUIT_BREAKER["consecutive_failures"] = 0
        return result["text"], False
    else:
        logging.warning(f"[[{_c(task_agent)}]{task_agent}[/]] Falha na compressao Gemini Flash. Resultado: {result}")
        # Se _try_provider retornou ROUTE_BLOCKED ou SKIP_KEY, a compressao Gemini falhou.
        return None, True

async def _try_compress_openrouter(session: aiohttp.ClientSession, system_prompt: str, text: str, task_agent: str, _c) -> Optional[str]:
    OPENROUTER_ALTERNATIVE_MODELS = te.OPENROUTER_ALTERNATIVE_MODELS
    if not OPENROUTER_KEYS:
        return None

    # SOTA: Refatorar _try_compress_openrouter para usar _try_provider tambem.
    # Por enquanto, mantem a logica existente para evitar regressao.
    ...
    for i, key in enumerate(OPENROUTER_KEYS):
        try:
            ...
            response, _ = await call_openrouter(session, compression_model, system_prompt, text, key, dyn_timeout)
            COMPRESSION_CIRCUIT_BREAKER["consecutive_failures"] = 0
            return response
        except Exception as e:
            logging.warning(f"[[{_c(task_agent)}]{task_agent}[/]] OpenRouter (Chave {i+1}) indisponivel para compressao: {e}.")
            continue
    return None
...
async def _compress_context(text: str, task_agent: str, prefer_local_fallback: bool = False) -> str:
async def _compress_context(text: str, task_agent: str, manager: QueueManager, prefer_local_fallback: bool = False) -> str:
    ...
    try:
        ...
        if not prefer_local_fallback:
            response, gemini_failed = await _try_compress_gemini(session, system_prompt, text, task_agent, _c)
        ...
        if prefer_local_fallback or gemini_failed:
            # SOTA: _try_compress_openrouter tambem deveria ser refatorado para usar _try_provider
            # e receber o manager. Por enquanto, mantem a logica existente.
            response = await _try_compress_openrouter(session, system_prompt, text, task_agent, _c)
            if response:
                return response
