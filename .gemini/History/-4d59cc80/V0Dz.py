...
def parse_poker_range(range_str_list):
    active_hands = set()
    for item in range_str_list:
        ...
        # Range like 22-77
        if '-' in item and not item.endswith('s') and not item.endswith('o'):
            r1_char, r2_char = item.split('-')
            idx1 = RANKS.index(r1_char[0])
            idx2 = RANKS.index(r2_char[0])
            for i in range(min(idx1, idx2), max(idx1, idx2) + 1):
                active_hands.add((i, i))
        # Range like A2s-A5s
        elif '-' in item and item.endswith('s'):
            start, end = item.replace('s','').split('-')
            r1 = start[0]
            idx1 = RANKS.index(r1)
            idx_start = RANKS.index(start[1])
            idx_end = RANKS.index(end[1])
            for i in range(min(idx_start, idx_end), max(idx_start, idx_end) + 1):
                active_hands.add((idx1, i))
        # Range like A2o-A9o
        elif '-' in item and item.endswith('o'):
            start, end = item.replace('o','').split('-')
            r1 = start[0]
            idx1 = RANKS.index(r1)
            idx_start = RANKS.index(start[1])
            idx_end = RANKS.index(end[1])
            for i in range(min(idx_start, idx_end), max(idx_start, idx_end) + 1):
                active_hands.add((i, idx1))
        # Range like 22+
        elif item.endswith('+') and len(item) == 3:
            r = item[0]
            idx = RANKS.index(r)
        ...
        # Range like A2s+
            ...
            for i in range(idx1 + 1, idx2 + 1):
                active_hands.add((idx1, i))
        # Range like A2o+
        elif item.endswith('o+') and len(item) == 4:
            r1 = item[0]
            r2 = item[1]
            idx1 = RANKS.index(r1)
            idx2 = RANKS.index(r2)
        ...
        # Single hands
            ...
            if len(item) == 2: # Pair
                ...
                active_hands.add((idx, idx))
            elif item.endswith('s'):
                idx1 = RANKS.index(item[0])
                idx2 = RANKS.index(item[1])
                active_hands.add((min(idx1, idx2), max(idx1, idx2)))
            elif item.endswith('o'):
                idx1 = RANKS.index(item[0])
                idx2 = RANKS.index(item[1])
                active_hands.add((max(idx1, idx2), min(idx1, idx2)))
    return active_hands

# SOURCE: Aula 1.2
BTN_RAISE_HANDS = ['22+', 'A2s+', 'K2s+', 'Q2s+', 'J4s+', 'T6s+', '96s+', '86s+', '76s', '65s', '54s', 'A7o+', 'KTo+', 'QJo']

BB_SHOVE_HANDS = ['22-77', 'A2s-A5s', 'A8s-A9s', 'KJs-KTs', 'QJs', 'AJo-ATo']
BB_RAISE_HANDS = ['88+', 'AQs+', 'KQs', 'AKo', 'AQo']
BB_CALL_HANDS = [
    'A2s-A7s', 'A9s', 'K2s-K9s', 'Q2s-QTs', 'J2s-JTs', 'T2s-T9s', '92s-98s', '82s-87s', '72s-76s', '62s-65s', '52s-54s', '42s-43s', '32s',
    'A2o-A9o', 'K2o+', 'Q5o+', 'J7o+', 'T7o+', '97o+', '87o', '76o'
]
...
final_content = template.replace("{btn_grid_ts}", build_grid_string('BTN_ACTION_GRID', btn_grid)).replace("{bb_grid_ts}", build_grid_string('BB_ACTION_GRID', bb_grid))
