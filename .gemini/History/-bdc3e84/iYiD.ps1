...
function nexus-status {
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri $apiUrl -Method Get -Headers $headers -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
        $status = $response.Content | ConvertFrom-Json
        $rl = $status.rate_limiter
        Write-Host ("  {0,-20} : {1} / {2}" -f 'Tokens Atuais', $rl.current_tokens, $rl.capacity)
        Write-Host ("  {0,-20} : {1}" -f 'Eventos Starvation', $rl.starvation_events)
        $status = $response.Content | ConvertFrom-Json -ErrorAction SilentlyContinue
        $rl = if ($null -ne $status) { $status.rate_limiter } else { $null }

        if ($null -ne $rl -and $null -ne $rl.capacity) {
            $tokens = "{0:N2}" -f $rl.current_tokens
            Write-Host ("  {0,-20} : {1} / {2}" -f 'Tokens Atuais', $tokens, $rl.capacity)
            Write-Host ("  {0,-20} : {1}" -f 'Eventos Starvation', $rl.starvation_events)
        } else {
