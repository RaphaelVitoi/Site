...
function stop-worker { 
    ...
    $workers = Get-CimInstance Win32_Process -Filter "name LIKE 'python%'" | Where-Object { $_.CommandLine -match "task_executor.py" }
    if ($workers) {
...
function Set-NexusAutonomy {
    ...
    try {
        ...
        Invoke-WebRequest -Uri $apiUrl -Method Post -Body $body -cia Zero)"
...
function ask {
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri $apiUrl -Method Post -Body $body -Hne
