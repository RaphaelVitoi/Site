...
function stop-worker {
    ...
    if (Test-Path $PidFilePath) {
        try {
            ...
            if ($process) {
                Write-Host "[INFO] Tentando parada graciosa (SIGINT) para o processo PID: $pid..." -ForegroundColor DarkGray
                # Stop-Process sem -Force tenta um fechamento gracioso, que pode acionar o 'except KeyboardInterrupt' no Python.
                Stop-Process -Id $pid -ErrorAction SilentlyContinue | Out-Null
                ...
                Write-Warning "[AVISO] PID ($pid) encontrado em .nexus_worker.pid, mas o processo nao existe. Removendo arquivo obsoleto."
...
function nexus-db {
    ...
    if ($Arguments.Length -eq 0) {
        Write-Host "Uso: nexus-db <subcomando> [argumentos]" -ForegroundColor Yellow
        Write-Host "Exemplos:"
        Write-Host "  nexus-db get all         # Equivalente a 'nexus-list'"
        Write-Host "  nexus-db check-integrity # Equivalente a 'nexus-checkdb'"
        Write-Host "  nexus-db cleanup 30"
        Write-Host "  nexus-db backup-online"
        return
    }
