# Matriz de Distribuicao de Conhecimento - Workflow SOTA v7.0
...
### Tier 3: EXECUCAO ESPECIFICA (tasks.db + MEMORY.md)

**Autoridade:** Distribuida

| Topico                      | Localizacao                               | Atualizado Por                 | Frequencia                  |
| --------------------------- | ----------------------------------------- | ------------------------------ | --------------------------- |
| Historico de Operacoes      | `tasks.db` (SQLite)                       | task_executor.py (Orquestrador)| Real-time (Automático)      |
| Matriz de Recuperacao       | `tasks.db` (Metadados)                    | @auditor (quando novo cenario) | Quando novo padrao de falha |
| Metricas de Qualidade       | `tasks.db` (Tabela daily_usage)           | task_executor.py               | Real-time                   |
...
3. **Manifesto (`agents_manifest.json`):** Chaves, regex e routing adicionados
...
2. **tasks.db:** SQLite atualizado automaticamente via Pydantic
3. **project-context.md:** Estado Atual atualizado (se afeta interpretacao do projeto)
4. **task_executor.py:** Validacao de schema atualizada via Pydantic (Kernel SOTA v7.0)

---

### Cenario 3: Bug/Falha Operacional

1. **tasks.db (SQLite):** Status da tarefa vai para "failed" com traceback SOTA no log
...
2. **tasks.db (SQLite):** Tarefa marcada como "completed" pelo Orquestrador
...
project-context.md
   ...
   tasks.db ("Para auditoria, veja SQLite DAL...")
...
GLOBAL_INSTRUCTIONS.md
   ...
   tasks.db ("Para exemplos de operacao, consulte log do Orquestrador")
...
- tasks.db (operacoes persistidas em SQLite)
