...
if ($null -ne $TargetFile -and $TargetFile.FullName) {
    Clear-Host
    Write-Host ('=== 📖 RESULTADO: {0} ===' -f $TargetFile.Name) -ForegroundColor Cyan
    ...
    Write-Host ('❌ Nenhum resultado markdown encontrado com a chave: {0}' -f $TaskId) -ForegroundColor Red
}
