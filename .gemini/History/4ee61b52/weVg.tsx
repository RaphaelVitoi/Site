...
function SelectBtn( { label, active, impossible, onClick }: Readonly<{
  ...
  const bgClass = impossible
    ? 'bg-rose-500/10'
    : active ? 'bg-accent-indigo' : 'bg-slate-800/80 hover:bg-slate-700/80';

  const textClass = impossible
    ? 'text-accent-danger/60'
...
function MetricRow( { label, value, color, loading, tooltipTitle, tooltipDesc }: Readonly<{ label: string; value: string; color: string; loading?: boolean; tooltipTitle?: string; tooltipDesc?: string }> ) {
  ...
  return (
    <div className="flex justify-between items-center">
      ...
      <span className="text-[0.68rem] font-extrabold font-mono" style={ { color: loading ? 'var(--text-darker)' : color } }>{ loading ? '...' : value }</span>
    </div>
  );
}
...
export default function PmLensPanel( { anteSize = 12.5, heroInvested = 1.125, currentPot = 2.5, activePlayers = 2, heroPosition: _heroPosition = 'BB', blindsRisingSoon = false, initialStacks = STACKS, initialPrizes = PRIZES }: Readonly<PmLensPanelProps> ) {
  ...
  return (
    <div className="mt-8 px-5 py-5 bg-[#0a0f1e]/70 border border-indigo-500/25 rounded-[14px]">
      {/* Header */ }
      <div className="mb-4">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <div className={ `flex justify-between text-[0.52rem] font-bold uppercase tracking-widest mb-1.5 ${isHeroIP ? 'text-accent-indigo' : 'text-accent-danger'}` }>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono" style={ { color: isHeroIP ? 'var(--accent-indigo-light)' : 'var(--accent-danger-light)' } }>{ Math.round( realizationFactor * 100 ) }%</span>
                </div>
              </div>
            ...
            <div>
              <div className="flex justify-between text-[0.52rem] text-text-dim font-bold uppercase tracking-widest mb-1.5">
                ...
                <span className="text-accent-indigo-light font-mono">{ equity }%</span>
              </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <div className="flex justify-between text-[0.52rem] text-accent-pink font-bold uppercase tracking-widest mb-1.5">
                ...
                <span className="text-accent-pink-light font-mono">{ Math.round( kappa * 100 ) }%</span>
              </div>
          ...
          <div className="flex flex-col justify-end">
            <div className="p-3 bg-black/30 border border-indigo-500/20 rounded-lg">
              ...
              <PmLensTooltip title="Motor Quântico O(1)" desc="Invoca o Web Worker nativo em Rust para triturar as matrizes combinatórias via Monte Carlo (10.000 iterações em <50ms)." align="center">
                <button onClick={ handleCalculateEquity } disabled={ isCalculatingEq || !heroRange || !villainRange } className="w-full p-1.5 bg-indigo-500/15 text-accent-indigo-light border border-indigo-500/30 rounded-md text-[0.6rem] font-extrabold uppercase disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer">
        ...
        { streetMetrics.map( s => (
          <div key={ s.name } className="bg-slate-900/60 border border-white/5 rounded-xl p-5 flex flex-col gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
            ...
            <div className="mt-auto pt-3 border-t border-white/5 flex justify-between items-center">
              ...
              <span className="text-[1.2rem] font-black font-mono" style={ { color: pmColor( s.PM ) } }>{ s.loading ? '...' : fmtPct( s.PM ) }</span>
            </div>
          </div>
        ) ) }
      </div>
