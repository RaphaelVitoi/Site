...
export default function CodeBlock( { code, children, language = 'text', title, className = '' }: Readonly<CodeBlockProps> ) {
    ...
    return (
        <div className={ `relative rounded-xl overflow-hidden border border-white/10 bg-bg-deep my-6 shadow-2xl ${className}` }>
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-bg-elevated/50 border-b border-white/5 text-xs font-mono text-text-muted">
                <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 uppercase tracking-widest text-[10px] font-black text-text-dim">{ title || language }</span>
                </div>
                { contentStr && (
                    <button
                        onClick={ handleCopy }
                    ...
                    >
                        { copied ? (
                            <><i className="fa-solid fa-check text-accent-emerald"></i> <span className="text-accent-emerald">Copiado</span></>
                        ) : (
