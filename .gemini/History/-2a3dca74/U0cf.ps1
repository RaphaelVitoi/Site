...
Write-Host " 👁️  NEXUS EYE :: Monitor de Consciência Sistêmica (Tríade Viva)" -ForegroundColor Yellow
Write-Host "==========================================================================" -ForegroundColor Cyan
...
# Streaming Contínuo e Semântico (Tradução Visual para foco TDAH/AHSD)
Get-Content -Path $LogPath -Tail 30 -Wait | ForEach-Object {
    ...
    elseif ($line -match "\[WARNING\]|Simulação|ausente|transiente") {
        Write-Host "⚠️ $line" -ForegroundColor DarkYellow
    }
    ...
    elseif ($line -match "Ignificando pensamento|Acionando cognição") {
        Write-Host "🧠 $line" -ForegroundColor Cyan
    }
    elseif ($line -match "=== ORQUESTRADOR") {
        Write-Host "`n🚀 $line" -ForegroundColor White -BackgroundColor DarkBlue
    }
    else {
