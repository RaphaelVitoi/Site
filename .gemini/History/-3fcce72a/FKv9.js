export class NashSolver {
     ...
     * @param {number} aggressionFactor - Desvio comportamental estrito vs equilíbrio ICM (0.5 = passivo · 1.0 = equilíbrio).
     * @param {number} effStack - Profundidade do Stack Efetivo para cálculo do Death Zone Dinâmico.
     */
    solve(ipRp, oopRp, aggressionFactor = 1, effStack = 25) {
        ...
        const safeFactor = Math.max(0.1, Math.min(3, Number.parseFloat(aggressionFactor) || 1));
        const safeEffStack = Math.max(5, Number.parseFloat(effStack) || 25);

        // SOTA: Teto de Morte Dinâmico (Dynamic Death Zone)
        // Stacks curtos (< 25bb) exigem maior tolerância ao risco para manter a sobrevivência térmica (Death Zone sobe).
        // Stacks profundos (> 50bb) possuem aversão exponencial à ruína, tornando a colisão suicídio precoce (Death Zone desce).
        let dynamicDeathZone = 41 + (25 - safeEffStack) * 0.4;
        dynamicDeathZone = Math.max(25, Math.min(55, dynamicDeathZone)); // Clamped [25% - 55%]
        ...
        return {
            ...
            dynamicDeathZone: dynamicDeathZone.toFixed(1),
