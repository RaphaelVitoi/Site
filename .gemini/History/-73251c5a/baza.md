...
**Status Global:**  EM EXECUÇÃO (Escopo Duplo: Infra + Produto)
...
1.  **Track A (Infra):** Implementar arquivamento automático em `cleanup.ps1` e refatorar `do.ps1` para usar variáveis de ambiente globais.
2.  **Track B (Produto):**
    *   **[CONCLUÍDO]** Mapear estrutura de pastas (Raiz).
    *   **[PENDENTE]** Ingestão do relatório `STRUCTURE_SRC.md`.
    *   Identificar CSS/JS não utilizado (Dead Code Removal).
    *   Padronizar nomenclatura de arquivos (Kebab-case).
    *   **[CONCLUÍDO]** Ingestão do relatório (Estrutura Mínima Confirmada).
    *   **[NOVO OBJETIVO]** Definir Arquitetura Inicial (Scaffold).
    *   ~~Identificar CSS/JS não utilizado~~ (Cancelado - Greenfield).

---

**Veredito:** O `@implementor` iniciará a intervenção cirúrgica agora. Prioridade máxima para não quebrar a produção enquanto refatora.
