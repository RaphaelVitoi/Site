...
async function handleByokCustom(body: ProxyPayload) {
  ...
  if (!customApiKey || !customBaseUrl || !customModelName) {
    return NextResponse.json(
      { error: "Credenciais BYOK incompletas. Verifique as configurações." },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    if (urlObj.protocol !== "https:") {
      return NextResponse.json(
        {
          error:
            "Protocolo inválido. Apenas conexões seguras (HTTPS) são permitidas no BYOK.",
        },
        { status: 403 },
      );
    }
    if (urlObj.port !== "" && urlObj.port !== "443") {
      return NextResponse.json(
        {
          error:
            "Porta inválida. Exfiltração de credenciais por portas não padronizadas bloqueada.",
        },
        { status: 403 },
      );
    }
    if (!ALLOWED_BYOK_DOMAINS.has(urlObj.hostname)) {
      return NextResponse.json(
        {
          error:
            "Domínio BYOK não autorizado pelo Cortex Shield (Proteção SSRF).",
        },
        { status: 403 },
      );
    }
      ...
      messages: [
        {
          ...
          content:
            systemInstruction ||
            "Você é um assistente lógico especialista em Teoria dos Jogos e ICM no Poker.",
        },
  ...
  if (!byokRes.ok) {
    ...
    throw new Error(
      `Upstream Provider Error: ${byokRes.status} - ${errorData}`,
    );
  }
...
async function handlePokerRacionalCloud(body: ProxyPayload) {
    ...
    body: JSON.stringify({
      ...
      systemInstruction: {
        parts: [
          {
            text:
              systemInstruction || "Você é Raphael Vitoi, especialista em ICM.",
          },
        ],
      },
    }),
  ...
  if (!gRes.ok) {
    ...
    throw new Error(
      `Upstream Provider Error (Google): ${gRes.status} - ${errorData}`,
    );
  }
...
export async function POST(req: Request) {
  try {
    ...
    if (!prompt || typeof prompt !== "string" || prompt.length > 8000) {
      return NextResponse.json(
        { error: "Payload inválido ou excede a densidade de bytes permitida." },
        { status: 400 },
      );
    }
    ...
    const errorMessage =
      error instanceof Error
        ? error.message
        : "Erro desconhecido durante parseamento JSON ou fetch.";
    console.error("[AI_GATEWAY_ERROR] Falha na ponte proxy:", errorMessage);
    return NextResponse.json(
      { error: "Falha na conexão neural. Verifique sua chave e endpoint." },
      { status: 500 },
    );
  }
  // SOTA: Rota desativada em favor do Proxy Python unificado (porta 17043).
  // Esta rota é mantida apenas para evitar que builds antigos quebrem com 404.
  return NextResponse.json(
    {
      error:
        "Endpoint desativado. Utilize o Proxy SOTA em http://127.0.0.1:17043/generate",
    },
    { status: 410 }, // 410 Gone
  );
}
