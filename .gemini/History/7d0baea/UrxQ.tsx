...
export default function GemmaPortal() {
  ...
  const [status, setStatus] = useState<"offline" | "online" | "thinking">(
    "offline",
  );
  ...
  async function handleConsult() {
    ...
    try {
      ...
      console.debug("Gemma engine local bypass or error:", error);
      const errorMsg =
        "ERRO: O motor @gemma4 está em hibernação ou offline no seu PC. Inicie via `nexus-cli start-gemma`.";
      setMessages((prev) => [
        ...prev,
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full animate-pulse ${status === "online" ? "bg-accent-emerald" : status === "thinking" ? "bg-accent-indigo" : "bg-rose-500"}`}
              />
