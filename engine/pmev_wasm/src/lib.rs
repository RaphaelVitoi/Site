...
impl PmevEngine {
    ...
    pub fn simulate_decision_tree(
        ...
        let decisions = [
            ...
            ("RAISE", pm_raise, p_best_raise),
        ];

        let mut best_action = "FOLD".to_string();
        let mut pm_best = pm_fold;
        let mut p_best_outcome = p_best_fold;

        for (act, pm, p_best) in decisions {
            if pm > pm_best {
                pm_best = pm;
                best_action = act.to_string();
                p_best_outcome = p_best;
            }
        }
    ...
    pub fn calculate_perspective(
        ...
        let pmev_value = (clamped_eq * u_win) + ((1.0 - clamped_eq) * u_lose) - ev_fold_dynamic - structural_liability + amortized_edge;

        let opt_action = if pmev_value > 0.5 {
            "RAISE".to_string()
        } else if pmev_value >= 0.0 {
            "CALL".to_string()
        } else {
            "FOLD".to_string()
        };
...
mod tests {
    ...
    #[test]
    fn test_utility() {
        let engine = PmevEngine::new();
        assert!((engine.calculate_utility(100.0, 2.25) - 57.54).abs() < 0.1);
        assert!((engine.calculate_utility(-100.0, 2.25) + 129.47).abs() < 0.1);
    }
}
