...
pub fn solve_icm_distortion_zerocopy(payload: &[f64]) -> js_sys::Float64Array {
    // Previne WASM Trap (Panic) se a ponte JS/WASM dessincronizar o tamanho do array
    if payload.len() < 8 {
        return js_sys::Float64Array::new_with_length(3);
    }
    ...
    let fold = payload[5];
    let _call = payload[6]; // Corrige a assimetria do índice omitido
    let raise = payload[7];
