...
def call_anthropic(model: str, system_prompt: str, user_prompt: str, api_key: str) -> Tuple[str, Dict[str, Any]]:
    ...
    headers = {'Content-Type': CONTENT_TYPE_JSON, 'x-api-key': api_key, 'anthropic-version': '2023-06-01'}
...
def _evaluate_api_error(error_msg: str, provider_name: str, provider_key: str, block_on_429: bool) -> str:
...
async def _execute_provider_attempt(
    ...
    try:
        logging.info(f"[{task.agent}] Acionando {model} via {provider_name} (Chave {key_index}, Tentativa {attempt})...")
    ...
    for i, key in enumerate(keys):
        ...
        for attempt in range(max_retries):
            response_text, error_msg = await _execute_provider_attempt(
                provider_name, api_call_func, model, system_prompt, user_prompt, key, task, manager, usage_keys, attempt + 1, i + 1
            )
            if response_text:
                return response_text

            logging.warning(f"[{task.agent}] Falha em {provider_name} com {model} (Chave {i+1}): {error_msg}")
            action = _evaluate_api_error(error_msg, provider_name, provider_key, block_on_429_quota)
...
async def _dispatch_provider_call(model: str, system_prompt: str, user_prompt: str, gemini_keys: List[str], anthropic_keys: List[str], openrouter_keys: List[str], task: Task, manager: QueueManager) -> Optional[str]:
    if "/" in model or "deepseek" in model or "llama" in model:
        return await _try_provider("openrouter", call_openrouter, model, system_prompt, user_prompt, openrouter_keys, task, manager, max_retries=3, usage_keys=('prompt_tokens', 'completion_tokens'))
    elif "gemini" in model:
        return await _try_provider("gemini", call_gemini, model, system_prompt, user_prompt, gemini_keys, task, manager, max_retries=2, usage_keys=('promptTokenCount', 'candidatesTokenCount'))
    elif "claude" in model:
        return await _try_provider("anthropic", call_anthropic, model, system_prompt, user_prompt, anthropic_keys, task, manager, max_retries=3, usage_keys=('input_tokens', 'output_tokens'))
...
async def call_llm_api(task: Task, system_prompt: str, user_prompt: str, manager: QueueManager) -> str:
    ...
    for model in models_to_try:
        response = await _dispatch_provider_call(model, system_prompt, user_prompt, gemini_keys, anthropic_keys, openrouter_keys, task, manager)
        if response:
            return response
