...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        history.append(
            {
                ...
                "test_passed": test_summary.get("passed") if test_summary else None,
                "test_failed": test_summary.get("failed") if test_summary else None,
                "test_total": test_summary.get("total") if test_summary else None,
                "test_results": test_results.get("results") if test_results else None,
        ...
        if verbose:
            ...
            if test_results is not None:
                print_eval_stats("Test ", test_results["results"], 0)  # type: ignore
    ...
    if history:
        if test_set:
            ...
            best_score = f"{best.get('train_passed')}/{best.get('train_total')}"
    else:
        best = {
            "iteration": 0,
    ...
    if verbose:
        ...
        print(
            f"Best score: {best_score} (iteration {best.get('iteration')})",
            file=sys.stderr,
        )
    ...
    return {
        ...
        "best_train_score": f"{best.get('train_passed')}/{best.get('train_total')}",
...
def main():
    ...
    # pylint: disable=line-too-long
    parser.add_argument(
        "--report", default="auto", help="Generate HTML report at this path"
    )
    parser.add_argument(
        "--results-dir",
        default=None,
