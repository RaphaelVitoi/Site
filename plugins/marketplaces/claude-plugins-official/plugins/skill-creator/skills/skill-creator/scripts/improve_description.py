...
from scripts.utils import parse_skill_md  # type: ignore
...
def improve_description(
    ...
    prompt = (
        ...
        "whether to use the skill, and then if it does use the skill, it "
        "reads the .md file which has lots more details and "
        "potentially links to other resources in the skill folder like helper files and scripts "
        "and additional documentation or examples.\n\n"
        'The description appears in Claude\'s "available_skills" list. When a '
        "user sends a query, Claude decides whether to invoke the skill based "
        "solely on the title and on this description. "
        "Your goal is to write a description that triggers for relevant queries, "
        "and doesn't trigger "
        "for irrelevant ones.\n\n"
    ...
    prompt += (
        ...
        'trigger correctly. When I say "based on the failures", it\'s a bit of a tricky '
        "line to walk because we don't want to overfit to the specific cases you're seeing. "
        "So what I DON'T want you to do is produce an ever-expanding list of specific "
        "queries that this skill should or shouldn't trigger for. Instead, try to generalize "
        "from the failures to broader categories of user intent and situations where this "
        "skill would be useful or not useful. The reason for this is twofold:\n\n"
        "1. Avoid overfitting\n"
        "2. The list might get loooong and it's injected into ALL queries and there might be "
        "a lot of skills, so we don't want to blow too much space on any given description.\n\n"
        ...
        "you'll have multiple opportunities to try different approaches and we'll just grab "
        "the highest-scoring one at the end.\n\n"
