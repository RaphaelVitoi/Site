...
class RuleEngine:
    ...
    def evaluate_rules(
        ...
        for eval_rule in rules:
            if self._rule_matches(eval_rule, input_data):
                if eval_rule.action == "block":
                    blocking_rules.append(eval_rule)
                else:
                    warning_rules.append(eval_rule)
...
if __name__ == "__main__":
    ...
    result = engine.evaluate_rules([test_rule], test_input)
    print("Match result:", result)

    # Test non-matching input
    test_input2 = {"tool_name": "Bash", "tool_input": {"command": "ls -la"}}

    result2 = engine.evaluate_rules([test_rule], test_input2)
