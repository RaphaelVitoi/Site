...
class Rule:
    ...
    def from_dict(cls, frontmatter: Dict[str, Any], message: str) -> "Rule":
        ...
        if simple_pattern and not conditions:
            ...
            conditions = [
                Condition(
                    field=cond_field, operator="regex_match", pattern=simple_pattern
                )
            ]
...
def load_rules(event: Optional[str] = None) -> List[Rule]:
    ...
    for file_path in files:
        try:
            file_rule = load_rule_file(file_path)
            if not file_rule:
                continue

            # Filter by event if specified
            if event:
                if file_rule.event != "all" and file_rule.event != event:
                    continue
...
def load_rule_file(file_path: str) -> Optional[Rule]:
    ...
    try:
        ...
        parsed_rule = Rule.from_dict(frontmatter, message)
        return parsed_rule
...
"""
    ...
    test_rule = Rule.from_dict(fm, msg)
