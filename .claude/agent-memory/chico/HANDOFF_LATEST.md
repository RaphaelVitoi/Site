...
- **Matriz Canônica de Preferências (Arquitetura, Especialidade e Preço):**
  *Axioma de Capacidade Plena:* Todos os modelos da tríade possuem competência irrestrita para qualquer função técnica. As diretrizes abaixo definem preferências operacionais fundamentadas nas características físicas da arquitetura, custo por token e eficiência nativa, jamais barreiras funcionais:
  - **Gemini 3.8 Flash:** Preferência primária para **Orquestração de Fluxo Agêntico**, coordenação concorrente, context caching em grande escala em TPUs e mediação multimodal de alta velocidade.
  - **Gemini 3.5 Flash-Lite (fallover para 3.6 Flash):** Duo primário de **Fast Operations**, triagem determinística, tarefas de altíssima frequência e sanitização de dados com custo marginal mínimo (tanto em chaves gratuitas do Google AI Studio / OpenRouter quanto em chaves pagas).
  - **Claude 5 (Sonnet 5 / Opus 5):** Preferência primária para **Engenharia de Código Cirúrgica + Modelagem Matemática** (formalismo PMev, teoria dos jogos, equilíbrios de Nash, contratos Rust/WASM, tipagem estrita e refatoração em grafos de dependência multi-arquivos).
  - **ChatGPT 5.6 (Terra / Sol):** Preferência primária para **Raciocínio Profundo (Deep Reasoning / Max Thinking), Auditoria de Segurança / AppSec, Arquitetura Macro de Sistemas e Atividades de Altíssima Complexidade Conceitual** (provas axiomáticas de última linha e análise de risco sistêmico).
...
```mermaid
flowchart TD
    Req["Requisição do Cliente<br/><i>(Input Payload / OpenAPI Schema)</i>"] --> C1
    
    subgraph S1 ["Camada 1: Roteamento & Triagem Determinística"]
        C1["<b>Triagem & Ingestão de Baixa Latência</b><br/>• TTFT &lt; 600ms | \\$0,30 / 1M tokens<br/>• Validação OpenAPI & Zod Schema<br/>• Detecção de Intenção & Caching Hash"]
    end

    C1 --> C2

    subgraph S2 ["Camada 2: Cache Persistente & Memória KV"]
        C2["<b>TPU KV CachedContent</b><br/>• TTL Dinâmico (1h a 24h) | 75% a 90% Desconto<br/>• Contexto Global: Teoria PMev, Código & Game Trees<br/>• Limiar Mínimo Operacional: &ge; 32.768 tokens"]
    end

    C2 --> C3

    subgraph S3 ["Camada 3: Workhorse Agêntico & Orquestração"]
        C3["<b>Motor Agêntico Principal</b><br/>• Thinking Level: Medium / High (Budget até 64k)<br/>• Ferramentas: Grounding Web & ToolCodeExecution<br/>• Orquestração Local via Antigravity CLI"]
    end

    C3 --> Decision{"Limiar Crítico Superado?<br/><i>(Dedução de Fronteira, Prova Matemática)</i>"}
    
    Decision -- "NÃO (Fluxo Padrão)" --> OutStandard["Output Consolidado Padrão-Ouro<br/><i>(Código Rust/Python, Matrizes PMev, Solução)</i>"]
    
    Decision -- "SIM (Fronteira Teórica)" --> C4

    subgraph S4 ["Camada 4: Fronteira Analítica & Dedução Axiomática"]
        C4["<b>Fronteira Analítica de Raciocínio Profundo</b><br/>• Thinking Level: High / Max<br/>• Dedução Formal Matemática Crítica (PMev / Nash)<br/>• Auditoria Axiomática de Convergência Estocástica"]
    end

    C4 --> OutDeep["Output Analítico Axiomático<br/><i>(Solução de Fronteira Validada)</i>"]
