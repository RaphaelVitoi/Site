...
- ``#proposta`` - Rotina autonoma para arquivar PRDs velhos para o .archive automaticamente. Expurgar contextos desnecessarios da memoria vetorial do Chroma.
...
### Procedencia -- `.claude/agent-memory/organizador/MEMORY.md`
...
`#padrao` - A entropia nasce da redundância. Se uma informacao existe em dois lugares, um deles está mentindo. A fonte da verdade deve ser única e imaculada.
`#aprendizado` - A contagem canônica de agentes (19) e a transição para `tasks.db` são pontos críticos de consistência que devem ser vigiados constantemente na documentação (`project-context.md`, `GLOBAL_INSTRUCTIONS.md`).
