...
- `%OneDrive%\Documentos\Site\docs\tasks\aula-icm-rp\pesquisa.md` -- relatorio completo de pesquisa com material local, fontes web, conceitos nao cobertos no material original, gaps identificados, conexoes interdisciplinares e estrutura proposta
- `%OneDrive%\Documentos\Site\.cerebro\agents\planner.md` -- instrucoes do agente que vai receber este prompt (PRD.md e SPEC.md como entregaveis)
