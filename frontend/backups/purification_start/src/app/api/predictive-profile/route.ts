...
export async function GET() {
  ...
  try {
    ...
    const telemetry = events.map(
      (e: { evLoss: number | null; isCorrect: boolean; createdAt: Date }) => ({
        evLoss: e.evLoss ?? 0,
    ...
    try {
      ...
      const logEntry = `[${timestamp}] FATAL: Falha dupla (Nexus + Prisma). User: ${userId}. Erro: ${error instanceof Error ? error.message : String(error)}\n`;
      appendFileSync(logPath, logEntry, "utf8");
    } catch (fsError) {
      console.error(
        "[SOTA API] Falha crítica ao escrever log do God Mode:",
        fsError,
      );
    }
