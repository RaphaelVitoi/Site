...
const intensity = args.includes('--intensity') ? (args[args.indexOf('--intensity') + 1] ?? 'low') : 'low';
const target = args.includes('--target') ? (args[args.indexOf('--target') + 1] ?? 'worker') : 'worker';
