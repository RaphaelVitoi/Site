...
export default function PredictiveProfilePanel() {
  ...
  if (loading) {
    return (
      <div className="p-8 text-center text-accent-indigo animate-pulse font-mono uppercase tracking-widest text-[0.7rem] border border-white/5 rounded-3xl bg-black/20">
        Sincronizando Oráculo Preditivo...
      </div>
    );
  }
    ...
    <div className="glass-panel p-8 lg:p-10 rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl relative overflow-hidden group/pred">
      ...
      <div className="flex flex-col mb-10 relative z-10">
        <h3 className="text-[0.75rem] font-black text-accent-indigo-light uppercase tracking-[0.3em] m-0 flex items-center gap-3">
          ...
          Perfil Preditivo &middot;{" "}
          <span className="text-text-muted">Fator Ψ</span>
        </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
        {Object.entries(profile).map(([key, value]) => {
          ...
          return (
            <div
              key={key}
              className="flex flex-col gap-3 p-5 rounded-2xl bg-black/40 border border-white/5 hover:bg-black/60 transition-colors"
            >
              <div className="flex justify-between items-center">
                <span className="text-[0.65rem] text-text-muted uppercase font-black tracking-widest">
                  {key}
                </span>
                <span className="text-[0.75rem] font-mono font-black text-white">
                  {(value * 100).toFixed(0)}%
                </span>
              </div>
