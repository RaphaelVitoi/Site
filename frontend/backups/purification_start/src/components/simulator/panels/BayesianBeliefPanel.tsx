...
export default function BayesianBeliefPanel ( { initialRange = "33.6%", label: _label = "BTN RFI" }: Readonly<BayesianBeliefPanelProps> )
{
    const { currentBelief, maxBelief, applyAction, undoAction, resetBelief, history } = useBayesianRange(initialRange);
    ...
    const heatmapColors = (intensity: number) => {
        ...
        if (intensity > 80) return 'bg-accent-emerald text-slate-900 border-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.4)]';
        ...
        <div className="glass-panel flex flex-col gap-10 p-6 sm:p-8 lg:p-12 rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl relative overflow-hidden transition-all duration-300">
            ...
            <div className="flex justify-between items-center flex-wrap gap-8 pb-8 border-b border-white/5">
                <div>
                    ...
                    <p className="m-0 mt-2 text-[0.65rem] text-text-dim font-medium uppercase tracking-wider">Mente Preditiva &middot; Street {history.length + 1}</p>
                </div>

                <div className="flex gap-4">
                    {history.length > 0 && (
                        <button onClick={undoAction} className="btn-secondary px-4 py-2 text-[0.6rem] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95">
                            <i className="fa-solid fa-rotate-left mr-2" /> Desfazer
                        </button>
                    )}
                    <button onClick={resetBelief} className="btn-secondary px-4 py-2 text-[0.6rem] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95 text-accent-danger border-accent-danger/20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-12">
                        ...
                        { RANKS.map( ( r1, i ) => (
                            <React.Fragment key={ `row-${ r1 }` }>
                                { RANKS.map( ( r2, j ) => {
                                    ...
                                    const intensity = getBeliefIntensity(currentBelief, hand, maxBelief);
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className={intensity === 0 ? 'opacity-10' : 'opacity-100'}>{ hand }</span>
                                            {intensity > 5 && (
                                                <span className="text-[0.4rem] opacity-60 mt-0.5">{ Math.round(intensity) }%</span>
                                            )}
                                        </div>
                                    );
                                } ) }
                ...
                <div className="space-y-8">
                    <div className="bg-black/40 p-6 rounded-3xl border border-white/5 shadow-inner">
                        <h5 className="text-[0.6rem] font-black text-text-muted uppercase tracking-[0.2em] mb-6">Filtros de Ação (Evidência)</h5>
                        <div className="flex flex-col gap-4">
                            {mockActions.map(action => (
                                ...
                                >
                                    <div className="text-[0.65rem] font-black text-white uppercase tracking-widest mb-1 group-hover:text-accent-indigo-light">{action.name}</div>
                                    <div className="text-[0.55rem] text-text-darker uppercase font-bold tracking-tighter">Likelihood: {action.likelihood}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="p-6 bg-accent-indigo/5 border border-accent-indigo/10 rounded-3xl">
                         <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-2">
                                <h6 className="text-[0.6rem] font-black text-white uppercase tracking-widest m-0">Invariante Bayesiana</h6>
                                <p className="text-[0.65rem] text-text-muted leading-relaxed m-0 font-medium">
                                    À medida que o oponente toma ações, a probabilidade de certas mãos colapsa. O mapa visualiza a &quot;Crença Posterior&quot; do motor de IA.
                                </p>
                            </div>
                         </div>
                    </div>
