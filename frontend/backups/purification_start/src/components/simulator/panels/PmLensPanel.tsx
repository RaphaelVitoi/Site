...
/**
 * IDENTITY: PM Lens â€” Framework PM incidindo sobre o Referencial (Aula 1.2)
 * PATH: src/components/simulator/panels/PmLensPanel.tsx
 * ROLE: Dado o FT real e a equity da mÃ£o fornecida pelo usuÃ¡rio, calcula por street:
 *         EV_fold = âˆ’heroCost  [1Âª ordem â€” dominante]
 *         E = EsperanÃ§a MatemÃ¡tica (ICM, sem R)
 *         P = Expectativa (E Ã— R â€” realizaÃ§Ã£o posicional)
 *         PM = P âˆ’ EV_fold  [positivo â†’ aÃ§Ã£o preferÃ­vel ao fold]
 *
...
export default function PmLensPanel({
  ...
  const [villainIndices, setVillainIndices] = useState<number[]>(() => {
    ...
    const valid = Array.from(
      { length: initialStacks.length },
      (_, i) => i,
    ).filter((i) => i !== getInitialHeroIdx());
  ...
  const [deltaHabilidade] = useDebouncedLocalStorage<number>(
    "vitoi_pm_delta_habilidade",
    50,
  );
  const [activeNodelock, setActiveNodelock] =
    useState<NodelockConstraint | null>(null);
  ...
  const isHeroIP =
    getPostFlopOrder(heroIdx) > getPostFlopOrder(primaryVillainIdx);

  const spr = Math.max(
    0.1,
    (initialStacks[heroIdx] || 10) / Math.max(1, currentPot),
  );
  ...
  useEffect(() => {
    ...
    setVillainIndices((prev) => {
      const valid = prev.filter(
        (idx) => idx < initialStacks.length && idx !== heroIdx,
      );
      if (valid.length === numVillains) return valid;
      const newVillains = [];
      for (
        let i = 0;
        i < initialStacks.length && newVillains.length < numVillains;
        i++
      ) {
        if (i !== heroIdx) newVillains.push(i);
      }
      return newVillains.length > 0
        ? newVillains
        : [Math.max(0, initialStacks.length - 3)];
    });
    ...
    <div className="glass-panel flex flex-col gap-10 p-6 sm:p-8 lg:p-12 rounded-4xl bg-bg-panel/80 backdrop-blur-xl border border-white/10 shadow-2xl relative overflow-hidden transition-all duration-300">
      ...
      <div className="flex flex-col gap-2 pb-6 border-b border-white/5">
        ...
        <p className="text-[0.65rem] text-text-dim mt-2 m-0 font-medium uppercase tracking-wider">
          Telemetria ContÃ­nua de Sunk Cost &middot; Motor ICM Isolado (SOTA
          Offloading)
        </p>
      </div>
          ...
          <div className="bg-black/40 border border-white/5 rounded-3xl p-8 flex flex-col gap-8 shadow-inner">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
              <div className="space-y-5">
                ...
                <input
                  aria-label="Ajuste de RealizaÃ§Ã£o Posicional"
                  type="range"
                  min={0.1}
                  max={2}
                  step={0.05}
              ...
              <div className="space-y-5">
                ...
                />
                <p className="text-[0.45rem] text-text-darker leading-tight m-0 uppercase tracking-[0.15em] font-black">
                  Agg extrema OOP destrÃ³i sua RealizaÃ§Ã£o (R).
                </p>
              </div>
            </div>
            {customR === null && (
              <div className="p-4 bg-accent-indigo/5 border border-accent-indigo/10 rounded-2xl flex items-center gap-3">
                ...
                <p className="text-[0.6rem] text-text-muted m-0 leading-relaxed font-medium">
                  ...
                  SPR {spr.toFixed(1)} &middot; Pos {absoluteHeroPos} &middot;
                  Agg {aggFactor.toFixed(1)}x
                  <strong className="text-white uppercase tracking-widest mr-2">
                    Motor R Ativo:
                  </strong>{" "}
                  SPR {spr.toFixed(1)} &middot; Pos {absoluteHeroPos} &middot;
          ...
          <div className="bg-black/40 border border-white/5 rounded-3xl p-8 flex flex-col gap-6 shadow-inner">
            ...
            {activeNodelock && (
              <p className="text-[0.65rem] text-accent-indigo-light m-0 leading-relaxed font-medium italic border-l-2 border-accent-indigo/30 pl-4 py-1">
                A estrutura SPR foi mitigada (+40%/street). Axioma Lipe Piv
                acionado (Îº:{" "}
          ...
          <div className="relative z-10 space-y-8">
            <div className="flex flex-col gap-5">
              ...
              <p className="text-[0.45rem] text-text-darker leading-tight m-0 uppercase tracking-widest font-black">
                No vÃ¡cuo &middot; Cega para FGS, RIO e PressÃ£o ICM.
              </p>
            </div>
          ...
          >
            ...
            <div className="flex flex-col gap-3 flex-1">
              <MetricRow
                ...
                loading={s.loading}
                tooltipDesc="Fichas investidas nÃ£o lhe pertencem mais. Elas ditam a profundidade do custo irrecuperÃ¡vel de desistir."
              />
              <MetricRow
                label="Piso (EV_fold)"
                ...
                loading={s.loading}
                tooltipDesc="A EsperanÃ§a de simplesmente desistir e ceder o pote. Qualquer aÃ§Ã£o deve superar matematicamente esta Ã¢ncora."
              />
              <MetricRow
                label="Passivo (RIO)"
                ...
                loading={s.loading}
                tooltipDesc="O custo passivo de 'acertar e continuar perdendo'. Infla geometricamente (xÂ²) em cenÃ¡rios Multiway."
              />
              <MetricRow
                label="FGS Health"
                ...
                loading={s.loading}
                tooltipDesc="PuniÃ§Ã£o gravitacional na Ã³rbita. Antecipa o dano do Big Blind iminente, forÃ§ando agressÃ£o para nÃ£o morrer cego."
              />
              <MetricRow
                label="InsolvÃªncia (Cáµ¢)"
                ...
                loading={s.loading}
                tooltipDesc="Se Cáµ¢ < 1, as Pot Odds mentem. A mÃ£o nÃ£o possui equidade suficiente para superar o passivo do RIO e do ICM."
              />
            </div>

            <div className="mt-2 pt-4 border-t border-white/5 flex flex-col gap-2">
              <div className="flex justify-between items-center">
                <span className="text-[0.6rem] font-black text-text-muted group-hover/card:text-white uppercase tracking-[0.2em] transition-colors">
                  Perspectiva
                </span>
                <span
                  className={`text-2xl font-black font-mono tabular-nums tracking-tighter ${s.loading ? "text-text-darker" : getPmColorClass(s.PM)}`}
                >
