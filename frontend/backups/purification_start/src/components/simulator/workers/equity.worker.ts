...
let initPromise: Promise<any> | null = null; // SOTA: Promise Guard evita Race Conditions na injeção assíncrona do WebAssembly
...
globalThis.onmessage = async (e: MessageEvent<EquityMessageData>) => {
	...
	try {
		...
		const equity = (calculate_equity_monte_carlo_binary as any)(
			heroMask,
			villainMask,
			cleanBoard,
			iterations,
		...
		console.warn(
			'[SOTA Worker] Falha silenciada na inferência WASM:',
			errorMessage,
		);
