...
export function useQuantumEngine({
	...
	const quantumPerspectiva = useMemo(() => {
		...
		const realizationFactor =
			heroIsIp ? 1 : baseRealization + 0.15 * (1 - effectiveKappa);
		try {
	...
	const postFlopRps = useMemo(() => {
		...
		const turnFutureRpInfluence =
			heroIsIp ? (river?.ipRp ?? 0) : (river?.oopRp ?? 0);
		...
		const flopFutureRpInfluence =
			heroIsIp ? (turn?.ipRp ?? 0) : (turn?.oopRp ?? 0);
	...
	const effectiveSprData = useMemo(() => {
		...
		const getRp = (streetObj: any, fb: number) => {
		...
		if (activeNodelock?.type === 'block_bet') {
			...
			return {
				flop: {
					...
					ip_bet_small:
						heroIsIp ? override : stableStreetFreqs.flop.ip_bet_small,
				},
				turn: {
					...stableStreetFreqs.turn,
					ip_bet_small:
						heroIsIp ? override : stableStreetFreqs.turn.ip_bet_small,
				},
				river: {
					...stableStreetFreqs.river,
					ip_bet_small:
						heroIsIp ? override : stableStreetFreqs.river.ip_bet_small,
				},
			};
		}
