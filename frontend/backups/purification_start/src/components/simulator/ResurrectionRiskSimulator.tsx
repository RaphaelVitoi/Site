...
export default function ResurrectionRiskSimulator() {
  ...
  const results = useMemo(() => {
    ...
    const { amortizedEdge: clEdgeWin } = calculateAmortizedEdge(edgeFactor, clStackWin, msStack);
    const { amortizedEdge: clEdgeLossRaw } = calculateAmortizedEdge(edgeFactor, clStackLoss, ssStackWin);
    const clEdgeLoss = clEdgeLossRaw * psychologicalPenalty;
    const { amortizedEdge: clEdgeFold } = calculateAmortizedEdge(edgeFactor, clStack - bb, msStack);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-white/5 pb-8">
        <div className="space-y-2">
          ...
          <p className="text-xs text-text-muted leading-relaxed max-w-2xl font-medium">
            ...
            Ressuscitar um oponente destrÃ³i a{" "}
            <strong className="text-white">Fear Equity</strong> da mesa. Sob o
            Axioma PsicolÃ³gico, a sua Edge Relativa despenca se vocÃª sangrar o
