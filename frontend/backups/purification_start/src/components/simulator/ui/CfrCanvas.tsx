...
interface SotaWebGpuContext {
  configure(config: { device: any; format: string; alphaMode: string }): void;
  getCurrentTexture(): { createView(): any };
}

export const CfrCanvas = forwardRef<CfrCanvasRef, Readonly<CfrCanvasProps>>(
  ({ nodes }, ref) => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const deviceRef = useRef<any>(null);
    const pipelineRef = useRef<any>(null);
    const matrixBufferRef = useRef<any>(null);
    ...
    useEffect(() => {
      ...
      async function initWebGPU() {
        ...
        try {
          ...
          if (!adapter)
            throw new Error("Adaptador WebGPU negou o pedido de contexto.");
          ...
          if (!context)
            throw new Error(
              "Falha ao ancorar o contexto de desenho WebGPU no DOM.",
            );
