...
export const BubbleFactorDiagnostic = ({
  ipRp,
  oopRp,
}: Readonly<BubbleFactorDiagnosticProps>) => {
      ...
      >
        ...
        <div>
          ...
          >
            {deltaRp > 0 ? "+" : ""}
            {deltaRp.toFixed(1)}%{" "}
            <span className="text-sm ml-1 opacity-60">ΔRP</span>
          </p>
          <p className="text-xs text-text-muted m-0 mt-1 font-medium">
            {deltaLabel}
          </p>
        </div>
      </div>
