...
export const StreetCard = ({
  ...
  const entrapmentColorClass = getPotEntrapmentColorClass(
    result.potEntrapmentRatio,
  );

  const valColorClass =
    result.valuationStreet < 1 ? "text-accent-amber" : "text-accent-emerald";
  const sprText =
    result.sprRemanescente === Infinity
      ? "∞"
      : result.sprRemanescente.toFixed(1);
