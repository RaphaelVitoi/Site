...
export function GtoCfrSimulator() {
  ...
  const spot = use(SotaSpotContext);
  ...
  const posteriorBelief = useMemo(
    () => calculateBayesianUpdate(debouncedPrior, actionProb.val, debouncedBlf),
    [debouncedPrior, actionProb.val, debouncedBlf],
  );
  ...
  useEffect(() => {
    ...
    workerRef.current ??= new Worker(
      new URL("./workers/cfr.worker.ts", import.meta.url),
      { type: "module" },
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-8 rounded-3xl border border-accent-indigo/20 flex flex-col gap-6 shadow-2xl relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start relative z-10">
            ...
            <span className="text-[0.55rem] font-black uppercase tracking-widest text-accent-indigo/50 bg-accent-indigo/5 px-2 py-1 rounded">
              Optimal Sizing
            </span>
          </div>
            ...
            >
              ...
              >
                ...
                <Tooltip
                  ...
                  itemStyle={{ color: "var(--accent-indigo-light)" }}
                />
              </RadarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 relative z-10">
            {geometricPath.map((node) => (
              ...
              >
                <div className="text-[0.55rem] text-text-darker font-black uppercase tracking-[0.2em] mb-1">
                  {node.street}
                </div>
                <div className="text-sm font-mono text-accent-indigo-light font-black">
                  {node.betSize.toFixed(1)}
                  <span className="text-[0.6rem] text-accent-indigo/50 ml-1">
                    bb
                  </span>
                </div>
              </div>
        ...
        <div className="glass-panel p-8 rounded-3xl border border-accent-emerald/20 flex flex-col gap-6 shadow-2xl relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start relative z-10">
            ...
            <span className="text-[0.55rem] font-black uppercase tracking-widest text-accent-emerald/50 bg-accent-emerald/5 px-2 py-1 rounded">
              {archetype}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 mt-auto relative z-10">
            <div className="space-y-1">
              <div className="flex justify-between text-[0.55rem] font-black text-text-muted uppercase tracking-widest">
                ...
                <span className="font-mono text-accent-emerald-light">
                  {(priorBelief * 100).toFixed(0)}%
                </span>
              </div>
              <input
                title="Prior Belief"
                aria-label="Prior Belief"
                type="range"
                min="0"
                max="1"
                step="0.05"
                value={priorBelief}
                onChange={(e) =>
                  setPriorBelief(Number.parseFloat(e.target.value))
                }
                className="w-full h-1.5 bg-white/10 rounded-full appearance-none cursor-pointer accent-accent-emerald"
              />
            </div>
            <div className="space-y-1">
              <div className="flex justify-between text-[0.55rem] font-black text-text-muted uppercase tracking-widest">
                ...
                <span className="font-mono text-accent-amber">
                  {(actionProb.blf * 100).toFixed(0)}%
                </span>
              </div>
              <input
                title="Probabilidade de Blefe"
                aria-label="Probabilidade de Blefe"
                type="range"
                min="0"
                max="1"
                step="0.05"
                value={actionProb.blf}
                onChange={(e) =>
                  setActionProb({
                    ...actionProb,
                    blf: Number.parseFloat(e.target.value),
                  })
                }
                className="w-full h-1.5 bg-white/10 rounded-full appearance-none cursor-pointer accent-accent-amber"
              />
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-12 items-center relative z-10">
            <div className="w-40 h-40 shrink-0 relative min-w-40 min-h-40">
              ...
              >
                <PieChart>
                  ...
                  <Pie
                    data={cfrStrategy.map((item, index) => ({
                      ...item,
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-[0.5rem] font-black text-text-darker uppercase tracking-[0.3em] font-mono">
                  Nash
                </span>
                <span className="text-[0.7rem] font-black text-white uppercase tracking-widest font-mono">
                  Eq
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-6 w-full">
              {cfrStrategy.map((item, index) => {
                const barProps = {
                  style: {
                    transform: `translateX(-${100 - item.strategy}%)`,
                    backgroundColor: COLORS[index % COLORS.length],
                  },
                };
                return (
                  <div
                    key={`strat-${item.action}`}
                    className="bg-black/40 p-5 rounded-2xl border border-white/5 shadow-inner group-hover:border-white/10 transition-colors"
                  >
                    <div className="flex justify-between items-center mb-3">
                      <span className="text-[0.65rem] font-black text-text-muted font-mono uppercase tracking-widest">
                        {item.action}
                      </span>
                      <span className="text-[0.8rem] font-black text-white font-mono">
                        {item.strategy.toFixed(1)}%
                      </span>
                    </div>
                    <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden border border-white/5 relative">
                      <div
                        className="absolute inset-0 w-full h-full transition-transform duration-1000 ease-out origin-left shadow-[0_0_10px_rgba(255,255,255,0.5)]"
                        {...barProps}
                      ></div>
                    </div>
                  </div>
