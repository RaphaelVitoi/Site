...
export function solveIcmDistortion(
	...
	// SOTA v4.6 GOLD: Paridade Absoluta com math_sota.py e wasm-equity (Equação Unificada)
	// A pressão sentida pelo OOP combina o RP médio basal com a penalização assimétrica
	// pelo desequilíbrio topológico (Risco de Ressurreição).
	const pressure = avgRp + Math.abs(deltaRp) * 0.25;
	const driftBase = 0.004 * (streetIdx + 1);
	const driftPenalty =
		chipEvFreqs.oop_raise * (pressure * driftBase * (1 + gravity * 0.5));

	const raiseShift =
		chipEvFreqs.oop_raise * (effectiveAggression - 1) -
		driftPenalty -
		pressure * 0.003 * activePlayers;

	const newRaise = Math.max(0, chipEvFreqs.oop_raise + raiseShift);

	// Fold Shift: Limitado pelo Teto de RP (D5/D6)
	const maxFoldAllowed = 88 - Math.min(30, gravity * 5);
	const foldShift =
		chipEvFreqs.oop_fold * (pressure * 0.012) +
