...
import {
  generateDynamicICMQuiz,
  type SimulatorState,
} from "@/components/quiz/icmQuizGenerator";
import {
  SotaMetricsContext,
  SotaWasmContext,
} from "@/components/simulator/SotaContext";
import { useSotaTelemetry } from "@/components/simulator/hooks/useSotaTelemetry";
import React, {
  useCallback,
  use,
  useEffect,
...
export const SimulatorQuizWidget: React.FC<SimulatorQuizWidgetProps> = ({
  simulatorState,
}) => {
  const [keySeed, setKeySeed] = useState(0);
  const [ghostMode, setGhostMode] = useState(false);

  const metricsCtx = use(SotaMetricsContext);
  const wasmCtx = use(SotaWasmContext);
  const { track } = useSotaTelemetry("SimulatorQuizWidget");
  const lastQuestionTime = useRef<number>(0);

  // SOTA: Cronômetro de tempo de reação iniciado na montagem
  useEffect(() => {
    lastQuestionTime.current = performance.now();
  }, []);
  ...
  const questions = useMemo(() => {
    ...
    simulatorState,
    metricsCtx?.apiQuantumMetrics,
    metricsCtx?.predictiveProfile,
  ]);
  ...
  const handleAnswer = useCallback(
    (
      ...
      const currentLatency = now - lastQuestionTime.current;
      lastQuestionTime.current = now; // Reseta para a próxima medição de tempo de reação

      const trackData: any = {
        isCorrect,
        evLoss,
        latency: currentLatency, // O(1) Time-to-Act
      };

      if (actionData?.userAction !== undefined) {
        trackData.userAction = actionData.userAction;
      }
      if (actionData?.optimalAction !== undefined) {
        trackData.optimalAction = actionData.optimalAction;
      }
      if (actionData?.metadata !== undefined) {
        trackData.metadata = actionData.metadata;
      }

      track(category as any, trackData);
    },
    [track],
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
        <div>
          ...
          <p className="text-text-muted text-[1.05rem] leading-relaxed m-0 max-w-3xl">
            O sistema leu as variáveis da sua simulação atual. Baseado nessa
            topologia de mesa, teste seu raciocínio SOTA:
          </p>
        </div>
        {ghostMode ? (
          <button
          ...
          >
            <i className="fa-solid fa-ghost" aria-hidden="true" /> Ocultar
            Solver
          </button>
        ) : (
          <button
          ...
          >
            <i className="fa-solid fa-ghost" aria-hidden="true" /> Ghost Solver
          </button>
        )}
      </div>
      ...
      {ghostMode && wasmCtx?.nashResults?.flop && (
        <div className="absolute inset-0 z-50 bg-bg-base/80 backdrop-blur-sm rounded-2xl flex items-center justify-center p-6 border border-accent-indigo/30 animate-sota-in">
          <div className="bg-bg-deep p-8 rounded-xl border border-accent-indigo/20 max-w-2xl w-full text-center shadow-2xl">
            ...
            <h4 className="text-xl font-black text-white uppercase tracking-widest mb-6">
              Ghost Solver Ativo
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                <div className="text-[0.65rem] text-text-muted uppercase mb-2 font-bold tracking-widest">
                  Agressão Média (IP)
                </div>
                <div className="text-accent-indigo font-mono font-black text-2xl">
                  {(
                    (wasmCtx.nashResults.flop.ip.bet_small.center +
                      wasmCtx.nashResults.flop.ip.bet_large.center) /
                    2
                  ).toFixed(1)}
                  %
                </div>
              ...
              <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                ...
                <div className="text-accent-emerald font-mono font-black text-2xl">
                  {wasmCtx.nashResults.flop.oop.call.center.toFixed(1)}%
                </div>
              </div>
              <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                <div className="text-[0.65rem] text-text-muted uppercase mb-2 font-bold tracking-widest">
                ...
                <div className="text-accent-danger font-mono font-black text-2xl">
                  {wasmCtx.nashResults.flop.oop.fold.center.toFixed(1)}%
                </div>
              </div>
            </div>
            <p className="text-[0.8rem] text-text-dim leading-relaxed mb-8">
              A lente térmica GTO revela a estrutura profunda deste Spot. A
              agressão ou passividade é extraída da Matriz de Nash subjacente
              (Flop). Use essa intuição antes de responder.
            </p>
            <button
              onClick={() => setGhostMode(false)}
