    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          {...({
            className:
              "fixed top-0 left-0 right-0 h-5 hover:h-7 flex items-center justify-center z-100 bg-slate-900/50 backdrop-blur-sm border-b border-white/5 cursor-pointer hover:bg-slate-800/80 transition-all duration-300",
            onClick: handleScrub,
        ...
        >
          <motion.div
            style={
              { scaleX, backgroundSize: "200% 200%", backgroundPosition } as any
            }
            {...({
              className:
                "absolute top-0 left-0 h-full bg-linear-to-r from-emerald-400 via-sky-400 to-fuchsia-500 origin-left shadow-[0_0_12px_rgba(217,70,239,0.5)]",
            } as any)}
          />
          <motion.span
            {...({
              className:
                "relative text-xs font-mono text-white mix-blend-difference font-bold",
            } as any)}
          >
            {percentage}
          </motion.span>
