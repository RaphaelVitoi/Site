...
export default function GemmaPortal() {
	...
	async function handleConsult() {
		...
		// Modulador RAG SOTA: Exige extração transparente das fontes
		const ragDirective = forceRAG
			? "\n\n[DIRETRIZ DE AUDITORIA EPISTÊMICA SOTA]: Embase sua resposta rigorosamente no RAG. Ao final da sua resposta, crie uma seção 'FONTES RECUPERADAS' listando explicitamente os fragmentos teóricos exatos e origens que você utilizou."
			: '';
		const enhancedMsg = userMsg + ragDirective;
				...
				>
					<div className="flex items-center justify-between mb-6">
						<div className="flex items-center gap-3">
							...
							<span className="text-[0.65rem] font-black text-accent-sky-light bg-accent-sky/10 px-2 py-0.5 rounded border border-accent-sky/20 animate-in fade-in zoom-in flex items-center gap-2 shadow-sm">
								<i className="fa-solid fa-database" /> RAG DB
							</span>
						</div>
						<div className="text-[0.65rem] text-text-muted font-mono">
							MODEL: Gemma 4 (31B Cloud / 4B Effective) | LATENCY: EDGE
						</div>
					</div>
					...
					<div className="relative">
						<textarea
							...
							className="w-full bg-black/60 border border-white/10 rounded-xl p-4 pr-4 focus:outline-none focus:border-accent-indigo/50 transition-all resize-none h-32 pb-14 text-sm"
							disabled={isStreaming}
						/>
						<div className="absolute right-2 sm:right-4 bottom-2 sm:bottom-4 flex items-center gap-3 sm:gap-6 bg-bg-panel/40 sm:bg-transparent backdrop-blur-md sm:backdrop-blur-none px-3 sm:px-0 py-2 sm:py-0 rounded-2xl">
							<label className="flex items-center gap-2 text-[0.55rem] sm:text-[0.6rem] text-text-muted font-black uppercase tracking-widest cursor-pointer hover:text-white transition-colors select-none">
								<input
									type="checkbox"
									checked={forceRAG}
								...
								/>
								<span className="hidden sm:inline">Exibir Fontes RAG</span>
