...
export default async function AnalyticsPage() {
    ...
    let dbError = false;
    try {
        events = await ( prisma as any ).telemetryEvent.findMany( {
    ...
    const topVazamento = sortedCategories.length > 0 ? sortedCategories[0][0] : null;
            ...
            <div className="sota-container mt-8">
                { dbError ? (
                    <GlassPanel className="flex flex-col items-center justify-center p-12 bg-accent-danger/5 border-accent-danger/20 text-center">
                        ...
                        <h2 className="text-accent-danger text-xl font-black mb-2 uppercase tracking-widest font-heading">Falha Termodinâmica</h2>
                        <p className="text-text-muted max-w-lg leading-relaxed">
                            O núcleo de telemetria local encontra-se inacessível. O Panóptico SOTA não pode processar o Sangramento de EV no momento.
                        </p>
                    </GlassPanel>
                ) : (
                    <>
                        ...
                        <GlassPanel className="flex flex-col items-center justify-center p-8 text-center hover:-translate-y-1 transition-transform">
                            <span className="text-accent-indigo text-xs font-black uppercase tracking-[0.2em] mb-4">Precisão Neural</span>
                            <span className="text-5xl sm:text-6xl font-black tracking-tighter text-white mb-2">{ winrate.toFixed( 1 ) }<span className="text-3xl text-text-muted">%</span></span>
                            <span className="text-text-dim text-xs font-medium uppercase tracking-widest">{ totalQuestions } decisões mapeadas</span>
                        </GlassPanel>
                        ...
                        <GlassPanel className="flex flex-col items-center justify-center p-8 text-center bg-accent-danger/5 border-accent-danger/20 hover:bg-accent-danger/10 hover:-translate-y-1 transition-all">
                            <span className="text-accent-danger text-xs font-black uppercase tracking-[0.2em] mb-4">Sangria Total (EV Loss)</span>
                            <span className="text-5xl sm:text-6xl font-black tracking-tighter text-accent-danger mb-2">-{ totalEvLoss.toFixed( 2 ) }</span>
                            <span className="text-text-dim text-xs font-medium uppercase tracking-widest">Fichas / ROI dissipado</span>
                        </GlassPanel>

                        {/* KPI: ZONA CRÍTICA (Maior Furo) */ }
                        <GlassPanel className="flex flex-col items-center justify-center p-8 text-center hover:-translate-y-1 transition-transform">
                            <span className="text-accent-amber text-xs font-black uppercase tracking-[0.2em] mb-4">Ponto de Ruptura</span>
                            ...
                            <span className="text-text-dim text-xs font-medium uppercase tracking-widest">Principal foco de estudo exigido</span>
                        </GlassPanel>
                        ...
                        <GlassPanel className="lg:col-span-3 p-8 sm:p-12">
                            ...
                            { sortedCategories.length === 0 ? (
                                <div className="text-center text-text-muted py-16 flex flex-col items-center">
                                    ...
                                    <p className="font-medium text-lg">Nenhuma entropia detectada.</p>
                                    <p className="text-sm">Seu jogo permanece perfeitamente selado. Continue no laboratório.</p>
                                </div>
                            ) : (
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex justify-between items-end mb-3 text-text-muted group-hover:text-white transition-colors">
                                                    ...
                                                    <span className="text-accent-danger font-mono font-black tracking-tighter">-{ loss.toFixed( 2 ) } EV</span>
                                                </div>
