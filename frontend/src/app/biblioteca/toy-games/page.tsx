...
export default function ToyGamesPage () {
  ...
  else if ( dB < 0 ) bluffBadgeClass = 'text-rose-400 bg-rose-500/10 border-rose-500/20';

  let defenseBadgeClass = 'bg-slate-800 border-slate-700 text-slate-400';
  if ( dD > 0 ) defenseBadgeClass = 'text-sky-400 bg-sky-500/10 border-sky-500/20';
  else if ( dD < 0 ) defenseBadgeClass = 'text-rose-400 bg-rose-500/10 border-rose-500/20';
          ...
          { activeScenario && (
            <div className="glass-panel p-8 md:p-12 border-t-4 border-accent-indigo">
              <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-12 border-b border-white/5 pb-8">
                <div>
                  <span className="text-[10px] font-black text-accent-indigo-light bg-accent-indigo/10 px-3 py-1.5 rounded-md uppercase tracking-widest border border-accent-indigo/20">{ activeScenario.env }</span>
                  <h2 className="text-3xl font-black text-white mt-4 tracking-tighter uppercase">{ activeScenario.title }</h2>
                </div>
                <div className={ `px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border ${activeScenario.verdict.className}` }>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                ...
                <div className="bg-black/40 p-8 rounded-3xl border border-sky-500/20 relative overflow-hidden group">
                  <h3 className="text-sky-400 font-black text-[10px] uppercase tracking-[0.2em] mb-6 text-center">IP / Agressor</h3>
                  <div className="text-center mb-8">
                    <div className="text-4xl font-black text-white mb-2 tracking-tighter">{ activeScenario.ip.pos }</div>
                    <div className="inline-block font-mono text-xs text-slate-400 bg-slate-900 px-3 py-1 rounded-lg border border-white/5">{ activeScenario.ip.stack }</div>
                  </div>
                  
                  <div className="relative w-40 h-40 mx-auto mb-8">
                    <svg viewBox="0 0 36 36" className="w-full h-full rotate-[-90deg]">
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" strokeWidth="2" className="text-white/5" />
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" strokeWidth="2" strokeDasharray={`${(activeScenario.ip.rp / 26) * 100} 100`} strokeLinecap="round" className="text-sky-500 shadow-glow-sky" />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-black text-white">{ activeScenario.ip.rp }%</span>
                      <span className="text-[8px] font-black text-sky-400 uppercase tracking-widest">Risk Premium</span>
                    </div>
                  </div>
                  
                  <div className="text-center">
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-3">Morfologia</p>
                    <span className="text-xs font-bold text-sky-300 bg-sky-500/10 px-4 py-2 rounded-xl border border-sky-500/20">{ activeScenario.ip.morph }</span>
                  </div>
                </div>

                {/* OOP */ }
                <div className="bg-black/40 p-8 rounded-3xl border border-rose-500/20 relative overflow-hidden group">
                  <h3 className="text-rose-400 font-black text-[10px] uppercase tracking-[0.2em] mb-6 text-center">OOP / Defensor</h3>
                  <div className="text-center mb-8">
                    <div className="text-4xl font-black text-white mb-2 tracking-tighter">{ activeScenario.oop.pos }</div>
                    <div className="inline-block font-mono text-xs text-slate-400 bg-slate-900 px-3 py-1 rounded-lg border border-white/5">{ activeScenario.oop.stack }</div>
                  </div>

                  <div className="relative w-40 h-40 mx-auto mb-8">
                    <svg viewBox="0 0 36 36" className="w-full h-full rotate-[-90deg]">
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" strokeWidth="2" className="text-white/5" />
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" strokeWidth="2" strokeDasharray={`${(activeScenario.oop.rp / 26) * 100} 100`} strokeLinecap="round" className="text-rose-500 shadow-glow-rose" />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-black text-white">{ activeScenario.oop.rp }%</span>
                      <span className="text-[8px] font-black text-rose-400 uppercase tracking-widest">Risk Premium</span>
                    </div>
                  </div>

                  <div className="text-center">
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-3">Morfologia</p>
                    <span className="text-xs font-bold text-rose-300 bg-rose-500/10 px-4 py-2 rounded-xl border border-rose-500/20">{ activeScenario.oop.morph }</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-slate-900/40 p-8 rounded-3xl border border-white/5">
                  <div className="flex justify-between items-start mb-6">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Teto de Agressão (Bluff)</h4>
                    <span className={ `text-[10px] font-bold px-2 py-1 rounded-lg border ${bluffBadgeClass}` }>
                      { dB > 0 ? '+' : '' }{ dB.toFixed( 1 ) }% vs cEV
                    </span>
                  </div>
                  <div className="flex items-baseline gap-2 mb-4">
                    <span className="text-5xl font-black text-white tracking-tighter">{ dynamics.bluff.toFixed( 1 ) }</span>
                    <span className="text-xl font-bold text-sky-400">%</span>
                  </div>
                  <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden shadow-inner">
                    <div className="h-full bg-sky-500 shadow-glow-sky transition-all duration-1000 ease-out" style={ { width: `${dynamics.bluff}%` } }></div>
                  </div>
                </div>

                <div className="bg-slate-900/40 p-8 rounded-3xl border border-white/5">
                  <div className="flex justify-between items-start mb-6">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">MDF de ICM (Call)</h4>
                    <span className={ `text-[10px] font-bold px-2 py-1 rounded-lg border ${defenseBadgeClass}` }>
                      { dD > 0 ? '+' : '' }{ dD.toFixed( 1 ) }% vs cEV
                    </span>
                  </div>
                  <div className="flex items-baseline gap-2 mb-4">
                    <span className="text-5xl font-black text-white tracking-tighter">{ dynamics.defense.toFixed( 1 ) }</span>
                    <span className="text-xl font-bold text-rose-400">%</span>
                  </div>
                  <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden shadow-inner">
                    <div className="h-full bg-rose-500 shadow-glow-rose transition-all duration-1000 ease-out" style={ { width: `${dynamics.defense}%` } }></div>
                  </div>
                </div>
              </div>

              <div className="mt-12 pt-8 border-t border-white/5">
                <div className="prose prose-invert max-w-none prose-sm" dangerouslySetInnerHTML={ { __html: activeScenario.theory } } />
              </div>
            </div>
          ) }
        </div>
