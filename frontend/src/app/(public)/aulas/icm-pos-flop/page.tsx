...
export const metadata = {
	...
	description:
		'Perspectiva Matemática por street: R dinâmico, RIO multiway, Valuation ICM, Coeficiente de Insolvência e Pot Entrapment.',
};
...
const D6_CONCEPTS = [
	...
	},
	{
		...
		intuition:
			'O erro fatal é tratar Multiway como HU linear. A penalidade é uma curva exponencial.',
		color: 'accent-rose',
	},
	{
		num: '03',
		...
		intuition:
			'Quanto maior o Risk Premium, menor o Valuation. No river, Val é o filtro final.',
		color: 'accent-indigo',
	},
	{
		num: '04',
		...
		intuition:
			'PM > EV_fold é o único critério de verdade. Se PM < investido, o erro é continuar.',
...
export default function IcmPosFlopPage() {
	const pageUrl = `${SITE_CONFIG.baseUrl}${ROUTES.AULAS.POS_FLOP}`;
	const pageTitle = `ICM Pós-Flop — Framework D6 | ${SITE_CONFIG.author}`;
	...
	return (
		<div className="min-h-screen bg-bg-base text-text-bright overflow-x-hidden font-body pb-24">
			...
			<div className="sota-container mb-12">
				<SprPipeline
					stages={[
						{ name: 'PRE', potSize: 2.5, rpValue: 24 },
						{ name: 'FLOP', potSize: 7.5, rpValue: 18.5 },
						{ name: 'TURN', potSize: 22.5, rpValue: 12 },
						{ name: 'RIVER', potSize: 67.5, rpValue: 4.5 },
					]}
					activeStage={1}
				/>
			</div>
			...
			<div className="sota-container grid grid-cols-1 lg:grid-cols-2 gap-6 mb-24">
				{D6_CONCEPTS.map((c) => (
					<GlassPanel
						key={c.num}
						className="p-8 border-white/5 hover:border-accent-indigo/20 transition-all group"
					>
						<div className="flex justify-between items-start mb-6">
							<span
								className={`text-[0.6rem] font-black uppercase tracking-[0.3em] px-3 py-1 rounded-lg bg-white/5 text-${c.color}`}
							>
								D6.{c.num} — {c.symbol}
							</span>
							<i
								className={`fa-solid fa-square-root-variable text-${c.color} opacity-20 group-hover:opacity-100 transition-opacity`}
							/>
						</div>
						<h3 className="text-xl font-black text-text-bright uppercase tracking-tighter mb-4">
							{c.title}
						</h3>
						...
						<div className="space-y-4">
							<p className="text-sm text-text-muted leading-relaxed">
								<strong className="text-text-bright uppercase text-[0.6rem] tracking-widest block mb-1">
									Definição:
								</strong>{' '}
								{c.desc}
							</p>
							<p className="text-sm text-text-muted leading-relaxed italic">
								<strong className="text-accent-emerald uppercase text-[0.6rem] tracking-widest block mb-1">
									Intuição:
								</strong>{' '}
								{c.intuition}
							</p>
					...
					<div className="flex items-center gap-4 mb-6">
						...
						<div>
							<h3 className="text-xl font-black text-text-bright uppercase tracking-tighter m-0">
								PER — Pot Entrapment Ratio
							</h3>
							<p className="text-[0.6rem] font-black text-accent-rose-light uppercase tracking-widest m-0">
								Protocolo de Emergência
							</p>
						</div>
					</div>
					<p className="text-text-muted leading-relaxed mb-6">
						Mede a fração do seu stack já comprometida.{' '}
						<strong className="text-text-bright">PER &gt; 50%</strong> indica
						aprisionamento severo. Quando PER é alto e Ci &lt; 1, você está diante de um
						colapso matemático: o fold é a única manobra de salvamento,
						independentemente das cartas.
					</p>
					<div className="flex justify-center">
						<Link
							href={ROUTES.SIMULADOR}
							className="btn-primary pulse-glow px-12 py-4 text-sm font-black tracking-widest rounded-2xl"
						>
			...
			<div className="sota-container mb-24">
				...
				<GlassPanel className="p-8 sm:p-12 lg:p-16 mt-12">
					<div className="prose prose-invert prose-lg max-w-none text-text-muted">
						...
						<p>
							No ICMev, observamos o fenômeno do{' '}
							<strong className="text-accent-indigo-light">Downward Drift</strong>:
							sizings grandes migram para pequenos (B20/B33) e sizings médios
							tornam-se checks. A presença de 7% de Lead do BB sugere que o ICM altera
							a topologia da agressão em favor de quem tem o Risk Premium menor.
						</p>

						<div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-16">
							{[
								{
									label: 'C-bet Strategy',
									chip: 'Mix (S/M/L)',
									icm: 'Small (B20/B33)',
									trend: 'Compressão',
								},
								{
									label: 'BB Reaction',
									chip: 'MDF Padrão',
									icm: 'Overfold Seletivo',
									trend: 'Preservação',
								},
								{
									label: 'River Shoves',
									chip: 'Frequentes',
									icm: 'Ultra-Polarizados',
									trend: 'Cautela',
								},
							].map((row) => (
								<div
									key={row.label}
									className="p-6 rounded-2xl bg-white/3 border border-white/5"
								>
									<span className="text-[0.5rem] font-black text-text-darker uppercase tracking-[0.2em] mb-4 block">
										{row.label}
									</span>
									<div className="flex flex-col gap-2">
										<div className="flex justify-between text-xs">
											<span className="text-text-dim">ChipEV:</span>{' '}
											<span className="font-mono">{row.chip}</span>
										</div>
										<div className="flex justify-between text-xs">
											<span className="text-accent-indigo">ICMev:</span>{' '}
											<span className="font-mono text-text-bright">
												{row.icm}
											</span>
										</div>
										<div className="mt-4 pt-4 border-t border-white/5 text-center font-black text-[0.6rem] text-accent-emerald uppercase tracking-widest">
											{row.trend}
										</div>
									</div>
								</div>
						...
						<blockquote className="text-2xl font-light italic text-text-bright border-l-2 border-accent-indigo/30 pl-10 py-4 my-16">
							...
							e a precisão tática de evitar colisões onde sua edge é amortizada.&quot;
						</blockquote>
					</div>
				</GlassPanel>
			</div>
			...
			<div className="sota-container">
				<ContentFooter
					...
					shareUrl={pageUrl}
					backLinkHref={ROUTES.AULAS.MASTERCLASS}
