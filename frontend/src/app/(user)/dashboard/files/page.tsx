...
export default function FilesDashboardPage() {
  ...
  // Helper to extract numerical fields from spreadsheet data for graphing
  const getChartData = (data: NonNullable<FileContent['data']>) => {
        ...
        <GlassPanel className="border-accent-indigo/20 flex h-187.5 flex-col p-6 lg:col-span-4">
          ...
          <div className="grow space-y-4 overflow-y-auto pr-1">
            {loading ? (
              ...
              tree.map((src) => {
                const files = filterFiles(src.files);
              ...
              <div className="flex min-h-0 grow flex-col">
                {loadingContent ? (
                  ...
                  <div className="flex min-h-0 grow flex-col">
                    ...
                    {/* Render Document (LibreOffice/RAG Extracted Text) */}
                            ...
                            <tbody>
                              {fileContent.data.rows
                                ...
                                .map((row: unknown[], rIdx: number) => (
                                  <tr key={rIdx} className="border-b border-white/5 transition-colors hover:bg-white/5">
                                    {row.map((val: unknown, cIdx: number) => (
                                      <td key={cIdx} className="text-text-muted max-w-xs truncate p-3">
                                        {String(val || '')}
                                      </td>
                                    ))}
                                  </tr>
                          ...
                          <div className="rounded-lg border border-white/5 bg-white/5 p-4">
                            ...
                            <div className="text-text-muted flex h-32 items-end justify-between border-b border-l border-white/10 px-2 pt-2 font-mono text-[9px]">
                              {chartData.map((pt, idx) => {
                                const keys = Object.keys(pt).filter((k) => k !== 'name');
                                const key = keys[0];
                                if (!key) return null;
                                ...
                                return (
                                  <div key={idx} className="group relative flex grow flex-col items-center">
                                    <div
                    ...
                    {/* Render Archives (ZIP/RAR contents) */}
                      ...
                      <div className="flex grow flex-col space-y-4">
                        ...
                        <div className="max-h-120 grow overflow-auto rounded-lg border border-white/5 bg-black/20">
                          <table className="w-full border-collapse text-left text-xs">
                            ...
                            <tbody>
                              {fileContent.files.map((file, idx) => (
