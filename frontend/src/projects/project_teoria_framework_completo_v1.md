...
- Diluição de Equidade Realizada (R): menor
- Pico de RIO: probabilidade de segunda melhor mão multiplicada pelo número de oponentes
...
Se Ci < 1, as pot odds mentem.
Com 4-5 jogadores, Ci torna-se negativo → call é destrutivo para o sistema.
```
...
```
Er(S) = (ΔHabilidade / σ(Variância)) × log(S)
```
