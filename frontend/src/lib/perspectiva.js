...
export function calculateMapaICM(stacks, prizes) {
    ...
    if (totalChips === 0 || k === 0)
        return { positionProbs, equities, totalChips };
    // SOTA: Memorização via chave bitwise inteira (O(1) hashing / sem alocação de string/arrays)
    const memo = new Map();
    function compute(activeMask, posIdx, currTotal) {
        if (posIdx >= k || activeMask === 0 || currTotal === 0) {
            return {
                posC: Array.from({ length: n }, () => new Array(k).fill(0)),
                eqC: new Array(n).fill(0),
            };
        }
        const stateKey = (posIdx << 10) | activeMask;
        ...
        for (let i = 0; i < n; i++) {
            ...
            const sub = compute(activeMask ^ (1 << i), posIdx + 1, currTotal - stack);
            for (let j = 0; j < n; j++) {
                const posRow = posC[j];
                if (!posRow)
                    continue;
...
function _resolveRealizationFactor(input, stackHero, stackVillain, potSize, numPlayersInPot) {
    ...
    // Se o humanNoiseFactor (AggFactor) for > 1, aplicamos o damping de 0.15
    ...
    if (input.realizationFactor === 1 && numPlayersInPot === 2) {
        const isHeroOop = input.realizationFactor < 1 ||
...
export function calculateAmortizedEdge(edgeBase, stackHero, stackVillain, isVacuum = false, spr) {
...
export function calculatePerspectivaVitoi(input) {
    ...
    if (!validation.success) {
        if (process.env.NODE_ENV !== 'production') {
            logger.warn('PERSPECTIVA', 'Sanitizing input due to validation mismatch', {
                details: validation.error.issues,
            });
        }
    }
    const { stacks, prizes, heroIdx, villainIdx, potSize, heroCost, winProb, numPlayersInPot = 2, kappa = 0.5, humanNoiseFactor = 0, } = input;
    ...
    const { edgePenalty, amortizedEdge } = calculateAmortizedEdge(input.edgeBase, stackHero, stackVillain, isVacuum, input.spr);
    ...
    const denom = deltaWinPct * R * valuation * fgsHealth * amortizedEdge -
        prospectDeltaLose +
        bountyValue * R;
...
heroInvested, currentPot, heroRawStack, heroPosition, baseRioLiability, activePlayers = 2, humanNoiseFactor = 0, mitigationFactor = 1) {
    ...
    return Math.min(1, (baseRioLiability * mwNoiseMultiplier) / 100 +
