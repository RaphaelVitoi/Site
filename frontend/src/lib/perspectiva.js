...
export function calculateMapaICM(stacks, prizes) {
    ...
    function compute(mask, posIdx, currTotal) {
        ...
        for (let i = 0; i < n; i++) {
            if ((mask & (1 << i)) === 0)
                continue; // Bit-test: jogador já eliminado
...
function _calculateValuationAndRio(current, deltaWinPct, deltaLosePct, perspWin, input, totalPrizes, stackHero) {
...
function _buildDiagnostico(params) {
    const { perspectivaPct, rioLiability, payjumpBonus, edgePenalty, investidoAcumulado, stackHero, kappa, humanNoiseFactor, riskAdvantage, } = params;
...
export function calculatePerspectivaVitoi(input) {
    ...
    const { valuation, rioLiability, riskAdvantage } = _calculateValuationAndRio(current, deltaWinPct, deltaLosePct, perspWin, input, totalPrizes, stackHero);
