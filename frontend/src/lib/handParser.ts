...
type TournamentSummary = Pick<
  ParsedHandHistory,
  'totalEntries' | 'totalPrizePool' | 'declaredTotalChips' | 'paidPlaces' | 'prizes' | 'fullPrizes' | 'remainingPrizePool' | 'payoutUnit'
>;
...
function parseSeats(opening: string, tableId?: string): ParsedHandHistory['players'] {
  const players: ParsedHandHistory['players'] = [];
  for (const line of opening.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!/^Seat \d+:/.test(trimmed)) continue;
    const m = /^Seat (\d+): (.+) \(([$ۣ]?)([\d,.]+)(?: in chips)?(?:, .+? bounty)?\)(?:.*)$/.exec(trimmed);
    ...
    players.push({ id: `seat_${seat}`, seat, name: m[2]!, stack, ...(tableId ? { tableId } : {}) });
  }
  if (new Set(players.map(p => p.id)).size !== players.length) throw new Error('Assentos repetidos na HH.');
  return players;
}

function parseTournamentSummary(text: string, playersCount: number): TournamentSummary {
  ...
  let summary: TournamentSummary = {
    totalEntries: entries ? amount(entries[1]!) : undefined,
  ...
  if (
    fullPrizes.length &&
    summary.totalPrizePool !== undefined &&
    positions.every((pos, i) => pos === i + 1) &&
    Math.abs(sum - summary.totalPrizePool) < 1e-6
  ) {
    const prizes = fullPrizes.slice(0, playersCount);
    summary = {
      ...summary,
      fullPrizes,
      prizes,
      paidPlaces: fullPrizes.length,
      payoutUnit: 'absolute',
...
export function parseHandHistoryDetails(raw: string): ParsedHandHistory {
  ...
  let room: 'PokerStars' | 'GGPoker' | undefined;
