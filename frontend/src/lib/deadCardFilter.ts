...
export function parseBoardToMask(boardStr: string): { cards: CardObject[]; mask: bigint } {
	...
	const cleaned = boardStr.replaceAll(/[^2-9TJQKAStjqkashdcSHDC]/g, '');
...
export function generateAll1326Combos(): Array<{ handClass: string; c1: CardObject; c2: CardObject; comboStr: string }> {
	...
	for (let i = 1; i < allCards.length; i++) {
		for (let j = 0; j < i; j++) {
			...
			// Define a classe canônica (ex: AKs, AKo, AA)
			let handClass: string;
			if (card1.rankIdx === card2.rankIdx) {
				handClass = `${card1.rankChar}${card2.rankChar}`;
			} else {
				const high = card1.rankIdx > card2.rankIdx ? card1 : card2;
				const low = card1.rankIdx > card2.rankIdx ? card2 : card1;
				const suited = card1.suitIdx === card2.suitIdx;
				handClass = `${high.rankChar}${low.rankChar}${suited ? 's' : 'o'}`;
			}

			const comboStr = `${card1.rankChar}${card1.suitChar}${card2.rankChar}${card2.suitChar}`;
			combos.push({
				handClass,
				c1: card1,
				c2: card2,
...
export function computeBlockerSummary(boardStr: string): BlockerSummary {
	...
	for (const combo of ALL_1326_COMBOS) {
		...
		if (!handClassDetails[combo.handClass]) {
