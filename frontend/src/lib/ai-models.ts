 ...
 */
export function simulateCfrRegretMatching(
    evs: Record<string, number>,
    _iterations: number = 1000
): CfrAction[] {
    ...
    const totalRegret = actions.reduce( ( sum, a ) => sum + Math.max( 0, evs[a] ), 0 );

    return actions.map( a => {
        const strategy = totalRegret > 0 ? ( Math.max( 0, evs[a] ) / totalRegret ) : ( 1 / actions.length );
        return {
            action: a,
            regret: Math.max( 0, evs[a] * 1.5 ), // Escala do "Arrependimento"
...
export function calculateBayesianUpdate(
    ...
    const totalActionProb = ( actionLikelihoodValue * priorValueProb ) + ( actionLikelihoodBluff * priorBluffProb );
