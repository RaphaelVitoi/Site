...
function resolveClientBase() {
    return trimTrailingSlash(process.env.NEXT_PUBLIC_NEXUS_API_BASE ||
        process.env.NEXT_PUBLIC_API_BASE ||
        DEFAULT_NEXUS_API_BASE);
}
function resolveServerBase() {
    return trimTrailingSlash(process.env.NEXUS_API_BASE ||
...
function safeBuildUrl(base, targetPath) {
    try {
        ...
        if (url.hostname !== baseUrl.hostname)
            throw new Error('Hostname override violation');
        ...
        logger.error('API_CONTRACT', 'Tentativa de corromper o Roteamento SOTA', { error: e });
        return 'http://127.0.0.1/sota-blocked-request';
    }
}
export function buildNexusClientUrl(path) {
    return safeBuildUrl(NEXUS_CLIENT_API_BASE, path);
}
export function buildNexusServerUrl(path) {
