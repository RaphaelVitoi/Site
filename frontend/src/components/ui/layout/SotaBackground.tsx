...
export default function SotaBackground() {
	...
	return (
		<div className="fixed inset-0 z-[-1] overflow-hidden bg-bg-base pointer-events-none">
			...
			<div
				ref={orb1Ref}
				className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-accent-indigo/20 rounded-full blur-[180px] mix-blend-screen transition-transform duration-1500 ease-out"
			/>
			<div
				ref={orb2Ref}
				className="absolute bottom-[-15%] right-[-10%] w-[70%] h-[70%] bg-accent-emerald/15 rounded-full blur-[180px] mix-blend-screen transition-transform duration-1500 ease-out delay-75"
			/>
			<div
				ref={orb3Ref}
				className="absolute top-[30%] left-[50%] w-[40%] h-[40%] bg-accent-rose/10 rounded-full blur-[150px] mix-blend-screen transition-transform duration-2000 ease-out delay-150"
			/>

			{/* Vinheta SOTA Gold (Profundidade Máxima) */}
			<div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_20%,#020617_100%)] opacity-90 z-10" />
		</div>
	);
}
