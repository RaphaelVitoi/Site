...
const markdownComponents: Components = {
  h1: ({ children, ...props }) => <h1 className="text-2xl font-black text-text-bright mt-8 mb-4 uppercase tracking-tighter" {...props}>{children}</h1>,
  h2: ({ children, ...props }) => <h2 className="text-xl font-bold text-accent-indigo mt-6 mb-3 tracking-tight" {...props}>{children}</h2>,
  h3: ({ children, ...props }) => <h3 className="text-lg font-bold text-emerald-400 mt-4 mb-2" {...props}>{children}</h3>,
  p: ({ children, ...props }) => <p className="mb-4 text-[0.95rem] text-text-muted" {...props}>{children}</p>,
  ul: ({ children, ...props }) => <ul className="list-disc pl-6 mb-6 text-[0.95rem] text-text-muted space-y-2" {...props}>{children}</ul>,
  ol: ({ children, ...props }) => <ol className="list-decimal pl-6 mb-6 text-[0.95rem] text-text-muted space-y-2" {...props}>{children}</ol>,
  li: ({ children, ...props }) => <li className="pl-1" {...props}>{children}</li>,
  strong: ({ children, ...props }) => <strong className="font-bold text-text-light" {...props}>{children}</strong>,
  blockquote: ({ children, ...props }) => <blockquote className="border-l-4 border-accent-indigo/50 pl-4 py-1 italic bg-white/5 rounded-r-lg my-6" {...props}>{children}</blockquote>,
