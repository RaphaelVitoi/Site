...
export function calculateBaseFgsErosion(
	...
	if (blindsRisingSoon) {
		...
		if (heroPosition === 'IP') return timeErosion - 1.5;
		if (heroPosition === 'BB') return timeErosion - 0.5;
		return timeErosion;
	}
	return 0;
}
...
export function getHandStatus(
	...
	if (Object.hasOwn(overrides, hand)) {
		const custom = Object.getOwnPropertyDescriptor(overrides, hand)?.value as string | undefined;
