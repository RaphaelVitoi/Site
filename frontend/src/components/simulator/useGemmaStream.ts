...
export function useGemmaStream() {
    ...
    const generateAnalysis = useCallback(async (prompt: string, maxTokens: number = 1024) => {
        ...
        const baseUrl = process.env.NEXT_PUBLIC_OLLAMA_URL || 'http://127.0.0.1:11434';

        // SOTA Guard: Validação estrita de protocolo prevenindo anomalias de .env
        if (!baseUrl.startsWith('http://') && !baseUrl.startsWith('https://')) {
            setError('Bloqueio de Segurança SOTA: OLLAMA_URL deve utilizar estritamente HTTP ou HTTPS.');
        ...
        try {
            ...
            if (errorMessage.includes('Failed to fetch') || errorMessage.includes('Load failed') || errorMessage.includes('NetworkError')) {
                setError('Motor Neural Local (Ollama/llama.cpp) offline ou bloqueado por CORS. Verifique se o host está rodando em 0.0.0.0 e a porta (11434/1234) responde.');
            } else {
