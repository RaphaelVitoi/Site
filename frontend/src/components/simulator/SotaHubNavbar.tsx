            ...
            <div className="sota-glass-pill px-5 py-3 flex flex-col lg:flex-row items-center justify-between gap-4 w-full">
                ...
                <div className="flex items-center justify-center flex-wrap gap-x-4 gap-y-2 bg-black/60 px-5 py-2 rounded-3xl border border-white/10 shadow-inner text-xs font-mono text-text-muted w-full lg:w-auto mt-2 lg:mt-0 backdrop-blur-md">
                    <div className="flex items-center gap-2">
                        <span className="text-accent-emerald font-black uppercase tracking-widest text-[0.65rem]">Hero:</span> <span className="font-mono text-white font-bold">{ physics.heroStack }bb</span>
                    </div>
                    <div className="hidden sm:block w-px h-4 bg-white/20"></div>
                    <div className="flex items-center gap-2">
                        <span className="text-accent-indigo-light font-black uppercase tracking-widest text-[0.65rem]">Pote:</span> <span className="font-mono text-white font-bold">{ physics.pot }bb</span>
                    </div>
                    <div className="hidden sm:block w-px h-4 bg-white/20"></div>
                    <div className="flex items-center gap-2">
                        <span className={ physics.position === 'IP' ? 'text-accent-emerald font-black uppercase tracking-widest text-[0.65rem]' : 'text-accent-danger font-black uppercase tracking-widest text-[0.65rem]' }>{ physics.position }</span>
                    </div>
                    <div className="hidden sm:block w-px h-4 bg-white/20"></div>
                    <div className="flex items-center gap-2">
                        <select
                        ...
                        >
                            <option value="baseline" className="bg-bg-deep text-white">Baseline (EV)</option>
                            <option value="tilt" className="bg-bg-deep text-white">Stuck / Tilt</option>
                            <option value="protecting" className="bg-bg-deep text-white">Protecting Win</option>
                            <option value="bubble" className="bg-bg-deep text-white">Bubble Survival</option>
                        </select>
                    </div>
                </div>
            </div>
