...
export default function SprPipeline({ stages, activeStage = 0 }: Readonly<SprPipelineProps>) {
	...
	return (
		<div className="glass-panel p-6 border-white/5 relative overflow-hidden group">
			...
			<div className="relative flex justify-between items-center py-4 px-2">
				...
				{stages.map((stage, idx) => {
					...
					return (
						...
						>
							<div
								className={`w-14 h-14 rounded-full flex flex-col items-center justify-center border-2 bg-bg-deep backdrop-blur-md ${activeClasses}`}
							>
								<span className="text-[0.55rem] text-text-dim font-black uppercase tracking-tighter leading-none mb-0.5">
									{stage.name}
								</span>
								<span
									className={`text-sm font-black font-mono tracking-tighter ${valueColor}`}
								>
			...
			<div className="mt-8 pt-4 border-t border-white/5 text-[0.7rem] text-text-muted leading-relaxed font-medium">
				<strong className="text-text-bright">Axioma do Aprisionamento ao Pote:</strong> O RP
				residual indica o custo da colisão <em>nesta</em> street. Conforme o pote cresce
				(Multiway ou Action), a stack perde valuation e a dor do fold (Sunk Cost) supera o
				risco de eliminação. A agressão pós-flop deve antever essa dissipação — o Teto do RP
				cai.
			</div>
		</div>
	);
