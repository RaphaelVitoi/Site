...
export function SotaTooltip( { title, content, desc, align = 'center', theme = 'indigo', children }: Readonly<SotaTooltipProps> ) {
    ...
    return (
        <div className="relative group inline-flex items-center cursor-help">
            { children }
            <div className={ `absolute bottom-full ${alignClasses[align]} mb-2 w-64 sm:w-72 max-w-[85vw] p-3 bg-bg-deep border rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 pointer-events-none ${themeClasses[theme]}` }>
