...
export function SotaTooltip( { title, content, desc, align = 'center', position = 'top', theme = 'indigo', fullWidth = false, children }: Readonly<SotaTooltipProps> ) {
    const alignClasses = {
    ...
    const animationClasses = position === 'bottom' ? '-translate-y-4 group-hover:translate-y-0' : 'translate-y-4 group-hover:translate-y-0';
    const gapBridge = position === 'bottom' ? '-top-4 left-0 w-full h-4' : '-bottom-4 left-0 w-full h-4';
    const arrowClasses = position === 'bottom' ? '-top-2 border-l border-t border-inherit' : '-bottom-2 border-r border-b border-inherit';

    return (
        <div className={`relative group cursor-help items-center ${fullWidth ? 'flex w-full' : 'inline-flex'}`}>
            { children }
            <div className={ `absolute ${positionClasses} ${alignClasses[align]} w-max max-w-[280px] p-5 backdrop-blur-3xl border rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible ${animationClasses} transition-all duration-300 ease-out z-[99999] pointer-events-none ${themeClasses[theme]}` }>
                ...
                <div className={`absolute left-1/2 -translate-x-1/2 w-4 h-4 bg-inherit rotate-45 backdrop-blur-3xl ${arrowClasses}`} />
            </div>
        </div>
    );
}
