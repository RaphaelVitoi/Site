...
function getThemeConfig( isDeathZone: boolean ) {
  return {
    wrapperClass: isDeathZone ? 'border-accent-danger/40 shadow-[0_0_40px_rgba(244,63,94,0.15)] animate-pulse' : 'border-accent-indigo/20 shadow-[0_8px_30px_rgba(99,102,241,0.15)] animate-sota-in',
    ...
    narrativeBg: isDeathZone ? 'bg-gradient-to-r from-accent-danger/5 to-transparent' : 'bg-transparent'
  };
}
...
export default function QuantumSynthesis( {
  ...
  const estabilidadeEvColor = isNearPayjump || avgRp > 20 ? "text-accent-pink" : "text-accent-blue";
  const esperancaMat = getEsperancaMatConfig( isVacuum, isDeathZone, avgRp );

  const { wrapperClass, gaugeBorderTop, badgeColor, badgeBg, badgeText, rpColor, narrativeBorder, narrativeBg } = getThemeConfig( isDeathZone );
      ...
      <div className="flex justify-between items-start mb-5">
        <div className="flex gap-6 items-center">
          ...
          <div className="w-20 h-11 relative overflow-hidden shrink-0">
            ...
            <div className={ `w-20 h-20 rounded-full border-8 border-transparent absolute top-0 left-0 -rotate-45 ${gaugeBorderTop} ${borderRightClass}` }></div>
            <div
          ...
          <div>
            <span className={ `text-[0.55rem] font-black uppercase tracking-[0.2em] px-2.5 py-1 rounded-md ${badgeColor} ${badgeBg}` }>
              { badgeText }
            </span>
            <h4 className="mt-2 text-lg text-text-main font-black font-heading tracking-tighter">{ scenarioName }</h4>
          </div>
        </div>

        <div className="text-right">
          <span className="text-[0.55rem] text-text-dim uppercase font-black block tracking-widest">Tensão Sistêmica</span>
