...
function computeNodeCfr(
  ...
  // SOTA BOLT: Removed redundant `?? 0` checks. TypedArray bounded access returns 0, not undefined. Avoids branch/check overhead in hot loop.
  ...
  localStrategy[idx0] += s0;
  localStrategy[idx1] += s1;
  localStrategy[idx2] += s2;
