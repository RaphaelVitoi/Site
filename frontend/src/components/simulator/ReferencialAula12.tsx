 ...
 *       SOTA v6.2.1 GOLD: Soberania Matemática e Purificação de Damping.
 */
		...
		<div className="flex flex-col gap-12">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-8 bg-slate-900/60 rounded-4xl border border-white/10 shadow-2xl relative overflow-hidden">
				...
				<div className="space-y-2">
					<h4 className="text-[0.7rem] font-black text-accent-indigo uppercase tracking-[0.3em]">
						HRC Context
					</h4>
				...
				<div className="space-y-2">
					<h4 className="text-[0.7rem] font-black text-accent-amber uppercase tracking-[0.3em]">
						Bubble Factor
					</h4>
					<p className="text-[0.8rem] text-text-muted leading-relaxed font-medium">
						Dívida de Equidade.
				...
				<div className="space-y-2">
					<h4 className="text-[0.7rem] font-black text-accent-emerald uppercase tracking-[0.3em]">
						Risk Premium
					</h4>
					<p className="text-[0.8rem] text-text-muted leading-relaxed font-medium">
						Imposto do ICM.
						<br />
						<span className="font-mono text-white bg-white/10 px-1.5 py-0.5 rounded-sm">
							RP = (BF-1)/BF
						</span>
					</p>
				</div>
				<div className="space-y-2">
					<h4 className="text-[0.7rem] font-black text-accent-danger uppercase tracking-[0.3em]">
						Âncora SOTA
					</h4>
					<p className="text-[0.8rem] text-text-muted leading-relaxed font-medium">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
				{[
					...
					<div
						key={item.title}
						className={`p-6 rounded-3xl border transition-all ${item.color} ${item.active ? 'ring-2 ring-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : ''}`}
					>
						<h5 className="text-[0.75rem] font-black text-white mb-3 tracking-widest">
							{item.title}
						</h5>
						<p className="text-[0.7rem] text-text-muted leading-relaxed font-medium m-0">
							{item.desc}
						</p>
					</div>
				))}
	...
	return (
		<div className="flex flex-col gap-5 group/grid">
			<div className="flex flex-col gap-1">
				<h4 className="m-0 text-[1.1rem] font-black text-white uppercase tracking-tight">
					{title}
				</h4>
				<p className="m-0 text-[0.7rem] font-bold text-text-darker uppercase tracking-[0.15em]">
					{subtitle}
				</p>
			</div>
						...
						return (
							...
							>
								...
								<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
									<span
										className={`text-[0.45rem] lg:text-[0.55rem] font-black font-mono transition-colors drop-shadow-md select-none ${isEmpty ? 'text-white/10 group-hover/cell:text-white/30' : 'text-white/50 group-hover/cell:text-white'}`}
									>
										{getHandLabel(r, c)}
									</span>
								</div>
...
function BoardAndTableLeft({}: Readonly<{ rpBtn: number; rpBb: number }>) {
	const W = 540;
	const H = 380;
	const rx = 180;
	const ry = 110;
	...
	return (
		<div className="flex flex-col gap-12">
			<div>
				<p className="m-0 mb-6 text-[0.8rem] font-black text-text-darker uppercase tracking-[0.3em]">
					Board Reference
				</p>
				<div className="flex justify-center gap-4">
					{[
						...
						<div
							key={rank + suit}
							className={`w-20 h-28 rounded-2xl ${bg} border-2 ${borderClass} flex flex-col items-center justify-center gap-1 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.8)] backdrop-blur-2xl group hover:-translate-y-2 transition-transform duration-500 relative overflow-hidden`}
						>
							<div className="absolute inset-0 bg-linear-to-b from-white/10 to-transparent opacity-50" />
							<span
								className={`text-[2.4rem] font-black leading-none drop-shadow-md relative z-10 ${colorClass}`}
							>
								{rank}
							</span>
							<span
								className={`text-[1.6rem] leading-none drop-shadow-md relative z-10 ${colorClass}`}
							>
								{suit}
							</span>
						</div>
				...
				<div className="bg-slate-950/80 rounded-5xl border border-white/10 p-8 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)] overflow-hidden backdrop-blur-3xl relative">
					...
					>
						...
						<text
							...
							textAnchor="middle"
							fill="currentColor"
							className="text-[0.65rem] font-black text-white/10 uppercase tracking-[0.8em]"
						>
							SOTA
						</text>
						...
						{TABLE_PLAYERS.map((p) => {
							...
							return (
								<g
									key={p.name}
									className="group/player transition-all hover:scale-110 cursor-default"
								...
								>
									...
									<text
										...
										textAnchor="middle"
										fill="currentColor"
										className={`text-[0.65rem] font-mono font-bold ${isHero ? 'text-emerald-200/80' : isVillain ? 'text-indigo-200/80' : 'text-slate-400/80'}`}
									>
										{p.stack}bb
									</text>
								</g>
...
function RiskAndPrizesRight({}: Readonly<{
	rpBtn: number;
	rpBb: number;
	btnStack: number;
	bbStack: number;
	...
	return (
		<div className="flex flex-col gap-12">
			<div className="space-y-6">
				<p className="m-0 text-[0.8rem] font-black text-text-darker uppercase tracking-[0.3em]">
					...
					{PRIZES.map(({ pos, val, jump }, i) => {
						...
						return (
							<div key={pos} className="flex items-center gap-6 group/jump">
								<span className="text-[0.75rem] font-black text-text-darker w-6 font-mono">
									{pos}
								</span>
								...
								<div className="flex items-center gap-4 w-32 justify-end">
									{jump > 0 && (
										<span className="text-[0.65rem] font-bold text-emerald-500/40 opacity-0 group-hover/jump:opacity-100 transition-opacity">
											Δ{jump.toFixed(1)}
										</span>
									)}
									<span
										className={`text-[0.85rem] font-black font-mono ${valColorClass}`}
									>
										${val.toFixed(2)}
									</span>
								</div>
			...
			<div className="space-y-6">
				...
				<div className="bg-slate-900/60 rounded-5xl border border-white/10 p-10 space-y-8 shadow-2xl relative overflow-hidden">
					...
					<div className="flex items-start gap-6">
						...
						<div className="space-y-2">
							<h5 className="text-[0.8rem] font-black text-white uppercase tracking-widest">
								Sizing Drift Logic
							</h5>
							<p className="text-[0.75rem] text-text-muted leading-relaxed m-0 font-medium">
								...
								<strong className="text-white">
									abordagens de menor sizing (20-50%)
								</strong>
								. Esta prática é quase inexistente em cenários ChipEV tradicionais.
							</p>
						</div>
					</div>
					<div className="flex items-start gap-6">
						...
						<div className="space-y-2">
							...
							<p className="text-[0.75rem] text-text-muted leading-relaxed m-0 font-medium">
								Simulações HRC apresentam um{' '}
								<strong className="text-white">e-Nash reduzido</strong>, eliminando
								os ruídos operacionais comuns no GTO Wizard e garantindo maior
	...
	return (
		<div className="mt-6">
			<div className="flex flex-col gap-1 mb-12">
				<p className="m-0 text-2xl font-black text-white uppercase tracking-tighter">
					Strategic Toy Games
				</p>
				<p className="m-0 text-[0.8rem] font-bold text-text-darker uppercase tracking-[0.2em]">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
				{[
					...
					<div
						key={term}
						className="p-8 bg-slate-900/50 rounded-4xl border border-indigo-500/10 hover:border-indigo-500/30 transition-all group"
					>
						<span className="text-[0.9rem] font-black text-indigo-400 uppercase tracking-[0.2em] block mb-4 group-hover:translate-x-1 transition-transform">
							{term}
						</span>
						<span className="text-[0.9rem] text-text-muted leading-relaxed font-medium">
							{def}
						</span>
					</div>
				))}
				...
				<table className="w-full border-collapse bg-slate-900/40">
					<thead>
						<tr className="border-b border-white/10 bg-slate-900/80">
							<th className="py-5 px-8 text-left font-black text-text-darker text-[0.8rem] uppercase tracking-widest whitespace-nowrap">
								Node
							</th>
							<th className="py-5 px-8 text-right font-black text-indigo-400 text-[0.8rem] uppercase tracking-widest whitespace-nowrap">
								RP IP
							</th>
							<th className="py-5 px-8 text-right font-black text-rose-500 text-[0.8rem] uppercase tracking-widest whitespace-nowrap">
								RP OOP
							</th>
							<th className="py-5 px-8 text-right font-black text-emerald-500 text-[0.8rem] uppercase tracking-widest whitespace-nowrap">
								ΔRP
							</th>
							<th className="py-5 px-8 text-right font-black text-text-muted text-[0.8rem] uppercase tracking-widest whitespace-nowrap">
								Bluff IP
							</th>
							<th className="py-5 px-8 text-right font-black text-text-muted text-[0.8rem] uppercase tracking-widest whitespace-nowrap">
								Def OOP
							</th>
							<th className="py-5 px-8 text-left font-black text-text-darker text-[0.8rem] uppercase tracking-widest">
								Effect
							</th>
						</tr>
					</thead>
				...
				<summary className="cursor-pointer list-none flex items-center justify-between gap-4 p-12 lg:p-16 hover:bg-white/5 transition-all select-none outline-none border-b border-white/5">
					<div className="flex items-center gap-8">
						...
						<div className="flex flex-col gap-1.5">
							<span className="text-[0.7rem] font-black uppercase tracking-[0.5em] text-accent-indigo-light opacity-80">
								Reference Layer 01
							</span>
							<h3 className="text-xl sm:text-2xl font-black text-white m-0 tracking-tight uppercase">
								Âncora Científica SOTA v6.2.1 GOLD
							</h3>
						</div>
					</div>
					...
					<div className="grid grid-cols-1 xl:grid-cols-2 gap-24 w-full">
						...
						<div className="space-y-20">
							<RiskAndPrizesRight
								rpBtn={rpBtn}
								rpBb={rpBb}
								btnStack={BF_STACKS[6] ?? 0}
								bbStack={BF_STACKS[8] ?? 0}
							/>
						</div>
					</div>
								...
								<thead>
									<tr className="bg-slate-900/95 backdrop-blur-2xl">
										<th className="py-5 px-6 text-left border-r border-white/10 sticky left-0 z-30 bg-slate-900">
											<div className="text-[0.6rem] font-black text-indigo-400/70 uppercase text-center leading-tight">
												Hero
												<br />
												(Def)
											</div>
										...
										{matrixData.mPlayers.map((p, i) => (
											<th
												key={p}
												className="py-5 px-6 text-text-muted font-black text-[0.8rem] text-center whitespace-nowrap uppercase tracking-tighter border-b border-white/10"
											>
							...
							<div className="p-16 sm:p-20 lg:p-24 relative z-10">
								<div className="flex items-center gap-6 mb-16">
									...
									<h3 className="text-3xl sm:text-4xl font-black text-white m-0 tracking-tight uppercase">
										Audit Protocol SOTA v6.2.1 GOLD
									</h3>
								</div>
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-24 text-left items-center">
									<div className="flex flex-col gap-10 justify-center">
										...
										<p className="text-[1.1rem] text-text-muted leading-relaxed m-0 font-medium max-w-xl">
											Nesta topologia 9P, o {isBBUnderPressure ? 'BB' : 'BTN'}{' '}
											sofre uma pressão assimétrica catastrófica. A{' '}
											<strong className="text-white">
												Gravidade da Distância
											</strong>{' '}
											entre RPs ({pressureDelta}%) desloca o equilíbrio para
											um regime de{' '}
										...
										<div className="bg-black/60 border border-white/10 rounded-4xl p-12 mt-2 shadow-inner grid grid-cols-2 gap-12">
											<div className="flex flex-col gap-4">
												<span className="text-[0.7rem] font-black uppercase tracking-widest text-text-darker">
													BF BTN (Agressor)
												</span>
											...
											<div className="flex flex-col gap-4">
												<span className="text-[0.7rem] font-black uppercase tracking-widest text-accent-danger/60">
													BF BB (Defensor)
												</span>
												<span className="text-4xl font-mono font-black text-accent-danger">
													{bfBb.toFixed(2)}x
												</span>
											</div>
										</div>
									...
									<div className="flex flex-col gap-10">
										<div className="bg-white/5 border border-white/10 rounded-5xl p-12 hover:bg-white/10 transition-all hover:border-white/20 shadow-3xl group/diag relative overflow-hidden">
											...
											<h4 className="text-[0.9rem] font-black text-accent-amber uppercase tracking-widest mb-6 flex items-center gap-4 leading-none">
												...
												Structural Vulnerability (RIO)
											</h4>
											<p className="text-[1rem] text-text-muted leading-relaxed m-0 font-medium">
												O range do BB contém mãos marginais que, apesar do
												acerto, sofrem de{' '}
												<strong className="text-white">
													Reverse Implied Odds severas
												</strong>
												. O custo de colisão é inflado pelo Bubble Factor de{' '}
												{bfBb.toFixed(2)}x, tornando o call um erro de
												valuation sistêmico.
											</p>
										</div>
										...
										<div className="bg-white/5 border border-white/10 rounded-5xl p-12 hover:bg-white/10 transition-all hover:border-white/20 shadow-3xl group/diag relative overflow-hidden">
											...
											<p className="text-[1rem] text-text-muted leading-relaxed m-0 font-medium">
												Com um Risk Premium menor ({rpBtn.toFixed(1)}%), o
												BTN opera sob regime de{' '}
												<strong className="text-white">
													Agressão Descontada
												</strong>
												. O limiar de equidade é reduzido, tornando o
												overshove no River matematicamente inquestionável.
											</p>
										</div>
									</div>
								</div>
