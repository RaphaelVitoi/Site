...
function StackInput({
	...
	return (
		<div
			className={`flex flex-col gap-1.5 px-3 py-2 rounded-xl border transition-all ${isActive ? 'bg-accent-indigo/10 border-accent-indigo/30 ring-1 ring-accent-indigo/20' : 'bg-transparent border-white/5 opacity-40 hover:opacity-100'}`}
		>
			<span
				className={`text-[0.5rem] font-black uppercase tracking-tighter ${isActive ? 'text-accent-indigo-light' : 'text-text-darker'}`}
			>
				{label}
			</span>
	...
	return (
		<div className="flex flex-col gap-4 group">
			<div className="flex justify-between items-end border-b border-white/5 pb-2">
				<span className="text-[0.65rem] text-accent-indigo-light font-black uppercase tracking-widest">
					{label}
				</span>
				<div className="flex items-baseline gap-1 bg-black/60 px-2.5 py-1 rounded-lg border border-white/10 shadow-inner">
					<span className="text-[0.85rem] text-white font-mono font-black tabular-nums">
						{val.toFixed(1)}
					</span>
			...
			<div className="flex flex-col gap-1">
				<div className="flex justify-between text-[0.55rem] font-black uppercase tracking-tighter text-text-darker">
					...
					<span className="text-accent-danger/80">
						-{(val + anteSize / 100).toFixed(2)} bb
					</span>
				</div>
				<div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
					<div
						className="h-full bg-accent-danger/30 rounded-full"
...
export default function PostFlopPanel({
	...
	const isMasterControlled = Boolean(
		scenarioId && initialStacks?.length && initialPrizes?.length,
	);
	...
	const gpuWinRate = wasmCtx?.insolvencyMatrixData?.winRate
		? wasmCtx.insolvencyMatrixData.winRate * 100
		: null;
			...
			<div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 relative z-10">
				<div className="max-w-2xl">
					<div className="flex items-center gap-3 mb-2">
						<span className="text-[0.55rem] font-black text-accent-indigo-light uppercase tracking-widest bg-accent-indigo/10 border border-accent-indigo/20 px-2 py-0.5 rounded-md shadow-inner">
							Post-Flop Analyzer
						</span>
						<h3 className="text-base sm:text-lg font-black text-white m-0 tracking-tight uppercase group-hover/pfp:text-glow-indigo transition-all duration-500">
							LaboratÃ³rio de Perspectiva{' '}
							<span className="text-accent-indigo opacity-60 font-mono">v6.2.1</span>
						</h3>
					</div>
					<p className="text-[0.7rem] text-text-muted mt-1.5 mb-0 leading-relaxed font-medium">
						Mapeamento termodinÃ¢mico de EV_fold e Risk Premium. O descarte (
						<code className="text-accent-danger/80">Fold</code>) possui valor
						intrÃ­nseco de sobrevivÃªncia baseado no Payjump e na inÃ©rcia da mesa.
					</p>
				</div>
				...
				<div className="flex items-center gap-2 bg-black/40 p-1.5 rounded-xl border border-white/5 shadow-inner">
					...
					>
						<i
							className={`fa-solid ${isLocked ? 'fa-link' : 'fa-link-slash'} text-[0.6rem]`}
						/>
			...
			<div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
				<div className="lg:col-span-4 flex flex-col gap-5">
					<div className="space-y-2">
						<span className="text-[0.55rem] font-black text-text-darker uppercase tracking-[0.2em] flex items-center gap-2 px-1">
							<i className="fa-solid fa-flask-vial text-accent-indigo" /> Presets de
							LaboratÃ³rio
						</span>
					...
					<div className="space-y-2">
						<span className="text-[0.55rem] font-black text-text-darker uppercase tracking-[0.2em] flex items-center gap-2 px-1">
							<i className="fa-solid fa-arrows-left-right text-accent-sky" />{' '}
							Posicionamento Hero
						</span>
					...
					<div className="space-y-2">
						<span className="text-[0.55rem] font-black text-text-darker uppercase tracking-[0.2em] flex items-center gap-2 px-1">
							<i className="fa-solid fa-users-viewfinder text-accent-violet" />{' '}
							Entropia Multiway
						</span>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 relative z-10">
				{results.map(([street, result]) =>
					result ? (
						...
						<div
							key={street}
							className="bg-black/40 border border-white/5 rounded-2xl p-8 flex flex-col items-center justify-center text-center opacity-30 shadow-inner"
						>
			...
			<div className="bg-bg-deep/80 backdrop-blur-md border border-white/10 rounded-3xl p-5 lg:p-7 shadow-2xl relative overflow-hidden group mt-2">
				...
				<div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-white/5 pb-4 mb-4 relative z-10 gap-4">
					<div className="flex items-center gap-3">
						<div
							className={`w-1.5 h-1.5 rounded-full ${isCalculatingWasm ? 'bg-accent-emerald animate-pulse shadow-[0_0_10px_var(--accent-emerald)]' : 'bg-accent-indigo shadow-[0_0_10px_var(--accent-indigo)]'}`}
						></div>
						<strong className="text-[0.7rem] font-black text-white uppercase tracking-widest">
							InteligÃªncia HÃ­brida (WASM + PM Lens)
						</strong>
						<span
							className={`px-2 py-0.5 rounded border text-[0.55rem] font-mono tabular-nums ${isLocked ? 'bg-accent-indigo/10 border-accent-indigo/30 text-accent-indigo-light' : 'bg-white/5 border-white/10 text-text-muted'}`}
						>
					...
					<div className="flex flex-wrap items-center gap-2">
						{pm !== null && (
							<div className="flex items-center gap-2 bg-black/40 px-2.5 py-1 rounded-lg border border-white/5 shadow-inner">
								<span className="text-[0.55rem] font-black text-text-dim uppercase tracking-widest">
									PM Global:
								</span>
								<span
									className={`text-[0.7rem] font-black font-mono tabular-nums ${getPmColorClass(pm)}`}
								>
									{formatPct(pm)}
								</span>
							</div>
						...
						{ci !== null && (
							<div className="flex items-center gap-2 bg-black/40 px-2.5 py-1 rounded-lg border border-white/5 shadow-inner">
								...
								<span
									className={`text-[0.7rem] font-black font-mono tabular-nums ${ci < 1 ? 'text-accent-danger' : 'text-accent-emerald'}`}
								>
									{formatCi(false, ci)}
								</span>
							</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
					<div className="flex flex-col gap-2.5">
						...
						<div className="bg-black/50 border border-white/5 rounded-xl p-3.5 flex flex-col justify-center gap-3.5 h-full shadow-inner">
							<div className="flex justify-between items-center">
								...
								<span
									className={`text-[0.8rem] font-mono font-black tabular-nums ${winRateClass}`}
								>
									{winRateText}
								</span>
							</div>
							...
							<div className="flex justify-between items-center">
								...
								<span
									className={`text-[0.8rem] font-mono font-black tabular-nums ${riskIndexClass}`}
								>
									{riskIndexText}
								</span>
							</div>
					...
					<div className="md:col-span-2 flex flex-col justify-center">
						...
						<p className="text-[0.7rem] text-text-dim leading-relaxed m-0 italic font-medium">
							...
							<code className="text-accent-danger font-bold bg-accent-danger/10 px-1 rounded">
								EV_fold
							</code>{' '}
							e{' '}
							<code className="text-accent-indigo-light font-bold bg-accent-indigo/10 px-1 rounded">
								Risk Premium
							</code>
							. A{' '}
							...
							desmascara a ilusÃ£o do ChipEV, enquanto o Motor WebGPU injeta
							Pathfinding A*, mitigando o viÃ©s do vÃ¡cuo pÃ³s-flop.
						</p>
						{!isLocked && (
							<button
