...
export default function TournamentTableImport({ onApply, initialContext, defaultRoom }: Readonly<{ defaultRoom?: PokerRoom; onApply: (context: AppliedTournamentContext) => void; initialContext?: AppliedTournamentContext | null }>) {
  ...
  function receiveFile(file?: File) {
    ...
    setError(null);
    if (!/\.(?:json|txt|hh)$/i.test(file.name) || file.size > 5 * 1024 * 1024) {
      setError('Escolha .txt/.hh ou JSON (Hand Config ou estruturas) de até 5 MB. No HRC, use Save As → JSON; .hrcz e .hrcv contêm dados adicionais do solver.');
      return;
    }
    const reader = new FileReader();
    reader.onload = () => { if (revision === inputRevision.current && typeof reader.result === 'string') receiveText(reader.result.replace(/^\uFEFF/, '')); };
  ...
  return <section aria-label="Importar torneio e selecionar mesa" className="space-y-4 rounded-2xl border border-white/15 p-4 text-sm text-text-light">
    ...
    <label className="block">Carregar HH ou configuração HRC
      <input aria-label="Carregar HH ou configuração HRC" type="file" accept=".txt,.hh,.json,text/plain,application/json" className={fieldClass}
        onChange={event => { receiveFile(event.target.files?.[0]); event.target.value = ''; }} />
    </label>
    <p>Cole uma HH em inglês do PokerStars/GGPoker ou abra um arquivo. A leitura preenche os dados disponíveis automaticamente. Para HRC: Save As → JSON (Hand Config), ou uma coleção JSON de estruturas junto da HH. Revise os defaults de premiação quando a HH não trouxer payouts.</p>
    ...
    {snapshot && <>
      ...
      {snapshot.sourceFormat === 'hrc-hand-config' && <p>Configuração HRC reconhecida: stacks, blinds, ante e payouts preenchidos. A mesa veio selecionada; confirme a sala. Estratégias e EVs do solver não são importados como resultados desta bancada.</p>}
