...
export default function TournamentConditionsPanel({ value, onChange }: Readonly<{
  ...
  return <fieldset className="space-y-3 rounded-xl border border-white/15 p-4 text-sm text-text-light">
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {fields.map(([key, label, description]) => <label key={key} className="min-w-0" title={description}>
        ...
        <input aria-label={label} aria-describedby={`mtt-${key}-hint`} type="number" min={key.includes('PrizePool') || key === 'declaredTotalChipsBb' ? 0 : 1}
          step={key.includes('PrizePool') || key === 'declaredTotalChipsBb' ? 'any' : '1'} value={Number.isNaN(value[key]) ? '' : (value[key] ?? '')}
