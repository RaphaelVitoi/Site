...
import type { StreetChipEvFreqs, Scenario, HeroPosition } from '@/components/simulator/engine/types';
...
export function useSimulatorState(initialScenario: Scenario) {
    ...
    const [streetFreqs, setStreetFreqs] = useState<StreetChipEvFreqs>(initialScenario.defaultStreetFreqs);
