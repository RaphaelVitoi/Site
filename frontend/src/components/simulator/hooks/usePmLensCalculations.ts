...
export function usePmLensCalculations({
    ...
    const streetProgression = useMemo(() => {
        ...
        return [
            { name: 'PRE', potSize: basePot, cumulative: Math.abs(heroInvested) },
            { name: 'FLOP', potSize: basePot + (firstBet * 2), cumulative: Math.abs(heroInvested) + firstBet },
            {
                name: 'TURN',
                potSize: (basePot + (firstBet * 2)) * (1 + (2 * normalizedSizing)),
                cumulative: Math.abs(heroInvested) + firstBet + ((basePot + (firstBet * 2)) * normalizedSizing),
            },
            {
                name: 'RIVER',
                potSize: ((basePot + (firstBet * 2)) * (1 + (2 * normalizedSizing))) * (1 + (2 * normalizedSizing)),
                cumulative:
    ...
    }, [heroIdx, primaryVillainIdx, equity, pkoValue, kappa, simulatedActivePlayers, blindsRisingSoon, deltaHabilidade, streetProgression, initialStacks, initialPrizes, handleIcmResult, absoluteHeroPos, realizationFactor, activeNodelock]);
