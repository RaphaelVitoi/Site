...
import { SotaDissolution } from '@/components/ui/layout/SotaDissolution';
			...
			<div className="flex flex-col gap-4 text-left mb-10">
				<div className="bg-slate-800/40 border-l-[3px] border-accent-indigo p-4 rounded-md">
					...
					<span className="text-[0.85rem] text-text-dim">
						Estar Fora de Posição dilui estruturalmente o seu poder de fogo. O Fator R revela que mãos que parecem lucrativas no vácuo tornam-se insustentáveis no mundo real.
					</span>
				</div>
				<div className="bg-slate-800/40 border-l-[3px] border-accent-pink p-4 rounded-md">
					...
					<span className="text-[0.85rem] text-text-dim">
						O Axioma Lipe Piv utiliza a Regressão Bayesiana para forçar a equidade teórica de volta ao baseline passivo sempre que a Credibilidade (κ) for baixa. Não confie cegamente no vácuo.
					</span>
				</div>
				<div className="bg-slate-800/40 border-l-[3px] border-accent-amber p-4 rounded-md">
					...
					<span className="text-[0.85rem] text-text-dim">
						Qualquer decisão agressiva negativa (-3bb) será lucrativa pela Lente da Perspectiva se o Custo Afundado da desistência for ainda pior (-5bb). O EV do Fold é o seu piso inegociável.
					</span>
				</div>
				<div className="bg-slate-800/40 border-l-[3px] border-accent-pink p-4 rounded-md">
					<strong className="text-accent-pink-light block mb-1 font-bold">
					...
					<span className="text-[0.85rem] text-text-dim">
						O preço barato mascara o Passivo Estrutural das Reverse Implied Odds (RIO). Se o Coeficiente Cᵢ for menor que 1, as odds estão mentindo e a sua equity real será devorada pela entropia (Multiway).
					</span>
				</div>
			</div>
			<button
				onClick={onRestart}
				className="py-3 px-8 bg-accent-indigo text-white border-none rounded-lg font-extrabold cursor-pointer uppercase tracking-wider"
			>
...
interface ActiveScenarioStepProps {
...
export default function IcmQuizVisceral() {
	...
	useEffect(() => {
		...
		fetchVisceralScenarios()
			...
			.catch((error: unknown) => {
				console.warn(
					'[Quiz] Falha ao sincronizar cenários:',
	...
	if (scenarios.length === 0) {
		return (
			<div className="p-8 text-center text-text-dim italic tracking-wider">
	...
	const handleSelect = (choiceId: string) => {
		...
		setTimeout(() => setIsRevealing(false), 850);
	};
	...
	return (
		<div className="max-w-3xl my-6 mx-auto p-6 sm:p-8 bg-slate-900/60 backdrop-blur-md rounded-2xl border border-white/5 shadow-2xl">
			<div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
				<h2 className="m-0 text-[1.1rem] text-text-light font-extrabold uppercase tracking-wider">
			...
			<SotaDissolution isDissolved={isDissolving}>
			...
			{!isFinished && (
				<div className="flex justify-between mt-8 pt-4 border-t border-white/5">
					<button
						...
						className={`py-2 px-4 rounded-md font-semibold transition-all duration-200 border ${currentStep === 0 || isRevealing ? 'bg-transparent text-white/10 border-white/5 cursor-not-allowed' : 'bg-transparent text-text-dim border-white/20 cursor-pointer hover:text-white hover:border-white/45'}`}
					>
						&larr; Voltar
					</button>
					<button
						onClick={handleNext}
						disabled={!isAnswered || isRevealing}
						className={`py-2 px-6 rounded-md font-bold transition-all duration-200 border-none ${isAnswered && !isRevealing ? 'bg-accent-indigo text-white cursor-pointer hover:shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'bg-indigo-500/10 text-white/30 cursor-not-allowed'}`}
					>
