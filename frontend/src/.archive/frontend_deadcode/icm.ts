...
export function calculateICM(stacks: number[], prizes: number[]): { id: string; equity: number }[] {
	...
	if (!stacks || stacks.length === 0 || stacks.some((s) => typeof s !== 'number' || s < 0)) {
		throw new Error(
			'As pilhas de fichas (stacks) devem ser um array de números positivos e não podem estar vazias.',
		);
	}
	if (!prizes || prizes.length === 0 || prizes.some((p) => typeof p !== 'number' || p < 0)) {
		throw new Error(
			'Os prêmios (prizes) devem ser um array de números positivos e não podem estar vazios.',
		);
	}
...
if (import.meta.vitest) {
	...
	describe('calculateICM', () => {
		it('deve retornar equities iguais para stacks iguais e um prêmio', () => {
			...
			expect(result[0]?.equity ?? 0).toBeCloseTo(50);
			expect(result[1]?.equity ?? 0).toBeCloseTo(50);
		});
		...
		it('deve retornar maior equity para stack maior com um prêmio', () => {
			...
			const result = calculateICM(stacks, prizes);
			expect(result.length).toBe(2);
			expect(result[0]?.equity ?? 0).toBeCloseTo(66.66666666666666); // (200/300) * 100
			expect(result[1]?.equity ?? 0).toBeCloseTo(33.33333333333333); // (100/300) * 100
		});
		...
		it('deve retornar equity zero para stack zero e um prêmio', () => {
			...
			const result = calculateICM(stacks, prizes);
			expect(result.length).toBe(2);
			expect(result[0]?.equity ?? 0).toBeCloseTo(100);
			expect(result[1]?.equity ?? 0).toBeCloseTo(0);
		});
		...
		it('deve retornar equities para múltiplos prêmios', () => {
			...
			expect(result[0]?.equity ?? 0).toBeCloseTo(67.5);
			expect(result[1]?.equity ?? 0).toBeCloseTo(49.166666666666664);
			expect(result[2]?.equity ?? 0).toBeCloseTo(33.33333333333333);
		});

		it('deve lançar erro para stacks vazias', () => {
			const stacks: number[] = [];
			const prizes = [100];
			expect(() => calculateICM(stacks, prizes)).toThrow(
				'As pilhas de fichas (stacks) devem ser um array de números positivos e não podem estar vazias.',
			);
		});
		...
		it('deve lidar com caso onde todos os jogadores têm stack zero', () => {
			...
			expect(result[0]?.equity ?? 0).toBeCloseTo(0);
			expect(result[1]?.equity ?? 0).toBeCloseTo(0);
			expect(result[2]?.equity ?? 0).toBeCloseTo(0);
		});
		...
		it('deve lidar com caso onde um jogador tem stack zero e outros não', () => {
			...
			const prizes = [100, 50];
			const result = calculateICM(stacks, prizes);
			expect(result[0]?.equity ?? 0).toBeGreaterThan(0);
			expect(result[1]?.equity ?? 0).toBeGreaterThan(0);
			expect(result[2]?.equity ?? 0).toBeCloseTo(0); // Player with 0 stack gets 0 equity
