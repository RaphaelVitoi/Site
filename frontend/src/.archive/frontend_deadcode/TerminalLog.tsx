...
export function TerminalLog({
	lines,
	isProcessing = false,
	title = 'NEXUS TERMINAL SOTA',
	height = 'h-96',
}: Readonly<TerminalLogProps>) {
	...
	return (
		<ClientOnly
			fallback={
				<div
					className={`w-full ${height} bg-black/20 animate-pulse rounded-xl border border-white/5`}
				/>
			}
		>
			<div
				className={`flex flex-col w-full ${height} bg-[#0a0a0f] rounded-xl border border-white/10 shadow-2xl overflow-hidden font-mono text-[0.8rem]`}
			>
				{/* Header do Terminal */}
				<div className="flex items-center justify-between px-4 py-2.5 bg-white/5 border-b border-white/10 shrink-0">
					<div className="flex items-center gap-3">
						...
						<span className="font-black text-text-dim uppercase tracking-widest text-[0.65rem] select-none">
							{title}
						</span>
					</div>
					{isProcessing && (
						<div className="flex items-center gap-2">
							<span className="text-accent-indigo text-[0.65rem] font-bold uppercase tracking-widest animate-pulse select-none">
								Executando
							</span>
				...
				<div
					ref={scrollRef}
					className="flex-1 overflow-y-auto p-5 space-y-1.5 scroll-smooth"
				>
					{lines.length === 0 ? (
						<div className="text-text-darker italic font-semibold select-none">
							Aguardando telemetria...
						</div>
					) : (
						lines.map((line, idx) => (
							// SOTA: Chave hibrida e interpolada previne colapso de renderizacao
							<div
								key={`${idx}-${line.substring(0, 16)}`}
								className="text-text-light wrap-break-word whitespace-pre-wrap leading-relaxed flex"
							>
