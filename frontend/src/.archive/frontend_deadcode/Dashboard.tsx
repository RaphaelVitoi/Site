...
export default function Dashboard() {
	...
	} = useSWR<TaskStatus[]>(buildNexusClientUrl('/status'), fetcher, { refreshInterval: 3000 });
	const { data: keysData } = useSWR<KeyHealthSummary>(
		buildNexusClientUrl('/key-health-summary'),
		fetcher,
	...
	const pendingTasks = useMemo(
		() =>
			Array.isArray(statusData)
				? statusData.filter((t: TaskStatus) => t.status === 'pending').length
				: 0,
		[statusData],
	);
	const activeWorkers = useMemo(
		() =>
			Array.isArray(statusData)
				? statusData.filter((t: TaskStatus) => t.status === 'processing').length
				: 0,
		[statusData],
	);
	const recentTasks = useMemo(
		() => (Array.isArray(statusData) ? statusData.slice(0, 10) : []),
		[statusData],
	);
	const onlineKeys = useMemo(
		() => (keysData ? Math.round((keysData.online_rate ?? 0) * (keysData.total_keys ?? 0)) : 0),
		[keysData],
	);
		...
		<div className="container pt-16 pb-20 max-w-6xl mx-auto">
			...
			{statusError && (
				<div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-lg mb-8">
					<strong>Aviso Critico:</strong> O Orquestrador Python (task_executor.py) parece
					estar offline ou inacessivel na porta 17042.
				</div>
			)}
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
				...
				<div className="p-6 rounded-2xl bg-bg-panel/40 border border-white/10 backdrop-blur-xl shadow-2xl hover:bg-bg-panel/60 transition-colors relative overflow-hidden group">
					...
					<h3 className="text-[0.65rem] uppercase tracking-widest text-text-muted font-black mb-2 relative z-10">
						Backlog (SQLite)
					</h3>
				...
				<div className="p-6 rounded-2xl bg-bg-panel/40 border border-white/10 backdrop-blur-xl shadow-2xl hover:bg-bg-panel/60 transition-colors relative overflow-hidden group">
					...
					<div className="text-xs mt-2 text-accent-emerald font-bold">Workers Ativos</div>
				</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				<div className="rounded-2xl border border-white/10 bg-bg-panel/40 backdrop-blur-xl shadow-2xl overflow-hidden flex flex-col">
					<div className="p-5 border-b border-white/10 bg-black/20">
						<h2 className="text-sm uppercase tracking-widest font-black text-white">
							Operacoes em Fila
						</h2>
					</div>
							...
							<tbody className="divide-y divide-white/5">
								{recentTasks.length === 0 ? (
									<tr>
										<td
											colSpan={3}
											className="p-8 text-center text-text-muted italic text-xs"
										>
											Friccao Zero. Fila limpa.
										</td>
									</tr>
								) : (
									recentTasks.map((task: TaskStatus) => (
										...
										>
											<td className="p-4 font-mono text-[0.65rem] text-text-darker font-bold">
											...
											<td className="p-4">
												<span className="flex items-center gap-2 text-xs font-bold text-text-muted">
													<span
														className={`w-1.5 h-1.5 rounded-full ${task.status === 'processing' ? 'bg-accent-amber animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.6)]' : 'bg-text-darker'}`}
													></span>
													{task.status}
												</span>
											</td>
				...
				<div className="rounded-2xl border border-white/10 bg-bg-panel/40 backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden">
					...
					<div className="p-5 flex-1 bg-[#0a0a0c]/80 font-mono text-[0.65rem] text-accent-emerald/80 overflow-y-auto max-h-100 shadow-inner">
						<p className="mb-2 opacity-70">
							&gt; [SYS] Interface SWR estabelecida com orquestrador local.
						</p>
						<p className="mb-2 opacity-70">
							&gt; [SYS] Aguardando telemetria WebSocket ou Polling via SQLite...
						</p>
						{statusData && (
							<p className="mb-2 text-text-dim">
								&gt; [POLLING] Sync concluido. {pendingTasks} pendentes,{' '}
								{activeWorkers} em processamento.
							</p>
						)}
