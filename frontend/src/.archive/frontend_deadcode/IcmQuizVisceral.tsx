...
function QuizSynthesis({ onRestart }: { readonly onRestart: () => void }) {
	return (
		<motion.div
			...
			{...({ className: 'text-center py-4' } as any)}
		>
			...
			<p className="text-text-muted text-[0.95rem] leading-relaxed max-w-150 mx-auto mb-8">
				As leis da PM Lens destroem a ilusão linear do ChipEV e ancoram suas decisões no
				chão da realidade:
			</p>
			<div className="flex flex-col gap-4 text-left mb-10">
				<div className="bg-slate-800/40 border-l-[3px] border-accent-indigo p-4 rounded-md">
					...
					<span className="text-[0.85rem] text-text-dim">
						Estar Fora de Posição dilui estruturalmente o seu poder de fogo. O Fator R
						revela que mãos que parecem lucrativas no vácuo tornam-se insustentáveis no
						mundo real.
					</span>
				</div>
				<div className="bg-slate-800/40 border-l-[3px] border-accent-pink p-4 rounded-md">
					...
					<span className="text-[0.85rem] text-text-dim">
						O Axioma Lipe Piv utiliza a Regressão Bayesiana para forçar a equidade
						teórica de volta ao baseline passivo sempre que a Credibilidade (κ) for
						baixa. Não confie cegamente no vácuo.
					</span>
				</div>
				<div className="bg-slate-800/40 border-l-[3px] border-accent-amber p-4 rounded-md">
					...
					<span className="text-[0.85rem] text-text-dim">
						Qualquer decisão agressiva negativa (-3bb) será lucrativa pela Lente da
						Perspectiva se o Custo Afundado da desistência for ainda pior (-5bb). O EV
						do Fold é o seu piso inegociável.
					</span>
				</div>
				<div className="bg-slate-800/40 border-l-[3px] border-accent-pink p-4 rounded-md">
					<strong className="text-accent-pink-light block mb-1 font-bold">
					...
					<span className="text-[0.85rem] text-text-dim">
						O preço barato mascara o Passivo Estrutural das Reverse Implied Odds (RIO).
						Se o Cᵢ for menor que 1, as odds estão mentindo e a sua equity real será
						devorada pela entropia (Multiway).
					</span>
				</div>
			</div>
			<button
		...
		>
			...
			{isRevealing && (
				<motion.div
					...
					{...({
						className:
							'mt-8 p-6 text-center bg-slate-900/40 rounded-lg border border-dashed border-accent-indigo/20',
					} as any)}
				>
					<span className="text-[0.75rem] text-accent-indigo uppercase tracking-widest font-bold">
						<i className="fa-solid fa-microchip fa-fade mr-2" /> Processando Vetores de
						Perspectiva...
					</span>
				</motion.div>
			)}
...
export default function IcmQuizVisceral() {
	...
	if (!isFinished && !scenario) return null;
	...
	return (
		<div className="max-w-200 my-12 mx-auto p-8 bg-slate-900/60 rounded-xl border border-white/5">
			<div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
				<h2 className="m-0 text-[1.25rem] text-text-light font-extrabold uppercase tracking-wider">
