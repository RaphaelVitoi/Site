...
export const NashMatrixProfiler: React.FC = () => {
	...
	const processDistortion = useCallback((payload: any) => {
		...
		f64Array[5] = payload.freqs.oop_fold ?? 0;
		f64Array[6] = payload.freqs.oop_call ?? 0;
		f64Array[7] = payload.freqs.oop_raise ?? 0;
	...
	const handleConcurrentStressTest = () => {
		for (let i = 0; i < 50; i++) {
			dispatchQuantized({
				...
				active_players: 3,
				freqs: { oop_fold: 0.2, oop_call: 0.4, oop_raise: 0.4 },
			});
		}
	};
	...
	return (
		<Profiler id="NashMatrix" onRender={onProfilerRender}>
			<div className="p-5 font-mono text-white bg-[#121212]">
				<h2 className="text-[#00ffcc] text-xl font-bold mb-4">
					Nash Distortion Profiler (SOTA)
				</h2>
				<button
					onClick={handleConcurrentStressTest}
					className="p-2.5 bg-[#333] text-[#00ffcc] border border-[#00ffcc] cursor-pointer hover:bg-[#444] transition-colors rounded"
				>
					Disparar Concorrência (50 Mutações / Stress Test)
				</button>
				<div className="mt-2.5 text-[#aaa] text-sm">
					Fase de Renderização Atual: {renderPhase}
				</div>
