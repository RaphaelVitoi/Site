...
const intensity = args.includes('--intensity')
	? (args[args.indexOf('--intensity') + 1] ?? 'low')
	: 'low';
const target = args.includes('--target')
	? (args[args.indexOf('--target') + 1] ?? 'worker')
	: 'worker';
...
async function simulateFrontendChaos() {
	...
	await delay(900);
	console.log(
		`[CHICO] Bombardeando \`updatePhysics\` com parâmetros degenerados (NaN, Infinity, -1)...`,
	);
	await delay(1500);
	...
	if (safetyScore >= 3) {
		...
		console.log(
			`[VITORIA] Os inputs invalidados colapsaram no limite de segurança estrito (\`Math.max(...)\`). Nenhum Crash.\n`,
		);
	} else {
		console.error(
...
async function runChaos() {
	...
	console.log(
		`[CHICO] Alvo: ${target.toUpperCase()} | Intensidade: ${intensity.toUpperCase()}\n`,
	);
