...
const SCENARIOS_DATABASE: Scenario[] = [
	...
	},
	{
		...
		icon: '⚔️',
		verdict: {
			label: 'Fome de Laddering',
			className: 'text-emerald-400 border-emerald-500/30 bg-emerald-950/50',
		},
	...
	},
	{
		...
		icon: '👑',
		verdict: {
			label: 'Criação de Monstros',
			className: 'text-fuchsia-400 border-fuchsia-500/30 bg-fuchsia-950/50',
		},
		ip: {
			pos: 'God Mode (CL)',
			stack: '90 bb',
			rp: 12,
			morph: 'Polar Controlado',
		},
	...
	},
	{
		...
		verdict: {
			label: 'MDF Perfeito',
			className: 'text-slate-400 border-slate-500/30 bg-slate-900/50',
		},
		...
		<main className="container mx-auto px-4 py-16">
			<div className="text-center mb-16 animate-fade-up">
				...
				<p className="text-slate-400 max-w-2xl mx-auto italic text-sm">
					Isolando a mecânica do Risk Premium. Sinta a impunidade de agredir quando o
					oponente está paralisado na Death Zone.
				</p>
			</div>

			{/* Seleção de Cenário */}
			<div className="flex flex-wrap justify-center gap-4 mb-12 animate-fade-up [animation-delay:100ms]">
			...
			{/* Palco do Cenário */}
			<div className="max-w-4xl mx-auto animate-fade-up min-h-75">
				{activeScenario && (
					<div className="glass-panel p-8 border-t-4 border-indigo-500 shadow-[inset_0_0_50px_rgba(99,102,241,0.05)]">
						<div className="flex justify-between items-start mb-6 border-b border-white/5 pb-4">
							<div>
								<span className="text-[10px] font-black text-slate-400 bg-slate-800/50 px-3 py-1.5 rounded-md uppercase tracking-widest border border-slate-700/50">
									{activeScenario.env}
								</span>
							...
							<span
								className={`px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest border ${activeScenario.verdict.className}`}
							>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
							<div className="bg-slate-950/50 p-6 rounded-xl border border-sky-500/30 relative overflow-hidden group">
								...
								<h3 className="text-sky-400 font-mono text-xs uppercase tracking-widest mb-4 flex items-center justify-center gap-2">
									<span>IP / Agressor</span>
								</h3>
								<div className="text-center mb-6">
									<div className="text-3xl font-bold text-white mb-1">
										{activeScenario.ip.pos}
									</div>
									<div className="inline-block data-mono text-sm text-slate-400 font-medium bg-slate-900/80 px-3 py-1 rounded-md border border-slate-800">
										Stack: {activeScenario.ip.stack}
									</div>
								</div>
								...
								<div className="relative w-32 h-32 mx-auto my-6">
									<svg viewBox="0 0 36 36" className="block w-full h-full">
										...
										<path
											...
											strokeWidth="2.5"
											strokeLinecap="round"
											className="stroke-sky-500 drop-shadow-[0_0_8px_rgba(14,165,233,0.6)] transition-all duration-1000 ease-out"
											strokeDasharray={`${Math.min(100, (activeScenario.ip.rp / 26) * 100)}, 100`}
											d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
										/>
									</svg>
									<div className="absolute inset-0 flex flex-col items-center justify-center">
										<span className="data-mono text-3xl font-black text-white">
											{activeScenario.ip.rp}%
										</span>
										<span className="text-[9px] font-bold text-sky-400 uppercase tracking-widest mt-1">
											R. Premium
										</span>
									</div>
								</div>
								...
								<div className="text-center">
									...
									<span className="inline-block text-xs font-bold text-sky-300 bg-sky-950/30 px-4 py-2 rounded-lg border border-sky-500/20">
							...
							<div className="bg-slate-950/50 p-6 rounded-xl border border-red-500/30 relative overflow-hidden group">
								...
								<h3 className="text-rose-400 font-mono text-xs uppercase tracking-widest mb-4 flex items-center justify-center gap-2">
									<span>OOP / Defensor</span>
								</h3>
								<div className="text-center mb-6">
									<div className="text-3xl font-bold text-white mb-1">
									...
									<div className="inline-block data-mono text-sm text-slate-400 font-medium bg-slate-900/80 px-3 py-1 rounded-md border border-slate-800">
										Stack: {activeScenario.oop.stack}
									</div>
								</div>
								...
								<div className="relative w-32 h-32 mx-auto my-6">
									<svg viewBox="0 0 36 36" className="block w-full h-full">
										...
										<path
											...
											strokeWidth="2.5"
											strokeLinecap="round"
											className="stroke-rose-500 drop-shadow-[0_0_8px_rgba(225,29,72,0.6)] transition-all duration-1000 ease-out"
											strokeDasharray={`${Math.min(100, (activeScenario.oop.rp / 26) * 100)}, 100`}
											d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
										/>
									</svg>
									<div className="absolute inset-0 flex flex-col items-center justify-center">
										...
										<span className="text-[9px] font-bold text-rose-400 uppercase tracking-widest mt-1">
											R. Premium
										</span>
									</div>
								</div>
								...
								<div className="text-center">
									...
									<span className="inline-block text-xs font-bold text-rose-300 bg-rose-950/30 px-4 py-2 rounded-lg border border-rose-500/20">
							...
							<div className="bg-slate-900/60 p-6 rounded-xl border-t-4 border-t-sky-500 border-x border-b border-white/5 hover:border-t-sky-400 transition-colors">
								<div className="flex justify-between items-start mb-6">
									<div className="space-y-1">
										...
										<p className="text-[10px] text-slate-500 font-bold uppercase">
											Frequência Ótima de Bluff
										</p>
									</div>
									<span
										className={`data-mono text-xs font-bold px-2 py-1 rounded border ${bluffBadgeClass}`}
									>
										{dB > 0 ? '+' : ''}
										{dB.toFixed(1)}% vs cEV
									</span>
								...
								<div>
									<div className="flex items-baseline gap-2 mb-2">
										<span className="data-mono text-5xl font-black text-white tracking-tighter">
									...
									<div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden mt-4 shadow-inner">
										<div
											...
											ref={(el) => {
												if (el) el.style.width = `${dynamics.bluff}%`;
											}}
										></div>
									</div>
								</div>
							...
							<div className="bg-slate-900/60 p-6 rounded-xl border-t-4 border-t-rose-500 border-x border-b border-white/5 hover:border-t-rose-400 transition-colors">
								<div className="flex justify-between items-start mb-6">
									<div className="space-y-1">
										...
										<p className="text-[10px] text-slate-500 font-bold uppercase">
											Limiar de Indiferença (Call)
										</p>
									</div>
									<span
										className={`data-mono text-xs font-bold px-2 py-1 rounded border ${defenseBadgeClass}`}
									>
										{dD > 0 ? '+' : ''}
										{dD.toFixed(1)}% vs cEV
									</span>
								...
								<div>
									...
									<div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden mt-4 shadow-inner">
										<div
											...
											ref={(el) => {
												if (el) el.style.width = `${dynamics.defense}%`;
											}}
										></div>
									</div>
								</div>
