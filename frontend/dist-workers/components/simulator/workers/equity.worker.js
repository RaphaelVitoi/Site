...
globalThis.onmessage = async (e) => {
    ...
    if (id === undefined || !heroRange || !villainRange) {
        console.warn('[SOTA Worker] Invalid payload discarded.');
        return;
    }
    try {
        if (!initPromise) {
        ...
        const equity = calculate_equity_monte_carlo_binary(heroMask, villainMask, cleanBoard, iterations, seed, kappa);
        // Honestidade Intelectual: Interceptando os contratos de entropia do Rust
        if (equity === -1)
            throw new Error('Sintaxe inválida: Hero (Ex: AhKh, AKs, QQ).');
        if (equity === -2)
            throw new Error('Sintaxe inválida: Vilão (Ex: QdQc, AJo).');
        if (equity === -3)
            throw new Error('Sintaxe inválida: Board (Use até 5 cartas exatas ou deixe em branco).');
        if (equity === -4)
            throw new Error('Anomalia Quântica: Cartas duplicadas (colisão) detectadas na entrada.');
        if (equity < 0)
            throw new Error('Falha matemática no motor WASM.');
