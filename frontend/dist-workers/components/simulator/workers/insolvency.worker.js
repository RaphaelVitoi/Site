/** @format */
import init, { solve_insolvency_matrix_binary, solve_icm_distortion_v2, calculate_perspectiva_vitoi_wasm, alloc_range_buffer, free_range_buffer, calculate_multiway_equity_zerocopy, } from '../../../lib/engine/vitoi_equity_engine';
...
async function handleMatrixPayload(data, id) {
    ...
    const matrix = solve_insolvency_matrix_binary(villainMask, data.board || '', data.rpFactor, data.heroInvested, data.currentPot, data.activePlayers, 5000, seed, data.kappa);
...
async function handlePerspectivePayload(data, id) {
    const p = data.payload;
    const start = performance.now();
    // Stress Test: Executa o loop de 21 pontos (usado no gráfico) para validar performance
    const results = [];
    for (let i = 0; i <= 100; i += 5) {
        const r = calculate_perspectiva_vitoi_wasm(i, // winProb
        p.potSize * 0.8, // deltaWin (simulado)
        -p.heroCost * 2, // deltaLose (simulado)
        -p.heroCost, // dynamicEvFold
        p.realization, 1, // fgsHealth
        p.numPlayers, p.heroCost, p.potSize, p.stackEff, p.heroRp, p.villainRp, p.bountyValue, p.edgeBase, p.humanNoiseFactor);
        results.push(r);
    }
...
async function handleDistortionPayload(data, id) {
    ...
    const nashResults = {
        flop: formatStreetResults(solve_icm_distortion_v2(data.ipRpFlop, data.oopRpFlop, data.topologicAggression, data.activePlayers, potFlop, 0, data.freqFlop.oop_fold / 100, data.freqFlop.oop_raise / 100), data.freqFlop, data.ipRpFlop, data.oopRpFlop),
        turn: formatStreetResults(solve_icm_distortion_v2(data.ipRpTurn, data.oopRpTurn, data.topologicAggression, data.activePlayers, potTurn, 1, data.freqTurn.oop_fold / 100, data.freqTurn.oop_raise / 100), data.freqTurn, data.ipRpTurn, data.oopRpTurn),
        river: formatStreetResults(solve_icm_distortion_v2(data.ipRpRiver, data.oopRpRiver, data.topologicAggression, data.activePlayers, potRiver, 2, data.freqRiver.oop_fold / 100, data.freqRiver.oop_raise / 100), data.freqRiver, data.ipRpRiver, data.oopRpRiver),
    };
    globalThis.postMessage({ type: 'DISTORTION', nashResults, id });
}
async function handleMultiwayPayload(data, id) {
