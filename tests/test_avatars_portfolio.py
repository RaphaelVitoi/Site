...
def test_maverick_capabilities_and_math(avatar_config):
    ...
    Valida que o Maverick usa o modelo llama3.1:8b (local, otimizado para tarefas especificas de poker)
    ...
    assert maverick["ollama_model"] == "llama3.1:8b"
...
def test_gemma4_capabilities(avatar_config):
    ...
    Valida que Gemma4 usa o modelo local gemma4:4b (otimizado para processamento em hardware local)
    e possui o prompt focado em calibracao e execucao de borda.
    """
    gemma4 = avatar_config["personas"]["gemma4"]
    assert gemma4["ollama_model"] == "gemma4:4b"
...
@pytest.mark.asyncio
async def test_maverick_multimodal_agentic_pipeline(avatar_config):
    """
    ...
    """
    avatar_config["personas"]["maverick"]
