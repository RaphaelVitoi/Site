...
- **Sincronização e Simetria de Profiles (Host & Guest)**: Estabelecemos paridade absoluta entre o Windows PowerShell (host) e o WSL Debian (guest). Isolamos os ambientes virtuais fixando `$env:UV_PROJECT_ENVIRONMENT = ".venv"` no host e `export UV_PROJECT_ENVIRONMENT=.venv-wsl` no guest. Criamos mapeamentos de interoperação recíproca (`nexus`, `wsl-root`/`win-powershell`, `wsl-python`/`win-python`, etc.), silenciamos e tratamos erros nos scripts de inicialização, e geramos links simbólicos em `/usr/local/bin` apontando para o Node.js v24.16.0 do NVM para garantir execução limpa sob o usuário `root` e comandos não-interativos.
